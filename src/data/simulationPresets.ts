...
export const SIMULATION_PRESETS: SimulationPreset[] = [
  // ── SHM & WAVES ──
  {
    ...
    formula: 'A * 100 * cos(sqrt(k / m) * t)',
    resultName: 'x', resultUnit: 'm',
    ...
    defaultVariables: [
      ...
      { name: 'A', label: 'Amplitude', unit: 'm', value: 1.5, min: 0.2, max: 3.0, step: 0.1, color: '#22c55e' },
    ],
    suggestedObjects: [
      { type: 'wall', label: 'Fixed Wall', x: 100, y: 250, size: 4, color: '#64748b', behavior: 'none', sensitivity: 1 },
      { type: 'spring', label: 'Spring (k)', x: 100, y: 250, size: 3, color: '#8b5cf6', behavior: 'none', sensitivity: 1 },
      { type: 'block', label: 'Mass (m)', x: 400, y: 250, size: 5, color: '#3b82f6', behavior: 'x-position', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['x', 'k', 'm', 'A'], maxPoints: 200, label: 'SHM Displacement vs Time' },
    questions: [
  ...
  },
  {
    ...
    formula: '25 * cos(sqrt(g / L) * t)',
    resultName: 'theta', resultUnit: 'deg',
    ...
    defaultVariables: [
      ...
      { name: 'theta0', label: 'Max Angle', unit: 'deg', value: 25, min: 5, max: 60, step: 1, color: '#ec4899' },
    ],
    suggestedObjects: [
      { type: 'pendulum-arm', label: 'Swinging Pendulum', x: 400, y: 100, size: 4, color: '#f59e0b', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['theta', 'L', 'g'], maxPoints: 100, label: 'Angle vs Time' },
    questions: [
  ...
  },
  {
    ...
    category: 'Waves & SHM', icon: '🌊',
    defaultVariables: [
      { name: 'f1', label: 'Frequency 1', unit: 'Hz', value: 2, min: 1, max: 10, step: 1, color: '#8b5cf6' },
      { name: 'f2', label: 'Frequency 2', unit: 'Hz', value: 4, min: 1, max: 10, step: 1, color: '#ec4899' },
      { name: 'A1', label: 'Amplitude 1', unit: 'm', value: 2, min: 0.5, max: 5, step: 0.5, color: '#3b82f6' },
      { name: 'A2', label: 'Amplitude 2', unit: 'm', value: 1, min: 0.5, max: 5, step: 0.5, color: '#f59e0b' },
    ],
    suggestedObjects: [
      { type: 'wave-fourier', label: 'Live Wave Synthesizer', x: 400, y: 250, size: 3, color: '#06b6d4', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['y', 'f1', 'f2'], maxPoints: 150, label: 'Wave Displacement vs Time' },
    questions: [
  ...
  },
  {
    ...
    resultName: 'f_prime', resultUnit: 'Hz',
    ...
    suggestedObjects: [
      { type: 'wave-fourier', label: 'Doppler Wave Shift', x: 400, y: 250, size: 5, color: '#ef4444', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['f_prime', 'f0', 'v_source'], maxPoints: 100, label: 'Observed Frequency vs Source Speed' },
    questions: [
  ...
  // ── GRAVITATION ──
  {
    ...
    suggestedObjects: [
      { type: 'atom-molecule', label: 'Orbiting System', x: 400, y: 250, size: 6, color: '#3b82f6', behavior: 'none', sensitivity: 1 },
    ],
    questions: [
  ...
  // ── CLASS 11 & 12 ADVANCED PRESETS ──
  {
    id: 'fourier-synthesis',
    name: 'Fourier Wave Synthesis & Superposition',
    formula: 'A1 * sin(2 * pi * f1 * t) + A2 * sin(2 * pi * f2 * t)',
    resultName: 'Y', resultUnit: 'm',
    description: 'y(t) = A₁ sin(2π f₁ t) + A₂ sin(2π f₂ t) — Wave superposition',
    theory: 'Any complex wave can be synthesized by adding pure sine waves (harmonics). Superposition produces beat frequencies, timbre in acoustics, and wave interference.',
    category: 'Waves & Math', icon: '📊',
    defaultVariables: [
      { name: 'A1', label: 'Harmonic 1 Amp', unit: 'm', value: 2, min: 0.1, max: 5, step: 0.1, color: '#3b82f6' },
      { name: 'f1', label: 'Harmonic 1 Freq', unit: 'Hz', value: 1, min: 0.1, max: 10, step: 0.1, color: '#06b6d4' },
      { name: 'A2', label: 'Harmonic 2 Amp', unit: 'm', value: 1, min: 0.1, max: 5, step: 0.1, color: '#ec4899' },
      { name: 'f2', label: 'Harmonic 2 Freq', unit: 'Hz', value: 3, min: 0.1, max: 10, step: 0.1, color: '#f59e0b' },
    ],
    suggestedObjects: [
      { type: 'wave-fourier', label: 'Fourier Wave Canvas', x: 400, y: 220, size: 5, color: '#a855f7', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['t', 'Y', 'A1', 'A2'], maxPoints: 200, label: 'Fourier Superposition Wave' },
  },
  {
    id: 'rlc-circuit',
    name: 'RLC AC Circuit & Phase Angle',
    formula: 'V0 / sqrt(R*R + (2*pi*f*L - 1/(2*pi*f*C))*(2*pi*f*L - 1/(2*pi*f*C)))',
    resultName: 'I0', resultUnit: 'A',
    description: 'I₀ = V₀ / Z where Impedance Z = √(R² + (X_L - X_C)²)',
    theory: 'In an AC circuit with Resistor R, Inductor L, and Capacitor C, impedance Z dictates current flow. Resonance occurs when X_L = X_C (f_0 = 1 / (2π√(LC))).',
    category: 'Electricity & Magnetism', icon: '⚡',
    defaultVariables: [
      { name: 'V0', label: 'Peak Voltage', unit: 'V', value: 120, min: 10, max: 240, step: 5, color: '#ef4444' },
      { name: 'R', label: 'Resistance', unit: 'Ω', value: 50, min: 5, max: 500, step: 5, color: '#f59e0b' },
      { name: 'L', label: 'Inductance', unit: 'H', value: 0.1, min: 0.01, max: 1, step: 0.01, color: '#8b5cf6' },
      { name: 'C', label: 'Capacitance', unit: 'F', value: 0.00005, min: 0.000001, max: 0.001, step: 0.000005, color: '#06b6d4' },
      { name: 'f', label: 'Frequency', unit: 'Hz', value: 60, min: 10, max: 500, step: 5, color: '#22c55e' },
    ],
    suggestedObjects: [
      { type: 'circuit-loop', label: 'RLC Circuit Loop', x: 400, y: 220, size: 5, color: '#06b6d4', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['f', 'I0', 'R'], maxPoints: 200, label: 'Current vs Frequency' },
    questions: [
      { id: 'rlc-q1', question: 'What is the resonant frequency f0 = 1 / (2π√(LC)) for L = 0.1 H and C = 50 μF?', answer: 71.18, tolerance: 1, hint: '1 / (2 * π * √(0.1 * 0.00005))', unit: 'Hz' },
    ],
  },
  {
    id: 'projectile-vectors',
    name: 'Projectile Motion & Vectors',
    formula: 'v0 * v0 * sin(2 * theta * pi / 180) / g',
    resultName: 'Range', resultUnit: 'm',
    description: 'R = (v₀² sin 2θ) / g — Trajectory & velocity components',
    theory: 'A projectile moving under gravity follows a parabolic trajectory: x(t) = v₀ cosθ · t, y(t) = v₀ sinθ · t - ½gt². Maximum range occurs at θ = 45°.',
    category: 'Mechanics', icon: '🎯',
    defaultVariables: [
      { name: 'v0', label: 'Initial Velocity', unit: 'm/s', value: 25, min: 5, max: 100, step: 1, color: '#3b82f6' },
      { name: 'theta', label: 'Launch Angle', unit: '°', value: 45, min: 5, max: 85, step: 1, color: '#f59e0b' },
      { name: 'g', label: 'Gravity', unit: 'm/s²', value: 9.81, min: 1, max: 20, step: 0.1, color: '#ef4444' },
    ],
    suggestedObjects: [
      { type: 'projectile-path', label: 'Projectile Trajectory', x: 400, y: 220, size: 5, color: '#3b82f6', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['theta', 'Range', 'v0'], maxPoints: 200, label: 'Range vs Launch Angle' },
  },
  {
    id: 'acid-base-titration',
    name: 'Acid-Base Titration & pH Curve',
    formula: '7 + 0.5 * (log10(Ka) + log10(C))',
    resultName: 'pH', resultUnit: 'pH',
    description: 'pH calculations & neutralization equivalence point',
    theory: 'In acid-base titration, adding a titrant (e.g. NaOH) to an analyte (e.g. HCl or CH₃COOH) causes a steep pH shift at the equivalence point where moles of acid = moles of base.',
    category: 'Chemistry', icon: '🧪',
    defaultVariables: [
      { name: 'Ka', label: 'Acid Constant (Ka)', unit: '', value: 0.000018, min: 0.000001, max: 0.01, step: 0.000001, color: '#8b5cf6' },
      { name: 'C', label: 'Concentration', unit: 'M', value: 0.1, min: 0.01, max: 2, step: 0.01, color: '#06b6d4' },
    ],
    suggestedObjects: [
      { type: 'titration-flask', label: 'Titration Assembly', x: 400, y: 220, size: 5, color: '#06b6d4', behavior: 'none', sensitivity: 1 },
    ],
    dataLog: { enabled: true, variables: ['C', 'pH'], maxPoints: 200, label: 'pH vs Titrant Volume' },
  },
  {
    id: 'enzyme-kinetics',
    name: 'Enzyme Kinetics (Michaelis-Menten)',
    formula: 'Vmax * S / (Km + S)',
    resultName: 'v', resultUnit: 'μmol/min',
    description: 'v = (Vmax · [S]) / (Km + [S]) — Reaction velocity',
    theory: 'The Michaelis-Menten equation models enzyme reaction rate. At low substrate [S], reaction rate is first-order; at high [S], rate approaches saturation Vmax. Km is [S] at half Vmax.',
    category: 'Biology', icon: '🧬',
    defaultVariables: [
      { name: 'Vmax', label: 'Max Velocity', unit: 'μmol/min', value: 100, min: 10, max: 500, step: 5, color: '#ef4444' },
      { name: 'Km', label: 'Michaelis Const', unit: 'mM', value: 5, min: 0.5, max: 20, step: 0.5, color: '#8b5cf6' },
      { name: 'S', label: 'Substrate [S]', unit: 'mM', value: 10, min: 0.1, max: 50, step: 0.5, color: '#22c55e' },
    ],
    suggestedObjects: [
      { type: 'cell', label: 'Enzyme-Substrate Complex', x: 400, y: 220, size: 6, color: '#22c55e', behavior: 'size', sensitivity: 2 },
      { type: 'molecule', label: 'Substrate', x: 250, y: 220, size: 4, color: '#f59e0b', behavior: 'x-position', sensitivity: 5 },
    ],
    dataLog: { enabled: true, variables: ['S', 'v', 'Km'], maxPoints: 200, label: 'Velocity vs Substrate [S]' },
  },
];
