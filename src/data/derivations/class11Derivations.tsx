...
export const CLASS11_DERIVATIONS: Record<string, DerivationConfig> = {
  ...
  dimensional_wavelength: {
    ...
    steps: [
      { label: 'Identify Dependencies', formula: 'lambda = f(h, m, v) => h^a m^b v^c', detail: 'ðŸ”¬ You\'re an electron microscope designer trying to see individual atoms. You know electrons behave like waves — but what determines their wavelength? You guess it depends on Planck\'s constant h (the quantum world\'s ruler), the electron\'s mass m (heavier = shorter wavelength), and its velocity v (faster = shorter wavelength). You write: Î» = h^a m^b v^c, where a, b, c are unknown powers you must find.' },
      { label: 'Write Dimensions', formula: '[L] = [ML^{2}T^{-1}]^a [M]^b [LT^{-1}]^c', detail: 'ðŸ“ You pull out your dimensional analysis toolkit. Planck\'s constant h has dimensions [ML²Tâ»¹] (energy × time), mass m has [M], velocity v has [LTâ»¹]. The wavelength Î» has [L]. You set up: [L] = [ML²Tâ»¹]^a [M]^b [LTâ»¹]^c. It\'s like a puzzle where each dimension M, L, T must match on both sides.' },
      { label: 'Solve Power Equations', formula: 'M: a+b=0, L: 2a+c=1, T: -a-c=0', detail: 'ðŸ§© You solve three simultaneous equations: For M: a+b=0, for L: 2a+c=1, for T: -a-c=0. Working through: from T: c=-a, from L: 2a-a=1 so a=1, from M: 1+b=0 so b=-1. Three equations, three unknowns — perfectly determined!' },
      { label: 'Final Formula', formula: 'lambda = \\text{constant} \\times h^{1} m^{-1} v^{-1} = h/mv', detail: 'âœ¨ Substituting back: Î» = constant × h¹ mâ»¹ vâ»¹ = h/(mv). When physicist Louis de Broglie proposed this in 1924, it was revolutionary — particles like electrons have wave nature! Today, electron microscopes use this exact principle, achieving resolutions of 0.05 nm — enough to image individual atoms!' },
    ],
    sliders: [
  ...
  dimensional_pendulum: {
    ...
    steps: [
      ...
      { label: 'Write Dimensions', formula: '[T] = [M]^a [L]^b [1]^c [LT^{-2}]^d', detail: 'ðŸ“ You write dimensions: [T] = [M]^a [L]^b [1]^c [LTâ»²]^d. Angle Î¸ is dimensionless (ratio of arc to radius). Time T has dimension T. Equating: M: a=0 (mass doesn\'t matter!), L: b+d=0, T: -2d=1. The angle\'s power c=0 too!' },
      { label: 'Solve Power Equations', formula: 'a=0, d=-1/2, b=1/2, c=0', detail: 'ðŸŽ¯ Solving: a=0, d=-1/2, b=1/2, c=0. The period doesn\'t depend on mass at all — Galileo was right! A heavier bob doesn\'t swing faster or slower. Length has power 1/2, gravity has power -1/2. The angle doesn\'t matter (for small swings).' },
      { label: 'Final Formula', formula: 'T = \\text{constant} \\times l^{1/2} g^{-1/2} = 2\\pi \\sqrt{l/g}', detail: 'â° Final formula: T = constant × âˆš(l/g). The constant turns out to be 2Ï€ (from solving the SHM equation for small angles). T = 2Ï€âˆš(l/g). In 1656, Christiaan Huygens built the first pendulum clock using this formula. A 1-meter pendulum gives T â‰ˆ 2 seconds — perfect for a clock\'s tick-tock!' },
    ],
    sliders: [
  ...
  rectangular_components: {
    ...
    steps: [
      { label: 'Right Triangle Formed', formula: 'Angle theta between A and x-axis', detail: 'ðŸ—ºï¸ You\'re a pilot navigating from New York to London. Your flight path forms a 3,000 km vector at 30° north of east. But your navigation system works in east-west (x) and north-south (y) components. You need to break this vector apart — just like resolving forces on an inclined plane or calculating the effective wind component on a runway.' },
      { label: 'Cosine for Adjacent (Ax)', formula: 'cos theta = A_x / A => A_x = A cos theta', detail: 'ðŸ“ The vector A makes angle Î¸ with the x-axis. Dropping a perpendicular to the x-axis creates a right triangle. The x-component is the adjacent side: Ax = A cos Î¸. For your flight: Ax = 3000 × cos 30° = 2598 km eastward.' },
      { label: 'Sine for Opposite (Ay)', formula: 'sin theta = A_y / A => A_y = A sin theta', detail: 'ðŸ“ The y-component is the opposite side: Ay = A sin Î¸. For your flight: Ay = 3000 × sin 30° = 1500 km northward. These two perpendicular components are independent — you can adjust east-west and north-speed speeds separately without affecting each other.' },
      { label: 'Vector from Components', formula: 'A = \\sqrt{A_x^{2} + A_y^{2}}, tan theta = A_y/A_x', detail: 'ðŸ”„ Given components, you can always reconstruct the original vector: magnitude A = âˆš(Ax² + Ay²) (Pythagoras) and direction Î¸ = tanâ»¹(Ay/Ax). This works for forces, velocities, electric fields — any vector quantity. GPS uses this to calculate your precise position from satellite signals!' },
    ],
    sliders: [
  ...
  first_equation: {
    ...
    steps: [
      ...
      { label: 'Multiply Both Sides by t', formula: 'a \\times t = v_f - v_i', detail: 'ðŸ“Š By definition: a = (vf - vi)/t. In your car: a = (16.7 - 0)/5 = 3.33 m/s². Your speed increases by 3.33 m/s every second. Cross-multiplying: a × t = vf - vi. The change in velocity equals acceleration multiplied by time.' },
      { label: 'Add v_i to Both Sides', formula: 'v_f = v_i + a t', detail: 'âž— Rearranging: vf = vi + at. This is the first equation of motion. For your car: vf = 0 + 3.33 × 5 = 16.7 m/s. Simple but powerful — it tells you your final speed after any acceleration for any duration.' },
      { label: 'Graphical Meaning', formula: 'Slope of v-t graph = a', detail: 'ðŸ“ˆ On a velocity-time graph, this equation gives a straight line. The slope is acceleration, the y-intercept is initial velocity. Drag racers use this: from 0 to 100 km/h (27.8 m/s) in 3 seconds means a = 9.27 m/s² — nearly 1 g of acceleration!' },
    ],
    sliders: [
  ...
  second_equation: {
    ...
    steps: [
      ...
      { label: 'Rectangle Area', formula: 'S_rect = v_i \\times t', detail: 'ðŸ“ The rectangle part: initial velocity × time. If you start the corner at 15 m/s and take 4 seconds, the rectangle area = 15 × 4 = 60 m. This is how far you\'d go at constant speed.' },
      { label: 'Triangle Area', formula: 'S_tri = \\frac{1}{2} \\times (vf - vi) \\times t = \\frac{1}{2} \\times (at) \\times t', detail: 'ðŸ“ The triangle part: the extra distance from acceleration. Area = ½ × (vf - vi) × t = ½ × (at) × t = ½at². For acceleration 5 m/s² over 4 s: ½ × 5 × 16 = 40 m. The triangle sits on top of the rectangle.' },
      { label: 'Add Both Areas', formula: 'S = v_i t + \\frac{1}{2} a t^{2}', detail: 'âœ… Total distance S = vit + ½at². For your F1 car: S = 15×4 + ½×5×16 = 60 + 40 = 100 m. You precisely know where the car will be. This equation is used everywhere — from calculating stopping distances to launching rockets!' },
    ],
    sliders: [
  ...
  third_equation: {
    ...
    steps: [
      ...
      { label: 'Multiply Both Sides by a/a', formula: 'S = \\frac{1}{2}(vi + vf) \\times (vf - vi)/a', detail: 'ðŸ“ Start with the trapezoid area: S = ½(vi + vf) × t. This is distance = average velocity × time. Smart — it doesn\'t assume constant velocity, just constant acceleration.' },
      { label: 'Difference of Squares', formula: '2aS = (vi + vf)(vf - vi) = vf^{2} - vi^{2}', detail: 'ðŸ§® Eliminate t using the first equation: t = (vf - vi)/a. Substitute: S = ½(vi + vf) × (vf - vi)/a. Multiply both sides by 2a: 2aS = (vi + vf)(vf - vi) = vf² - vi² (difference of squares).' },
      { label: 'Final Form', formula: '2aS = vf^{2} - vi^{2}', detail: 'âœ… 2aS = vf² - vi². For braking from 30 m/s to 0 with deceleration 5 m/s²: 2×5×S = 0 - 900, so S = 90 m. That\'s your stopping distance! This equation powers everything from airbag deployment calculations to roller coaster safety systems.' },
    ],
    sliders: [
  ...
  projectile_height: {
    ...
    steps: [
      { label: 'Initial Vertical Velocity', formula: 'v_iy = v_i sin theta', detail: 'ðŸ€ You\'re a basketball player taking a jump shot. You launch the ball at 10 m/s at 60° above horizontal. How high will the ball go? The vertical component determines the height: v_iy = vi sin Î¸ = 10 × sin 60° = 10 × 0.866 = 8.66 m/s. The horizontal component (5 m/s) doesn\'t affect how high the ball goes — it just carries it forward.' },
      { label: 'At Maximum Height', formula: 'v_fy = 0', detail: 'ðŸ“ˆ At the maximum height, the ball\'s vertical velocity is momentarily zero — it\'s the turning point between going up and coming down. v_fy = 0. This is the peak of the parabolic arc.' },
      { label: 'Use Third Equation', formula: 'v_fy^{2} = v_iy^{2} + 2a_y H', detail: 'ðŸ§® Use the third equation of motion in the vertical direction: v_fy² = v_iy² + 2a_yH. Here a_y = -g (gravity pulls down). Substituting: 0 = (vi sin Î¸)² - 2gH. Rearranging: 2gH = (vi sin Î¸)².' },
      { label: 'Solve for H', formula: '0 = (v_i sin theta)^2 - 2gH => H = v_i^{2} sin^{2} theta / 2g', detail: 'âœ… H = (vi² sin²Î¸)/(2g). For your shot: H = 100 × 0.75 / 19.62 = 3.82 m. Add release height of 2.5 m, and the ball peaks at 6.32 m — well above the 3.05 m rim! The maximum height for a given speed occurs at Î¸ = 90° (straight up): H_max = vi²/(2g).' },
    ],
    sliders: [
  ...
  projectile_range: {
    ...
    steps: [
      ...
      { label: 'Time of Flight', formula: 'T = 2 v_i sin theta / g', detail: 'â±ï¸ The ball stays in the air for time T = 2vi sin Î¸ / g (twice the time to reach max height). For your kick: T = 2 × 25 × 0.5 / 9.81 = 25/9.81 = 2.55 seconds. That\'s your hang time — you need it to clear the wall.' },
      { label: 'Range = v_x x T', formula: 'R = (v_i cos theta) \\times (2 v_i sin theta / g)', detail: 'ðŸ“ Range = horizontal velocity × time of flight: R = (vi cos Î¸) × (2vi sin Î¸ / g) = 2vi² sin Î¸ cos Î¸ / g. For your kick: R = 2 × 625 × 0.866 × 0.5 / 9.81 = 541.25/9.81 = 55.2 m.' },
      { label: 'Use sin 2theta = 2 sin theta cos theta', formula: 'R = v_i^{2} sin 2theta / g', detail: 'ðŸŽ¯ Using 2 sin Î¸ cos Î¸ = sin 2Î¸: R = vi² sin 2Î¸ / g. For Î¸ = 45°: sin 90° = 1, R_max = vi²/g = 625/9.81 = 63.7 m. That\'s the maximum range at this speed. But in soccer, you want accuracy, not just distance — top players like Messi use angles around 25-30° for precision!' },
    ],
    sliders: [
      { label: 'Initial Velocity (vi)', key: 'vi', min: 5, max: 50, step: 1, default: 20, unit: ' m/s' },
  ...
  elastic_collision: {
    ...
    steps: [
      ...
      { label: 'Conservation of KE', formula: '0.5m1 u1^2 + 0.5m2 u2^2 = 0.5m1 v1^2 + 0.5m2 v2^2', detail: 'âš¡ Momentum conservation: mâ‚uâ‚ + mâ‚‚uâ‚‚ = mâ‚vâ‚ + mâ‚‚vâ‚‚. With uâ‚=3, uâ‚‚=0, mâ‚=mâ‚‚: 0.17×3 = 0.17vâ‚ + 0.17vâ‚‚, so vâ‚ + vâ‚‚ = 3. But one equation, two unknowns — you need more information.' },
      { label: 'Relative Speed Relation', formula: 'u1 - u2 = v2 - v1', detail: 'ðŸ”‹ Kinetic energy conservation: ½mâ‚uâ‚² + ½mâ‚‚uâ‚‚² = ½mâ‚vâ‚² + ½mâ‚‚vâ‚‚². Canceling ½m: 9 = vâ‚² + vâ‚‚². Combined with vâ‚ + vâ‚‚ = 3: solving gives vâ‚ = 0, vâ‚‚ = 3. The cue ball stops dead, the 8-ball rolls away at 3 m/s — a perfect transfer!' },
      { label: 'Solve for v1 and v2', formula: 'v1 = (m1-m2)/(m1+m2) u1 + 2m2/(m1+m2) u2', detail: 'ðŸŽ¯ The general solution: vâ‚ = (mâ‚-mâ‚‚)/(mâ‚+mâ‚‚) × uâ‚ + 2mâ‚‚/(mâ‚+mâ‚‚) × uâ‚‚. For equal masses: the first ball stops, the second takes its speed. For a heavy car hitting a light car: the light one flies backward. For a neutron hitting a proton in a nuclear reactor: the neutron transfers most of its energy — this is how nuclear moderators work!' },
    ],
    sliders: [
  ...
  linear_angular: {
    ...
    steps: [
      ...
      { label: 'Tangential Acceleration', formula: 'a_t = v/t = r omega/t = r alpha', detail: 'ðŸ”„ Linear speed v = S/t = rÎ¸/t = rÏ‰. If the Ferris wheel completes a rotation (2Ï€ rad) in 40 seconds: Ï‰ = Ï€/20 â‰ˆ 0.157 rad/s, and your speed v = 20 × 0.157 = 3.14 m/s. The kid at half the radius (10 m) moves at only 1.57 m/s.' },
      { label: 'Direction Matters', formula: 'a_c = v^{2}/r (centripetal)', detail: 'ðŸŽ¯ Tangential acceleration a_t = rÎ± (where Î± is angular acceleration). But there\'s also centripetal acceleration a_c = v²/r pulling you toward the center. Total acceleration is the vector sum of these two perpendicular components — your Ferris wheel seat experiences both changing speed and changing direction!' },
    ],
    sliders: [
  ...
  centripetal_acceleration: {
    ...
    steps: [
      { label: 'Similar Triangles (Position)', formula: 'delta r / r = delta theta', detail: 'ðŸŽ¢ You\'re riding a roller coaster through a sharp curve. You feel pushed sideways — that\'s centripetal acceleration pulling you toward the center. But where does this formula come from? Imagine a particle moving in a circle: in a small time Î”t, it moves through a small angle Î”Î¸. The position vectors before and after make a small triangle — similar to the velocity vectors before and after.' },
      ...
      { label: 'Acceleration = delta v / delta t', formula: 'a = v (delta theta/delta t) = v omega', detail: 'ðŸ§® Acceleration a = Î”v/Î”t = v(Î”Î¸/Î”t) = vÏ‰. Since Î”v = vÎ”Î¸ (from similar triangles), dividing by Î”t: a = v(Î”Î¸/Î”t) = vÏ‰. The direction of a is perpendicular to v — toward the center of the circle.' },
      { label: 'Substitute v = r omega', formula: 'a_c = v^{2}/r = r omega^{2}', detail: 'âœ… Substituting Ï‰ = v/r: a_c = v(v/r) = v²/r. Or a_c = (rÏ‰)Ï‰ = rÏ‰². For a car taking a curve of radius 50 m at 20 m/s: a_c = 400/50 = 8 m/s² â‰ˆ 0.8g. This is why you feel thrown outward — your inertia wants to go straight while the road pushes you centripetally!' },
    ],
    sliders: [
  ...
  kinetic_energy: {
    ...
    steps: [
      ...
      { label: 'Substitute F = ma', formula: 'W = m \\times a \\times d', detail: 'ðŸ“ Work W = F × d. From Newton\'s Second Law: F = ma. So W = (ma) × d. The force that stops you does work over the crumple zone distance.' },
      { label: 'Use Third Equation of Motion', formula: 'd = (vf^{2} - vi^{2}) / 2a', detail: 'ðŸ§® Use the third equation of motion: 2ad = vf² - vi². If stopping from vi to 0: 2ad = -vi², so d = -vi²/(2a). Substituting into W: W = ma × (-vi²/(2a)) = -½mvi². The work is negative — energy is being removed from the car.' },
      { label: 'Final Result', formula: 'KE = \\frac{1}{2} m (vf^{2} - vi^{2}) or \\frac{1}{2} mv^{2}', detail: 'âœ… KE = ½mv². Your 1500 kg car at 20 m/s: KE = ½ × 1500 × 400 = 300,000 J. That\'s enough to lift a 1000 kg car 30 m in the air! Modern crumple zones absorb this energy over about 0.5 m of deformation, requiring an average force of 600,000 N — about 60 tons of force!' },
    ],
    sliders: [
  ...
  archimedes: {
    ...
    steps: [
      ...
      { label: 'Net Upward Force', formula: 'F = F2 - F1 = rho g A (h2 - h1)', detail: 'ðŸ§® Net upward (buoyant) force: F_up = Fâ‚‚ - Fâ‚ = ÏgA(hâ‚‚ - hâ‚) = ÏgAÎ”h. But AÎ”h = V — the volume of the object! So F_up = ÏgV. This is the weight of the displaced fluid: ÏV × g.' },
  ...
  continuity: {
    ...
    steps: [
      { label: 'Mass Entering Pipe', formula: 'delta m1 = rho1 A1 v1 delta t', detail: 'ðŸŒŠ You\'re standing by a river. The river is wide and slow upstream, but narrows into a fast-flowing rapids downstream. The same amount of water must pass every point each second — water is incompressible. Mass entering = mass leaving.' },
      { label: 'Mass Leaving Pipe', formula: 'delta m2 = rho2 A2 v2 delta t', detail: 'ðŸ“ Mass entering in time Î”t: Î”mâ‚ = ÏAâ‚vâ‚Î”t. Density Ï × area Aâ‚ × velocity vâ‚ × time Î”t. This is the mass of water flowing through cross-section Aâ‚ in time Î”t.' },
      ...
      { label: 'Cancel Common Terms', formula: 'A1 v1 = A2 v2 = \\text{constant}', detail: 'âœ… Flow rate is constant: Av = constant. If the river is 10 m wide and 2 m deep (Aâ‚ = 20 m²) flowing at 1 m/s, then through a narrows of 5 m²: vâ‚‚ = 20 × 1 / 5 = 4 m/s — four times faster! This explains how garden hose nozzles work: squeezing the opening makes the water shoot farther.' },
    ],
    sliders: [
  ...
  bernoulli: {
    ...
    steps: [
      ...
      { label: 'Net Work = Change in Energy', formula: '(P1 - P2)V = (0.5mv2^2 - 0.5mv1^2) + (mgh2 - mgh1)', detail: 'ðŸ§® Net work = Change in mechanical energy: (Pâ‚ - Pâ‚‚)V = (½mvâ‚‚² - ½mvâ‚²) + (mghâ‚‚ - mghâ‚). Net work on the fluid = change in kinetic energy + change in gravitational potential energy (Work-Energy Theorem).' },
      { label: 'Divide by Volume', formula: 'P1 + 0.5 rho v1^2 + rho g h1 = P2 + 0.5 rho v2^2 + rho g h2', detail: 'âœ… Divide by volume (m = ÏV): P + ½Ïv² + Ïgh = constant along a streamline. For an airplane wing: air travels faster above (vâ‚‚ > vâ‚), so Pâ‚‚ < Pâ‚ — creating net upward force! A typical Boeing 747 wing generates about 0.5 psi pressure difference over 500 m², producing 170,000 N of lift per square meter!' },
    ],
    sliders: [
  ...
  doppler_listener_towards: {
    ...
    steps: [
      ...
      { label: 'Apparent Wavelength', formula: "lambda = v/f (unchanged)", detail: 'ðŸ“ The wavelength is determined by the stationary source: Î» = v/f = 340/440 = 0.773 m. The source isn\'t moving, so the wave pattern in air is unchanged — peaks are still 0.773 m apart.' },
      { label: 'Apparent Frequency', formula: "f' = v_relative / lambda = (v + v_L) / (v/f)", detail: 'ðŸ§® You encounter more wave peaks per second because you\'re moving toward them: f\' = v_relative / Î» = (v + vL)/(v/f) = f × (v + vL)/v. Each second, you cover the distance sound travels PLUS the distance you walk.' },
      { label: 'Final Formula', formula: "f' = (v + v_L)/v x f", detail: 'âœ… f\' = 440 × (340 + 2)/340 = 440 × 342/340 = 442.6 Hz. The pitch rises by 2.6 Hz. This is why an approaching train horn sounds higher pitched — and why police radar uses the Doppler shift of radio waves bouncing off your car to measure your speed!' },
    ],
    sliders: [
  ...
  stationary_waves_string: {
    ...
    steps: [
      ...
      { label: 'Wave Equation', formula: 'v = f_n \\times lambda_n', detail: 'ðŸ§® Wave speed v = f_n × Î»_n. The wave speed on a string depends on tension and linear density: v = âˆš(T/Î¼). For a typical guitar string: v â‰ˆ 100 m/s. Longer strings produce lower fundamental frequencies.' },
      { label: 'Frequency Formula', formula: 'f_n = n \\times v / 2L', detail: 'âœ… f_n = nv/(2L). For a guitar string L = 0.65 m, v = 100 m/s: fâ‚ = 100/(2×0.65) = 76.9 Hz. The harmonics: fâ‚‚ = 153.8 Hz, fâ‚ƒ = 230.7 Hz... These overtones give each instrument its unique timbre. The 12th fret sits exactly at the midpoint — pressing it halves the effective string length, doubling the frequency — that\'s an octave higher!' },
    ],
    sliders: [
  ...
  electric_field_point: {
    ...
    steps: [
      ...
      { label: 'Coulomb\'s Law', formula: 'F = k Q q / r^{2}', detail: 'ðŸ“ Coulomb\'s Law: the force between two charges Q and q separated by distance r is F = kQq/r², where k = 9×10â¹ N·m²/C². For the Van de Graaff dome (Q = 1×10â»â¶ C) and your hand with induced charge: the force is proportional to both charges and inversely proportional to distance squared.' },
      { label: 'Substitute F into E', formula: 'E = (k Q q / r^{2}) / q', detail: 'ðŸ§® Substituting F into E = F/q: E = (kQq/r²)/q = kQ/r². The test charge q cancels! The electric field depends only on the source charge Q, not on whatever you use to measure it. A larger Q or closer distance = stronger field.' },
      { label: 'Final Formula', formula: 'E = k Q / r^{2}', detail: 'âœ… E = kQ/r². Near the Van de Graaff dome (r = 0.1 m, Q = 1×10â»â¶ C): E = 9×10â¹ × 1×10â»â¶ / 0.01 = 9×10âµ N/C. That\'s 900,000 N/C — strong enough to ionize air molecules, creating the crackling sparks! Lightning is the same physics on a massive scale.' },
    ],
    sliders: [
  ...
  drift_velocity: {
    ...
    steps: [
      ...
      { label: 'Current = Charge / Time', formula: 'I = Q/t = (n A L e) / t', detail: 'ðŸ“ In a copper wire, there are n = 8.5×10²â¸ free electrons per cubic meter. Each has charge e = 1.6×10â»¹â¹ C. In a wire of cross-sectional area A, a length L contains total charge Q = nALe. That\'s a staggering number of electrons!' },
      { label: 'Distance / Time = Velocity', formula: 'I = n A e (L/t) = n A e v_d', detail: 'ðŸ§® Current I = Q/t = nALe/t = nAe(L/t) = nAev_d. Here v_d = L/t is the drift velocity — the average speed electrons drift along the wire. For a 1 A current in a 1 mm² copper wire: v_d = I/(nAe) = 1/(8.5×10²â¸ × 10â»â¶ × 1.6×10â»¹â¹) = 7.4×10â»âµ m/s.' },
      { label: 'Final Formula', formula: 'I = n e A v_d', detail: 'âœ… I = nAev_d. The drift velocity is only 0.074 mm/s — an electron takes over 3 hours to travel 1 meter! Yet the light turns on instantly because electrons throughout the wire are already there — like a tube filled with marbles: push one in, one immediately comes out the other end.' },
    ],
    sliders: [
  ...
  magnetic_force: {
    ...
    steps: [
      { label: 'Force on Moving Charges', formula: 'F = q v B sin theta', detail: 'ðŸ”Œ You\'re building an electric motor. A wire carrying current I placed in a magnetic field B experiences a force. This is the fundamental principle behind all electric motors — from the tiny vibration motor in your phone to the massive traction motors in electric trains.' },
      { label: 'Current = Moving Charges', formula: 'I = q/t so q = I t', detail: 'ðŸ“ A single charge q moving at velocity v through field B experiences force F = qvB sin Î¸ (perpendicular to both v and B). In a wire, many charges move together. Total charge q = I × t (current × time).' },
      { label: 'Velocity = L/t', formula: 'F = (I t) \\times (L/t) \\times B sin theta = I L B sin theta', detail: 'ðŸ§® The average velocity of charges is v = L/t (length of wire per unit time). Substituting: F = (It) × (L/t) × B sin Î¸ = ILB sin Î¸. The time cancels! The force depends on current, not on how long it flows.' },
      { label: 'Direction: Right Hand Rule', formula: 'F perpendicular to both I and B', detail: 'âœ… F = BIL sin Î¸. Maximum force when Î¸ = 90° (current perpendicular to field). For a motor coil: B = 0.5 T, I = 3 A, L = 0.2 m (per turn), N = 50 turns: F = 0.5 × 3 × (0.2×50) × 1 = 15 N. The right-hand rule gives the direction — it\'s the basis of every electric motor!' },
    ],
    sliders: [
  ...
  faraday_law: {
    ...
    steps: [
      ...
      { label: 'Magnetic Flux', formula: 'Phi = B A cos theta', detail: 'ðŸ“ Magnetic flux Î¦ = BA cos Î¸ — the amount of magnetic field passing through a surface. Think of it like the number of field lines piercing the area. A stronger field B, larger area A, or better alignment (Î¸ = 0) all increase flux.' },
      { label: 'Rate of Change', formula: 'epsilon = -N (delta Phi / delta t)', detail: 'ðŸ§® Faraday found that the induced EMF is proportional to the rate of change of flux through N turns: Îµ = -N(Î”Î¦/Î”t). If flux through a 100-turn coil changes by 0.5 Wb in 0.5 s: Îµ = -100 × 0.5/0.5 = -100 V. The faster the change, the higher the voltage!' },
  ...
  projectile_time_flight: {
    ...
    steps: [
      ...
      { label: 'Initial Vertical Velocity', formula: 'v_iy = v_i sin\u03B8', detail: 'ðŸ“ Your initial vertical velocity: v_iy = vi sin Î¸ = 9 × sin 20° = 9 × 0.342 = 3.08 m/s. This upward component is what keeps you off the ground. The higher your launch angle, the longer you stay up — but too high and you lose horizontal distance.' },
      { label: 'Displacement = 0 at Landing', formula: 'S_y = v_iy t + \u00BD a_y t\u00B2 = 0', detail: 'ðŸ§® Using the second equation of motion vertically: S_y = v_iy T + ½a_y T². At landing, S_y = 0 (same height as takeoff). a_y = -g. So 0 = (vi sin Î¸)T - ½gT². Factor T: T(vi sin Î¸ - ½gT) = 0.' },
      { label: 'Solve for T', formula: '0 = (v_i sin\u03B8)T \u2212 \u00BD g T\u00B2 \u21D2 T = 2v_i sin\u03B8/g', detail: 'âœ… Non-zero solution: T = 2vi sin Î¸ / g. For your jump: T = 2 × 9 × 0.342 / 9.81 = 6.16/9.81 = 0.628 seconds. World-class long jumpers achieve about 1 second of hang time with takeoff speeds near 10 m/s and angles around 22°. That second feels like an eternity!' },
    ],
    sliders: [
      { label: 'Initial Velocity (vi)', key: 'vi', min: 5, max: 50, step: 1, default: 20, unit: ' m/s' },
  ...
  angular_momentum: {
    ...
    steps: [
      { label: 'Definition of Angular Momentum', formula: 'L = r \u00D7 p = rmv sin90\u00B0 = rmv', detail: 'â›¸ï¸ You\'re watching an Olympic figure skater. She starts spinning with arms extended, then pulls her arms in — and suddenly spins much faster! This is conservation of angular momentum in action. Angular momentum L is the rotational equivalent of linear momentum (p = mv), and it\'s defined as L = r × p (position vector cross linear momentum).' },
      { label: 'Substitute Linear Momentum', formula: 'L = r \u00D7 (mv) = rmv', detail: 'ðŸ“ For an object moving in a circle of radius r: L = r × mv. Since r and v are perpendicular (circular motion): L = rmv. If a skater with mass 50 kg spins at radius 1 m with speed 3 m/s: L = 1 × 50 × 3 = 150 kg·m²/s.' },
      { label: 'Use v = r\u03C9', formula: 'L = r \u00D7 m \u00D7 (r\u03C9) = mr\u00B2\u03C9', detail: 'ðŸ”„ Substitute v = rÏ‰: L = r × m × (rÏ‰) = mr²Ï‰. The quantity I = mr² is called the moment of inertia — it measures how hard it is to spin an object. When the skater pulls arms in from r = 1 m to r = 0.3 m, I drops from 50 to 4.5 kg·m².' },
      { label: 'Moment of Inertia I = mr\u00B2', formula: 'L = I\u03C9', detail: 'ðŸŽ¯ Conservation of angular momentum: L = IÏ‰ = constant. If I decreases, Ï‰ must increase: Ï‰_new = Ï‰_old × (I_old/I_new). The skater\'s spin speed multiplies by 11! This also explains why neutron stars spin at hundreds of times per second — they\'re the collapsed cores of massive stars, with unimaginably small moments of inertia.' },
    ],
    sliders: [
  ...
  torque_inertia: {
    ...
    steps: [
      { label: 'Definition of Torque', formula: '\u03C4 = r F sin90\u00B0 = rF', detail: 'ðŸšª You\'re trying to open a heavy door. You instinctively push as far from the hinges as possible. Why? Torque Ï„ = rF depends on both the force F and the lever arm distance r. Pushing at the door edge (r = 0.8 m) with 20 N gives Ï„ = 16 N·m. Pushing near the hinge (r = 0.1 m) with the same force gives only Ï„ = 2 N·m — eight times less effective!' },
      { label: 'Apply Newton\u2019s Second Law', formula: '\u03C4 = r \u00D7 (ma)', detail: 'ðŸ—ï¸ Newton\'s Second Law says F = ma for linear motion. For rotation: Ï„ = rF = r(ma_t). The tangential acceleration a_t = rÎ±, where Î± is angular acceleration. Substituting: Ï„ = r × m × (rÎ±) = mr²Î±.' },
      { label: 'Use a_t = r\u03B1', formula: '\u03C4 = r \u00D7 m \u00D7 (r\u03B1) = mr\u00B2\u03B1', detail: 'ðŸ“ The quantity I = mr² is the moment of inertia — it\'s the rotational equivalent of mass. Just as heavier objects are harder to push linearly, objects with larger I are harder to spin. For the door (mass 30 kg, treat as concentrated at r = 0.4 m): I â‰ˆ 30 × 0.16 = 4.8 kg·m².' },
      { label: 'Moment of Inertia I = mr\u00B2', formula: '\u03C4 = I\u03B1', detail: 'âœ… Newton\'s Second Law for rotation: Ï„ = IÎ±. A wrench uses this — applying 50 N at 0.3 m gives Ï„ = 15 N·m. If the bolt\'s I = 0.01 kg·m², Î± = 1500 rad/s² — the bolt spins fast!' },
    ],
    sliders: [
      { label: 'Mass (m)', key: 'm', min: 0.5, max: 10, step: 0.5, default: 2, unit: ' kg' },
  ...
  artificial_gravity: {
    ...
    steps: [
      ...
      { label: 'Substitute a_c = v\u00B2/R', formula: 'v\u00B2/R = g', detail: 'ðŸ“ Centripetal acceleration a_c = v²/R. Set this equal to Earth\'s gravity: v²/R = g = 9.81 m/s². Multiply both sides by R: v² = gR. The required linear speed v = âˆš(gR). For a station with radius R = 50 m: v = âˆš(9.81 × 50) = âˆš490.5 = 22.1 m/s â‰ˆ 80 km/h.' },
      { label: 'Solve for v', formula: 'v = \u221A(gR)', detail: 'ðŸ”„ Using v = Ï‰R: Ï‰²R² = gR, so Ï‰² = g/R, Ï‰ = âˆš(g/R). For R = 50 m: Ï‰ = âˆš(9.81/50) = âˆš0.1962 = 0.443 rad/s. One full rotation takes T = 2Ï€/Ï‰ = 2Ï€/0.443 â‰ˆ 14.2 seconds.' },
  ...
  terminal_velocity: {
    ...
    steps: [
      ...
      { label: 'Stokes\u2019 Law for Drag', formula: 'F_drag = 6\u03C0\u03B7 r v_t', detail: 'ðŸ“ For a small sphere in a viscous fluid, Stokes\' Law gives drag force: F_drag = 6Ï€Î·rv_t, where Î· is fluid viscosity, r is radius. A tiny raindrop (r = 0.5 mm) in air (Î· = 1.8×10â»âµ Pa·s) experiences drag proportional to its speed and radius.' },
      { label: 'Mass of Sphere', formula: 'm = \u03C1V = \u03C1 \u00D7 (4/3)\u03C0 r\u00B3', detail: 'ðŸ§® At terminal velocity: Weight = Drag. mg = 6Ï€Î·rv_t. Mass of a sphere: m = ÏV = Ï × (4/3)Ï€r³. So Ï(4/3)Ï€r³g = 6Ï€Î·rv_t. Cancel Ï€ and one r: Ï(4/3)r²g = 6Î·v_t.' },
      { label: 'Solve for v_t', formula: 'v_t = 2\u03C1 g r\u00B2 / 9\u03B7', detail: 'âœ… Solve for v_t: v_t = 2Ïgr²/(9Î·). For a raindrop (Ï = 1000 kg/m³, r = 0.5 mm): v_t = 2×1000×9.81×(5×10â»â´)²/(9×1.8×10â»âµ) = 30.3 m/s (109 km/h). Larger drops fall faster — that\'s why drizzle floats gently while heavy rain pounds down! Skydivers: v_t â‰ˆ 55 m/s (200 km/h) in belly-down position.' },
    ],
    sliders: [
  ...
  resistance_series: {
    ...
    steps: [
      ...
      { label: 'Current is Common', formula: 'I R_eq = I (R\u2081 + R\u2082 + R\u2083)', detail: 'ðŸ§® But by Ohm\'s Law for the equivalent resistance: V_total = IR_eq. Equating: IR_eq = I(Râ‚ + Râ‚‚ + Râ‚ƒ). The current I is common to all terms — cancel it!' },
      { label: 'Cancel Current I', formula: 'R_eq = R\u2081 + R\u2082 + R\u2083', detail: 'âœ… R_eq = Râ‚ + Râ‚‚ + Râ‚ƒ. For 100 Î© + 50 Î© + 25 Î©: R_eq = 175 Î©. Series resistance always adds up — total is always larger than the largest individual. That\'s why the old Christmas lights went dark when one bulb failed: the broken bulb broke the only path for current.' },
    ],
    sliders: [
  ...
  venturi_meter: {
    ...
    steps: [
      ...
      { label: 'Pressure Difference', formula: 'P\u2081 \u2212 P\u2082 = \u00BD\u03C1(v\u2082\u00B2 \u2212 v\u2081\u00B2)', detail: 'ðŸ“ In a horizontal pipe (hâ‚ = hâ‚‚), Bernoulli simplifies: Pâ‚ + ½Ïvâ‚² = Pâ‚‚ + ½Ïvâ‚‚². Rearranging: Pâ‚ - Pâ‚‚ = ½Ï(vâ‚‚² - vâ‚²). The pressure drop between the wide and narrow sections equals the change in dynamic pressure.' },
      { label: 'Continuity Equation', formula: 'v\u2082 = v\u2081 (A\u2081/A\u2082)', detail: 'ðŸ”„ From continuity: Aâ‚vâ‚ = Aâ‚‚vâ‚‚, so vâ‚‚ = vâ‚(Aâ‚/Aâ‚‚). The fluid must speed up in the narrow section by the ratio of areas. Substitute: Pâ‚ - Pâ‚‚ = ½Ïvâ‚²((Aâ‚/Aâ‚‚)² - 1).' },
      { label: 'Manometer Reading', formula: 'P\u2081 \u2212 P\u2082 = \u03C1gh', detail: 'ðŸ“Š The pressure difference is measured by a manometer: Pâ‚ - Pâ‚‚ = Ïgh. Equating: Ïgh = ½Ïvâ‚²((Aâ‚/Aâ‚‚)² - 1). Solve: vâ‚ = âˆš(2gh/((Aâ‚/Aâ‚‚)² - 1)). This formula is used in carburetors to mix fuel with air, in perfume atomizers, and in medical nebulizers!' },
    ],
    sliders: [
  ...
  elastic_potential_energy: {
    ...
    steps: [
      { label: 'Force-Extension Graph', formula: 'Area under F-x graph = Work done', detail: 'ðŸ¹ You\'re an archer drawing a bow. The more you pull back the string, the harder it becomes to pull further — Robert Hooke discovered this in 1660: F = kx, where k is the spring constant. A typical bow has k = 200 N/m. Pulling back 0.5 m requires 100 N of force.' },
      { label: 'Work = Area of Triangle', formula: 'W = \u00BD \u00D7 F \u00D7 x', detail: 'ðŸ“ On a force-extension graph, F = kx is a straight line through the origin. The work done to stretch the spring equals the area under this line — which is a triangle! Area = ½ × base × height = ½ × x × F.' },
      { label: 'Substitute F = kx', formula: 'W = \u00BD \u00D7 (kx) \u00D7 \\times = \u00BD k x\u00B2', detail: 'ðŸ§® Substitute F = kx: W = ½ × x × kx = ½kx². If you pull your bow 0.5 m with k = 200 N/m: W = ½ × 200 × 0.25 = 25 J. That 25 joules of work is stored as elastic potential energy.' },
      { label: 'Elastic Potential Energy', formula: 'U = \u00BD k x\u00B2', detail: 'âœ… U = ½kx². When released, this energy converts to kinetic energy of the arrow: ½mv² = ½kx². An arrow of mass 0.03 kg: v = âˆš(kx²/m) = âˆš(200×0.25/0.03) = âˆš1667 = 40.8 m/s (147 km/h)! This same principle powers everything from trampolines to vehicle suspension systems.' },
    ],
    sliders: [
  ...
  work_done_gas: {
    ...
    steps: [
      ...
      { label: 'Work = Force \u00D7 Distance', formula: 'W = F \u00D7 \u0394y', detail: 'ðŸ“ The gas exerts force F = P × A on the piston. Work = force × distance: W = F × Î”y = (PA) × Î”y. The piston moves a distance Î”y as the gas expands.' },
      { label: 'Substitute Force', formula: 'W = (P \u00D7 A) \u00D7 \u0394y = P \u00D7 (A \u0394y)', detail: 'ðŸ§® Substitute: W = P × (AÎ”y). But A × Î”y = Î”V — the change in volume of the gas! So W = PÎ”V. For a car cylinder: if gas at 5×10âµ Pa expands by 0.5 L (5×10â»â´ m³): W = 5×10âµ × 5×10â»â´ = 250 J per stroke.' },
      { label: 'A\u0394y = Change in Volume', formula: 'W = P \u0394V', detail: 'ðŸ”¥ A four-cylinder engine at 3000 RPM does 4 cylinders × 2 strokes/rotation = 6000 power strokes per minute. That\'s 6000 × 250 = 1,500,000 J/min = 25,000 W â‰ˆ 33 horsepower! The negative sign (W = -PÎ”V) means work is done ON the gas during compression — like in a diesel engine\'s compression stroke.' },
    ],
    sliders: [
  ...
  cop_refrigerator: {
    ...
    steps: [
      ...
      { label: 'Work Input', formula: 'W = Q\u2081 \u2212 Q\u2082', detail: 'ðŸ“ COP (Coefficient of Performance) = Qâ‚‚/W (for cooling) — the heat removed per unit work input. A COP of 6 means you remove 6 J of heat for every 1 J of electrical work. Better than 100% \'efficiency\' because you\'re moving heat, not creating it!' },
      ...
      { label: 'Using Q \u221D T', formula: 'COP = T\u2082 / (T\u2081 \u2212 T\u2082)', detail: 'âœ… COP = 270/(310-270) = 270/40 = 6.75. For every 1 J of electricity, 6.75 J of heat is removed from your food! Your kitchen gets Qâ‚ = Qâ‚‚ + W = 6.75 + 1 = 7.75 J of heat — that\'s why the back of the fridge feels warm. Modern refrigerators achieve COP of 3-4 in practice.' },
    ],
    sliders: [
  ...
  doppler_listener_away: {
    ...
    steps: [
      { label: 'Relative Speed of Sound', formula: 'v_relative = v \u2212 v_L', detail: 'ðŸš¶ You\'re walking away from a fixed speaker playing the same 440 Hz note. Now the pitch drops! Walking away at vL = 2 m/s, the sound waves reach you slower — effective speed decreases to v - vL = 338 m/s.' },
      ...
      { label: 'Apparent Frequency', formula: "f' = v_relative / \u03BB = (v \u2212 v_L) / (v/f)", detail: 'ðŸ§® You encounter fewer wave peaks per second: f\' = v_relative / Î» = (v - vL)/(v/f) = f × (v - vL)/v. You\'re running away from the waves, so they take longer to catch up to you.' },
      { label: 'Final Formula', formula: "f' = (v \u2212 v_L)/v \u00D7 f", detail: 'âœ… f\' = 440 × (340 - 2)/340 = 440 × 338/340 = 437.4 Hz. The pitch drops by 2.6 Hz. The same shift magnitude as approaching, just in the opposite direction. This symmetry is used in Doppler echocardiography — doctors measure blood flow velocity by comparing the frequency shift of ultrasound waves reflected from moving red blood cells!' },
    ],
    sliders: [
      { label: 'Source Freq (f)', key: 'f', min: 100, max: 1000, step: 10, default: 440, unit: ' Hz' },
      { label: 'Listener Speed (vL)', key: 'vL', min: 0, max: 50, step: 1, default: 10, unit: ' m/s' },
  ...
  doppler_source_towards: {
    ...
    steps: [
      ...
      { label: 'Wave Compression', formula: '\u0394\u03BB = v_S/f', detail: 'ðŸ“ In one period T = 1/f, the source moves distance vS × T toward you while the wave travels v × T. The new wavelength: Î»\' = vT - vST = (v - vS)/f. The waves are bunched up — compressed!' },
      { label: 'Apparent Frequency', formula: "f' = v/\u03BB' = v / ((v \u2212 v_S)/f)", detail: 'ðŸ§® You hear the compressed waves at: f\' = v/Î»\' = v/((v - vS)/f) = f × v/(v - vS). Since the denominator is smaller than v, the frequency increases. The speed of sound v = 340 m/s is fixed, but the spacing between wavefronts is squeezed.' },
      { label: 'Final Formula', formula: "f' = v/(v \u2212 v_S) \u00D7 f", detail: 'âœ… Ambulance at 30 m/s: f\' = 440 × 340/(340-30) = 440 × 340/310 = 482.6 Hz. The pitch jumps by 42.6 Hz — a noticeable higher pitch! The formula breaks down when vS approaches v (the sound barrier) — at vS = v, the frequency becomes infinite, and you hear a sonic boom!' },
    ],
    sliders: [
      { label: 'Source Freq (f)', key: 'f', min: 100, max: 1000, step: 10, default: 440, unit: ' Hz' },
  ...
  stationary_waves_air_column: {
    ...
    steps: [
      ...
      { label: 'Wave Equation', formula: 'v = f_n \u03BB_n', detail: 'ðŸ§® Using v = fÎ»: f_n = nv/(2L). The speed of sound in air v = 343 m/s at 20°C. A 0.6 m flute: fâ‚ = 343/(2×0.6) = 285.8 Hz (around middle C#). Warmer air increases v, raising the pitch — that\'s why orchestras tune before performances!' },
      { label: 'Frequency Formula', formula: 'f_n = n v / 2L', detail: 'âœ… A closed pipe (one end closed) is different: it has a node at the closed end and an antinode at the open end. L = (2n-1)Î»_n/4, so f_n = (2n-1)v/(4L) — only odd harmonics! A 0.6 m closed pipe: fâ‚ = 343/(4×0.6) = 142.9 Hz (lower, hollow sound). This is why clarinets (closed pipe) sound different from flutes (open pipe)!' },
    ],
    sliders: [
  ...
  potential_gradient: {
    ...
    steps: [
      ...
      { label: 'Work = Change in PE', formula: 'W = \u2212q \u0394V', detail: 'ðŸ“ Work = force × displacement: W = FÎ”r = (qE)Î”r. Moving a charge through a uniform field E over distance Î”r requires force qE (overcoming the field\'s push).' },
      ...
      { label: 'Solve for E', formula: 'E = \u2212\u0394V/\u0394r', detail: 'âœ… E = -Î”V/Î”r. The electric field equals the negative potential gradient. For a capacitor with 12 V across 0.01 m gap: E = -12/0.01 = -1200 V/m. The negative sign means the field points from HIGH to LOW potential. Lightning forms when the potential gradient exceeds 3×10â¶ V/m!' },
    ],
    sliders: [
  ...
  maximum_power_output: {
    ...
    steps: [
      ...
      { label: 'Current with Internal Resistance', formula: 'I = \u03B5 / (R + r)', detail: 'ðŸ“ Power delivered to load R: P_out = I²R. Using Ohm\'s Law with internal resistance: I = Îµ/(R + r). So P_out = Îµ²R/(R + r)².' },
      { label: 'Substitute Current', formula: 'P_out = \u03B5\u00B2 R / (R + r)\u00B2', detail: 'ðŸ§® For R = 2 Î©: P = 400×2/(6)² = 800/36 = 22.2 W. For R = 4 Î©: P = 400×4/(8)² = 1600/64 = 25.0 W. For R = 8 Î©: P = 400×8/(12)² = 3200/144 = 22.2 W. The power is maximized when R = r!' },
      { label: 'Max When R = r', formula: 'P_max = \u03B5\u00B2 / 4r', detail: 'âœ… P_max = Îµ²/(4r). For Îµ = 20 V, r = 4 Î©: P_max = 400/16 = 25 W. This is the Maximum Power Transfer Theorem — load resistance equals source resistance for maximum power. It applies everywhere — from solar panels to wireless charging!' },
    ],
    sliders: [
  ...
  wheatstone_bridge: {
    ...
    steps: [
      ...
      { label: 'Voltage Drops in Upper Branch', formula: 'V_P = V_R (I\u2081P = I\u2082R)', detail: 'ðŸ“ At balance, no current flows through the galvanometer — points B and D are at the same potential. In the upper branch: voltage across P = voltage across R. Since B and D are at same potential: Iâ‚P = Iâ‚‚R.' },
      ...
      { label: 'Divide Equations', formula: 'P/Q = R/S \u21D2 R = PS/Q', detail: 'âœ… Divide the equations: (Iâ‚P)/(Iâ‚Q) = (Iâ‚‚R)/(Iâ‚‚S). Cancel currents: P/Q = R/S, so R = PS/Q. If P = 100 Î©, Q = 50 Î©, S = 75 Î©: R = 100×75/50 = 150 Î©. The bridge gives precision to 0.01% because it detects ZERO current!' },
    ],
    sliders: [
  ...
  force_between_conductors: {
    ...
    steps: [
      ...
      { label: 'Force on Second Wire', formula: 'F = B\u2081 I\u2082 L', detail: 'ðŸ“ A long straight wire carrying current Iâ‚ produces a magnetic field at distance r: Bâ‚ = Î¼â‚€Iâ‚/(2Ï€r), where Î¼â‚€ = 4Ï€×10â»â· T·m/A is the permeability of free space. This field circulates around the wire like concentric rings.' },
      { label: 'Substitute B\u2081', formula: 'F/L = (\u03BC\u2080 I\u2081 / 2\u03C0r) \u00D7 I\u2082 = \u03BC\u2080 I\u2081 I\u2082 / 2\u03C0r', detail: 'ðŸ§® The second wire (carrying Iâ‚‚) experiences force F = Bâ‚Iâ‚‚L in this magnetic field. Substitute Bâ‚: F/L = (Î¼â‚€Iâ‚/(2Ï€r)) × Iâ‚‚ = Î¼â‚€Iâ‚Iâ‚‚/(2Ï€r). Parallel currents attract; opposite currents repel.' },
      { label: 'Definition of Ampere', formula: '1 A is current producing 2\u00D710\u207B\u2077 N/m at r = 1 m', detail: 'âœ… F/L = Î¼â‚€Iâ‚Iâ‚‚/(2Ï€r). For Iâ‚ = Iâ‚‚ = 1 A, r = 1 m: F/L = (4Ï€×10â»â·)/(2Ï€) = 2×10â»â· N/m. This tiny force defines the ampere! In a high-voltage line with I = 1000 A, r = 0.5 m: F/L = (4Ï€×10â»â·)×10â¶/(2Ï€×0.5) = 0.4 N/m.' },
    ],
    sliders: [
  ...
  charged_particle_b_field: {
    ...
    steps: [
      ...
      { label: 'Forces Balance', formula: 'qvB = mv\u00B2 / r', detail: 'ðŸ§® Set magnetic force equal to centripetal force: qvB = mv²/r. Rearranging: r = mv/(qB). For an electron in a CRT: m = 9.11×10â»³¹ kg, v = 2×10â· m/s, B = 0.01 T: r = (9.11×10â»³¹ × 2×10â·)/(1.6×10â»¹â¹ × 0.01) = 1.14×10â»² m = 1.14 cm.' },
  ...
  velocity_selector: {
    ...
    steps: [
      { label: 'Electric Force', formula: 'F_E = qE', detail: 'ðŸ§ª You\'re operating a mass spectrometer — a device that identifies molecules by their mass. But first, you need a beam of particles all moving at the same velocity. Enter the velocity selector: crossed electric and magnetic fields that filter particles by speed.' },
      ...
      { label: 'Solve for v', formula: 'v = E/B', detail: 'âœ… When forces balance: qE = qvB, cancel q: v = E/B. Only particles with exactly this speed pass straight through. For E = 3000 N/C and B = 0.1 T: v = 3000/0.1 = 30,000 m/s. Mass spectrometers use this to identify molecules — this is how airports detect explosives!' },
    ],
    sliders: [
  ...
  doppler_source_away: {
    ...
    steps: [
      { label: 'Expanded Wavelength', formula: "\u03BB' = \u03BB + \u0394\u03BB = (v + v_S)/f", detail: 'ðŸš‘ The ambulance passes you and races away. The pitch drops dramatically. Now the source is moving away, stretching the sound waves behind it — like a boat creating longer waves behind it as it moves forward.' },
      { label: 'Wave Expansion', formula: '\u0394\u03BB = v_S/f', detail: 'ðŸ“ In one period, the source moves vS × T away from you. The wave travels v × T. The new wavelength behind: Î»\' = vT + vST = (v + vS)/f. The waves are stretched — longer wavelength means lower frequency!' },
      { label: 'Apparent Frequency', formula: "f' = v/\u03BB' = v / ((v + v_S)/f)", detail: 'ðŸ§® Apparent frequency: f\' = v/Î»\' = v/((v + vS)/f) = f × v/(v + vS). Now the denominator is larger than v, so frequency decreases. The waves are being \'left behind\' by the receding source.' },
      { label: 'Final Formula', formula: "f' = v/(v + v_S) \u00D7 f", detail: 'âœ… Ambulance at 30 m/s moving away: f\' = 440 × 340/(340+30) = 440 × 340/370 = 404.3 Hz. The pitch drops by 35.7 Hz — the classic \'wooo\' as it passes. Astronomers use the same principle to measure how fast stars and galaxies are moving toward or away from Earth!' },
    ],
    sliders: [
      { label: 'Source Freq (f)', key: 'f', min: 100, max: 1000, step: 10, default: 440, unit: ' Hz' },
      { label: 'Source Speed (vS)', key: 'vS', min: 0, max: 50, step: 1, default: 10, unit: ' m/s' },
  ...
  potentiometer: {
    ...
    steps: [
      { label: 'Potential Gradient Along Wire', formula: 'V/L = \\text{constant}', detail: 'ðŸ”‹ You\'re comparing two batteries to find the EMF of an unknown cell. You can\'t use a voltmeter directly because it draws some current and gives inaccurate readings. Instead, use a potentiometer — a long uniform wire with a sliding contact. The wire has a constant potential gradient (voltage per unit length is uniform).' },
      ...
      { label: 'Divide Equations', formula: '\u03B5\u2082/\u03B5\u2081 = l\u2082/l\u2081', detail: 'âœ… Îµâ‚‚/Îµâ‚ = lâ‚‚/lâ‚. If Îµâ‚ = 1.5 V gives lâ‚ = 60 cm, and unknown cell gives lâ‚‚ = 40 cm: Îµâ‚‚ = 1.5 × 40/60 = 1.0 V. The beauty: at balance, the cell draws ZERO current — you measure the true EMF!' },
    ],
    sliders: [
  ...
  dirac_momentum: {
    ...
    steps: [
      ...
      { label: 'Square Both Sides', formula: 'E\u00B2 = m\u00B2c\u2074 + p\u00B2c\u00B2', detail: 'ðŸ“ Einstein\'s energy-momentum relation: E² = (mc²)² + (pc)². This is the relativistic Pythagorean theorem — rest energy and momentum energy are perpendicular legs, total energy is the hypotenuse. For a stationary particle (p=0): E = mc². For a massless photon (m=0): E = pc.' },
      { label: 'Rearrange for p\u00B2c\u00B2', formula: 'p\u00B2c\u00B2 = E\u00B2 \u2212 m\u00B2c\u2074', detail: 'ðŸ§® Squared: E² = m²câ´ + p²c². Rearranging: p²c² = E² - m²câ´. Divide by c²: p² = (E² - m²câ´)/c².' },
      { label: 'Solve for p', formula: 'p = \u00B1\u221A(E\u00B2 \u2212 m\u00B2c\u2074)/c', detail: 'âœ… p = âˆš(E² - m²câ´)/c. For a 1 TeV proton (m_pc² = 0.938 GeV) at the LHC: p â‰ˆ 1 TeV/c. The ± sign Dirac noticed: E = ±âˆš(m²câ´ + p²c²). This led Dirac to predict antimatter — the positron — in 1928! Today, PET scanners in hospitals use positron annihilation to detect cancer.' },
    ],
    sliders: [
    ...
    compute: (v: any) => {
      ...
      return { traces: [{ label: 'p = âˆš(E\u00B2-m\u00B2c\u2074)/c = ', value: `âˆš(${v.E.toExponential(3)}²-(${mc2.toExponential(3)})²)/3e8` }], result: p.toExponential(3) + ' kg\u00B7m/s' };
    },
