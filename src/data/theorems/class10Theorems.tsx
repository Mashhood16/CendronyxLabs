  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
        <p className="text-xs text-blue-700 font-mono">{t("Equation:")} {a}{t("x² +")} {b}{t("x +")} {cVal} = 0<br />{t("D = b²âˆ’4ac =")} {b}²âˆ’4({a})({cVal}) = <strong>{discriminant}</strong><br />{discriminant >= 0 ? <>Roots: x = [{root1.toFixed(2)}, {root2.toFixed(2)}]</> : <span className="text-red-500">{t("No real roots")}</span>}</p>
      </div>
      ...
      {checkResult === 'incorrect' && <p className="text-xs text-red-500 flex items-center gap-1"><XCircle className="w-3 h-3" /> {t("Try x = [âˆ’b ± âˆš(b²âˆ’4ac)] / 2a")}</p>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        ...
        <div><label className="text-xs font-semibold">{t("Angle Î±")}</label><input type="range" min="1" max="179" step="1" value={alpha} onChange={e => { setAlpha(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-rose-500" /><span className="text-xs">{alpha}°</span></div>
      </div>
      <div className="bg-rose-50 rounded-lg p-3 border border-rose-200"><p className="text-xs text-rose-700 font-mono">{t("a² =")} {b}² + {c}² âˆ’ 2({b})({c}{t(") cos(")}{alpha}°)<br />{t("a =")} <strong>{aCalc.toFixed(2)}</strong></p></div>
      ...
      {checkResult === 'incorrect' && <p className="text-xs text-red-500"><XCircle className="w-3 h-3 inline" /> {t("Try a² = b² + c² âˆ’ 2bc cos Î±")}</p>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        ...
        <div><label className="text-xs font-semibold">âˆ Î±</label><input type="range" min="1" max="178" step="1" value={alpha} onChange={e => { setAlpha(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-sky-500" /><span className="text-xs">{alpha}°</span></div>
        <div><label className="text-xs font-semibold">âˆ Î²</label><input type="range" min="1" max={178-alpha} step="1" value={beta} onChange={e => { setBeta(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-sky-500" /><span className="text-xs">{beta}°</span></div>
      </div>
      <div className="bg-sky-50 rounded-lg p-3 border border-sky-200"><p className="text-xs text-sky-700 font-mono">Î³ = 180°âˆ’{alpha}°âˆ’{beta}° = {gamma}°<br />{t("b = a × sin Î² / sin Î± =")} {a} {t("× sin(")}{beta}{t("°)/sin(")}{alpha}°) = <strong>{b.toFixed(2)}</strong></p></div>
...
function HerosFormulaInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200"><p className="text-xs text-emerald-700 font-mono">S = ({a}+{b}+{c})/2 = <strong>{S.toFixed(1)}</strong><br />Î” = âˆš[{S.toFixed(1)}×{(S-a).toFixed(1)}×{(S-b).toFixed(1)}×{(S-c).toFixed(1)}] = <strong>{area.toFixed(2)}</strong></p></div>
...
function CircumRadiusInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-orange-50 rounded-lg p-3 border border-orange-200"><p className="text-xs text-orange-700 font-mono">Î” = {area.toFixed(2)}<br />R = ({a}×{b}×{c})/(4×{area.toFixed(2)}) = <strong>{R.toFixed(2)}</strong></p></div>
...
function CircleChordInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-indigo-50 rounded-lg p-3 border border-indigo-200"><p className="text-xs text-indigo-700 font-mono">{t("Half-chord = âˆš(")}{radius}²âˆ’{chordOffset}²) = {halfChord.toFixed(1)}<br />{t("Full chord =")} <strong>{(halfChord*2).toFixed(1)}</strong></p></div>
      ...
      {checkResult === 'incorrect' && <p className="text-xs text-red-500"><XCircle className="w-3 h-3 inline" /> {t("Try chord = 2âˆš(r²âˆ’d²)")}</p>}
    </div>
  );
}
...
function CyclicQuadInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div><label className="text-xs font-semibold">âˆ A</label><input type="range" min="1" max="179" step="1" value={angleA} onChange={e => { setAngleA(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-cyan-500" /><span className="text-xs">{t("âˆ A =")} {angleA}°</span></div>
      <div className="bg-cyan-50 rounded-lg p-3 border border-cyan-200"><p className="text-xs text-cyan-700 font-mono">{t("âˆ A + âˆ C =")} {angleA}° + {angleC}° = <strong>180°</strong> âœ“</p></div>
      ...
      {checkResult === 'correct' && <p className="text-xs text-emerald-600"><CheckCircle className="w-3 h-3 inline" /> {t("Correct! âˆ A+âˆ C=180°")}</p>}
      {checkResult === 'incorrect' && <p className="text-xs text-red-500"><XCircle className="w-3 h-3 inline" /> {t("Try âˆ C = 180° âˆ’ âˆ A")}</p>}
    </div>
  );
}
...
function AlternateSegmentInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div><label className="text-xs font-semibold">{t("âˆ APT")}</label><input type="range" min="10" max="80" step="1" value={angle} onChange={e => { setAngle(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-pink-500" /><span className="text-xs">{t("âˆ APT =")} {angle}°</span></div>
      <div className="bg-pink-50 rounded-lg p-3 border border-pink-200"><p className="text-xs text-pink-700 font-mono">{t("Alternate Segment Theorem: âˆ APT = âˆ ABP")}<br />{t("âˆ ABP =")} <strong>{angle}°</strong></p></div>
      ...
      {checkResult === 'correct' && <p className="text-xs text-emerald-600"><CheckCircle className="w-3 h-3 inline" /> {t("Correct! âˆ ABP =")} {angle}°</p>}
...
function SemicircleAngleInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 rounded-lg p-3 border border-blue-200"><p className="text-xs text-blue-700 font-mono">{t("âˆ APB =")} <strong>90°</strong> {t("(angle in a semicircle)")}</p></div>
...
export const CLASS10_THEOREMS: Record<string, TheoremConfig> = {
  ...
  quadratic_formula: {
    ...
    keyInsight: 'The discriminant D = b²âˆ’4ac tells you everything: D > 0 means 2 real roots (parabola crosses x-axis twice), D = 0 means 1 root (touches once), D < 0 means no real roots (never crosses). Architects use this to design arches that perfectly span a given distance!',
    steps: [
      { label: 'Start with standard form', formula: 'ax^2 + bx + c = 0', detail: 'ðŸ—ï¸ You\'re an architect designing a parabolic arch. The curve follows ax² + bx + c = 0. You need a formula that works for ANY arch shape. Start with the general quadratic equation.' },
      { label: 'Move constant term', formula: 'ax^2 + bx = -c', detail: 'Shift the constant c to the right side: ax² + bx = âˆ’c. The arch\'s vertical offset doesn\'t affect its shape — it just shifts it up or down.' },
      ...
      { label: 'Complete the square', formula: '\(x + \{b}{2a}\)^2 = \{b^2 - 4ac}{4a^2}', detail: 'Add (b/2a)² to both sides: x² + (b/a)x + (b/2a)² = (b/2a)² âˆ’ c/a. The left simplifies to (x + b/2a)², and the right becomes (b²âˆ’4ac)/(4a²).' },
      { label: 'Take square root and solve', formula: 'x = \{-b \\pm \\sqrt{b^2 - 4ac}}{2a}', detail: 'Take the square root: x + b/2a = ±âˆš(b²âˆ’4ac)/(2a). Isolate x to get the Quadratic Formula. This is the single most used formula in algebra!' },
    ],
    practice: { question: 'Solve x² âˆ’ 5x + 6 = 0 using the formula.', hint: 'a=1, b=âˆ’5, c=6. D = 25âˆ’24 = 1, x = [5±1]/2', answer: 3, tolerance: 0.5, explanation: 'x = 2 or 3. The quadratic formula finds both roots!', errorHint: 'Use x = [âˆ’b ± âˆš(b²âˆ’4ac)] / 2a' },
  ...
  vector_diff: {
    ...
    steps: [
      ...
      { label: 'Substitute coordinates', formula: '\\overrightarrow{AB} = (x_2 - x_1, y_2 - y_1)', detail: 'AB = (xâ‚‚âˆ’xâ‚, yâ‚‚âˆ’yâ‚). The components tell you the displacement in each direction. The magnitude |AB| = âˆš((xâ‚‚âˆ’xâ‚)² + (yâ‚‚âˆ’yâ‚)²) gives the straight-line distance.' },
    ],
  ...
  law_cosines: {
    ...
    steps: [
      ...
      { label: 'Apply Pythagoras in right triangle ADC', formula: 'b^2 = (c \\sin \\beta)^2 + (a - c \\cos \\beta)^2', detail: 'b² = c² \\sin²Î² + a² + c² \\cos²Î² âˆ’ 2ac \\cos Î².' },
      { label: 'Simplify using \\sin²+\\cos²=1', formula: 'b^2 = a^2 + c^2 - 2ac \\cos \\beta', detail: 'b² = a² + c²(\\sin²Î² + \\cos²Î²) âˆ’ 2ac \\cos Î² = a² + c² âˆ’ 2ac \\cos Î². When Î² = 90°, this becomes the Pythagorean Theorem!' },
    ],
    practice: { question: 'b=5, c=6, Î±=60°. Find side a.', hint: 'a² = 5² + 6² âˆ’ 2×5×6×cos 60° = 25+36âˆ’60×0.5 = 31', answer: 5.57, tolerance: 0.1, explanation: 'a = âˆš31 â‰ˆ 5.57 units.', errorHint: 'a² = b² + c² âˆ’ 2bc cos Î±' },
  ...
  law_sines: {
    ...
    steps: [
      ...
      { label: 'Equate the expressions', formula: 'c \\sin \\alpha = a \\sin \\gamma \â‡’ \{a}{\\sin \\alpha} = \{c}{\\sin \\gamma}', detail: 'Since both equal the same altitude: c \\sin Î± = a \\sin Î³ → a/\\sin Î± = c/\\sin Î³.' },
    ...
    practice: { question: 'a=7, Î±=50°, Î²=70°. Find side b.', hint: 'b = 7 × sin(70°)/sin(50°) = 7×0.9397/0.7660', answer: 8.59, tolerance: 0.1, explanation: 'b â‰ˆ 8.59 units.', errorHint: 'a/sin Î± = b/sin Î²' },
  ...
  law_tangents: {
    ...
    practice: { question: 'Î±=80°, Î²=40°. Find tan[(Î±+Î²)/2]/tan[(Î±âˆ’Î²)/2].', hint: '(80+40)/2 = 60°, (80-40)/2 = 20°. tan60°/tan20°', answer: 4.76, tolerance: 0.5, explanation: 'tan60°/tan20° â‰ˆ 1.732/0.364 â‰ˆ 4.76', errorHint: 'Use tan[(Î±+Î²)/2] / tan[(Î±âˆ’Î²)/2]' },
    interactive: LawCosinesInteractive,
  },
  half_angle_cos: {
    ...
    steps: [
      { label: 'Start with Law of Cosines', formula: '\\cos \\alpha = \{b^2 + c^2 - a^2}{2bc}', detail: 'ðŸ“Š \\cos Î± = (b²+c²âˆ’a²)/(2bc). This gives \\cos of the full angle in terms of side lengths.' },
      { label: 'Use double-angle identity', formula: '2cos^2(\\alpha/2) - 1 = \{b^2 + c^2 - a^2}{2bc}', detail: 'Since \\cos Î± = 2cos²(Î±/2) âˆ’ 1, substitute: 2cos²(Î±/2) âˆ’ 1 = (b²+c²âˆ’a²)/(2bc).' },
      { label: 'Solve for \\cos²(Î±/2)', formula: '\\cos^2(\\alpha/2) = \{S(S-a)}{bc}', detail: 'After algebra using the semi-perimeter S = (a+b+c)/2, we get \\cos²(Î±/2) = S(Sâˆ’a)/(bc).' },
    ...
    practice: { question: 'a=5, b=6, c=7. S=9. Find cos(Î±/2).', hint: 'S=9, Sâˆ’a=4, bc=42. cos(Î±/2) = âˆš(9×4/42) = âˆš(36/42)', answer: 0.926, tolerance: 0.05, explanation: 'cos(Î±/2) â‰ˆ 0.926', errorHint: 'cos(Î±/2) = âˆš[S(Sâˆ’a)/(bc)]' },
  ...
  half_angle_sin: {
    ...
    steps: [
      { label: 'Use \\cos Î± = 1 âˆ’ 2sin²(Î±/2)', formula: '1 - 2sin^2(\\alpha/2) = \{b^2 + c^2 - a^2}{2bc}', detail: 'ðŸ”„ Starting with \\cos Î± = 1 âˆ’ 2sin²(Î±/2), substitute the Law of Cosines.' },
      { label: 'Solve for \\sin²(Î±/2)', formula: '\\sin^2(\\alpha/2) = \{(S-b)(S-c)}{bc}', detail: 'After algebraic manipulation using the semi-perimeter S, we get \\sin²(Î±/2) = (Sâˆ’b)(Sâˆ’c)/(bc).' },
    ...
    practice: { question: 'a=5, b=6, c=7. S=9. Find sin(Î±/2).', hint: 'Sâˆ’b=3, Sâˆ’c=2, bc=42. sin(Î±/2) = âˆš(3×2/42) = âˆš(6/42)', answer: 0.378, tolerance: 0.05, explanation: 'sin(Î±/2) â‰ˆ 0.378', errorHint: 'sin(Î±/2) = âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)]' },
    interactive: HerosFormulaInteractive,
  },
  area_sas: {
    ...
    steps: [
      ...
      { label: 'Substitute', formula: '\\Delta = \\frac{1}{2} c \\times (b \\sin \\alpha) = \\frac{1}{2} bc \\sin \\alpha', detail: 'If Î± = 90°, \\sin 90° = 1, so Î” = ½bc — the familiar right triangle area formula!' },
    ],
    practice: { question: 'b=5, c=6, Î±=30°. Find the area.', hint: 'sin 30° = 0.5. Î” = ½×5×6×0.5', answer: 7.5, tolerance: 0.1, explanation: 'Î” = ½×5×6×sin30° = 15×0.5 = 7.5 sq units.', errorHint: 'Î” = ½ bc sin Î±' },
    interactive: LawCosinesInteractive,
  },
  area_aas: {
    ...
    steps: [
      { label: 'Start with SAS formula', formula: '\\Delta = \\frac{1}{2} ab \\sin \\gamma', detail: 'ðŸ“‹ When you know two angles (Î², Î³) and the included side a (AAS), start from Î” = ½ ab \\sin Î³.' },
      { label: 'Use Law of Sines to find b', formula: 'b = \{a \\sin \\beta}{\\sin \\alpha}', detail: 'From Law of Sines: b/\\sin Î² = a/\\sin Î± → b = a \\sin Î² / \\sin Î±.' },
      { label: 'Substitute', formula: '\\Delta = \\frac{1}{2} a^2 \{\\sin \\beta \\sin \\gamma}{\\sin \\alpha}', detail: 'Î” = ½ a × (a \\sin Î² / \\sin Î±) × \\sin Î³. This formula requires only one side length and two angles!' },
    ],
    practice: { question: 'a=7, Î²=70°, Î³=60°. Î±=50°. Find the area.', hint: 'Î” = ½×49×sin70°×sin60°/sin50°', answer: 24.5, tolerance: 1, explanation: 'Î” â‰ˆ 24.5 sq units.', errorHint: 'Î” = ½ a² sin Î² sin Î³ / sin Î±' },
    interactive: LawSinesInteractive,
  },
  heros_formula: {
    ...
    steps: [
      { label: 'Start with Î” = ½ bc \\sin Î±', formula: '\\Delta = \\frac{1}{2} bc \\sin \\alpha', detail: 'ðŸ“ You\'ve measured all three sides of a triangular garden: a=5m, b=6m, c=7m. Start with Î” = ½ bc \\sin Î±.' },
    ...
    practice: { question: 'a=5, b=12, c=13. Find S and the area.', hint: 'S = (5+12+13)/2 = 15. Î” = âˆš[15×10×3×2]', answer: 30, tolerance: 0.1, explanation: 'Î” = âˆš900 = 30. This is a 5-12-13 right triangle!', errorHint: 'Î” = âˆš[S(Sâˆ’a)(Sâˆ’b)(Sâˆ’c)]' },
    interactive: HerosFormulaInteractive,
  },
  circum_radius: {
    ...
    steps: [
      ...
      { label: 'Apply sine in right triangle', formula: '\{a}{2R} = \\sin \\alpha', detail: 'In right triangle BCD (angle in a semicircle): a/(2R) = \\sin Î± → R = a/(2 \\sin Î±).' },
      { label: 'Express in terms of sides and area', formula: '\\sin \\alpha = \{2\\Delta}{bc}', detail: 'From area formula: Î” = ½ bc \\sin Î± → \\sin Î± = 2Î”/(bc).' },
      { label: 'Substitute to get formula', formula: 'R = \{abc}{4\\Delta}', detail: 'R = a/(2 × 2Î”/(bc)) = abc/(4Î”). The circumradius relates all three sides and the area!' },
    ],
    practice: { question: 'a=5, b=6, c=7, Î”â‰ˆ14.7. Find R.', hint: 'R = (5×6×7)/(4×14.7) = 210/58.8', answer: 3.57, tolerance: 0.1, explanation: 'R â‰ˆ 3.57 units.', errorHint: 'R = abc/(4Î”)' },
  ...
  in_radius: {
    ...
    steps: [
      ...
      { label: 'Factor and simplify', formula: '\\Delta = r \· \{a+b+c}{2} = rS', detail: 'Î” = r(a+b+c)/2 = rS. So r = Î”/S.' },
    ],
  ...
  ex_radius: {
    ...
    steps: [
      ...
      { label: 'Simplify', formula: '\\Delta = \\frac{1}{2} r_1(b + c - a) = r_1(S - a)', detail: 'b + c âˆ’ a = 2S âˆ’ 2a = 2(Sâˆ’a). So Î” = ½râ‚ × 2(Sâˆ’a) = râ‚(Sâˆ’a).' },
    ],
    practice: { question: 'a=5, b=6, c=7, S=9, Î”â‰ˆ14.7. Find râ‚.', hint: 'râ‚ = 14.7 / (9âˆ’5) = 14.7/4', answer: 3.68, tolerance: 0.1, explanation: 'râ‚ â‰ˆ 3.68. Compare with inradius r â‰ˆ 2.10 — exradius is larger!', errorHint: 'râ‚ = Î”/(Sâˆ’a)' },
    interactive: InRadiusInteractive,
  },
  ...
  centre_bisects: {
    ...
    practice: { question: 'In a circle of radius 10, a chord has midpoint 6 units from centre. Find half the chord length.', hint: 'By Pythagoras: half-chord = âˆš(10²âˆ’6²)', answer: 8, tolerance: 0.5, explanation: 'Half-chord = âˆš(100âˆ’36) = âˆš64 = 8. Full chord = 16.', errorHint: 'Use Pythagoras: half-chord = âˆš(r²âˆ’d²)' },
    interactive: CircleChordInteractive,
  },
  perp_bisects: {
    ...
    steps: [
      ...
      { label: 'Prove triangles congruent', formula: '\\triangle OAC \\cong \\triangle OBC', detail: 'OA = OB (radii), OC = OC (common), âˆ OCA = âˆ OCB = 90°. So triangles are congruent by RHS. Therefore AC = BC.' },
    ],
    practice: { question: 'A chord 16 units long has distance 6 from the centre. Find the radius.', hint: 'r² = 6² + (16/2)² = 36 + 64', answer: 10, tolerance: 0.5, explanation: 'r = âˆš(36+64) = âˆš100 = 10.', errorHint: 'r² = d² + (c/2)²' },
    interactive: CircleChordInteractive,
  },
  ...
  congruent_arcs: {
    ...
    steps: [
      { label: 'Take two congruent arcs', formula: '\{Arc } AB \\cong \{Arc } CD', detail: 'ðŸŽµ Imagine two identical arcs on a circle — like equal-length curved sections of a circular railway track.' },
  ...
  equal_chords_angles: {
    ...
    practice: { question: 'Chord AB = chord CD in the same circle. If âˆ AOB = 60°, what is âˆ COD?', hint: 'Equal chords → equal central angles.', answer: 60, tolerance: 1, explanation: 'âˆ COD = 60°. Equal chords subtend equal angles at the centre.', errorHint: 'Equal chords subtend equal central angles' },
    interactive: CircleChordInteractive,
  },
  equal_angles_chords: {
    ...
    practice: { question: 'Central angles âˆ AOB = âˆ COD = 45°. Are chords AB and CD equal?', hint: 'Equal central angles → equal chords.', answer: 1, tolerance: 0.5, explanation: 'Yes, equal central angles always subtend equal chords.', errorHint: 'Equal central angles → equal chords' },
    interactive: CircleChordInteractive,
  },
  ...
  tangent_perp_radius: {
    ...
    practice: { question: 'Radius OB meets tangent at B. What is the angle between them?', hint: 'Tangent is perpendicular to radius at point of contact.', answer: 90, tolerance: 1, explanation: 'The angle is always 90°.', errorHint: 'Tangent âŸ‚ radius = 90°' },
    interactive: TangentCircleInteractive,
  },
  equal_tangents: {
    ...
    steps: [
      ...
      { label: 'Join centre to points', formula: "OB \\perp AB, OC \\perp AC", detail: 'Join OA, OB, OC. By Theorem 10.2: OB âŸ‚ AB and OC âŸ‚ AC, so âˆ OBA = âˆ OCA = 90°.' },
      { label: 'Prove triangles congruent', formula: '\\triangle OBA \\cong \\triangle OCA \{ (RHS)}', detail: 'OB = OC (radii), OA = OA (common), âˆ OBA = âˆ OCA = 90°. RHS congruence → AB = AC.' },
    ],
  ...
  alternate_segment: {
    ...
    practice: { question: 'âˆ APT = 55°. What is âˆ ABP?', hint: 'Alternate Segment Theorem: âˆ ABP = âˆ APT', answer: 55, tolerance: 1, explanation: 'âˆ ABP = 55°, the same as âˆ APT.', errorHint: 'âˆ APT = âˆ ABP (alternate segment)' },
  ...
  same_segment: {
    ...
    practice: { question: 'âˆ ACB = 40° in a segment. What is âˆ ADB if D is also in the same segment?', hint: 'Angles in the same segment are equal.', answer: 40, tolerance: 1, explanation: 'âˆ ADB = 40° since both are in the same segment.', errorHint: 'Angles in same segment are equal' },
    interactive: SemicircleAngleInteractive,
  },
  semicircle: {
    ...
    steps: [
      ...
      { label: 'Apply central angle theorem', formula: "\\angle AOB = 180^\\circ \\Rightarrow \\angle APB = \\frac\\frac{1}{2} \\times 180^\\circ = 90^\\circ", detail: 'The central angle for chord AB is âˆ AOB = 180°. By Theorem 10.7, âˆ APB = ½ × 180° = 90°.' },
      { label: 'Works for any point P', formula: '\angle APB = 90^\\circ \{ for any } P \{ on the semicircle}', detail: 'No matter where P is on the semicircle arc, âˆ APB always equals 90°!' },
    ],
    practice: { question: 'AB is a diameter. Point P is on the semicircle. What is âˆ APB?', hint: 'Angle in a semicircle is always...', answer: 90, tolerance: 1, explanation: 'âˆ APB = 90° always.', errorHint: 'Angle in a semicircle = 90°' },
    interactive: SemicircleAngleInteractive,
  },
  major_segment: {
  ...
  cyclic_quad: {
    ...
    steps: [
      ...
      { label: 'Apply central angle theorem', formula: "\\angle BAD = \\frac\\frac{1}{2}\\text{(reflex } \\angle BOD), \\angle BCD = \\frac\\frac{1}{2}\\angle BOD", detail: 'âˆ A = ½×(reflex âˆ BOD) and âˆ C = ½×(âˆ BOD). The two central angles sum to 360°.' },
      { label: 'Sum of opposite angles = 180°', formula: "\\angle A + \\angle C = \\frac\\frac{1}{2}(360^\\circ) = 180^\\circ", detail: 'âˆ A + âˆ C = ½(360°) = 180°. Similarly, âˆ B + âˆ D = 180°. Opposite angles of a cyclic quadrilateral are supplementary!' },
    ],
    practice: { question: 'In a cyclic quadrilateral, âˆ A = 80°. Find âˆ C.', hint: 'âˆ A + âˆ C = 180° (opposite angles supplementary)', answer: 100, tolerance: 1, explanation: 'âˆ C = 180° âˆ’ 80° = 100°.', errorHint: 'âˆ A + âˆ C = 180° (cyclic quadrilateral)' },
