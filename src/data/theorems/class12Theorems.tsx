...
function CalcInteractive({ onAnswer, mode }: { onAnswer: (correct: boolean) => void; mode?: string }) {
  ...
  if (mode === 'sin') {
    ...
    formula = `d/dx(sinâ»¹x) at x=0.5 = 1/âˆš(1-0.5²)`;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800">
        <p className="text-xs text-emerald-700 dark:text-emerald-300 font-mono">
          {t("âˆ«â‚áµ‡ xâ¿ dx = [xâ¿âº¹/(n+1)]â‚áµ‡")}<br />
      ...
      {checkResult === 'incorrect' && <p className="text-xs text-red-500"><XCircle className="w-3 h-3 inline" /> {t("Apply âˆ« xâ¿ = xâ¿âº¹/(n+1)")}</p>}
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div className="bg-violet-50 dark:bg-violet-900/20 rounded-lg p-3 border border-violet-200 dark:border-violet-800">
        <p className="text-xs text-violet-700 dark:text-violet-300 font-mono">
          ...
          {t("radius² =")} {gVal}² + {fVal}² âˆ’ {cVal} = {gVal * gVal + fVal * fVal - cVal}<br />
...
export const CLASS12_THEOREMS: Record<string, TheoremConfig> = {
  ...
  power_rule: {
    ...
    finalFormulaDesc: 'The derivative of xâ¿ is n·xâ¿â»¹ for any positive integer n.',
    ...
    steps: [
      { label: 'Definition of derivative', formula: "f'(x) = lim_{h \→ 0} \{f(x+h) - f(x)}{h}", detail: 'ðŸ“ˆ A rocket scientist needs to calculate the velocity of a spacecraft at every instant. Position s(t) = t³. By definition, velocity = derivative = limit of (s(t+h)âˆ’s(t))/h as h→0.' },
      { label: 'Expand using binomial theorem', formula: "(x+h)^n = x^n + nx^{n-1}h + \\frac{n(n-1)}{2!}x^{n-2}h^2 + ... + h^n", detail: 'ðŸ“ Expand (x+h)â¿ using the binomial theorem. The terms are: xâ¿ + n·xâ¿â»¹·h + n(nâˆ’1)/2!·xâ¿â»²·h² + ... + hâ¿.' },
      { label: 'Cancel xâ¿ terms', formula: "\\frac{(x+h)^n - x^n}{h} = nx^{n-1} + \\frac{n(n-1)}{2!}x^{n-2}h + ... + h^{n-1}", detail: 'ðŸ§® Subtract xâ¿ and divide by h. The xâ¿ cancels, and every remaining term contains h. Factor h: the numerator becomes h[n·xâ¿â»¹ + ... + hâ¿â»¹]. Cancel h with denominator.' },
      { label: 'Take limit as h→0', formula: "lim_{h \\to 0} nx^{n-1} + \\frac{n(n-1)}{2!}x^{n-2}h + ... + h^{n-1} = nx^{n-1}", detail: 'âœ… As h→0, all terms with h vanish, leaving only n·xâ¿â»¹. For the rocket: if s(t) = t³, velocity v(t) = 3t². At t=5 seconds: v = 3×25 = 75 m/s. The power rule makes differentiation instant!' },
    ],
    practice: { question: 'd/dx xâµ = ?', hint: 'd/dx xâ¿ = n·xâ¿â»¹, so d/dx xâµ = 5·xâ´', answer: 0, tolerance: 10, explanation: 'd/dx xâµ = 5xâ´', errorHint: 'd/dx xâ¿ = n·xâ¿â»¹' },
  ...
  sin_derivative: {
    ...
    keyInsight: 'This is the foundation of all trigonometric calculus! The derivative of sine is cosine because the slope of sin x at any point equals the value of cos x at that point. This relationship is what makes simple harmonic motion (springs, pendulums) so elegant: d²x/dt² = âˆ’Ï‰²x.',
    steps: [
      ...
      { label: 'Apply known limits', formula: "lim_{h \\to 0} \\frac{cos h - 1}{h} = 0, lim_{h \\to 0} \\frac{sin h}{h} = 1", detail: 'âœ… The two fundamental limits: (1) lim(cos h âˆ’ 1)/h = 0 (the graph of cos is flat at h=0), (2) lim(sin h)/h = 1 (the small-angle approximation). Result: d/dx sin x = sin x·0 + cos x·1 = cos x. Similarly, d/dx cos x = âˆ’sin x.' },
    ],
    practice: { question: 'd/dx sin(Ï€/3) = cos(Ï€/3) = ?', hint: 'cos(60°) = 0.5', answer: 0.5, tolerance: 0.05, explanation: 'cos(60°) = 0.5', errorHint: 'd/dx sin x = cos x' },
  ...
  inverse_trig_derivatives: {
    ...
    keyInsight: 'Inverse trig derivatives appear in many integration problems (âˆ« 1/âˆš(1âˆ’x²) dx = sinâ»¹ x + C). They\'re also essential in physics for calculating angles from ratios, like finding the launch angle of a projectile from its velocity components!',
    steps: [
      { label: 'Let y = sinâ»¹ x â‡’ x = sin y', formula: "x = sin y, \\quad -pi/2 \\le y \\le pi/2", detail: 'ðŸŽ¯ A ballistics expert measures the ratio of vertical to horizontal velocity (x) and needs the angle. If x = 0.5, then y = sinâ»¹(0.5) = 30°. Implicit differentiation finds how the angle changes with the ratio.' },
      { label: 'Differentiate implicitly', formula: "1 = cos y \\frac{dy}{dx} \\Rightarrow \\frac{dy}{dx} = \\frac{1}{cos y}", detail: 'ðŸ“ Using implicit differentiation: d/dx(x) = d/dx(sin y) → 1 = cos y · dy/dx → dy/dx = 1/cos y.' },
      { label: 'Substitute cos y = âˆš(1âˆ’sin²y)', formula: "\\frac{dy}{dx} = \\frac{1}{\\sqrt{1 - sin^2 y}} = \\frac{1}{\\sqrt{1 - x^2}}", detail: 'ðŸ§® Using the identity cos²y + sin²y = 1: cos y = âˆš(1âˆ’sin²y) = âˆš(1âˆ’x²). Therefore d/dx sinâ»¹ x = 1/âˆš(1âˆ’x²).' },
      { label: 'Same process for tanâ»¹ x', formula: "x = tan y \\Rightarrow 1 = sec^2 y \\frac{dy}{dx} \\Rightarrow \\frac{1}{sec^2 y} = \\frac{1}{1+tan^2 y} = \\frac{1}{1+x^2}", detail: 'âœ… For tanâ»¹: differentiate x = tan y → 1 = sec²y·dy/dx. Since sec²y = 1 + tan²y = 1 + x², we get d/dx tanâ»¹ x = 1/(1+x²). These formulas are the key to integrating rational functions!' },
    ],
    practice: { question: 'd/dx sinâ»¹(0) = 1/âˆš(1âˆ’0²) = ?', hint: '1/âˆš1 = 1', answer: 1, tolerance: 0.1, explanation: 'd/dx sinâ»¹(0) = 1', errorHint: 'd/dx sinâ»¹ x = 1/âˆš(1âˆ’x²)' },
  ...
  basic_integrals: {
    ...
    keyInsight: 'Integration is the inverse of differentiation. Every differentiation rule gives an integration rule! The power rule d/dx xâ¿âº¹/(n+1) = xâ¿ reverses to âˆ« xâ¿ dx = xâ¿âº¹/(n+1) + C. This is the foundation of all integral calculus.',
    steps: [
      { label: 'Reverse the power rule', formula: "\\int x^n dx = \\frac{x^{n+1}}{n+1} + c, (n neq -1)", detail: 'ðŸ—ï¸ A civil engineer calculating the volume of an irregular foundation uses integration. If the cross-sectional area is A(x) = x², the volume from x=0 to x=3 is âˆ«â‚€³ x² dx = [x³/3]â‚€³ = 9 cubic meters.' },
      ...
      { label: 'Integral of exponential aË£', formula: "\\frac{d}{dx}[\\frac{a^x}{ln a}] = \\frac{a^x ln a}{ln a} = a^x \\Rightarrow \\int a^x dx = \\frac{a^x}{ln a} + c", detail: 'ðŸ§® Differentiate aË£/ln a: the derivative is aË£·ln a / ln a = aË£. Therefore âˆ« aË£ dx = aË£/ln a + C. For the natural exponential: âˆ« eË£ dx = eË£ + C (since ln e = 1).' },
      { label: 'Linearity of integration', formula: "\\int [f(x) + g(x)] dx = \\int f(x) dx + \\int g(x) dx, \\int kf(x) dx = k \\int f(x) dx", detail: 'âœ… Integration is linear: the integral of a sum is the sum of integrals, and constants factor out. This is why âˆ« (3x² + 2x + 1) dx = 3·x³/3 + 2·x²/2 + x + C = x³ + x² + x + C.' },
    ],
    practice: { question: 'âˆ« x³ dx = ? (Evaluate at x=2 minus x=0)', hint: 'âˆ« x³ dx = xâ´/4. From 0 to 2: 2â´/4 = 16/4 = 4', answer: 4, tolerance: 0.5, explanation: 'âˆ«â‚€² x³ dx = 2â´/4 âˆ’ 0â´/4 = 4', errorHint: 'âˆ« xâ¿ dx = xâ¿âº¹/(n+1)' },
  ...
  substitution_integrals: {
    ...
    keyInsight: 'Trigonometric substitution transforms square roots into trigonometric functions! âˆš(a²âˆ’x²) → x = a sin Î¸ → a cos Î¸, eliminating the radical. This technique was used by Newton to derive the area of a circle: âˆ« âˆš(a²âˆ’x²) dx gives the area of a circular segment!',
    steps: [
      { label: 'Substitute x = a sin Î¸', formula: "x = a sin theta, dx = a cos theta d theta", detail: 'ðŸ“ An astronomer is calculating the area of a circular telescope mirror of radius a. The integral âˆ« âˆš(a²âˆ’x²) dx appears. Substitute x = a sin Î¸, which converts the circle’s equation into a trigonometric form.' },
      { label: 'Simplify the square root', formula: "\\sqrt{a^2 - x^2} = \\sqrt{a^2 - a^2 sin^2 theta} = a cos theta", detail: 'ðŸ§® âˆš(a²âˆ’x²) = âˆš(a²âˆ’a²sin²Î¸) = aâˆš(1âˆ’sin²Î¸) = a cos Î¸. The integral becomes âˆ« a cos Î¸ · a cos Î¸ dÎ¸ = a² âˆ« cos²Î¸ dÎ¸.' },
      { label: 'Integrate using half-angle', formula: "a^2 \\int \\frac{1+cos 2theta}{2} dtheta = \\frac{a^2}{2}(theta + \\frac{sin 2theta}{2}) + c", detail: 'ðŸ“ Use cos²Î¸ = (1+cos 2Î¸)/2. Integrate: a²[Î¸/2 + sin 2Î¸/4] + C = (a²/2)Î¸ + (a²/4) sin 2Î¸ + C.' },
      { label: 'Convert back to x', formula: "theta = sin^{-1}\\frac{x}{a}, sin 2theta = 2 sin theta cos theta = 2frac{x}{a}\\frac{\\sqrt{a^2 - x^2}}{a}", detail: 'âœ… Substituting back: âˆ«âˆš(a²âˆ’x²)dx = (a²/2)sinâ»¹(x/a) + (xâˆš(a²âˆ’x²))/2 + C. This formula gives the area of a circular segment of height x — used in civil engineering for water flow in partially filled pipes!' },
    ],
    practice: { question: 'âˆ«â‚€³ 1/âˆš(9âˆ’x²) dx = sinâ»¹(3/3) âˆ’ sinâ»¹(0/3) = ?', hint: 'sinâ»¹(1) = Ï€/2 â‰ˆ 1.57', answer: 1.57, tolerance: 0.1, explanation: 'âˆ«â‚€³ 1/âˆš(9âˆ’x²) dx = sinâ»¹(1) âˆ’ sinâ»¹(0) = Ï€/2 â‰ˆ 1.57', errorHint: 'Use the formula âˆ« 1/âˆš(a²âˆ’x²) dx = sinâ»¹(x/a) + C' },
    interactive: IntegralInteractive,
  },
  ...
  fundamental_theorem_calc: {
    ...
    steps: [
      ...
      { label: "Show A'(x) = f(x)", formula: "A'(x) = lim_{h \\to 0} \\frac{A(x+h) - A(x)}{h} = f(x)", detail: 'ðŸ“ By definition: A\'(x) = lim[A(x+h)âˆ’A(x)]/h as h→0. But A(x+h)âˆ’A(x) = âˆ«â‚“Ë£âºÊ° f(t) dt â‰ˆ f(x)·h by the Mean Value Theorem. So A\'(x) = f(x). The area function is an antiderivative of f!' },
      ...
      { label: 'Final result', formula: "int_a^b f(x) dx = A(b) = F(b) - F(a)", detail: 'âœ… But A(a) = âˆ«â‚áµƒ f(t) dt = 0, so F(b)âˆ’F(a) = A(b) = âˆ«â‚áµ‡ f(x) dx. The definite integral equals the antiderivative evaluated at the bounds. The river delta volume: if A\'(x) = 2x (linear increase), volume from x=0 to x=5 = [x²]â‚€âµ = 25 cubic meters!' },
    ],
    practice: { question: 'âˆ«â‚³ (2x) dx. Find F(3) âˆ’ F(1) where F(x) = x².', hint: 'F(3) = 9, F(1) = 1, so 9 âˆ’ 1 = 8', answer: 8, tolerance: 0.5, explanation: 'F(3)âˆ’F(1) = 9âˆ’1 = 8.', errorHint: 'FTC: âˆ«â‚áµ‡ f(x) dx = F(b)âˆ’F(a)' },
  ...
  concurrency_altitudes: {
    ...
    steps: [
      ...
      { label: 'Slope of BC → altitude AD', formula: "m_{BC} = \\frac{y_3 - y_2}{x_3 - x_2} \\Rightarrow m_{AD} = -\\frac{x_3 - x_2}{y_3 - y_2}", detail: 'ðŸ“ The slope of BC is (yâ‚ƒâˆ’yâ‚‚)/(xâ‚ƒâˆ’xâ‚‚). Altitude AD âˆ¦ BC, so its slope is the negative reciprocal: m_AD = âˆ’(xâ‚ƒâˆ’xâ‚‚)/(yâ‚ƒâˆ’yâ‚‚).' },
  ...
  concurrency_bisectors: {
    ...
    steps: [
      { label: 'Midpoint and slope of BC', formula: "M_{BC} = (\\frac{x_2+x_3}{2}, \\frac{y_2+y_3}{2}), m_{\\perp} = -\\frac{x_3-x_2}{y_3-y_2}", detail: 'ðŸ“ GPS triangulation: Three satellite positions A, B, C are known. To find your location, construct the perpendicular bisectors of AB and BC — their intersection is your position (circumcenter)!' },
  ...
  concurrency_medians: {
    ...
    steps: [
      { label: 'Median AD: vertex A to midpoint of BC', formula: "M_{BC} = (\\frac{x_2 + x_3}{2}, \\frac{y_2 + y_3}{2})", detail: 'âš–ï¸ An architect is designing a triangular steel truss for a bridge roof. The center of mass (centroid) is where the medians intersect — this is where the main support column should be placed.' },
      ...
      { label: 'Determinant of coefficients = 0', formula: "R_1 \\to R_1 + R_2 + R_3 \\Rightarrow |D| = 0", detail: 'âœ… Write all three median equations. Their coefficient determinant becomes zero after adding rows, proving concurrency. The intersection point G = ((xâ‚+xâ‚‚+xâ‚ƒ)/3, (yâ‚+yâ‚‚+yâ‚ƒ)/3) — the centroid. This is also the average of the vertices, making it the center of mass!' },
    ],
  ...
  homogeneous_second_degree: {
    ...
    keyInsight: 'A homogeneous equation ax² + 2hxy + by² = 0 has all terms of degree 2. It always factors into two linear equations, meaning it represents two straight lines through the origin. If h² âˆ’ ab > 0, the lines are distinct and real — used in optics to analyze light cone sections!',
    steps: [
      { label: 'Multiply both sides by a', formula: "a^2 x^2 + 2ahxy + aby^2 = 0", detail: 'ðŸ”¦ A physicist is analyzing a beam of light spreading from a point source. The light cone is described by a homogeneous equation: ax² + 2hxy + by² = 0. The goal is to factor it into two distinct rays.' },
      { label: 'Add and subtract h²y²', formula: "a^2x^2 + 2ahxy + h^2y^2 - h^2y^2 + aby^2 = 0", detail: 'ðŸ“ The key insight: complete a perfect square by adding and subtracting h²y².' },
      { label: 'Condense into perfect square', formula: "(ax + hy)^2 - y^2(h^2 - ab) = 0", detail: 'ðŸ§® The first three terms form (ax + hy)². The remaining terms become âˆ’y²(h²âˆ’ab).' },
      { label: 'Factor as difference of squares', formula: "(ax + hy + ysqrt{h^2-ab})(ax + hy - ysqrt{h^2-ab}) = 0", detail: 'âœ… As a difference of squares: (ax + hy)² âˆ’ (yâˆš(h²âˆ’ab))² = 0. This factors into two linear equations: ax + y(h ± âˆš(h²âˆ’ab)) = 0. Both pass through the origin (no constant term). If h² > ab: two distinct real lines. If h² = ab: one repeated line. If h² < ab: complex (imaginary) lines.' },
    ],
    practice: { question: 'Does x² âˆ’ y² = 0 factor into two lines?', hint: 'x² âˆ’ y² = (xâˆ’y)(x+y) = 0 gives lines x=y and x=âˆ’y', answer: 1, tolerance: 0.5, explanation: 'Yes! x² âˆ’ y² = (xâˆ’y)(x+y) = 0 is two lines through the origin: x = y and x = âˆ’y.', errorHint: 'Check if h² âˆ’ ab > 0' },
    interactive: ConcurrencyInteractive,
  },
  ...
  circle_general: {
    ...
    finalFormulaDesc: 'The general equation represents a circle with center (âˆ’g, âˆ’f) and radius âˆš(g²+f²âˆ’c).',
    keyInsight: 'By completing the square, any equation x² + y² + 2gx + 2fy + c = 0 can be rewritten as a circle. If g²+f²âˆ’c > 0, it’s a real circle; if equal to 0, it’s a point circle; if negative, the equation has no real graph. This is how GPS receivers determine your position from three circle equations!',
    steps: [
      { label: 'Start with general equation', formula: "x^2 + y^2 + 2gx + 2fy + c = 0", detail: 'ðŸ—ºï¸ A GPS receiver detects signals from three satellites. Each satellite’s signal defines a circle: x² + y² + 2gx + 2fy + c = 0. The receiver needs to find the center and radius of each circle to triangulate your position.' },
      { label: 'Group x and y terms', formula: "(x^2 + 2gx) + (y^2 + 2fy) = -c", detail: 'ðŸ“ Separate the x and y terms, moving c to the right side.' },
      { label: 'Complete the square for x and y', formula: "(x^2 + 2gx + g^2) - g^2 + (y^2 + 2fy + f^2) - f^2 = -c", detail: 'ðŸ§® Add g² (to complete (x+g)²) and f² (to complete (y+f)²). Balance by subtracting g² and f² on the same side.' },
      { label: 'Rewrite in standard form', formula: "(x+g)^2 + (y+f)^2 = g^2 + f^2 - c", detail: 'âœ… This matches the standard circle form (xâˆ’h)² + (yâˆ’k)² = r² where h = âˆ’g, k = âˆ’f, r = âˆš(g²+f²âˆ’c). The center is (âˆ’g, âˆ’f) and radius is âˆš(g²+f²âˆ’c). For a real circle, we need g²+f²âˆ’c > 0!' },
    ],
    practice: { question: 'x²+y²+4x+6y+4=0. Find the center x-coordinate.', hint: 'Center = (âˆ’g, âˆ’f) = (âˆ’2, âˆ’3). x = âˆ’2', answer: -2, tolerance: 0.5, explanation: 'g=2, f=3, c=4. Center = (âˆ’2, âˆ’3). Radius = âˆš(4+9âˆ’4) = 3.', errorHint: 'Complete the square: center = (âˆ’g, âˆ’f)' },
  ...
  circle_tangent: {
    ...
    keyInsight: 'The tangent to a circle at a point is perpendicular to the radius at that point. The formula xâ‚x + yâ‚y + g(x + xâ‚) + f(y + yâ‚) + c = 0 is derived by applying the slope condition. This is known as "T = 0" — replace x² → xâ‚x, y² → yâ‚y, x → (x+xâ‚)/2, y → (y+yâ‚)/2!',
    steps: [
      ...
      { label: 'Solve for dy/dx', formula: "\\frac{dy}{dx} = -\\frac{x+g}{y+f}", detail: 'ðŸ“ Differentiate implicitly: 2x + 2y·y\' + 2g + 2f·y\' = 0. Solve for y\': dy/dx = âˆ’(x+g)/(y+f).' },
      ...
      { label: 'Rearrange to T = 0 form', formula: "(y_1+f)(y - y_1) = -(x_1+g)(x - x_1) \\Rightarrow x_1x + y_1y + g(x + x_1) + f(y + y_1) + c = 0", detail: 'âœ… Cross-multiply and rearrange. The result is the elegant T = 0 formula: replace x² → xâ‚x, y² → yâ‚y, 2gx → g(x+xâ‚), 2fy → f(y+yâ‚), c → c. The tangent equation is written without computing the derivative explicitly!' },
    ],
    practice: { question: 'Circle x²+y²=25 at point (3,4). Tangent slope = âˆ’xâ‚/yâ‚ = âˆ’3/4 = ?', hint: 'âˆ’3/4 = âˆ’0.75', answer: -0.75, tolerance: 0.05, explanation: 'Slope = âˆ’3/4 = âˆ’0.75. The tangent equation is 3x+4y=25.', errorHint: 'Slope = âˆ’(xâ‚+g)/(yâ‚+f)' },
  ...
  parabola_standard: {
    ...
    steps: [
      ...
      { label: 'Place focus at (a, 0) and directrix x = âˆ’a', formula: "\\sqrt{(x-a)^2 + y^2} = |x + a|", detail: 'ðŸ“ Choose coordinate axes so the focus F = (a, 0) and the directrix is x = âˆ’a. The distance from P(x,y) to F is âˆš[(xâˆ’a)²+y²]. The distance to the directrix is |xâˆ’(âˆ’a)| = |x + a|.' },
      { label: 'Square both sides', formula: "(x-a)^2 + y^2 = (x+a)^2", detail: 'ðŸ§® Square both sides (both are non-negative): (xâˆ’a)² + y² = (x+a)².' },
      { label: 'Simplify to y² = 4ax', formula: "x^2 - 2ax + a^2 + y^2 = x^2 + 2ax + a^2 \\Rightarrow y^2 = 4ax", detail: 'âœ… Expand: x²âˆ’2ax+a²+y² = x²+2ax+a². Cancel x² and a² on both sides. Solve: y² = 4ax. This is the standard equation of a parabola opening to the right. For satellite dishes: the receiver is at the focus (a, 0) and the dish follows y² = 4ax!' },
    ],
  ...
  parabola_tangent: {
    ...
    steps: [
      { label: 'Substitute line into parabola', formula: "y = mx + c \\Rightarrow (mx + c)^2 = 4ax", detail: 'ðŸŽ¯ An architect is designing a parabolic arch and needs to find the tangent line at a point to calculate the support beam angle. Substitute the line equation y = mx + c into the parabola y² = 4ax.' },
      { label: 'Expand to quadratic in x', formula: "m^2x^2 + 2mcx + c^2 - 4ax = 0 \\Rightarrow m^2x^2 + 2(mc - 2a)x + c^2 = 0", detail: 'ðŸ“ Expand: m²x² + 2mcx + c² = 4ax. Rearrange: m²x² + 2(mcâˆ’2a)x + c² = 0. This is a quadratic in x.' },
      { label: 'Set discriminant = 0 for tangency', formula: "D = 4(mc-2a)^2 - 4m^2c^2 = 0", detail: 'ðŸ§® For tangency, the line must intersect at exactly one point. This means the quadratic has a double root: its discriminant D = b²âˆ’4ac must be zero.' },
      { label: 'Solve for c', formula: "4(mc-2a)^2 - 4m^2c^2 = 0 \\Rightarrow -16amc + 16a^2 = 0 \\Rightarrow mc = a \\Rightarrow c = \\frac{a}{m}", detail: 'âœ… Expand: 4(m²c²âˆ’4amc+4a²) âˆ’ 4m²c² = 0 → âˆ’16amc + 16a² = 0 → mc = a → c = a/m. The tangent equation in slope form: y = mx + a/m. This formula allows architects to find the support beam angle at any point on a parabolic arch!' },
    ],
  ...
  ellipse_standard: {
    ...
    finalFormulaDesc: 'An ellipse with foci at (±c, 0) where c² = a²âˆ’b².',
    keyInsight: 'An ellipse is the set of points where the sum of distances to two foci is constant (2a). This is why planetary orbits are elliptical with the Sun at one focus (Kepler’s First Law). The flattening depends on eccentricity e = c/a = âˆš(1âˆ’b²/a²)!',
    steps: [
      { label: 'Definition: sum of distances to foci = 2a', formula: "|PF_1| + |PF_2| = 2a", detail: 'ðŸŒ Johannes Kepler discovered that planets orbit the Sun in elliptical paths. The Sun is at one focus. The sum of distances from any point on the ellipse to the two foci is constant — the length of the major axis.' },
      { label: 'Set foci at (±c, 0) and point P(x, y)', formula: "\\sqrt{(x+c)^2 + y^2} + \\sqrt{(x-c)^2 + y^2} = 2a", detail: 'ðŸ“ Place foci at Fâ‚(âˆ’c, 0) and Fâ‚‚(c, 0). For any point P(x,y) on the ellipse: âˆš[(x+c)²+y²] + âˆš[(xâˆ’c)²+y²] = 2a.' },
      { label: 'Isolate and square twice', formula: "a \\sqrt{(x-c)^2 + y^2} = a^2 - cx \\Rightarrow (a^2-c^2)x^2 + a^2y^2 = a^2(a^2-c^2)", detail: 'ðŸ§® Isolate one radical, square both sides, isolate the remaining radical, square again. After simplifying: (a²âˆ’c²)x² + a²y² = a²(a²âˆ’c²).' },
      { label: 'Let b² = a²âˆ’c² to get standard form', formula: "b^2x^2 + a^2y^2 = a^2b^2 \\Rightarrow \\frac{x^2}{a^2} + \\frac{y^2}{b^2} = 1", detail: 'âœ… Define b² = a²âˆ’c². Then b²x² + a²y² = a²b² → x²/a² + y²/b² = 1. For Earth’s orbit: a â‰ˆ 149.6 million km, b â‰ˆ 149.5 million km (nearly circular, e â‰ˆ 0.017). Mars has e â‰ˆ 0.093 — noticeably more elliptical!' },
    ],
    practice: { question: 'Ellipse x²/25 + y²/9 = 1. Find c (distance from center to focus).', hint: 'a²=25, b²=9. c² = a²âˆ’b² = 25âˆ’9 = 16. c = 4', answer: 4, tolerance: 0.5, explanation: 'a=5, b=3, c=âˆš(25âˆ’9)=4.', errorHint: 'c² = a²âˆ’b²' },
  ...
  hyperbola_standard: {
    ...
    steps: [
      ...
      { label: 'Set foci at (±c, 0) and point P(x, y)', formula: "\\sqrt{(x+c)^2 + y^2} - \\sqrt{(x-c)^2 + y^2} = 2a", detail: 'ðŸ“ The absolute difference of distances to Fâ‚(âˆ’c,0) and Fâ‚‚(c,0) equals 2a. Choose the branch where PFâ‚ > PFâ‚‚.' },
      ...
      { label: 'Let b² = c²âˆ’a² to get standard form', formula: "b^2x^2 - a^2y^2 = a^2b^2 \\Rightarrow \\frac{x^2}{a^2} - \\frac{y^2}{b^2} = 1", detail: 'âœ… Define b² = c²âˆ’a². The standard form x²/a² âˆ’ y²/b² = 1 has two branches (left and \). Asymptotes: y = ±(b/a)x. The LORAN ship: if stations are 100km apart (2c=100) and the constant difference is 60km (2a=60), then b² = 50²âˆ’30² = 1600, and the ship lies on x²/900 âˆ’ y²/1600 = 1!' },
    ],
    practice: { question: 'Hyperbola x²/16 âˆ’ y²/9 = 1. Find c (focus distance from center).', hint: 'a²=16, b²=9. c² = a²+b² = 16+9 = 25. c = 5', answer: 5, tolerance: 0.5, explanation: 'a=4, b=3, c=âˆš(16+9)=5.', errorHint: 'For hyperbola, c² = a²+b²' },
