...
function ProductLogInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="bg-teal-50 dark:bg-teal-900/20 rounded-lg p-3 border border-teal-200 dark:border-teal-800">
      <p className="text-xs text-teal-700 dark:text-teal-300 font-mono">{t("log")}<sub>{b}</sub>({m}) = {logM.toFixed(4)}<br />{t("log")}<sub>{b}</sub>({n}) = {logN.toFixed(4)}<br />{t("log")}<sub>{b}</sub>({m} × {n}{t(") = log")}<sub>{b}</sub>({m * n}) = <strong>{logMN.toFixed(4)}</strong><br />{t("Sum:")} <strong>{(logM + logN).toFixed(4)}</strong> {t("= log")}<sub>{b}</sub>({m * n}) âœ“</p>
    </div>
...
function PowerLogInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="bg-rose-50 dark:bg-rose-900/20 rounded-lg p-3 border border-rose-200 dark:border-rose-800">
      <p className="text-xs text-rose-700 dark:text-rose-300 font-mono">{t("log")}<sub>{b}</sub>({m}) = {logM.toFixed(4)}<br />{t("log")}<sub>{b}</sub>({m}<sup>{n}</sup>{t(") = log")}<sub>{b}</sub>({Math.pow(m, n)}) = <strong>{logMN.toFixed(4)}</strong><br />{t("n × log")}<sub>{b}</sub>{t("(m) =")} {n} × {logM.toFixed(4)} = <strong>{(n * logM).toFixed(4)}</strong> âœ“</p>
    </div>
...
function IntersectionAssociativeInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-3 border border-indigo-200 dark:border-indigo-800">
      <p className="text-xs text-indigo-700 dark:text-indigo-300"><strong>{t("Theorem:")}</strong> {t("(A âˆ© B) âˆ© C = A âˆ© (B âˆ© C)")}<br /><span className="italic mt-1 block">{t("Logical AND is associative — just like (P AND Q) AND R = P AND (Q AND R). Grouping doesn't change which elements are in all three sets.")}</span></p>
    </div>
  </div>);
}
...
function DistributeInterUnionInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="bg-sky-50 dark:bg-sky-900/20 rounded-lg p-3 border border-sky-200 dark:border-sky-800">
      <p className="text-xs text-sky-700 dark:text-sky-300"><strong>{t("Theorem:")}</strong> {t("A âˆ© (B âˆª C) = (A âˆ© B) âˆª (A âˆ© C)")}<br /><span className="italic mt-1 block">{t("An element is in A âˆ© (B âˆª C) if it's in A AND (in B OR C). This distributes to (Aâˆ©B) OR (Aâˆ©C) — just like the distributive law in algebra!")}</span></p>
    </div>
  </div>);
}
...
function PythagoreanIdentityInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    <div><label className="text-xs font-semibold text-slate-600 dark:text-slate-300">{t("Angle Î¸")}</label><input type="range" min="0" max="360" step="1" value={angle} onChange={e => { setAngle(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-emerald-500" /><span className="text-sm font-mono text-emerald-600">Î¸ = {angle}°</span></div>
    ...
    <div className="flex gap-2"><input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t("sin²Î¸ + cos²Î¸ = ?")} className="flex-1 px-3 py-2 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-emerald-500 outline-none" /><button onClick={handleCheck} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-colors">{t("Check")}</button></div>
    {checkResult === 'correct' && <p className="text-xs text-emerald-600"><CheckCircle className="w-3 h-3 inline" /> {t("Correct! sin²Î¸ + cos²Î¸ = 1 always!")}</p>}
...
function DistanceFormulaInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="bg-violet-50 dark:bg-violet-900/20 rounded-lg p-3 border border-violet-200 dark:border-violet-800"><p className="text-xs text-violet-700 dark:text-violet-300 font-mono">{t("d = âˆš((")}{x2} âˆ’ {x1})² + ({y2} âˆ’ {y1})²) = âˆš({dx}² + {dy}²) = âˆš{dx*dx + dy*dy} = <strong>{distance.toFixed(2)}</strong></p></div>
    ...
    {checkResult === 'incorrect' && <p className="text-xs text-red-500"><XCircle className="w-3 h-3 inline" /> {t("Try d = âˆš((xâ‚‚-xâ‚)² + (yâ‚‚-yâ‚)²)")}</p>}
  </div>);
}
...
function NormalFormInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    <div className="grid grid-cols-2 gap-3">
      <div><label className="text-xs font-semibold">{t("Î± (inclination)")}</label><input type="range" min="0" max="90" step="1" value={alpha} onChange={e => { setAlpha(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-orange-500" /><span className="text-xs">{alpha}°</span></div>
...
function AngleBetweenLinesInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="bg-rose-50 dark:bg-rose-900/20 rounded-lg p-3 border border-rose-200 dark:border-rose-800"><p className="text-xs text-rose-700 dark:text-rose-300 font-mono">{t("tan Î¸ = |(")}{m2}âˆ’{m1})/(1+{m1}×{m2})| = <strong>{thetaDeg.toFixed(1)}°</strong></p></div>
    ...
    {checkResult === 'correct' && <p className="text-xs text-emerald-600"><CheckCircle className="w-3 h-3 inline" /> {t("Correct! Î¸ =")} {thetaDeg.toFixed(1)}°</p>}
...
export const CLASS9_THEOREMS: Record<string, TheoremConfig> = {
  product_log: {
    ...
    practice: { question: 'logâ‚‚(4) = 2, logâ‚‚(8) = 3. Find logâ‚‚(32).', hint: '32 = 4 × 8. Use product law: logâ‚‚(4) + logâ‚‚(8)', answer: 5, tolerance: 0.1, explanation: 'logâ‚‚(32) = 5. Indeed, 2âµ = 32!', errorHint: 'log_b(mn) = log_b m + log_b n' },
  ...
  quotient_log: {
    ...
    keyInsight: 'pH = âˆ’log[Hâº] uses this: a solution at pH 3 is 100× more acidic than pH 5, because 10^(5âˆ’3) = 10² = 100 on the logarithmic scale.',
    steps: [
  ...
  power_log: {
    ...
    keyInsight: 'Astronomers love logs: the distance to the farthest galaxies is 10²â¶ meters — logâ‚â‚€(10²â¶) = 26 × logâ‚â‚€(10) = 26. A 26 on the log scale is easy to write and compare!',
    steps: [
      { label: 'Set up the equation', formula: 'log_b m = x', detail: 'ðŸ’° You\'re an investor watching your money grow. Your initial investment of m = $3,000 grows at b = 2× per year. After n = 4 years, your money multiplies by 2^4. Let log_b m = x, meaning b^x = m.' },
      { label: 'Convert to exponential', formula: 'm = b^x', detail: 'Convert m = b^x. Your initial $3,000 = 2^x means x = logâ‚‚(3000) â‰ˆ 11.55 — the "doubling power" of your investment.' },
    ...
    practice: { question: 'logâ‚‚(3) â‰ˆ 1.585. Find logâ‚‚(81).', hint: '81 = 3â´. Use power law: 4 × logâ‚‚(3)', answer: 6.34, tolerance: 0.1, explanation: 'logâ‚‚(81) = 4 × 1.585 = 6.34. The power pulls down in front!', errorHint: 'log_b(m^n) = n log_b m' },
  ...
  change_base: {
    ...
    steps: [
      ...
      { label: 'Apply the Power Law', formula: 'log_a m = x \· log_a b \â‡’ x = \{log_a m}{log_a b}', detail: 'log_a m = x · log_a b. The exponent x "pulls down" in front. Now isolate x: x = log_a m / log_a b.' },
      { label: 'Substitute back', formula: 'log_b m = \{log_a m}{log_a b}', detail: 'Since x = log_b m, we get: log_b m = log_a m / log_a b. So logâ‚‚(8) = logâ‚â‚€(8) / logâ‚â‚€(2) = 0.9031 / 0.3010 = 3. Indeed, 2³ = 8.' },
    ],
  ...
  union_associative: {
    ...
    steps: [
      ...
      { label: 'Re-group the ORs', formula: '(x \\in A \{ OR } x \\in B) \{ OR } x \\in C = x \\in A \{ OR } (x \\in B \{ OR } x \\in C)', detail: 'The logical OR is associative — (P OR Q) OR R = P OR (Q OR R). So (x âˆˆ A OR x âˆˆ B) OR x âˆˆ C = x âˆˆ A OR (x âˆˆ B OR x âˆˆ C). The school fair doesn\'t care how you group the teams!' },
  ...
  distributive_union_inter: {
    ...
    steps: [
      ...
      { label: 'Convert back to sets', formula: 'A \\cup (B \\cap C) = (A \\cup B) \\cap (A \\cup C)', detail: '(x âˆˆ A âˆª B) AND (x âˆˆ A âˆª C) means x âˆˆ (A âˆª B) âˆ© (A âˆª C). So A âˆª (B âˆ© C) âŠ† (A âˆª B) âˆ© (A âˆª C). The reverse containment also holds. Just like 2×(3+4) = 2×3 + 2×4!' },
    ],
  ...
  quotient_identities: {
    ...
    keyInsight: 'On a unit circle, the tangent at angle Î¸ is the y-coordinate divided by the x-coordinate — the slope of the radius line!',
    steps: [
      ...
      { label: 'Divide sin by cos', formula: '\{sin theta}{cos theta} = \{a/c}{b/c} = \{a}{b} = tan theta', detail: 'sin Î¸ / cos Î¸ = (a/c) / (b/c) = (a/c) × (c/b) = a/b. The hypotenuse (c) cancels out! And a/b is exactly tan Î¸.' },
    ...
    practice: { question: 'sin(30°) = 0.5, cos(30°) â‰ˆ 0.866. Find tan(30°).', hint: 'tan(30°) = sin(30°)/cos(30°) = 0.5/0.866', answer: 0.577, tolerance: 0.05, explanation: 'tan(30°) = 0.577. Matches sin/cos perfectly!', errorHint: 'tan Î¸ = sin Î¸ / cos Î¸' },
  ...
  pythagorean_identities: {
    ...
    keyInsight: 'From this one identity, we can derive: sec²Î¸ âˆ’ tan²Î¸ = 1 and cosec²Î¸ âˆ’ cot²Î¸ = 1. These are used in everything from signal processing to orbital mechanics!',
    steps: [
      { label: 'Start with the Pythagorean Theorem', formula: 'a^2 + b^2 = c^2', detail: 'ðŸ“ In any right triangle with legs a, b and hypotenuse c: a² + b² = c². This is the most famous relationship in all of geometry.' },
      { label: 'Express in trig terms', formula: 'sin^2 theta = \{a^2}{c^2}, cos^2 theta = \{b^2}{c^2}', detail: 'sin Î¸ = a/c and cos Î¸ = b/c. Square both: sin²Î¸ = a²/c² and cos²Î¸ = b²/c².' },
      { label: 'Add the squares', formula: 'sin^2 theta + cos^2 theta = \{a^2}{c^2} + \{b^2}{c^2} = \{c^2}{c^2} = 1', detail: 'sin²Î¸ + cos²Î¸ = a²/c² + b²/c² = (a² + b²)/c² = c²/c² = 1. The triangle\'s legs and hypotenuse cancel out perfectly!' },
      { label: 'The identity is born', formula: 'sin^2 theta + cos^2 theta = 1', detail: 'sin²Î¸ + cos²Î¸ = 1 — it doesn\'t matter what Î¸ is, this always holds true! From this, we can derive: sec²Î¸ âˆ’ tan²Î¸ = 1 and cosec²Î¸ âˆ’ cot²Î¸ = 1.' },
    ],
    practice: { question: 'sin(30°) = 0.5. Find sin²(30°) + cos²(30°).', hint: 'cos(30°) = 0.866. Compute 0.25 + 0.75', answer: 1, tolerance: 0.01, explanation: 'sin²(30°) + cos²(30°) = 1. Always 1, no matter the angle!', errorHint: 'sin²Î¸ + cos²Î¸ = 1 always!' },
  ...
  distance_formula: {
    ...
    steps: [
      ...
      { label: 'Apply Pythagoras', formula: 'd^2 = (Delta x)^2 + (Delta y)^2', detail: 'd² = (Î”x)² + (Î”y)² → d = âˆš((xâ‚‚âˆ’xâ‚)² + (yâ‚‚âˆ’yâ‚)²) = âˆš(6² + 4²) = âˆš(36 + 16) = âˆš52 â‰ˆ 7.21 units.' },
    ...
    practice: { question: 'Pâ‚(0,0), Pâ‚‚(3,4). Find the distance.', hint: 'd = âˆš(3² + 4²) = âˆš(9+16)', answer: 5, tolerance: 0.1, explanation: 'd = âˆš(9+16) = âˆš25 = 5. The classic 3-4-5 triangle!', errorHint: 'd = âˆš((xâ‚‚âˆ’xâ‚)² + (yâ‚‚âˆ’yâ‚)²)' },
  ...
  slope_intercept: {
    ...
    steps: [
      ...
      { label: 'Rearrange for y', formula: "y = mx + c", detail: 'm = (y âˆ’ c)/x → y âˆ’ c = mx → y = mx + c. That\'s it! All straight lines can be written this way.' },
    ],
  ...
  point_slope: {
    ...
    steps: [
      ...
      { label: 'The result', formula: 'y - y_1 = m(x - x_1)', detail: 'y âˆ’ yâ‚ = m(x âˆ’ xâ‚). For our example: y âˆ’ 3 = 1.5(x âˆ’ 2) → y = 1.5x.' },
    ],
  ...
  two_point_form: {
    ...
    steps: [
      { label: 'Two points define a line', formula: 'A(x_1, y_1), B(x_2, y_2)', detail: 'ðŸ”— You have two points A(1, 2) and B(4, 8). Only one straight line passes through both — the shortest path between them.' },
      ...
      { label: 'Apply Point-Slope Form', formula: 'y - y_1 = m(x - x_1)', detail: 'Now that we have m = 2, use the Point-Slope Form with either point: y âˆ’ yâ‚ = m(x âˆ’ xâ‚) → y âˆ’ 2 = 2(x âˆ’ 1).' },
  ...
  two_intercept_form: {
    ...
    steps: [
      ...
      { label: 'Use Point-Slope', formula: '\{x}{a} + \{y}{b} = 1', detail: 'Using point (0, b): y âˆ’ b = (âˆ’b/a)x → ay âˆ’ ab = âˆ’bx → bx + ay = ab.' },
  ...
  normal_form: {
    ...
    steps: [
      ...
      { label: 'Find intercepts', formula: 'x\{-intercept} = \{p}{cos alpha}, y\{-intercept} = \{p}{sin alpha}', detail: 'From right triangle OCA: cos Î± = p/OA → OA = p/cos Î± (x-intercept). From triangle OCB: sin Î± = p/OB → OB = p/sin Î± (y-intercept).' },
      { label: 'Apply Two-Intercept Form', formula: '\{x}{p/cos alpha} + \{y}{p/sin alpha} = 1', detail: 'Using x/a + y/b = 1: x/(p/cos Î±) + y/(p/sin Î±) = 1 → (x cos Î±)/p + (y sin Î±)/p = 1.' },
    ...
    practice: { question: 'Î± = 60°, p = 5. Find cos Î± and evaluate x cos Î± + y sin Î± at x=2, y=3.', hint: 'cos 60° = 0.5, sin 60° = 0.866. LHS = 2×0.5+3×0.866', answer: 5, tolerance: 0.3, explanation: '2×0.5 + 3×0.866 = 1 + 2.598 = 3.598 â‰  5. Point (2,3) is not on this line!', errorHint: 'x cos Î± + y sin Î± = p' },
  ...
  angle_between_lines: {
    ...
    steps: [
      ...
      { label: 'Apply tan to both sides', formula: 'tan theta = tan(alpha - beta) = \{tan alpha - tan beta}{1 + tan alpha tan beta}', detail: 'Î¸ = Î± âˆ’ Î². Take tan of both sides: tan Î¸ = tan(Î± âˆ’ Î²). Using the tangent subtraction formula: tan(Î±âˆ’Î²) = (tan Î± âˆ’ tan Î²)/(1 + tan Î± × tan Î²).' },
    ...
    practice: { question: 'mâ‚ = 1, mâ‚‚ = 2. Find the acute angle between them.', hint: 'tan Î¸ = |(2âˆ’1)/(1+2)| = |1/3| = 0.333', answer: 18.43, tolerance: 1, explanation: 'Î¸ = 18.4°. The lines are 18.4° apart.', errorHint: 'tan Î¸ = |(mâ‚‚âˆ’mâ‚)/(1+mâ‚mâ‚‚)|' },
