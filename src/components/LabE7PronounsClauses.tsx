          ...
          {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 p-4 min-h-0">
                ...
                <div className="flex flex-col gap-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 overflow-y-auto border border-slate-200 dark:border-slate-700">
                    ...
                    {feedback && (
                        <div className={`mt-auto p-4 rounded-lg flex items-center gap-3 ${feedback.includes('Perfect') || feedback.includes('Correct') ? 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-200' : feedback.includes('Hmm') || feedback.includes('Incorrect') ? 'bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-200' : 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200'}`}>
