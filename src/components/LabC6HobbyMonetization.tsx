    ...
    <div className="flex h-screen font-sans bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      <div className="flex-1 px-8 pb-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex gap-8 flex-1">
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 text-left transition-all ${
                        hobby === h.id ? 'border-indigo-500 bg-indigo-50 shadow-md' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 hover:border-slate-300 dark:border-slate-700 dark:border-slate-500 hover:bg-slate-50 dark:bg-slate-900'
                      }`}
                    >
                      <Icon className={`w-8 h-8 mb-3 ${hobby === h.id ? 'text-indigo-600' : 'text-slate-400'}`} />
                      <div className="font-bold text-slate-800 dark:text-slate-100 mb-1">{h.name}</div>
          ...
          <div className="w-1/2 bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-8 flex flex-col">
            ...
            <div className="flex flex-col gap-6 flex-1">
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Selling Price per item/service (Rs)</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-lg font-mono"
                  />
                </div>
              </div>

              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Cost Price per item/service (Rs)</label>
                <div className="relative">
                  <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
                  <input 
                    type="number" 
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Estimated Sales (Quantity)</label>
                <input 
                  type="number" 
                  value={sales}
                  onChange={e => setSales(e.target.value)}
                  className="w-full px-4 py-3 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-lg font-mono"
                />
              </div>

              <div className="mt-auto bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-xl p-6">
                <h3 className="font-bold text-slate-500 dark:text-slate-400 text-sm uppercase tracking-wider mb-4">Financial Summary</h3>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-slate-600 dark:text-slate-300">Profit Per Item (Selling - Cost):</span>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-800 dark:text-slate-100 font-bold text-lg">Total Estimated Profit:</span>
