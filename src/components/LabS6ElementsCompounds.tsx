...
export default function LabS6ElementsCompounds({ onExit }: LabProps) {
 ...
 <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans min-h-screen lg:h-screen overflow-x-hidden w-full">
  <LabHeader onExit={onExit} title={t('lab.s6elementscompounds_unit_6_elements_vs_compounds')} />
  ...
  <div className="w-full max-w-3xl bg-slate-50 dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-8">
   ...
   <h2 className="font-bold text-lg mb-2 flex items-center gap-2"><HelpCircle className="w-6 h-6" />  {t('lab.s6elementscompounds_identification_activity')}</h2>
   <p className="text-sm">
    An <strong>{t('lab.s6elementscompounds_element')}</strong>  {t('lab.s6elementscompounds_consists_of_only_one_type_of_a')} <strong>{t('lab.s6elementscompounds_compound')}</strong>  {t('lab.s6elementscompounds_consists_of_two_or_more_differ')}
                            </p>
   </div>
   ...
   {questions.map((q) => (
    ...
    <div className="flex gap-2 w-1/3 justify-center">
     ...
     >
                        ...
                        {t('lab.s6elementscompounds_element')}
                        </button>
     <button 
     ...
     >
                        ...
                        {t('lab.s6elementscompounds_compound')}
                        </button>
    </div>
   ...
   >
                             ...
                             {t('lab.s6elementscompounds_check_answers')}
                            </button>
   ) : (
   <div className="text-center p-6 bg-slate-100 dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#1c1b1b]">
    <h3 className="text-2xl font-black text-slate-800 dark:text-[#ffffff] mb-2">{t('lab.s6elementscompounds_score')} {calculateScore()} / {questions.length}</h3>
    {calculateScore() === questions.length ? (
    <p className="text-emerald-600 font-bold flex items-center justify-center gap-2"><CheckCircle className="w-5 h-5" />  {t('lab.s6elementscompounds_perfect_you_understand_the_dif')}</p>
    ) : (
    <button onClick={() => { setShowResults(false); setAnswers({}); }} className="px-6 py-2 bg-slate-100 dark:bg-[#121212] border-2 border-slate-300 dark:border-[#1c1b1b] rounded font-bold text-slate-600 dark:text-[#a1a1aa] hover:bg-slate-200 dark:bg-[#121212] transition-colors">{t('lab.s6elementscompounds_try_again')}</button>
    )}
   </div>
   )}
