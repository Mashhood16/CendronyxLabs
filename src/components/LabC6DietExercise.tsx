              ...
              <div className="space-y-2">
                {foods.map(f => (
                  <button 
                    ...
                    className="w-full flex justify-between p-3 rounded-lg border border-slate-200 hover:border-rose-300 hover:bg-rose-50 transition-colors"
                  >
                    <span className="font-medium">{f.name}</span>
                    <span className="text-slate-500">+{f.cal}</span>
                  </button>
                ))}
              </div>
              <button onClick={() => setCaloriesIn(2000)} className="w-full mt-4 text-sm text-slate-400 hover:text-slate-600">Reset to Base BMR (2000)</button>
            </div>
              ...
              <div className="space-y-2">
                {exercises.map(e => (
                  <button 
                    ...
                    className="w-full flex justify-between p-3 rounded-lg border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 transition-colors"
                  >
                    <span className="font-medium">{e.name}</span>
                    <span className="text-slate-500">+{e.cal}</span>
                  </button>
                ))}
              </div>
              <button onClick={() => setCaloriesOut(2000)} className="w-full mt-4 text-sm text-slate-400 hover:text-slate-600">Reset to Base BMR (2000)</button>
            </div>
          </div>
          ...
          <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 p-8 flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <div className="relative w-full max-w-md h-64 mb-12">
                <div className="absolute bottom-0 w-4 h-32 bg-slate-800 left-1/2 -translate-x-1/2 rounded-t-sm"></div>
                <div 
                  className="absolute bottom-32 w-full h-2 bg-slate-600 rounded-full transition-transform duration-500 ease-in-out origin-center"
              ...
              <div className="text-center p-8 bg-slate-50 rounded-2xl border border-slate-100 w-full max-w-md">
                <div className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Net Calories</div>
                <div className={`text-5xl font-bold mb-4 ${netCalories > 0 ? 'text-rose-500' : netCalories < 0 ? 'text-indigo-500' : 'text-slate-800'}`}>
