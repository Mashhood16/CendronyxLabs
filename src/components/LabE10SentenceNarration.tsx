      ...
      <div className="flex-1 lg:overflow-hidden">
        <div className="h-full grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
          ...
          <div className="bg-white dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-neutral-900 flex flex-col h-full overflow-hidden">
            <div className="p-6 bg-slate-100 dark:bg-[#121212] border-b border-slate-200 dark:border-[#1c1b1b] flex items-center gap-3">
              <BookOpen className="w-6 h-6 text-slate-600 dark:text-[#71717a]" />
              <h2 className="text-xl font-bold text-slate-900 dark:text-[#a1a1aa]">Rules of Narration</h2>
            </div>
            <div className="p-6 lg:overflow-y-auto flex-1 space-y-6 text-slate-700 dark:text-[#a1a1aa]">
              <section>
                <h3 className="text-lg font-bold text-slate-900 dark:text-[#ffffff] mb-2">Sentence Types</h3>
              ...
              <section>
                <h3 className="text-lg font-bold text-slate-900 dark:text-[#ffffff] mb-2">Direct to Indirect Speech</h3>
                <ul className="list-disc pl-5 space-y-2 text-sm">
          ...
          <div className="bg-white dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-neutral-900 p-6 flex flex-col h-full overflow-hidden">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200 dark:border-neutral-900">
              <div className="p-2 bg-slate-100 dark:bg-[#121212] text-slate-600 dark:text-[#71717a] rounded-lg">
                <Mic className="w-6 h-6" />
              </div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-[#a1a1aa]">Press Room Simulator</h2>
            </div>
            
            <div className="flex-1 flex flex-col">
              <div className="bg-slate-100 dark:bg-[#121212] p-6 rounded-xl border border-slate-200 dark:border-[#1c1b1b] text-center mb-6">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-[#71717a] block mb-2">
              ...
              <div className="mb-4">
                <p className="text-sm text-slate-600 dark:text-[#71717a] mb-2">Construct the Indirect Speech:</p>
                <div className="min-h-[80px] p-3 border-2 border-dashed border-slate-300 dark:border-[#1c1b1b] rounded-xl flex flex-wrap gap-2 items-start bg-slate-50 dark:bg-[#121212]/50">
                  {constructed.map((piece: any, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleRemovePiece(piece)}
                      className="px-3 py-2 bg-slate-700 text-white rounded-lg text-sm shadow hover:bg-[#121212] transition-colors flex items-center gap-1"
                    >
                      {piece.text}
                    </button>
                  ))}
              ...
              <div className="flex-1">
                <p className="text-sm text-slate-600 dark:text-[#71717a] mb-2">Available Pieces:</p>
                <div className="flex flex-wrap gap-2">
                  {availablePieces.map((piece: any, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleSelectPiece(piece)}
                      className="px-3 py-2 bg-white dark:bg-[#121212] border border-slate-300 dark:border-[#1c1b1b] text-slate-700 dark:text-[#a1a1aa] rounded-lg text-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                    >
                      {piece.text}
                    </button>
                  ))}
              ...
              {checkConstruction() && (
                <div className="mt-4 flex flex-col gap-4">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full py-2 bg-[#121212] hover:bg-[#000000] text-white rounded-lg font-bold transition-colors"
                  >
              ...
              {!assessmentSubmitted ? (
                <div className="space-y-6">
                        ...
                        {q.options.map((opt, oIdx) => (
                          <label key={oIdx} className="flex items-start gap-3 cursor-pointer group">
                            <input
                              type="radio"
                              name={`question-${qIdx}`}
                              className="mt-1 w-4 h-4 text-slate-600 dark:text-[#71717a] bg-slate-100 dark:bg-[#121212]/50 border-slate-300 dark:border-[#1c1b1b]"
                  ...
                  <button
                    ...
                    className="w-full mt-4 py-2 px-4 bg-[#121212] hover:bg-[#000000] disabled:bg-slate-300 dark:bg-slate-700 disabled:dark:bg-[#121212] text-white rounded-lg font-medium transition-colors"
                  >
                    Submit Evaluation
                  </button>
                </div>
                ...
                <div className="text-center py-8">
                  <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-slate-100 dark:bg-[#121212] text-slate-600 dark:text-[#71717a] mb-4">
