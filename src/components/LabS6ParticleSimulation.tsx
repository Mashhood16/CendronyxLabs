      ...
      <div className="flex-1 flex flex-col p-8 items-center">
        <div className="flex gap-4 mb-8">
          <button 
            ...
            className={`px-6 py-3 rounded-xl border-2 font-bold flex items-center gap-2 ${simulation === 'dissolving' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 hover:border-blue-300'}`}
          >
            <Droplets className="w-5 h-5" /> Dissolving Sugar
          </button>
          <button 
            onClick={startDiffusion}
            className={`px-6 py-3 rounded-xl border-2 font-bold flex items-center gap-2 ${simulation === 'diffusion' ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 hover:border-indigo-300'}`}
          >
        ...
        {simulation === 'dissolving' && (
          <div className="w-full max-w-2xl bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-8 flex flex-col items-center">
            ...
            <div className="mt-8 w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
        ...
        {simulation === 'diffusion' && (
          <div className="w-full max-w-2xl bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-8 flex flex-col items-center">
            ...
            <div className="relative w-96 h-64 border-2 border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded bg-slate-50 dark:bg-slate-900 overflow-hidden">
              {/* Spray nozzle */}
