...
import { ArrowLeft, CheckCircle, ChevronRight, ChevronLeft, Lightbulb, Users, Target } from 'lucide-react';
  ...
  const slides = [
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <p className="text-lg text-slate-600">What specific agricultural problem are farmers in Pakistan facing today?</p>
          <textarea 
            ...
            className="w-full h-32 p-4 border border-slate-300 rounded-lg text-lg resize-none focus:ring-2 focus:ring-emerald-500"
          />
        </div>
      )
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <p className="text-lg text-slate-600">How will your technology (app, website, IoT) solve this problem?</p>
          <textarea 
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <p className="text-lg text-slate-600">Who exactly will use this product and how will they benefit?</p>
          <textarea 
  ...
  return (
    <div className="flex h-screen font-sans bg-slate-50 text-slate-800">
      <div className="flex-1 p-8 flex flex-col overflow-y-auto">
        ...
        <h1 className="text-3xl font-bold mb-2">Tech Startup Pitch</h1>
        <p className="text-slate-600 mb-8">Prepare a presentation to pitch a new IT solution addressing a problem in the agriculture sector.</p>

        <div className="flex-1 flex flex-col items-center justify-center">
          
          {/* Presentation Viewer */}
          <div className="w-full max-w-4xl aspect-video bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden relative">
            ...
            <div className="flex-1 p-12 flex flex-col">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h2 className="text-4xl font-bold text-slate-800">{slides[slide].title}</h2>
              </div>
            ...
            <div className="bg-slate-50 border-t border-slate-200 p-4 flex justify-between items-center px-12">
              <button 
                onClick={() => setSlide(s => Math.max(0, s - 1))}
                disabled={slide === 0}
                className="flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-slate-900 disabled:opacity-30 font-bold transition-colors"
              >
