  ...
  const slides = [
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <p className="text-lg text-slate-600 dark:text-slate-300">What specific agricultural problem are farmers in Pakistan facing today?</p>
          <textarea 
            ...
            className="w-full h-32 p-4 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-lg text-lg resize-none focus:ring-2 focus:ring-emerald-500"
          />
        </div>
      )
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <p className="text-lg text-slate-600 dark:text-slate-300">How will your technology (app, website, IoT) solve this problem?</p>
          <textarea 
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <p className="text-lg text-slate-600 dark:text-slate-300">Who exactly will use this product and how will they benefit?</p>
          <textarea 
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col items-center text-center gap-6">
          ...
          <p className="text-xl text-slate-700 dark:text-slate-200 max-w-lg">
    ...
    <div className="flex h-screen font-sans bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      <div className="flex-1 px-8 pb-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          {/* Presentation Viewer */}
          <div className="w-full max-w-4xl aspect-video bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col overflow-hidden relative">
            ...
            <div className="flex-1 p-12 flex flex-col">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h2 className="text-4xl font-bold text-slate-800 dark:text-slate-100">{slides[slide].title}</h2>
              </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 dark:border-slate-500 p-4 flex justify-between items-center px-12">
              <button 
                onClick={() => setSlide(s => Math.max(0, s - 1))}
                disabled={slide === 0}
                className="flex items-center gap-2 px-4 py-2 text-slate-600 dark:text-slate-300 hover:text-slate-900 disabled:opacity-30 font-bold transition-colors"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg disabled:opacity-30 font-bold transition-colors shadow-sm"
              >
