        ...
        <section className="w-full lg:w-1/3 flex flex-col border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 lg:overflow-y-auto p-6">
          ...
          <div className="flex bg-slate-200 dark:bg-slate-700 p-1 rounded-lg mb-8 shrink-0">
            <button 
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-md transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'tenses' ? 'bg-white dark:bg-slate-800 shadow-sm text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'}`}
            >
              Time Machine (Tenses)
            </button>
            <button 
              onClick={() => setActiveTab('gerunds')}
              className={`flex-1 py-2 text-sm font-bold rounded-md transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'gerunds' ? 'bg-white dark:bg-slate-800 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'}`}
            >
              Gerund Scanner
            </button>
          </div>
          ...
          {activeTab === 'tenses' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-left-4">
              ...
              <div className="space-y-4">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">1. Base Verb</label>
                <div className="grid grid-cols-2 gap-2">
                  {VERBS.map((v, i) => (
                    <button
                      key={v.base}
                      onClick={() => setSelectedVerbIdx(i)}
                      className={`py-2 px-3 rounded-lg border font-mono text-center transition-colors whitespace-nowrap flex-shrink-0 ${
                        selectedVerbIdx === i 
                          ? 'bg-blue-100 border-blue-500 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200 dark:border-blue-400' 
              ...
              <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 min-w-0 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <button
                    onClick={handleCheckConjugation}
                    className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-bold transition-colors flex items-center gap-2 whitespace-nowrap flex-shrink-0"
                  >
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-slate-800 dark:text-slate-200">Task {activeSentenceIdx + 1} of {GERUND_TASKS.length}</h3>
                  <button 
                ...
                <div className="p-4 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl">
                  ...
                  <div className="flex flex-wrap gap-2 text-lg">
                    {GERUND_TASKS[activeSentenceIdx].words.map((w, idx) => (
                      <button
                        ...
                        className={`px-2 py-1 rounded transition-all whitespace-nowrap flex-shrink-0 ${
                          selectedWords.includes(idx) 
                            ? 'bg-emerald-500 text-white shadow-md' 
                            : 'bg-white dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-600'
                        }`}
                      >
                        {w.text}
                      </button>
                ...
                <button
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-lg font-bold transition-colors flex items-center justify-center gap-2 whitespace-nowrap flex-shrink-0"
                >
          ...
          {activeTab === 'tenses' ? (
            <div className="relative w-full max-w-2xl aspect-square sm:aspect-video bg-slate-950 rounded-2xl overflow-hidden shadow-2xl flex flex-col items-center justify-center border-4 border-slate-800">
              ...
              <div className={`relative w-48 h-48 md:w-64 md:h-64 rounded-full border-8 shadow-[0_0_60px_rgba(0,0,0,0.8)] flex items-center justify-center transition-all duration-1000 ${TENSES[timelineIndex].border} ${conjFeedback === 'success' ? 'scale-110' : 'scale-100'}`}>
                ...
                <div className="absolute -bottom-8 bg-slate-900 text-white px-4 py-1 rounded-full text-sm font-bold whitespace-nowrap border border-slate-700 z-10">
                  {TENSES[timelineIndex].name}
                </div>
              </div>
            ...
            <div className="relative w-full max-w-2xl min-h-[400px] bg-slate-950 rounded-2xl overflow-hidden shadow-2xl flex flex-col items-center justify-center border-4 border-slate-800 p-8">
               ...
               <div className="w-full bg-slate-900/80 border border-emerald-500/30 rounded-xl p-8 relative font-mono shadow-[0_0_30px_rgba(16,185,129,0.05)] backdrop-blur-sm">
