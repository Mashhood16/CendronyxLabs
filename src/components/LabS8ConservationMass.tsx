    ...
    <div className="overflow-y-auto flex flex-col h-screen bg-slate-50 font-sans">
      <div className="bg-white border-b p-4 flex items-center justify-between sticky top-0 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Act 6.2: Conservation of Mass</h1>
            <p className="text-sm text-slate-500">Mass remains constant before and after reaction</p>
          </div>
        </div>
        <button onClick={() => setStage('initial')} className="flex items-center gap-2 bg-slate-200 px-4 py-2 rounded-md hover:bg-slate-300 font-medium text-slate-700">
          <RefreshCw className="w-4 h-4" /> Reset
        </button>
      </div>

      <div className="flex-1 flex flex-col items-center justify-center p-6">
        <div className="bg-white p-8 rounded-3xl shadow-lg border border-slate-200 max-w-2xl w-full flex flex-col items-center relative min-h-[500px]">
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className={`absolute bottom-4 z-10 transition-transform duration-1000 origin-bottom ${stage === 'mixed' ? 'rotate-[360deg] scale-105' : ''}`}>
              {/* Flask */}
              <div className="w-32 h-32 border-4 border-slate-300 rounded-b-[40px] rounded-t-md relative bg-white/50 overflow-hidden flex flex-col justify-end">
                ...
                <div className={`w-full h-1/2 transition-colors duration-1000 ${stage === 'mixed' ? 'bg-slate-100' : 'bg-blue-50'}`}>
                  {/* Precipitate Formed */}
                  {stage === 'mixed' && (
                    <div className="absolute bottom-0 w-full h-1/2 bg-white border-t border-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-500 opacity-90 animate-fade-in">
                      CaSO4 Solid
                    </div>
                  )}
                </div>
