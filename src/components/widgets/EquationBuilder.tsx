...
// ---- Styling helpers ----
const NUM_BTN = `w-full py-1.5 sm:py-2 px-1 text-xs font-bold rounded-lg transition-all bg-white dark:bg-[#1c1b1b] text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-[#2a2a2a] hover:bg-slate-100 active:scale-95 text-center flex items-center justify-center select-none touch-manipulation`;
const OP_BTN = `w-full py-1.5 sm:py-2 px-1 text-xs font-bold rounded-lg transition-all bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-900/50 hover:bg-amber-100 active:scale-95 text-center flex items-center justify-center select-none touch-manipulation`;
const GREEK_BTN = `w-full py-1.5 sm:py-2 px-1 text-xs font-bold rounded-lg transition-all bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-900/50 hover:bg-purple-100 active:scale-95 text-center flex items-center justify-center select-none touch-manipulation`;
const FUNC_BTN = `w-full py-1.5 sm:py-2 px-1 text-[11px] sm:text-xs font-bold rounded-lg transition-all bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 border border-sky-200 dark:border-sky-900/50 hover:bg-sky-100 active:scale-95 text-center flex items-center justify-center select-none touch-manipulation`;
const VAR_BTN = `w-full py-1.5 sm:py-2 px-1 text-xs font-mono font-semibold rounded-md transition-all bg-slate-50 dark:bg-[#1c1b1b] text-slate-600 dark:text-slate-400 hover:bg-slate-200 active:scale-95 border border-slate-200 dark:border-transparent text-center flex items-center justify-center select-none touch-manipulation`;
const PHYS_VAR_BTN = `w-full py-1.5 sm:py-2 px-1 text-xs font-semibold rounded-md transition-all bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-200 active:scale-95 border border-indigo-200 dark:border-indigo-900/50 text-center flex items-center justify-center select-none touch-manipulation`;
const DANGER_BTN = `w-full py-1.5 sm:py-2 px-1 text-xs font-bold rounded-lg transition-all bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-900/50 hover:bg-red-100 active:scale-95 text-center flex items-center justify-center select-none touch-manipulation`;
...
export default function EquationBuilder({
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <div className="relative">
        <div className="relative">
          <input
            ...
            className={`w-full px-3 py-2 text-xs font-mono rounded-lg bg-white dark:bg-[#121212] border-2 border-slate-200 dark:border-[#2a2a2a] text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 outline-none transition-all duration-200 placeholder:text-slate-400 dark:placeholder:text-slate-600 ${
      ...
      {value && (
        <div className="bg-gradient-to-b from-[#1a1a2e] to-[#0f0f1a] rounded-xl px-3 py-2 text-center border border-[#2a2a3a] shadow-inner min-h-[36px] flex items-center justify-center overflow-x-auto">
          <MathFormula formula={value} className="text-sm font-bold text-yellow-300 drop-shadow-sm" />
        </div>
      )}

      {/* Category tabs */}
      <div className="grid grid-cols-5 gap-1 bg-slate-100 dark:bg-[#1a1a1a] p-1 rounded-xl w-full">
        <button
          onClick={() => { setActiveCategory(0); setShowVars(false); }}
          className={`flex items-center justify-center py-1 px-0.5 text-[10px] sm:text-xs font-bold rounded-lg transition-all w-full ${
        ...
        <button
          onClick={() => { setActiveCategory(1); setShowVars(false); }}
          className={`flex items-center justify-center py-1 px-0.5 text-[10px] sm:text-xs font-bold rounded-lg transition-all w-full ${
      ...
      {/* Button panel */}
      <div className="bg-slate-50 dark:bg-black/30 rounded-xl p-2 border border-slate-200 dark:border-[#1c1c1c] shadow-inner">
        {!showVars ? (
          <>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-1 w-full">
              {CATEGORIES[activeCategory].buttons.map((btn, idx) => (
                <button
                  key={idx}
                  onClick={() => handleButtonClick(btn.value)}
                  title={btn.title || btn.label}
                  className={getBtnStyle(activeCategory, btn.label, btn.value)}
                >
                  {btn.label}
                </button>
              ))}
          ...
          <>
            ...
            {/* Common variables */}
            <div className="mb-2">
              <p className="text-[9px] font-semibold text-slate-400 dark:text-slate-500 mb-1 uppercase tracking-wider">
                Common
              </p>
              <div className="grid grid-cols-5 sm:grid-cols-7 gap-1 w-full">
