...
export default function MathFormula({ formula, display = false, className = '' }: MathFormulaProps) {
  ...
  if (display) {
    return (
      <span className={`katex-block-wrapper block text-center my-1 max-w-full overflow-x-auto ${className}`} style={{ fontFamily: '"STIX Two Text", "Cambria Math", Georgia, "Times New Roman", serif' }}>
        {nodes}
      </span>
    );
  }
  ...
  return (
    <span className={`katex-inline-wrapper max-w-full overflow-x-auto inline-block align-middle ${className}`} style={{ fontFamily: '"STIX Two Text", "Cambria Math", Georgia, "Times New Roman", serif' }}>
      {nodes}
    </span>
  );
}
