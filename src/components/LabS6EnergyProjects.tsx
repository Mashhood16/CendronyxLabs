...
import { ArrowLeft, Activity, Image as ImageIcon, CheckSquare } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans">
      <div className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shadow-sm">
        ...
        <h1 className="text-xl font-bold text-slate-800">Unit 8: Energy Projects</h1>
      </div>
      ...
      <div className="flex-1 flex flex-col p-8 items-center overflow-y-auto">
        ...
        <div className="flex gap-4 mb-8">
          <button 
            ...
            className={`px-6 py-3 rounded-xl border-2 font-bold ${activeProject === 'windmill' ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-slate-200 text-slate-600 hover:border-purple-300'}`}
          >
            Project: Model Windmill
          </button>
          <button 
            onClick={() => setActiveProject('rollercoaster')}
            className={`px-6 py-3 rounded-xl border-2 font-bold ${activeProject === 'rollercoaster' ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 text-slate-600 hover:border-indigo-300'}`}
          >
        ...
        <div className="w-full max-w-4xl bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          {activeProject === 'windmill' && (
            <div className="space-y-8">
              <div className="flex gap-4 items-start bg-purple-50 border border-purple-200 p-6 rounded-xl">
                <Activity className="w-8 h-8 text-purple-600 shrink-0 mt-1" />
                <div>
                  <h2 className="text-xl font-bold text-purple-900 mb-2">Build a Working Windmill</h2>
              ...
              <div>
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><CheckSquare /> Construction Checklist</h3>
                <div className="space-y-3 bg-slate-50 p-6 rounded-xl border border-slate-200">
                  {['Cut a square piece of paper', 'Cut diagonally from corners towards center', 'Fold alternating corners to the center', 'Pin the center to a wooden stick', 'Blow on the windmill to test'].map((step, i) => (
                    <label key={i} className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className={`text-slate-700 ${checks[`wind_${i}`] ? 'line-through text-slate-400' : ''}`}>{step}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="border-t border-slate-200 pt-8">
                 <h3 className="font-bold text-slate-800 mb-4">Submission</h3>
                 <div className="p-8 text-center text-slate-400 border-2 border-dashed border-slate-300 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer">
          ...
          {activeProject === 'rollercoaster' && (
            <div className="space-y-8">
              <div className="flex gap-4 items-start bg-indigo-50 border border-indigo-200 p-6 rounded-xl">
                <Activity className="w-8 h-8 text-indigo-600 shrink-0 mt-1" />
                <div>
                  <h2 className="text-xl font-bold text-indigo-900 mb-2">Design a Roller Coaster Track</h2>
                  <p className="text-slate-700 text-sm leading-relaxed">
              ...
              <div>
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><CheckSquare /> Project Requirements</h3>
                <div className="space-y-3 bg-slate-50 p-6 rounded-xl border border-slate-200">
                  {['Track has a steep starting hill (Max PE)', 'Track includes a loop (Requires high KE)', 'Marble successfully completes the track without falling off', 'Created labels for "High PE" and "High KE"'].map((step, i) => (
                    <label key={i} className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className={`text-slate-700 ${checks[`roller_${i}`] ? 'line-through text-slate-400' : ''}`}>{step}</span>
                    </label>
                  ))}
                </div>
              </div>
