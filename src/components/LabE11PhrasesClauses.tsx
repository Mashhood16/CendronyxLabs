...
export default function LabE11PhrasesClauses({ onExit }: { onExit?: () => void }) {
 ...
 <div className="flex flex-col min- lg: bg-slate-50 dark:bg-[#121212] font-sans select-none text-slate-900 dark:text-[#ffffff] min-h-screen lg:h-screen overflow-x-hidden w-full">
  {/* Header */}
  <LabHeader onExit={onExit} title={t('lab.e11phrasesclauses_lab_phrases_clauses_highlighte')} rightContent={
          <div className="flex items-center gap-2 bg-black/20 dark:bg-white/10 px-4 py-2 rounded-full font-bold text-white text-xs">
            <span className="opacity-90">{t('lab.e11phrasesclauses_score')}</span>
  ...
  <section className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-100 dark:bg-[#121212] lg:dark:bg-[#0a0a0a] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] relative min-h-0 overflow-y-auto p-8 lg:min-h-[35vh] lg:min-h-[500px] flex-col '' : ''} ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   ...
   <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full pb-8">
    ...
    <div key={sentence.id} className={`rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-[#1c1b1b] leading-loose text-base md:text-lg flex-col ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
