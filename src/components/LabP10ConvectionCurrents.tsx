...
import { ArrowLeft, RefreshCw, Activity, BookOpen, LineChart, Info, Play, Square } from 'lucide-react';
    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 font-sans select-none">
      <div className="bg-white border-b p-4 flex items-center justify-between sticky top-0 shadow-sm z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Unit 10: Convection Currents</h1>
            <p className="text-sm text-slate-500">Determine the relationship between heater power and convection velocity.</p>
          </div>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 max-w-7xl mx-auto w-full">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
          ...
          <div className="space-y-4 text-slate-600 text-sm">
            ...
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
              <h3 className="font-bold text-purple-800 mb-2">Formulas</h3>
            ...
            <div className="flex items-start gap-2 bg-blue-50 text-blue-800 p-3 rounded-md">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
          ...
          <div className="flex-1 relative bg-slate-50 border-2 border-slate-200 rounded-xl flex justify-center items-end p-4 h-80 overflow-hidden">
            {/* Beaker */}
            ...
            <div className="absolute bottom-4 w-28 h-8 bg-zinc-800 rounded-t-lg flex items-center justify-center border-b-4 border-orange-500 z-0">
               {isRunning && <div className="absolute -top-4 w-16 h-8 bg-orange-500 rounded-full blur-md animate-pulse opacity-50" />}
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {!isRunning ? (
                <button onClick={startHeating} disabled={finished} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-md font-medium transition-colors disabled:opacity-50">
                  <Play className="w-4 h-4" /> Start Heating
                </button>
              ) : (
                <button onClick={stopHeating} className="flex-1 flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white py-2 rounded-md font-medium transition-colors">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-slate-600">Measure velocity when finished.</span>
            <button 
              onClick={recordData}
              disabled={!finished}
          ...
          <div className="max-h-32 overflow-y-auto border border-slate-200 rounded-md">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 sticky top-0 shadow-sm">
                <tr>
                  <th className="px-3 py-2 font-semibold text-slate-700">Power (W)</th>
                  <th className="px-3 py-2 font-semibold text-slate-700">Time (s)</th>
                  <th className="px-3 py-2 font-semibold text-slate-700">Vel (mm/s)</th>
                </tr>
              </thead>
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col items-center">
            <span className="text-xs font-bold text-slate-600 mb-2">Velocity vs Heater Power</span>
          ...
          <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-100 mt-auto">
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={checkAssessment} className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-md text-sm font-bold transition-colors">
                Check
              </button>
            </div>
