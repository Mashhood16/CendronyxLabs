...
export default function LabE7Tenses({ onExit }: { onExit?: () => void }) {
  ...
  const renderTimeline = () => {
    return (
      <div className="relative w-full h-48 flex items-center justify-center mt-12 bg-slate-100 dark:bg-slate-800 rounded-xl p-4">
        {/* Timeline Axis */}
        ...
        <div className="absolute left-[10%] flex flex-col items-center">
          ...
          <span className="mt-2 text-sm font-semibold text-slate-500 dark:text-slate-400">Past</span>
        </div>
        ...
        <div className="absolute left-[50%] flex flex-col items-center">
          ...
          <span className="mt-2 text-sm font-semibold text-slate-500 dark:text-slate-400">Present</span>
        </div>
        ...
        <div className="absolute right-[10%] flex flex-col items-center">
          ...
          <span className="mt-2 text-sm font-semibold text-slate-500 dark:text-slate-400">Future</span>
        </div>

        {/* Current action indicator */}
        <div className={`absolute top-4 px-4 py-2 bg-white dark:bg-slate-700 rounded-lg shadow-lg border-2 ${feedback === 'correct' ? 'border-emerald-400' : 'border-indigo-400'} text-slate-800 dark:text-slate-100 font-medium transition-all duration-500 ${
          currentQ.timelinePos === 'past' ? 'left-[10%] -translate-x-1/2' :
          currentQ.timelinePos === 'present' ? 'left-[50%] -translate-x-1/2' :
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <section className="w-full md:w-1/2 p-6 overflow-y-auto border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col">
          ...
          <div className="flex-1 flex flex-col justify-center space-y-6">
            ...
            {/* Sentence Builder */}
            <div className="p-6 bg-slate-50 dark:bg-slate-900 rounded-xl border-2 border-slate-200 dark:border-slate-700 shadow-sm">
              <p className="text-lg font-medium leading-relaxed mb-6 text-slate-700 dark:text-slate-200">
                    ...
                    {i < arr.length - 1 && (
                      <span className="mx-2 inline-block">
                        <input
                          ...
                          className={`w-32 sm:w-40 px-3 py-1 border-b-2 bg-transparent text-center focus:outline-none transition-colors ${
                            feedback === 'correct' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400 font-bold' :
                            feedback === 'incorrect' ? 'border-red-500 text-red-600 dark:text-red-400' :
                            'border-indigo-400 dark:border-indigo-500 text-indigo-700 dark:text-indigo-300 focus:border-indigo-600 dark:focus:border-indigo-400'
                          }`}
                        />
                      </span>
                    )}
              ...
              <div className="min-h-[60px] flex items-center">
                ...
                {feedback === 'incorrect' && (
                  <div className="flex flex-col text-red-500 dark:text-red-400 text-sm">
                    ...
                    <span className="text-slate-600 dark:text-slate-400 text-xs pl-7">
                      Hint: {currentQ.hint}
                    </span>
                  </div>
                )}
            ...
            <div className="flex gap-4 mt-auto">
              <button
                ...
                className="flex-1 whitespace-nowrap flex-shrink-0 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 dark:disabled:bg-indigo-800 text-white font-semibold rounded-lg shadow-sm transition-colors"
              >
                Check Answer
              </button>
              {feedback === 'correct' && (
                <button
                  onClick={handleNext}
                  className="whitespace-nowrap flex-shrink-0 py-3 px-6 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-lg shadow-sm transition-colors flex items-center gap-2"
                >
                  Next
        ...
        <section className="w-full md:w-1/2 p-6 bg-slate-100 dark:bg-slate-900 overflow-y-auto flex flex-col">
          <h2 className="text-lg font-bold mb-4 text-slate-800 dark:text-slate-100">Timeline Visualization</h2>
          
          <div className="flex-1 flex flex-col justify-center items-center">
            {renderTimeline()}

            <div className="mt-12 max-w-sm w-full bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
              <h3 className="font-semibold text-slate-700 dark:text-slate-200 mb-3 border-b border-slate-100 dark:border-slate-700 pb-2">Rule Reminder</h3>
