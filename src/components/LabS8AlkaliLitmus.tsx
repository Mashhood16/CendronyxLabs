...
export default function LabS8AlkaliLitmus({ onExit }: LabProps) {
 ...
 <div className="lg:overflow-y-auto flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
  ...
  <div className="bg-slate-50 dark:!bg-[#121212] rounded-2xl shadow-sm border p-8 flex flex-col items-center justify-center relative min-h-[450px] w-full">
   ...
   <h2 className="text-xl font-bold mb-8 text-slate-700 dark:text-[#ffffff]">{t('lab.s8alkalilitmus_aqueous_sodium_hydroxide_naoh')}</h2>
   ...
   <div className="absolute top-0 flex gap-12 w-full justify-center">
    ...
    {testedBlue === 'tested' && <div className="absolute top-1 left-1/2 -translate-x-1/2 text-[6px] font-bold text-blue-800 dark:text-[#ffffff]">{t('lab.s8alkalilitmus_stays_blue')}</div>}
    </div>
    ...
    {testedRed === 'tested' && <div className="absolute top-1 left-1/2 -translate-x-1/2 text-[6px] font-bold text-blue-800 dark:text-[#ffffff]">{t('lab.s8alkalilitmus_turned_blue')}</div>}
    </div>
   </div>
   </div>
   ...
   >
                             ...
                             {t('lab.s8alkalilitmus_dip_blue_litmus')}
                            </button>
   <button 
    onClick={dipRed}
   ...
   >
                             ...
                             {t('lab.s8alkalilitmus_dip_red_litmus')}
                            </button>
   </div>
   ...
   <div className="mt-8 px-6 py-4 bg-slate-100 dark:bg-[#121212] border border-slate-300 dark:border-[#1c1b1b] rounded-xl max-w-md text-center animate-fade-in">
    <h3 className="font-bold text-lg mb-2">{t('lab.s8alkalilitmus_observation')}</h3>
    {testedBlue === 'tested' && <p className="mb-1 text-blue-600 font-medium">{t('lab.s8alkalilitmus_blue_litmus_stayed_blue')}</p>}
    {testedRed === 'tested' && <p className="mb-1 text-blue-600 font-medium">{t('lab.s8alkalilitmus_red_litmus_turned_blue')}</p>}
    <p className="text-sm mt-2 text-slate-600 dark:text-[#a1a1aa]">{t('lab.s8alkalilitmus_this_indicates_that')} <strong>{t('lab.s8alkalilitmus_sodium_hydroxide')}</strong>  {t('lab.s8alkalilitmus_is_an_alkali_base')}</p>
   </div>
   )}
