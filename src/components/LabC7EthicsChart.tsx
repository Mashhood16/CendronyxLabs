...
export default function LabC7EthicsChart({ onExit }: LabProps) {
 ...
 return (
 <div className="flex flex-col font-sans bg-slate-100 dark:!bg-[#000000] text-slate-800 dark:text-[#ffffff] min-h-screen lg:h-screen overflow-x-hidden w-full">
 ...
 className={`p-3 rounded border-2 shadow-sm cursor-grab active:cursor-grabbing hover:-translate-y-1 transition-transform ${item.type === 'header' ? 'bg-blue-600 text-white font-black text-center text-xl' : ''} ${item.type === 'rule' ? 'bg-emerald-50 border-emerald-200 text-emerald-800 font-medium flex items-center' : ''} ${item.type === 'alert' ? 'bg-rose-50 border-rose-200 text-rose-800 font-bold flex items-center' : ''}`}
 >
 ...
 {/* Canvas */}
 <div 
 className="flex-1 bg-slate-50 dark:!bg-[#121212] border-4 border-slate-200 dark:border-[#1c1b1b] shadow-xl rounded-xl relative overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/cream-paper.png')]"
 ...
 <div 
 key={el.id}
 className={`absolute p-4 cursor-pointer shadow-md rounded border-2 group ${el.type === 'header' ? 'bg-blue-600 border-blue-700 text-white font-black text-3xl' : ''} ${el.type === 'rule' ? 'bg-emerald-50 border-emerald-300 text-emerald-900 font-medium text-lg flex items-center' : ''} ${el.type === 'alert' ? 'bg-rose-50 border-rose-300 text-rose-900 font-bold flex items-center' : ''}`}
