    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-[#000000]">
        <div className="text-center p-8">
          <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-2">{t('lab.m9theoremviewer_theorem_not_found')}</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('lab.m9theoremviewer_the_requested_theorem_could_no')}</p>
          {onExit && (
            <button onClick={onExit} className="mt-4 px-4 py-2 bg-violet-600 text-white text-sm font-bold rounded-lg">
              
                                      {t('lab.m9theoremviewer_go_back')}
                                    </button>
          )}
        </div>
      </div>
