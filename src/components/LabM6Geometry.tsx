...
export default function LabM6Geometry({ onExit }: { onExit?: () => void }) {
 ...
 return (
 <div className="flex flex-col min-min- bg-slate-50 dark:bg-[#000000] text-slate-800 dark:text-[#ffffff] font-sans transition-colors duration-300 min-h-screen lg:h-screen overflow-x-hidden w-full">
 <LabHeader onExit={onExit} title={t('lab.m6geometry_class_6_geometry_symmetry_cons')} />
 ...
 <Disc className="w-5 h-5" /> {t('lab.m6geometry_ferris_wheel_symmetry')}
 </h2>
 <div className="space-y-4">
 <div>
 <label className="block text-sm mb-1 font-medium">{t('lab.m6geometry_number_of_cabins_symmetry_line')} {symmetryLines}</label>
 ...
 <Navigation className="w-5 h-5" /> {t('lab.m6geometry_plumb_bob')}
 </h2>
 <p className="text-sm mb-3 text-slate-600 dark:text-[#71717a]">{t('lab.m6geometry_a_plumb_bob_points_perfectly_v')}</p>
 ...
 <section className="p-4 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800">
 <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-indigo-700 dark:text-indigo-300">
 <PencilRuler className="w-5 h-5" /> {t('lab.m6geometry_compass_construction')}
 </h2>
 <div className="space-y-4">
 <select 
 value={targetAngle}
 ...
 >
 <option value={60}>{t('lab.m6geometry_construct_60_angle')}</option>
 <option value={120}>{t('lab.m6geometry_construct_120_angle')}</option>
 <option value={90}>{t('lab.m6geometry_construct_90_angle')}</option>
 </select>
 
 <div className="flex flex-col gap-2">
 <button onClick={() => setCompassStep(0)} className={`p-2 text-left text-sm rounded transition-colors ${compassStep >= 0 ? 'bg-indigo-100 dark:bg-indigo-800 text-indigo-900 dark:text-indigo-100 font-medium' : ' hover:bg-slate-100 dark:hover:bg-slate-700'}`}>{t('lab.m6geometry_1_draw_base_line_set_origin_a')}</button>
 <button onClick={() => setCompassStep(1)} disabled={compassStep < 0} className={`p-2 text-left text-sm rounded transition-colors ${compassStep >= 1 ? 'bg-indigo-100 dark:bg-indigo-800 text-indigo-900 dark:text-indigo-100 font-medium' : ' hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50'}`}>{t('lab.m6geometry_2_draw_arc_from_a_cutting_line')}</button>
 <button onClick={() => setCompassStep(2)} disabled={compassStep < 1} className={`p-2 text-left text-sm rounded transition-colors ${compassStep >= 2 ? 'bg-indigo-100 dark:bg-indigo-800 text-indigo-900 dark:text-indigo-100 font-medium' : ' hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50'}`}>{t('lab.m6geometry_3_keep_radius_same_draw_arc_fr')}</button>
 ...
 <button onClick={() => setCompassStep(3)} disabled={compassStep < (targetAngle === 60 ? 2 : 2.5)} className={`p-2 text-left text-sm rounded transition-colors ${compassStep >= 3 ? 'bg-indigo-100 dark:bg-indigo-800 text-indigo-900 dark:text-indigo-100 font-medium' : ' hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50'}`}>{t('lab.m6geometry_4_draw_line_from_a_through_int')}</button>
 </div>
 </div>
 </section>
 ...
 {/* Right Column */}
 <div className="w-full lg:w-2/3 p-6 flex flex-col gap-6 lg:overflow-y-auto bg-slate-100 dark:bg-[#1c1b1b]/50 dark:bg-[#121212]/50">
 ...
 <h3 className="absolute top-4 left-4 text-sm font-bold text-slate-800 dark:text-[#ffffff]">{t('lab.m6geometry_rotational_symmetry')}</h3>
 ...
 <h3 className="absolute top-4 left-4 text-sm font-bold text-slate-800 dark:text-[#ffffff]">{t('lab.m6geometry_vertical_axis')}</h3>
 ...
 <text x="65" y="-115" className="text-xs fill-rose-700 dark:fill-rose-300 font-bold">{t('lab.m6geometry_c_60')}</text>
 </>
 )}
 ...
 <text x="-95" y="-115" className="text-xs fill-indigo-700 dark:fill-indigo-300 font-bold">{t('lab.m6geometry_d_120')}</text>
 </>
 )}
 ...
 <text x="10" y="-215" className="text-xs fill-teal-700 dark:fill-teal-300 font-bold">{t('lab.m6geometry_e_90')}</text>
 </>
 )}
