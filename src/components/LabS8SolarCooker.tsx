...
export default function LabS8SolarCooker({ onExit }: LabProps) {
 ...
 <div className="lg:overflow-y-auto flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
  ...
  <div className="w-full bg-sky-100 rounded-3xl shadow-xl border border-sky-200 p-8 flex flex-col items-center relative overflow-hidden h-[500px]">
   ...
   {/* Parabolic Reflector */}
   <div className="relative w-96 h-48 bg-gradient-to-b from-slate-200 to-slate-400 rounded-b-full shadow-[0_20px_30px_rgba(0,0,0,0.2)] border-b-8 border-slate-500 dark:border-[#1c1b1b] flex items-center justify-center z-10 overflow-hidden">
    {/* Foil texture */}
    ...
    <div className="absolute -top-4 w-24 h-20 bg-zinc-800 rounded-b-xl border-x-2 border-b-2 border-zinc-900 shadow-xl flex flex-col justify-end overflow-hidden">
     ...
     <div className={`w-full bg-amber-700 transition-all duration-[3000ms] ${sunlight ? 'h-full' : 'h-1/2'}`}>
      ...
      <div className="w-full h-full flex flex-wrap gap-1 p-1 opacity-50">
       <div className="w-3 h-3 bg-slate-50 dark:bg-[#121212] rounded-full animate-bounce" />
       <div className="w-2 h-2 bg-slate-50 dark:bg-[#121212] rounded-full animate-ping" />
       <div className="w-4 h-4 bg-slate-50 dark:bg-[#121212] rounded-full animate-bounce delay-75" />
      </div>
      )}
     </div>
    </div>
   ...
   {/* Stand */}
   <div className="w-4 h-24 bg-slate-700 dark:bg-[#121212] z-0" />
   <div className="w-48 h-4 bg-[#121212] dark:bg-[#121212] rounded-full z-0" />
   ...
   className={`absolute bottom-10 left-10 px-8 py-3 rounded-full font-bold shadow-lg transition-transform active:scale-95 border-2 ${sunlight ? 'bg-[#121212] dark:bg-[#121212] text-white border-[#1c1b1b] dark:border-slate-500' : 'bg-yellow-400 hover:bg-yellow-500 text-yellow-900 border-yellow-500'}`}
   >
