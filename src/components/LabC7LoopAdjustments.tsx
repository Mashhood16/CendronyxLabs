...
export default function LabC7LoopAdjustments({ onExit }: LabProps) {
 ...
 <div className="flex flex-col min- lg: font-sans bg-slate-50 dark:!bg-[#000000] text-slate-800 dark:text-[#ffffff] min-h-screen lg:h-screen overflow-x-hidden w-full">
  <LabHeader onExit={onExit} title={t('lab.c7loopadjustments_loop_adjustments')} />
  <div className="flex-1 px-8 pb-8 flex flex-col lg:overflow-y-auto items-center">

  <div className="max-w-4xl w-full">
   <p className="text-slate-600 dark:text-[#a1a1aa] mb-8">{t('lab.c7loopadjustments_change_the_repeat_count_and_mo')}</p>

   <div className="flex gap-8">
   {/* Block Editor */}
   <div className="w-80 bg-slate-50 dark:!bg-[#121212] p-6 rounded-xl shadow-lg border border-slate-200 dark:border-[#1c1b1b]">
    <h2 className="font-bold text-slate-700 dark:text-[#ffffff] mb-4 uppercase tracking-wider text-sm">{t('lab.c7loopadjustments_scratch_code')}</h2>
    ...
    <div className="flex items-center text-yellow-600 font-bold mb-2 bg-yellow-100 px-2 py-1 rounded w-fit">
                                      ...
                                      {t('lab.c7loopadjustments_when_clicked')}
                                     </div>
    ...
    <div className="bg-orange-400 text-white p-3 rounded-lg rounded-tl-none font-bold dark:bg-orange-500 dark:hover:bg-orange-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-orange-500/40">
     <div className="flex items-center mb-2">
                                          ...
                                          {t('lab.c7loopadjustments_repeat')} 
                                          <input 
      type="number" 
      value={repeatCount}
    ...
    >
     <Play className="w-5 h-5 mr-2 fill-current" />  {t('lab.c7loopadjustments_run')}
                                     </button>
    <button 
     onClick={reset}
    ...
    >
     <RotateCcw className="w-5 h-5 mr-2" />  {t('lab.c7loopadjustments_reset')}
                                     </button>
    </div>
   </div>
   ...
   <div className="flex-1 bg-slate-50 dark:!bg-[#121212] p-4 rounded-xl shadow-xl border-4 border-slate-300 dark:border-[#1c1b1b] relative overflow-hidden flex flex-col">
    ...
    <span className="absolute bottom-2 left-2 text-xs font-bold text-slate-400">{t('lab.c7loopadjustments_start_0')}</span>
    ...
    >
                                      ...
                                      {t('lab.c7loopadjustments_sprite')}
                                     </div>
    ...
    <div className="absolute top-4 right-4 bg-slate-50 dark:bg-[#121212] px-4 py-2 rounded-lg shadow border border-slate-200 dark:border-[#1c1b1b] font-mono text-sm font-bold text-slate-600 dark:text-[#a1a1aa]">
                                      ...
                                      {t('lab.c7loopadjustments_total_moved')} {position}
    </div>
    </div>
   </div>
   </div>
