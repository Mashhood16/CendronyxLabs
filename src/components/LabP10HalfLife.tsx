      ...
      <div className="flex-1 p-6 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6">
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2"><BookOpen className="w-5 h-5 text-rose-600"/> Theory</h2>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6">
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-rose-600"/> Setup</h2>
            
            <div className="space-y-5">
              <div>
                <div className="flex justify-between text-sm font-medium mb-1">
                  <span className="text-slate-700 dark:text-slate-200">Half-Life (s)</span>
              ...
              <div>
                <div className="flex justify-between text-sm font-medium mb-1">
                  <span className="text-slate-700 dark:text-slate-200">Initial Atoms (N₀)</span>
        ...
        <div className="bg-slate-900 dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-800 dark:border-slate-500 flex flex-col relative overflow-hidden h-full min-h-[500px]">
          ...
          <div className="flex-1 relative w-full flex items-center justify-center p-8">
            <div className="w-full h-full bg-slate-800 dark:bg-slate-800 rounded-xl border-4 border-slate-700 dark:border-slate-500 relative overflow-hidden shadow-inner">
              {atoms.map(atom => (
                <div 
                  key={atom.id}
                  className={`absolute w-2 h-2 rounded-full transition-colors duration-300 ${atom.decayed ? 'bg-slate-700 dark:bg-slate-800' : 'bg-rose-500 shadow-[0_0_6px_#f43f5e]'}`}
          ...
          <div className="bg-slate-800 dark:bg-slate-800 p-4 border-t border-slate-700 dark:border-slate-500 flex flex-col gap-2">
            <div>
            ...
            <div>
              ...
              <div className="w-full bg-slate-700 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                <div className="bg-slate-500 dark:bg-slate-800 h-full transition-all duration-300" style={{ width: `${((initialAtoms - activeCount) / initialAtoms) * 100}%` }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col h-[400px]">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2"><Table2 className="w-5 h-5 text-rose-600"/> Data Log</h2>
              <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-1 rounded text-xs font-bold">Auto-records every 1s</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-lg mb-4">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 dark:bg-slate-900 sticky top-0">
                  <tr>
                    <th className="p-3 font-semibold text-slate-700 dark:text-slate-200">Time (s)</th>
                    <th className="p-3 font-semibold text-slate-700 dark:text-slate-200">Active Atoms</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {loggedData.map((d, i) => (
                    <tr key={i} className="hover:bg-slate-50 dark:bg-slate-900">
                      <td className="p-3 font-mono text-slate-600 dark:text-slate-300">{d.time}</td>
