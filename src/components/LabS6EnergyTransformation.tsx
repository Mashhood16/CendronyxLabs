      ...
      <div className="flex-1 flex flex-col p-6 items-center">
        ...
        <div className="flex gap-4 mb-6">
          <button 
            ...
            className={`px-6 py-2 rounded font-bold border-2 ${tab === 'bouncing' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 text-slate-600 dark:text-slate-300 hover:border-amber-300'}`}
          >
            Activity 8.1: Bouncing Ball
          </button>
          <button 
            onClick={() => setTab('solar')}
            className={`px-6 py-2 rounded font-bold border-2 ${tab === 'solar' ? 'border-yellow-500 bg-yellow-50 text-yellow-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 text-slate-600 dark:text-slate-300 hover:border-yellow-300'}`}
          >
        ...
        {tab === 'bouncing' && (
          <div className="flex gap-8 w-full max-w-4xl">
            {/* Physics Engine View */}
            <div className="w-96 h-[500px] bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col items-center justify-end relative">
              <button 
                onClick={dropBall}
              ...
              <div className="w-full h-full relative border-b-8 border-slate-800 dark:border-slate-500 mt-12">
            ...
            {/* Energy Map */}
            <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-8 flex flex-col justify-center">
              <h2 className="text-2xl font-bold mb-8 text-slate-800 dark:text-slate-100">Energy State Analysis</h2>
              ...
              <div className="space-y-4">
                <div className={`p-4 rounded-xl border-2 transition-colors ${ballState === 'held' || ballState === 'peak' ? 'border-amber-500 bg-amber-50' : 'border-slate-100'}`}>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">Maximum Gravitational Potential Energy (GPE). Zero Kinetic Energy (KE).</p>
                </div>
                ...
                <div className={`p-4 rounded-xl border-2 transition-colors ${ballState === 'falling' ? 'border-blue-500 bg-blue-50' : 'border-slate-100'}`}>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">GPE is converting into Kinetic Energy (KE) as speed increases.</p>
                </div>
                ...
                <div className={`p-4 rounded-xl border-2 transition-colors ${ballState === 'impact' ? 'border-red-500 bg-red-50' : 'border-slate-100'}`}>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">KE converts into Elastic Potential Energy (EPE) as the ball squishes. Some energy is lost as Heat and Sound.</p>
                </div>
                ...
                <div className={`p-4 rounded-xl border-2 transition-colors ${ballState === 'rising' ? 'border-emerald-500 bg-emerald-50' : 'border-slate-100'}`}>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">EPE pushes the ball up, converting back to KE, which then converts back to GPE as it slows down.</p>
                </div>
              </div>

              <button onClick={() => setBallState('held')} className="mt-8 text-slate-500 dark:text-slate-400 font-bold hover:text-slate-800 dark:text-slate-100 self-start">Reset Simulation</button>
            </div>
          </div>
        )}

        {tab === 'solar' && (
          <div className="flex flex-col items-center bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-12 max-w-4xl w-full">
            <div className="flex justify-center gap-24 items-center mb-16">
              {/* Sun Toggle */}
              <div className="flex flex-col items-center">
                <button 
                  onClick={() => setSunlight(!sunlight)}
                  className={`w-24 h-24 rounded-full flex items-center justify-center transition-all ${sunlight ? 'bg-yellow-400 shadow-[0_0_50px_rgba(250,204,21,0.6)] text-white scale-110' : 'bg-slate-200 dark:bg-slate-800 text-slate-400'}`}
                >
                  <Sun className="w-12 h-12" />
                </button>
                <span className="mt-4 font-bold text-slate-600 dark:text-slate-300">{sunlight ? 'Sunlight Active' : 'Sunlight Blocked'}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                <button 
                  ...
                  className="w-20 h-10 bg-slate-300 dark:bg-slate-800 rounded-full relative shadow-inner"
                >
                  <div className={`w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-900 shadow absolute top-1 transition-all ${switchOn ? 'left-11' : 'left-1'}`}></div>
                </button>
                <span className="mt-4 font-bold text-slate-600 dark:text-slate-300">Circuit {switchOn ? 'Closed' : 'Open'}</span>
              </div>
            </div>

            {/* Apparatus */}
            <div className="relative w-full h-64 border-b-4 border-slate-300 dark:border-slate-700 dark:border-slate-500 flex items-end justify-center pb-4 gap-12">
              {/* Solar Panel */}
              <div className="relative">
                {sunlight && (
                  <div className="absolute -top-32 -left-12 w-64 h-64 bg-yellow-400/20 blur-2xl rounded-full"></div>
                )}
                <div className="w-32 h-24 bg-blue-900 border-4 border-slate-800 dark:border-slate-500 rounded flex flex-wrap content-start p-1 gap-1 -rotate-12 transform origin-bottom-right z-10 relative">
                  {[...Array(8)].map((_, i) => (
                    <div key={i} className="w-[calc(50%-2px)] h-[calc(25%-2px)] border border-blue-400/30"></div>
                  ))}
                </div>
                <div className="w-2 h-16 bg-slate-800 dark:bg-slate-800 absolute right-4 bottom-0"></div>
              </div>
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="w-4 h-12 bg-slate-400 dark:bg-slate-800 mt-2 rounded-t"></div>
                <div className="w-16 h-4 bg-slate-500 dark:bg-slate-800 rounded-t"></div>
              </div>
            </div>
