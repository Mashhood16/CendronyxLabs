      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-auto border border-slate-300 shadow-sm rounded relative">
             <table className="w-full text-sm font-mono border-collapse bg-white">
                <thead>
                  <tr className="bg-slate-200 text-slate-600 border-b border-slate-300">
                    ...
                    <th className="border-r border-slate-300 py-1 font-normal w-40">A</th>
                    <th className="border-r border-slate-300 py-1 font-normal w-24">B</th>
                    <th className="border-r border-slate-300 py-1 font-normal w-24">C</th>
                    <th className="border-r border-slate-300 py-1 font-normal w-24">D</th>
                    <th className="border-r border-slate-300 py-1 font-normal w-24">E</th>
                    <th className="border-r border-slate-300 py-1 font-normal w-48">F</th>
                    <th className="border-r border-slate-300 py-1 font-normal w-48">G</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={i} className="border-b border-slate-200">
                      <td className="bg-slate-100 border-r border-slate-300 text-center text-slate-500 py-1 select-none">{i + 1}</td>
                      {row.map((cell, j) => {
                         // Interactive cells
                         if (cell === 'F5') return <td key={j} className="border-r border-slate-200 p-0"><input value={f5} onChange={e=>setF5(e.target.value)} className={`w-full h-full px-2 outline-none font-bold ${isF5Correct ? 'bg-green-100 text-green-800' : 'bg-yellow-50 text-yellow-800'}`} placeholder="=..." /></td>
                         if (cell === 'F7') return <td key={j} className="border-r border-slate-200 p-0"><input value={f7} onChange={e=>setF7(e.target.value)} className={`w-full h-full px-2 outline-none font-bold ${isF7Correct ? 'bg-green-100 text-green-800' : 'bg-yellow-50 text-yellow-800'}`} placeholder="=..." /></td>
                         if (cell === 'G7') return <td key={j} className="border-r border-slate-200 p-0"><input value={g7} onChange={e=>setG7(e.target.value)} className={`w-full h-full px-2 outline-none font-bold ${isG7Correct ? 'bg-green-100 text-green-800' : 'bg-yellow-50 text-yellow-800'}`} placeholder="=..." /></td>
                         if (cell === 'D10') return <td key={j} className="border-r border-slate-200 p-0"><input value={d10} onChange={e=>setD10(e.target.value)} className={`w-full h-full px-2 outline-none font-bold ${isD10Correct ? 'bg-green-100 text-green-800' : 'bg-yellow-50 text-yellow-800'}`} placeholder="=..." /></td>
                         if (cell === 'E11') return <td key={j} className="border-r border-slate-200 p-0"><input value={e11} onChange={e=>setE11(e.target.value)} className={`w-full h-full px-2 outline-none font-bold ${isE11Correct ? 'bg-green-100 text-green-800' : 'bg-yellow-50 text-yellow-800'}`} placeholder="=..." /></td>
                         
                         // Static cells
                         return <td key={j} className={`border-r border-slate-200 px-2 py-1 ${i===1||j===0?'font-bold bg-slate-50':''}`}>{cell}</td>
                      })}
                    </tr>
                  ))}
                </tbody>
             ...
             {showChart && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] bg-white border border-slate-300 shadow-2xl p-6 flex flex-col rounded z-10 animate-fade-in">
                   ...
                   <div className="flex justify-around text-[10px] text-slate-500 mt-2 font-bold rotate-[-15deg]">
                      <span>Fakhar</span>
                      <span>Imam</span>
                      <span>Babar</span>
                      <span>Haris</span>
        ...
        <div className="w-96 bg-white p-8 flex flex-col shadow-[-10px_0_20px_rgba(0,0,0,0.1)] z-10 overflow-y-auto">
          ...
          <div className="space-y-4 mb-8">
            <div className={`p-4 rounded border ${isF5Correct ? 'bg-green-50 border-green-500' : 'bg-slate-50 border-slate-200'}`}>
               <p className="text-sm text-slate-600"><strong>F5:</strong> Write formula for Total Score of Babar Azam.</p>
               {isF5Correct && <span className="text-green-600 font-bold text-xs">✓ Correct ({f5})</span>}
            </div>
            <div className={`p-4 rounded border ${isF7Correct ? 'bg-green-50 border-green-500' : 'bg-slate-50 border-slate-200'}`}>
               <p className="text-sm text-slate-600"><strong>F7:</strong> Write SUM function for Total Score of M. Hafeez.</p>
               {isF7Correct && <span className="text-green-600 font-bold text-xs">✓ Correct</span>}
            </div>
            <div className={`p-4 rounded border ${isG7Correct ? 'bg-green-50 border-green-500' : 'bg-slate-50 border-slate-200'}`}>
               <p className="text-sm text-slate-600"><strong>G7:</strong> Write AVERAGE function for M. Hafeez.</p>
               {isG7Correct && <span className="text-green-600 font-bold text-xs">✓ Correct</span>}
            </div>
            <div className={`p-4 rounded border ${isD10Correct ? 'bg-green-50 border-green-500' : 'bg-slate-50 border-slate-200'}`}>
               <p className="text-sm text-slate-600"><strong>D10:</strong> Write MAX function for Match 2 column.</p>
               {isD10Correct && <span className="text-green-600 font-bold text-xs">✓ Correct</span>}
            </div>
            <div className={`p-4 rounded border ${isE11Correct ? 'bg-green-50 border-green-500' : 'bg-slate-50 border-slate-200'}`}>
               <p className="text-sm text-slate-600"><strong>E11:</strong> Write MIN function for Match 3 column.</p>
          ...
          <button 
            ...
            disabled={!allCorrect}
            className={`py-4 font-bold rounded-xl transition-all shadow-md ${allCorrect ? 'bg-blue-600 hover:bg-blue-700 text-white active:scale-95' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
          >
            Draw Bar Chart
          </button>
          {!allCorrect && <p className="text-xs text-center text-slate-500 mt-2">Complete all formulas to unlock charts.</p>}
