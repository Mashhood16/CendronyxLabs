...
const OBJECTS: ObjectToDrop[] = [
  { id: 'fe', name: 'Iron Coin', type: 'metal', color: 'bg-slate-400 dark:bg-slate-800', shape: 'circle', density: 'high' },
    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans">
      ...
      <div className="flex-1 p-6 flex flex-col md:flex-row gap-6 max-w-6xl mx-auto w-full">
        ...
        <div className="w-full md:w-64 flex flex-col gap-2">
          <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-2">Select Object</h3>
          {OBJECTS.map(o => (
            <button 
              key={o.id}
              onClick={() => { setSelected(o); setDropped(false); }}
              className={`p-3 text-left rounded-lg font-bold transition-all border-2 ${selected?.id === o.id ? 'border-cyan-500 bg-cyan-50 text-cyan-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 bg-slate-50 dark:bg-slate-900 hover:border-slate-300 dark:border-slate-700 dark:border-slate-500 text-slate-700 dark:text-slate-200'}`}
            >
              {o.name}
            </button>
          ))}
        ...
        <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]">
          ...
          <div className="relative w-64 h-80 flex flex-col justify-end items-center">
            ...
            <div className="absolute left-2 top-10 bottom-10 w-4 border-r border-white/50 z-30 opacity-50">
              {[0,1,2,3].map(i => <div key={i} className="w-2 h-px bg-slate-50 dark:bg-slate-900/50 absolute right-0" style={{top: `${i*25}%`}} />)}
            </div>
            ...
            <div className="w-full h-3/4 bg-cyan-400/40 relative z-0 border-t-2 border-cyan-300/60 overflow-hidden rounded-b-lg">
              ...
              <div className="absolute inset-0 opacity-30">
                <div className="w-2 h-2 rounded-full bg-slate-50 dark:bg-slate-900 absolute bottom-4 left-10 animate-[rise_2s_infinite]" />
                <div className="w-3 h-3 rounded-full bg-slate-50 dark:bg-slate-900 absolute bottom-10 right-16 animate-[rise_3s_infinite_1s]" />
                <div className="w-1.5 h-1.5 rounded-full bg-slate-50 dark:bg-slate-900 absolute bottom-20 left-24 animate-[rise_1.5s_infinite_0.5s]" />
              </div>
            </div>
          ...
          <div className="mt-8 text-center h-24">
            {!dropped ? (
              ...
              selected && (
                <div className={`px-6 py-4 rounded-xl border-2 animate-fade-in ${selected.density === 'high' ? 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 dark:border-slate-500 text-slate-800 dark:text-slate-100' : 'bg-cyan-50 border-cyan-200 text-cyan-800'}`}>
