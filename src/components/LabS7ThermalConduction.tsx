    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-amber-50 font-sans">
      ...
      <div className="flex-1 p-8 flex flex-col items-center">
        <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-amber-100 max-w-2xl w-full text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-300 mb-6">A metal spoon, a plastic spoon, and a wooden spoon each have a solid cube of butter placed on their handles. Their bowls are immersed in boiling water. Watch to see which material conducts heat the fastest to melt the butter.</p>
        ...
        <div className="relative w-full max-w-3xl h-96 flex justify-center items-end pb-12 mt-8">
          ...
          {/* Glass Beaker */}
          <div className="absolute bottom-0 w-80 h-48 border-4 border-slate-300 dark:border-slate-700 dark:border-slate-500/60 bg-blue-50/30 rounded-b-3xl backdrop-blur-sm z-20 flex justify-center items-end">
            {/* Boiling Water */}
            <div className={`w-full transition-all duration-1000 rounded-b-2xl overflow-hidden relative ${time > 0 ? 'h-32 bg-red-500/20' : 'h-32 bg-blue-300/30'}`}>
              {running && (
                <div className="absolute inset-0 flex flex-wrap gap-4 p-2 justify-center content-end overflow-hidden opacity-50">
                   <div className="w-2 h-2 rounded-full bg-slate-50 dark:bg-slate-900 animate-[bounce_0.5s_ease-in_infinite]"></div>
                   <div className="w-3 h-3 rounded-full bg-slate-50 dark:bg-slate-900 animate-[bounce_0.6s_ease-in_infinite_0.1s]"></div>
                   <div className="w-1 h-1 rounded-full bg-slate-50 dark:bg-slate-900 animate-[bounce_0.4s_ease-in_infinite_0.3s]"></div>
                   <div className="w-2 h-2 rounded-full bg-slate-50 dark:bg-slate-900 animate-[bounce_0.7s_ease-in_infinite_0.2s]"></div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-8 absolute bottom-12 z-10 w-full justify-center">
             ...
             <div className="flex flex-col items-center relative transform -rotate-12 translate-y-8">
               <div className="font-bold text-slate-700 dark:text-slate-200 absolute -top-8 bg-slate-50 dark:bg-slate-900/80 px-2 rounded z-20">Metal</div>
               <div className="relative">
                 ...
                 ></div>
                 <div className="w-12 h-64 bg-slate-300 dark:bg-slate-800 rounded-full border border-slate-400 dark:border-slate-500 shadow-md"></div>
               </div>
             </div>

             {/* Plastic Spoon */}
             <div className="flex flex-col items-center relative translate-y-12 z-30">
               <div className="font-bold text-slate-700 dark:text-slate-200 absolute -top-8 bg-slate-50 dark:bg-slate-900/80 px-2 rounded z-20">Plastic</div>
               <div className="relative">
                 {/* Butter */}
                 <div 
                   className="absolute -top-4 left-2 w-8 bg-yellow-300 rounded-sm shadow-sm transition-all duration-100 ease-linear z-20"
             ...
             <div className="flex flex-col items-center relative transform rotate-12 translate-y-8 z-10">
               <div className="font-bold text-slate-700 dark:text-slate-200 absolute -top-8 bg-slate-50 dark:bg-slate-900/80 px-2 rounded z-20">Wood</div>
               <div className="relative">
                 {/* Butter */}
                 <div 
                   className="absolute -top-4 left-2 w-8 bg-yellow-300 rounded-sm shadow-sm transition-all duration-100 ease-linear z-20"
        ...
        {time === 100 && (
          <div className="mt-4 p-6 bg-slate-50 dark:bg-slate-900 shadow-lg text-slate-800 dark:text-slate-100 rounded-xl border-t-4 border-red-500 max-w-2xl text-center">
