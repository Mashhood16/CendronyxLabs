...
interface LabHeaderProps {
 onExit?: () => void;
 title: string;
 /** Optional translation key. If provided, title is translated via t(titleKey). Useful for lab titles that should appear in Urdu/Roman Urdu. */
 titleKey?: string;
 subtitle?: string;
...
const SOLID_THEMES: SolidTheme[] = [
  {
    ...
    button: 'p-2 rounded-full transition-colors text-white hover:bg-white dark:bg-[#121212] dark:border-[#1c1b1b]/10',
  },
  {
...
export default function LabHeader({ onExit, title, titleKey, subtitle, subtitleKey, icon, rightContent }: LabHeaderProps) {
 ...
 const { t: ti18n } = useTranslate();
 const displayTitle = titleKey ? ti18n(titleKey) : title;
 ...
 return (
 <>
  <div className={`${containerClass} px-4 py-3 md:px-6 md:py-4 flex flex-wrap items-center justify-between gap-2 sticky top-0 z-10 shadow-sm shrink-0`}>
   ...
   <div className="flex items-center gap-2 md:gap-3 shrink-0 ml-2 md:ml-4">
    ...
    <button
     ...
     className={`${buttonClass} flex items-center gap-1.5 px-3 py-1.5`}
     title={language === 'en' ? 'Roman Urdu mein dekhein' : 'Switch to English'}
    >
     <Languages className="w-5 h-5" />
     <span className="font-bold">{language === 'en' ? 'EN' : 'RU'}</span>
    </button>
   </div>
  </div>
