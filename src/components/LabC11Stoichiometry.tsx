      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col gap-6">
          <div>
            ...
            <ul className="text-xs text-slate-500 dark:text-slate-400 mb-4 list-disc pl-4">
          ...
          <div className="flex-1 bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-500">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Salicylic Acid (g): {salicylicMass}</label>
            <input 
            ...
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Acetic Anhydride (mL): {anhydrideVol}</label>
            <input 
            ...
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Hot Plate Temp (°C): {temperature}</label>
            <input 
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="flex-1 py-2 bg-slate-700 dark:bg-slate-800 disabled:opacity-50 text-white rounded-lg font-semibold flex items-center justify-center gap-2"
              >
                <Filter className="w-4 h-4" /> Filter
              </button>
            </div>
            
            <button onClick={resetExperiment} className="w-full mt-3 py-2 text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-lg font-semibold hover:bg-slate-100 dark:bg-slate-800">
              Reset Glassware
            </button>
          </div>
        </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col items-center justify-center">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 w-full mb-4 text-center">Reaction Beaker</h2>
          
          <svg viewBox="0 0 200 300" className="w-full h-80 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-500">
            ...
            {stage === 'filtered' && actualYield !== null && (
              <text x="100" y="220" textAnchor="middle" className="text-xs font-bold fill-slate-700 bg-slate-50 dark:bg-slate-900">Yield: {actualYield.toFixed(2)} g</text>
            )}
          </svg>
          ...
          <div className="flex-1">
            ...
            <div className="overflow-y-auto max-h-48 border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-lg">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {logs.length === 0 ? (
                    <tr><td colSpan={3} className="px-3 py-4 text-center text-slate-500 dark:text-slate-400 italic">No yields recorded.</td></tr>
                  ) : (
                    logs.map((l, i) => (
          ...
          <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-500">
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50"
              />
              <button 
