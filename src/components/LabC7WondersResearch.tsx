        ...
        <div className="flex items-center gap-4 mb-8 max-w-4xl mx-auto w-full">
          {[1, 2, 3].map(s => (
            <div key={s} className="flex-1">
              <div className={`h-2 rounded-full mb-2 ${step >= s ? 'bg-blue-600' : 'bg-slate-200'}`} />
          ...
          {step === 1 && (
            <div className="p-8 flex flex-col h-full bg-white">
              ...
              <div className="max-w-xl w-full mx-auto relative flex">
                ...
                <input 
                  type="text" 
                  className="w-full border border-slate-300 rounded-full py-3 pl-12 pr-4 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm"
                ...
                <button 
                  ...
                  className="ml-2 bg-slate-100 hover:bg-slate-200 px-6 rounded-full font-medium"
                >
                  Search
                </button>
              </div>
              ...
              {hasSearched && (
                <div className="mt-8 max-w-2xl mx-auto animate-in fade-in">
                  <p className="text-sm text-slate-500 mb-4">About 3,000,000 results</p>
                  <div className="mb-6">
                    ...
                    <p className="text-sm text-slate-600">The New7Wonders of the World was a campaign started in 2000 to choose Wonders of the World from a selection of 200 existing monuments: Great Wall, Petra, Colosseum...</p>
                  </div>
                ...
                <div className="w-full max-w-xl bg-white shadow-lg border border-slate-200 min-h-full p-8 outline-none" contentEditable onInput={(e) => setDocContent(e.currentTarget.textContent || '')}>
                  ...
                  {hasImages ? (
                    <div className="grid grid-cols-2 gap-4 my-4">
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Taj Mahal</div>
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Colosseum</div>
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Machu Picchu</div>
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Christ the Redeemer</div>
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Chichen Itza</div>
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Petra</div>
                      <div className="bg-slate-200 aspect-video rounded border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">Great Wall</div>
                    </div>
                  ) : (
            ...
            <div className="flex flex-col h-full bg-white">
              {isSent ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-slate-600 max-w-sm">Your teacher has received your research document on the Seven Wonders.</p>
                </div>
              ) : (
                <>
                  <div className="bg-slate-100 border-b border-slate-200 p-4 font-bold text-slate-700">New Message</div>
