      ...
      <div className="flex-1 p-4 grid grid-cols-1 lg:grid-cols-3 gap-4 max-w-7xl mx-auto w-full">
        ...
        <div className="bg-slate-50 dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-5 flex flex-col gap-6 lg:overflow-y-auto">
          ...
          <div className="space-y-5">
            <div>
              <label className="block text-sm font-semibold text-slate-700 dark:text-[#ffffff] mb-2">Transport Mode</label>
              <select 
                value={mode} 
                onChange={(e) => { setMode(e.target.value); setDriveProgress(0); }}
                className="w-full p-2 border border-slate-300 dark:border-[#1c1b1b] rounded-lg bg-slate-50 dark:bg-[#121212] focus:ring-2 focus:ring-emerald-500"
              >
            ...
            <div>
              ...
              <input 
                ...
                className="w-full accent-emerald-600"
              />
              <p className="text-xs text-slate-500 dark:text-[#71717a] mt-1">Max capacity for {modes[mode].name} is {modes[mode].maxPax}.</p>
            </div>
          </div>
          
          <div className="mt-auto">
            <h3 className="text-sm font-bold text-slate-800 dark:text-[#ffffff] mb-2">Assessment</h3>
        ...
        <div className="bg-slate-50 dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-5 flex flex-col items-center justify-between">
          <div className="w-full flex justify-between gap-4 mb-4">
             <div className="flex-1 text-center bg-slate-100 dark:bg-[#121212] p-3 rounded-lg border border-slate-200 dark:border-[#1c1b1b]">
               <p className="text-xs text-slate-500 dark:text-[#71717a] uppercase font-bold tracking-wider">Total Emitted</p>
               <p className="text-2xl font-mono text-slate-800 dark:text-[#ffffff]">{simulatedEmissions.toFixed(2)} <span className="text-sm">kg</span></p>
             </div>
          ...
          <div className="flex gap-4 mt-4 w-full justify-center">
            <button 
              ...
              className={`flex flex-1 items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-all ${
                isDriving ? 'bg-slate-400 dark:bg-[#121212]' : 'bg-emerald-600 hover:bg-emerald-700'
              } disabled:opacity-50`}
            >
        ...
        <div className="bg-slate-50 dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-5 flex flex-col gap-4">
          <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff]">Results & Comparison</h2>
          ...
          <div className="h-48 border border-slate-200 dark:border-[#1c1b1b] rounded-lg lg:overflow-y-auto bg-slate-50 dark:bg-[#121212]">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-100 dark:bg-[#121212] sticky top-0 shadow-sm z-10">
                <tr>
                  <th className="px-3 py-2 font-semibold text-slate-700 dark:text-[#ffffff]">Mode</th>
                  <th className="px-3 py-2 font-semibold text-slate-700 dark:text-[#ffffff]">Pax</th>
                  <th className="px-3 py-2 font-semibold text-slate-700 dark:text-[#ffffff]">Total (kg)</th>
                  <th className="px-3 py-2 font-semibold text-slate-700 dark:text-[#ffffff]">Per Pax (kg)</th>
                </tr>
              </thead>
              <tbody>
                {dataLog.length === 0 ? (
          ...
          <div className="flex-1 bg-slate-50 dark:bg-[#121212] border border-slate-200 dark:border-[#1c1b1b] rounded-lg p-4 relative min-h-[200px] flex items-end justify-around pb-8 pt-4">
             {dataLog.length === 0 ? (
               ...
               <>
                 ...
                 <div className="absolute -left-4 top-1/2 -rotate-90 text-xs text-slate-500 dark:text-[#71717a] whitespace-nowrap">Per-Pax CO₂ (kg)</div>
