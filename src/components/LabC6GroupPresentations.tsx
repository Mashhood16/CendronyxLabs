...
export default function LabC6GroupPresentations({ onExit }: LabProps) {
  ...
  if (isPlaying) {
    return (
      <div className="flex h-screen bg-slate-900 dark:bg-slate-800 text-white flex-col items-center justify-center p-8 relative">
        <button 
        ...
        <div className="w-full max-w-5xl aspect-video bg-slate-800 dark:bg-slate-800 rounded-2xl shadow-2xl p-16 flex flex-col justify-center items-center text-center relative border border-slate-700 dark:border-slate-500">
          <div className="absolute top-8 right-8 flex items-center gap-3 text-slate-500 dark:text-slate-400">
    ...
    <div className="flex h-screen font-sans bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      <div className="flex-1 px-8 pb-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex justify-between items-end mb-6">
          <div>
            <p className="text-slate-600 dark:text-slate-300">Select a hardware topic and prepare your slides.</p>
          </div>
          ...
          <button 
            ...
            className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-blue-500/20"
          >
        ...
        <div className="flex gap-8 flex-1 h-[600px]">
          ...
          <div className="w-64 flex flex-col gap-6">
            <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-5">
              <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-3 text-sm tracking-wider uppercase">1. Select Topic</h3>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-5 flex-1 flex flex-col overflow-hidden">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-slate-700 dark:text-slate-200 text-sm tracking-wider uppercase">2. Slides</h3>
                  ...
                  <button
                    ...
                    className={`aspect-video rounded-lg border-2 flex items-center justify-center p-4 text-center transition-colors shadow-sm relative overflow-hidden ${
                      currentSlide === idx 
                        ...
                        : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 bg-slate-50 dark:bg-slate-900 hover:border-blue-300'
                    }`}
                  >
                    <div className="text-xs font-bold text-slate-600 dark:text-slate-300 line-clamp-3">
                      {slide.title || `Slide ${idx + 1}`}
                    </div>
                    <div className="absolute top-2 left-2 w-5 h-5 bg-slate-800 dark:bg-slate-800 text-white rounded-full text-[10px] flex items-center justify-center font-bold">
                      {idx + 1}
                    </div>
                  </button>
                ))}
          ...
          <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col">
            ...
            <div className="flex-1 p-12 bg-slate-100 dark:bg-slate-800/50 flex flex-col overflow-y-auto">
              {!selectedTopic ? (
                ...
                <div className="w-full max-w-3xl mx-auto aspect-video bg-slate-50 dark:bg-slate-900 shadow-lg border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-12 flex flex-col justify-center">
                  <input
                    type="text"
                    value={slides[currentSlide].title}
                    onChange={(e) => updateSlide('title', e.target.value)}
                    placeholder="Click to add title"
                    className="text-4xl font-bold text-center mb-8 outline-none border-b-2 border-transparent hover:border-slate-200 dark:border-slate-700 dark:border-slate-500 focus:border-blue-500 bg-transparent transition-colors placeholder:text-slate-300"
                  />
                  <textarea
                    value={slides[currentSlide].content}
                    onChange={(e) => updateSlide('content', e.target.value)}
                    placeholder="Click to add text"
                    className="text-xl text-center flex-1 outline-none border-2 border-transparent hover:border-slate-200 dark:border-slate-700 dark:border-slate-500 focus:border-blue-500 rounded-xl p-4 bg-transparent transition-colors resize-none placeholder:text-slate-300"
                  />
                </div>
              )}
            </div>
