...
export default function LabC6GroupPresentations({ onExit }: LabProps) {
  ...
  if (isPlaying) {
    return (
      <div className="flex h-screen bg-slate-900 text-white flex-col items-center justify-center p-8 relative">
        <button 
        ...
        <div className="w-full max-w-5xl aspect-video bg-slate-800 rounded-2xl shadow-2xl p-16 flex flex-col justify-center items-center text-center relative border border-slate-700">
          <div className="absolute top-8 right-8 flex items-center gap-3 text-slate-500">
        ...
        <div className="mt-12 flex gap-4">
          <button 
            ...
            className="px-6 py-3 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 rounded-lg font-bold"
          >
            Previous
          </button>
    ...
    <div className="flex h-screen font-sans bg-slate-50 text-slate-800">
      <div className="flex-1 p-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex justify-between items-end mb-6">
          <div>
            <h1 className="text-3xl font-bold mb-2">Group Presentation Builder</h1>
            <p className="text-slate-600">Select a hardware topic and prepare your slides.</p>
          </div>
        ...
        <div className="flex gap-8 flex-1 h-[600px]">
              ...
              <div className="flex flex-col gap-3 overflow-y-auto pr-2 pb-2 flex-1">
                {slides.map((slide, idx) => (
                  ...
                  >
                    ...
                    <div className="absolute top-2 left-2 w-5 h-5 bg-slate-800 text-white rounded-full text-[10px] flex items-center justify-center font-bold">
                      {idx + 1}
                    </div>
                  </button>
                ))}
          ...
          <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col">
            ...
            <div className="flex-1 p-12 bg-slate-100/50 flex flex-col overflow-y-auto">
              {!selectedTopic ? (
                ...
                <div className="w-full max-w-3xl mx-auto aspect-video bg-white shadow-lg border border-slate-200 p-12 flex flex-col justify-center">
                  <input
                    ...
                    className="text-4xl font-bold text-center mb-8 outline-none border-b-2 border-transparent hover:border-slate-200 focus:border-blue-500 bg-transparent transition-colors placeholder:text-slate-300"
                  />
                  <textarea
                    value={slides[currentSlide].content}
                    onChange={(e) => updateSlide('content', e.target.value)}
                    placeholder="Click to add text"
                    className="text-xl text-center flex-1 outline-none border-2 border-transparent hover:border-slate-200 focus:border-blue-500 rounded-xl p-4 bg-transparent transition-colors resize-none placeholder:text-slate-300"
                  />
                </div>
              )}
            </div>
