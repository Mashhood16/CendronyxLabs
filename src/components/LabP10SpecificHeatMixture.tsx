...
export default function LabP10SpecificHeatMixture({ onExit }: LabProps) {
 ...
 <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
  <LabHeader onExit={onExit} title={t('lab.p10specificheatmixture_unit_10_specific_heat_method_o')} subtitle={t('lab.subtitle_determine_specific_heat')} />
  ...
  <div className="lg:hidden w-full px-4 py-4 md:px-6 grid grid-cols-2 gap-2 flex-shrink-0 z-10 relative mb-4">
   ...
   >
                     ...
                     {t('lab.p10specificheatmixture_theory')}
                    </button>
   <button 
    onClick={() => setActiveMobileTab('lab')}
    className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
   >{t('lab.10specificheatmixture_lab')}</button>
  </div>
  ...
  <div className="lg:col-span-1 flex flex-col gap-6 ${activeMobileTab === 'theory' ? 'flex' : 'hidden'} lg:flex">
   ...
   <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 border-b pb-2">{t('lab.10specificheatmixture_theory')}</h2>
   <div className="text-sm text-slate-600 dark:text-[#a1a1aa] space-y-3">
    <p>{t('lab.p10specificheatmixture_when_a_hot_solid_is_placed_in_')}</p>
    <div className={`bg-slate-100 dark:bg-[#121212] p-3 rounded font-mono text-center text-xs text-slate-800 dark:text-[#ffffff] flex-col `}>
                                 ...
                                 {t('lab.p10specificheatmixture_heat_lost_heat_gained')}<br/>
    
                                 {t('lab.p10specificheatmixture_m_s_c_s_t_i_t_eq_m_w_c_w_m_c_c')}
                                 </div>
    <ul className="list-disc pl-5">
    <li>{t('lab.p10specificheatmixture_c_w_water_4186_j_kg_c')}</li>
    <li>{t('lab.p10specificheatmixture_m_c_calorimeter_0_020_kg')}</li>
    <li>{t('lab.p10specificheatmixture_c_c_calorimeter_385_j_kg_c')}</li>
    </ul>
   </div>
   </div>
   ...
   <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 border-b pb-2">{t('lab.10specificheatmixture_setup')}</h2>
   
   <div className="space-y-4">
    <div>
    <label className="block text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.10specificheatmixture_selectsolid')}</label>
    ...
    <div>
    <label className="block text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.p10specificheatmixture_mass_of_solid_g')} {massSolid}</label>
    ...
    <div>
    <label className="block text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.p10specificheatmixture_mass_of_water_g')} {massWater}</label>
    ...
    >
     {isRunning ? <><Pause className="w-4 h-4"/>{t('lab.10specificheatmixture_pause')}</> : <><Play className="w-4 h-4"/>{t('lab.10specificheatmixture_dropsolid')}</>}
    </button>
    </div>
   </div>
   </div>
   ...
   <div className={`bg-slate-50 dark:!bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-5 flex-col ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 border-b pb-2">{t('lab.10specificheatmixture_analysis')}</h2>
   <div className="space-y-3">
    <label className="block text-sm font-medium text-slate-700 dark:text-[#ffffff]">{t('lab.p10specificheatmixture_calculated_c_s_j_kg_c')}</label>
    <div className="flex gap-2">
    <input 
     type="number" 
     value={answerC}
     ...
     placeholder={t('lab.p10specificheatmixture_t_lab_10specificheatmixture_eg')}
    />
    <button onClick={checkAnswer} className="bg-blue-600 text-white px-4 py-2 rounded font-medium hover:bg-blue-700 dark:text-white dark:text-white dark:bg-blue-500 dark:hover:bg-blue-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-blue-500/40">{t('lab.10specificheatmixture_check')}</button>
    </div>
    {feedback && (
  ...
  <div className="lg:col-span-2 flex flex-col gap-6 ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex rounded-t-none lg:rounded-t-xl border-t-0 lg:border-t">
   ...
   <h3 className="absolute top-4 left-4 font-bold text-slate-700 dark:text-[#ffffff]">{t('lab.10specificheatmixture_calorimetersimulation')}</h3>
   <div className={`absolute top-4 right-4 bg-slate-100 dark:bg-[#121212] px-3 py-1 rounded text-sm font-mono border border-slate-200 dark:border-[#1c1b1b] flex-col `}>
                             ...
                             {t('lab.p10specificheatmixture_time')} {time}s
   </div>
   
   <div className="flex gap-16 mt-8">
    {/* Hot Water Bath */}
    <div className={`transition-opacity duration-500 ${time > 0 ? 'opacity-30' : 'opacity-100'} flex flex-col items-center`}>
     <div className="text-sm font-bold text-red-600 mb-2">{t('lab.10specificheatmixture_boilingwater')}</div>
     ...
     <div className="absolute -right-16 top-0 w-2 h-24 bg-slate-50 dark:bg-[#121212] border border-slate-400 dark:border-[#1c1b1b] rounded-full flex items-end">
      ...
      <span className="absolute -right-10 top-0 text-xs font-mono">{t('lab.p10specificheatmixture_100_c')}</span>
     </div>
     </div>
    </div>
    ...
    <div className="flex flex-col items-center">
     <div className="text-sm font-bold text-blue-600 mb-2">{t('lab.10specificheatmixture_calorimeter')}</div>
   ...
   <h3 className="font-bold text-slate-700 dark:text-[#ffffff] mb-2">{t('lab.10specificheatmixture_coolingcurvedatalog')}</h3>
   <div className="flex flex-col md:flex-row gap-4 h-full">
    ...
    <table className="w-full text-sm text-left">
     ...
     <tr>
      <th className="py-2 px-3 font-medium">{t('lab.p10specificheatmixture_time_s')}</th>
      <th className="py-2 px-3 font-medium">{t('lab.p10specificheatmixture_temp_c')}</th>
     </tr>
     </thead>
     <tbody>
     ...
     {dataPoints.length === 0 && (
      <tr><td colSpan={2} className="py-4 text-center text-slate-400">{t('lab.10specificheatmixture_runexperimenttologdata')}</td></tr>
     )}
     </tbody>
    </table>
    </div>
    ...
    {/* Graph */}
    <div className="w-full md:w-2/3 border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] rounded relative bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:bg-[#121212] lg:dark:bg-[#121212] ${activeMobileTab === 'lab' ? 'block' : 'hidden'} lg:block order-first lg:order-none rounded-b-none lg:rounded-b-xl border-b-0 lg:border-b">
    <div className="absolute top-2 left-2 text-xs font-bold text-slate-500 dark:text-[#71717a] rotate-0">{t('lab.p10specificheatmixture_temp_c')}</div>
    <div className="absolute bottom-2 right-2 text-xs font-bold text-slate-500 dark:text-[#71717a]">{t('lab.p10specificheatmixture_time_s')}</div>
