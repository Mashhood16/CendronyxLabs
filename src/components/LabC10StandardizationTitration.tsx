    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col gap-4">
            ...
            <p className="text-slate-600 dark:text-slate-300">Titration is used to determine the unknown concentration of an acid by neutralizing it with a standard base of known concentration.</p>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col items-center relative">
            ...
            <div className="flex gap-4 mb-8">
                <button onClick={addHCl} disabled={flaskFilled} className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded hover:bg-slate-200 dark:bg-slate-800 disabled:opacity-50 text-sm font-medium">Add HCl (25mL)</button>
                <button onClick={addIndicator} disabled={!flaskFilled || indicatorAdded} className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded hover:bg-slate-200 dark:bg-slate-800 disabled:opacity-50 text-sm font-medium">Add Indicator</button>
                <button onClick={fillBurette} disabled={buretteFilled} className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded hover:bg-slate-200 dark:bg-slate-800 disabled:opacity-50 text-sm font-medium">Fill Burette</button>
            </div>

            <div className="relative w-64 h-80 flex flex-col items-center">
                {/* Burette */}
                <div className="relative w-8 h-48 border-x-2 border-b-2 border-slate-400 dark:border-slate-500 rounded-b flex flex-col justify-end overflow-hidden mb-2">
                    {buretteFilled && <div className="w-full bg-blue-100/50 transition-all duration-300" style={{ height: `${((50 - naohAdded) / 50) * 100}%` }}></div>}
                    {buretteFilled && <div className="absolute top-0 w-full text-center text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-1">{naohAdded.toFixed(1)}mL</div>}
                </div>
                {/* Tap Controls */}
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col gap-6">
            ...
            <div className="bg-amber-50 p-5 rounded-xl border border-amber-200 mt-auto">
                ...
                <div className="flex gap-2">
                    <input type="number" step="0.001" value={assessmentAns} onChange={(e) => setAssessmentAns(e.target.value)} placeholder="Molarity (M)" className="flex-1 px-3 py-2 border border-amber-300 rounded focus:outline-none focus:ring-2 focus:ring-amber-500 bg-slate-50 dark:bg-slate-900" />
