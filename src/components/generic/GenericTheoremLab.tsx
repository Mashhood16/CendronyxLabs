...
// â”€â”€ Equation Normalization â”€â”€
...
function useTheoremT(config: TheoremConfig) {
  ...
  return (field: string, fallback: string): string => {
    ...
    if (config.theoremKey) {
      ...
      if (translated && translated.trim() !== '' && translated !== fullKey) return translated;
    }
    return fallback;
  };
}
...
export function GenericTheoremLab({ onExit, config }: { onExit?: () => void; config: TheoremConfig }) {
  ...
  const handleTestCheck = () => {
    ...
    const isCorrect = checkEquation(testEquationInput, expected);
    setTestStepStatus(isCorrect ? 'correct' : 'incorrect');
    if (isCorrect) {
      setCompletedSteps(prev => ({ ...prev, [currentTestStep]: true }));
      const allDone = config.steps.every((_, i) => i === currentTestStep || completedSteps[i]);
      if (allDone) {
        setTestFullyCompleted(true);
      } else {
        setTimeout(() => {
          const nextIdx = currentTestStep + 1;
          setCurrentTestStep(nextIdx);
      ...
      {activeTab === 'learn' ? (
        ...
        <>
          ...
          <div className="lg:hidden w-full px-4 py-4 md:px-6 grid grid-cols-2 gap-2 shrink-0">
            <button onClick={() => setActiveMobileTab('theory')}
              className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? `${config.accentColor} text-white shadow-md` : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.theorem_proof')}</button>
            <button onClick={() => setActiveMobileTab('lab')}
              className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? `${config.accentColor} text-white shadow-md` : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.theorem_interactive')}</button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 w-full">
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 w-full">
            ...
            <div className="flex-1 min-w-0">
              <div className="bg-white dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#1c1b1b] p-4 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-slate-800 dark:text-white">Proof Steps</h2>
              ...
              {testFullyCompleted ? (
                <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950/30 dark:to-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800 p-8 text-center">
                  ...
                  <h3 className="text-lg font-bold text-emerald-800 dark:text-emerald-300 mb-1">Theorem Mastered! ðŸŽ‰</h3>
            ...
            {/* â”€â”€ RIGHT PANEL: Equation Builder (Desktop Only) â”€â”€ */}
            {!testFullyCompleted && (
                ...
                <div className="bg-white dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#1c1b1b] p-5 sticky top-4">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500">Step {currentTestStep + 1} of {config.steps.length}</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 dark:bg-[#000000] rounded-lg border border-slate-200 dark:border-[#2a2a2a] p-4 mb-4">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2 block">
                      Equation for Step {currentTestStep + 1}
                    </label>
                    <EquationBuilder
                      value={testEquationInput}
                      onChange={v => { setTestEquationInput(v); if (testStepStatus !== 'idle') setTestStepStatus('idle'); }}
