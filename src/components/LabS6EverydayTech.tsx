      ...
      <div className="flex-1 flex flex-col p-8 items-center overflow-y-auto">
        ...
        <div className="flex gap-4 mb-8">
          <button 
            ...
            className={`px-6 py-3 rounded-xl border-2 font-bold flex items-center gap-2 ${activeTech === 'satellites' ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 text-slate-600 hover:border-indigo-300'}`}
          >
            <Rocket className="w-5 h-5" /> Satellites
          </button>
          <button 
            onClick={() => setActiveTech('gps')}
            className={`px-6 py-3 rounded-xl border-2 font-bold flex items-center gap-2 ${activeTech === 'gps' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:border-emerald-300'}`}
          >
            <Map className="w-5 h-5" /> GPS Systems
          </button>
          <button 
            onClick={() => setActiveTech('communication')}
            className={`px-6 py-3 rounded-xl border-2 font-bold flex items-center gap-2 ${activeTech === 'communication' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:border-blue-300'}`}
          >
          ...
          {activeTech === 'satellites' && (
            <div className="animate-fade-in">
              ...
              <div className="flex gap-8">
                ...
                <div className="w-64 h-64 bg-slate-900 rounded-xl relative overflow-hidden border-4 border-slate-800 shadow-inner flex items-center justify-center">
                  ...
                  <div className="absolute w-48 h-48 border border-white/20 rounded-full animate-[spin_10s_linear_infinite]">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-4 bg-slate-300 flex items-center justify-between px-1">
          ...
          {activeTech === 'gps' && (
            <div className="animate-fade-in">
              ...
              <div className="flex gap-8">
                ...
                <div className="w-64 h-64 bg-slate-100 rounded-xl border border-slate-300 shadow-inner relative flex items-center justify-center">
                  {/* Phone screen */}
