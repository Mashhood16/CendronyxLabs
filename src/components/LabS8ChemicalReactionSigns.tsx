...
const EXPERIMENTS = [
  ...
  { id: 'precipitate', name: 'CuSO4 + NaOH', resultType: 'Precipitate', initialColor: 'bg-blue-200', finalColor: 'bg-blue-200', dropsColor: 'bg-slate-100 dark:bg-slate-800', hasPrecipitate: true },
  { id: 'gas', name: 'Vinegar + Baking Soda', resultType: 'Gas Evolution', initialColor: 'bg-slate-100 dark:bg-slate-800', finalColor: 'bg-slate-100 dark:bg-slate-800', dropsColor: 'bg-slate-50 dark:bg-slate-900', hasBubbles: true },
];
        ...
        <div className="w-full md:w-64 flex flex-col gap-2">
          ...
          {EXPERIMENTS.map(e => (
            <button 
              ...
              className={`p-3 text-left rounded-lg font-bold border-2 ${selectedExp.id === e.id ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 bg-slate-50 dark:bg-slate-900 hover:border-slate-300 dark:border-slate-700 dark:border-slate-500'}`}
            >
              {e.name}
            </button>
          ))}
        ...
        <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col items-center justify-center relative min-h-[400px]">
          ...
          <div className="relative w-64 h-80 flex flex-col justify-end items-center">
            ...
            <div className={`absolute top-0 right-10 flex flex-col items-center transition-transform duration-500 ${stage === 'adding' ? 'translate-y-4' : '-translate-y-10 opacity-0'}`}>
              <div className="w-6 h-16 bg-slate-200 dark:bg-slate-800 rounded-full rounded-b-none border-x-2 border-t-2 border-slate-300 dark:border-slate-700 dark:border-slate-500" />
              <div className="w-2 h-10 bg-slate-300 dark:bg-slate-800 border-x border-slate-400 dark:border-slate-500" />
              {stage === 'adding' && (
            ...
            {/* */}
            <div className="w-48 h-48 border-4 border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-b-3xl border-t-0 relative overflow-hidden bg-slate-50 dark:bg-slate-900/50 backdrop-blur-sm z-10 shadow-inner flex flex-col justify-end">
              <div 
                ...
                {selectedExp.hasBubbles && stage === 'final' && (
                  <div className="absolute inset-0">
                    {Array.from({length: 15}).map((_, i) => (
                      <div key={i} className="absolute w-2 h-2 rounded-full border border-white/80 bg-slate-50 dark:bg-slate-900/20 animate-[rise_1s_infinite]" style={{ left: `${Math.random() * 90}%`, animationDelay: `${Math.random() * 2}s` }} />
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="mt-8 text-center flex gap-4">
            ...
            <button onClick={reset} className="bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-6 py-2 rounded-lg font-bold hover:bg-slate-300 dark:bg-slate-800">
              Reset
            </button>
          </div>
