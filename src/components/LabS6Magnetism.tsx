      ...
      <div className="flex-1 flex flex-col p-6 items-center">
        ...
        <div className="flex gap-4 mb-6">
          <button 
            ...
            className={`px-6 py-2 rounded font-bold border-2 ${view === 'field' ? 'border-red-500 bg-red-50 text-red-700' : 'border-slate-200 text-slate-600 hover:border-red-300'}`}
          >
            Activity 10.1: Magnetic Field Lines
          </button>
          <button 
            onClick={() => setView('alignment')}
            className={`px-6 py-2 rounded font-bold border-2 ${view === 'alignment' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:border-blue-300'}`}
          >
        ...
        {view === 'field' && (
          <div className="w-full max-w-4xl bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col items-center relative overflow-hidden">
            ...
            <div className="relative w-[600px] h-[400px] flex items-center justify-center">
              ...
              {/* The Bar Magnet */}
              <div className="flex w-64 h-16 shadow-lg relative z-10 rounded overflow-hidden border-2 border-slate-800">
        ...
        {view === 'alignment' && (
          <div className="w-full max-w-4xl bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col items-center">
            ...
            <div className="relative w-96 h-96 border-4 border-slate-200 rounded-full bg-slate-50 flex items-center justify-center overflow-hidden">
              {/* Compass directions */}
              ...
              {/* Thread */}
              <div className="absolute top-1/2 left-1/2 w-1 h-32 bg-slate-300 origin-top -rotate-[20deg] z-10 animate-[swing_4s_ease-in-out_infinite_alternate]">
                {/* Suspended Magnet */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-32 flex flex-col shadow-lg border-2 border-slate-800 rounded">
