      ...
      <div className="flex-1 p-4 grid grid-cols-1 lg:grid-cols-3 gap-4 max-w-7xl mx-auto w-full">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 flex flex-col gap-6 overflow-y-auto">
          ...
          <div className="space-y-5">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Insulating Material</label>
              <select 
                value={material} 
                onChange={(e) => { setMaterial(e.target.value); reset(); }}
                disabled={isRunning || time > 0}
                className="w-full p-2 border border-slate-300 rounded-lg bg-slate-50 focus:ring-2 focus:ring-blue-500"
              >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 flex flex-col items-center justify-between">
          <div className="w-full flex justify-between items-center mb-4">
             <div className="text-center bg-slate-100 px-4 py-2 rounded-lg border border-slate-200">
               ...
               <p className="text-2xl font-mono text-slate-800">{time.toString().padStart(2, '0')}:00</p>
             </div>
             <div className="text-center bg-slate-100 px-4 py-2 rounded-lg border border-slate-200">
               <p className="text-xs text-slate-500 uppercase font-bold tracking-wider">Thermometer</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 flex flex-col gap-4">
          ...
          <div className="h-48 border border-slate-200 rounded-lg overflow-y-auto bg-slate-50">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-100 sticky top-0 shadow-sm">
                <tr>
                  <th className="px-4 py-2 font-semibold text-slate-700">Time (min)</th>
              ...
              <tbody>
                {dataLog.length === 0 ? (
                  ...
                  dataLog.map((row, i) => (
                    <tr key={i} className="border-b border-slate-100 hover:bg-slate-100 transition-colors">
                      <td className="px-4 py-1.5 font-mono text-slate-600">{row.t}</td>
          ...
          <div className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-4 relative min-h-[250px]">
             {/* Graph SVG */}
