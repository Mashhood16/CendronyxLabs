...
export default function LabC12Agriculture({ onExit }: { onExit?: () => void }) {
 ...
 <text x="20" y="30" fill="white" className="text-sm">{t('lab.c12agriculture_ache_enzyme_activity')} {activity.toFixed(1)}%</text>
 ...
 {activity === 0 && <text x="120" y="180" fill="#ef4444" className="text-lg font-bold">{t('lab.c12agriculture_synaptic_overload')}</text>}
 </svg>
 );
 } else if (scenario === 'AcidRain') {
 ...
 return (
 <svg viewBox="0 0 400 300" className="w-full h-64 bg-[#000000] dark:bg-[#121212] rounded-lg shadow-inner">
 <text x="20" y="30" fill="white" className="text-sm">{t('lab.c12agriculture_ca_leached')} {leached.toFixed(0)} {t('lab.c12agriculture_units')}</text>
 ...
 <text x="320" y="100" fill="#ef4444" className="text-xs">{t('lab.c12agriculture_h_rain')}</text>
 ...
 {leached > 0 && <text x="250" y="270" fill="#3b82f6" className="text-xs">{t('lab.c12agriculture_ca_leached_1')}</text>}
 </svg>
 );
 } else {
 ...
 return (
 <svg viewBox="0 0 400 300" className="w-full h-64 bg-[#000000] dark:bg-[#121212] rounded-lg shadow-inner">
 <text x="20" y="30" fill="white" className="text-sm">{t('lab.c12agriculture_pest_survival')} {survival.toFixed(1)}%</text>
 ...
 {survival === 0 && <text x="130" y="250" fill="#ef4444" className="text-lg font-bold">{t('lab.c12agriculture_pests_eliminated')}</text>}
 </svg>
 );
 }
 };
 ...
 {/* Header */}
 <LabHeader onExit={onExit} title={t('lab.c12agriculture_interactive_agrochemicals_lab')} />
 ...
 {t('lab.c12agriculture_theory')}
 </button>
 <button 
 onClick={() => setActiveMobileTab('lab')}
 className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
 >{t('lab.c12agriculture_lab')}</button>
 </div>
 <main className="lg:flex-1 p-6 flex flex-col lg:grid lg:grid-cols-3 gap-0 lg:gap-6 max-w-7xl mx-auto w-full min-h-0 lg:overflow-hidden">
 
 {/* Left Column: Theory & Setup */}
 <div className={`w-full bg-slate-50 dark:!bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-6 flex-col gap-6 ${activeMobileTab === 'theory' ? 'flex' : activeMobileTab === 'lab' ? 'flex mb-4' : 'hidden'} lg:flex lg:order-none`}>
 ...
 {t('lab.c12agriculture_process_dynamics')}
 </h2>
 {scenario === 'Pesticide' && (
 <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mt-2 leading-relaxed">
 <strong>{t('lab.c12agriculture_organophosphate_pesticides')}</strong> {t('lab.c12agriculture_irreversibly_inhibit_acetylcho')}
 </p>
 )}
 {scenario === 'AcidRain' && (
 <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mt-2 leading-relaxed">
 <strong>{t('lab.c12agriculture_acid_rain')}</strong> {t('lab.c12agriculture_contains_elevated_h_ions_these')}
 </p>
 )}
 {scenario === 'GMCrops' && (
 <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mt-2 leading-relaxed">
 <strong>{t('lab.c12agriculture_gm_crops')}</strong> {t('lab.c12agriculture_are_engineered_to_express_bt_t')}
 </p>
 )}
 </div>

 <div className={`flex-1 ${activeMobileTab === 'lab' ? 'block' : 'hidden'} lg:block`}>
 <h3 className="font-semibold text-slate-700 dark:text-[#ffffff] mb-4">{t('lab.c12agriculture_experimental_setup')}</h3>
 ...
 <span className="text-xs text-slate-500 dark:text-[#71717a] uppercase font-bold tracking-wider">{t('lab.c12agriculture_elapsed_time')}</span>
 ...
 {t('lab.c12agriculture_agro_visualizer')}
 </h2>
 ...
 {t('lab.c12agriculture_data_analysis')}
 </h2>
 <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mb-6">
 
 {t('lab.c12agriculture_calculate_the_agrochemical_imp')}
 </p>

 <div className="bg-green-50 p-4 rounded-lg border border-green-100 mb-6 dark:bg-[#121212] dark:border-[#1c1b1b]">
 <h3 className="font-semibold text-green-900 mb-2 dark:text-[#ffffff]">{t('lab.c12agriculture_assignment_question')}</h3>
 {scenario === 'Pesticide' && (
 <p className="text-sm text-green-800 dark:text-[#ffffff]">
 
 {t('lab.c12agriculture_if_the_pesticide_concentration')} <strong>{t('lab.c12agriculture_5_0_m')}</strong>{t('lab.c12agriculture_what_will_be_the_exact_ache_en')} <strong>{t('lab.c12agriculture_t_5_0_s')}</strong>? <br/><br/><em>{t('lab.c12agriculture_rate_activity_loss_conc_time_2')}</em>
 </p>
 )}
 {scenario === 'AcidRain' && (
 <p className="text-sm text-green-800 dark:text-[#ffffff]">
 
 {t('lab.c12agriculture_at_a_rain')} <strong>{t('lab.c12agriculture_ph_of_4_0')}</strong>{t('lab.c12agriculture_calculate_the_total_amount_of_')} <strong>{t('lab.c12agriculture_t_5_0_s')}</strong>. <br/><br/><em>{t('lab.c12agriculture_rate_leached_time_10_6_ph')}</em>
 </p>
 )}
 {scenario === 'GMCrops' && (
 <p className="text-sm text-green-800 dark:text-[#ffffff]">
 
 {t('lab.c12agriculture_at_a_bt_toxin_expression_level')} <strong>{t('lab.c12agriculture_8_units')}</strong>{t('lab.c12agriculture_at_what_exact_time_in_seconds_')} <strong>0%</strong>? <br/><br/><em>{t('lab.c12agriculture_rate_survival_loss_expression_')}</em>
 </p>
 )}
 </div>
 ...
 <div>
 <label className="block text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.c12agriculture_your_answer')}</label>
 <input
 type="number"
 value={answer}
 onChange={(e) => setAnswer(e.target.value)}
 placeholder={t('lab.c12agriculture_enter_numerical_value')}
 ...
 {t('lab.c12agriculture_check_answer')}
 </button>
