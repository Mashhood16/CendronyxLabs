...
import { ArrowLeft, CheckCircle, AlertTriangle, Monitor, MousePointer, Eye, Volume2 } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen font-sans bg-slate-50 text-slate-800">
      <div className="flex-1 p-8 flex flex-col overflow-y-auto">
        ...
        <h1 className="text-3xl font-bold mb-2">Health Awareness Campaign</h1>
        <p className="text-slate-600 mb-8">Design a community awareness campaign about the adverse health effects of ICT use.</p>
          ...
          {isPublished ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              ...
              <div className={`p-6 rounded-xl border-2 border-dashed max-w-2xl w-full mb-8 ${topics.find(t => t.id === selectedTopic)?.bg}`}>
                <p className="text-xl font-medium text-slate-700">{campaignMessage}</p>
              </div>
              <button 
                onClick={() => setIsPublished(false)}
                className="px-6 py-3 bg-slate-200 hover:bg-slate-300 rounded-lg font-bold transition-colors"
              >
            ...
            <div className="flex flex-col h-full">
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 text-left transition-all flex items-start gap-4 ${
                        selectedTopic === topic.id ? 'border-blue-500 bg-blue-50' : 'border-slate-200 hover:border-slate-300'
                      }`}
                    >
                      ...
                      <div>
                        ...
                        <div className="text-sm text-slate-500">{topic.desc}</div>
                      </div>
                    </button>
                  );
                })}
              ...
              <div className={`transition-opacity duration-300 ${selectedTopic ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Awareness Message</label>
                    <textarea 
                      rows={3}
                ...
                <div className="flex justify-end mt-auto">
                  <button 
                    ...
                    className="flex items-center gap-2 px-8 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed transition-colors"
                  >
