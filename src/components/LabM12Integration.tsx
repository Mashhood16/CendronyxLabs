...
export default function LabM12Integration({ onExit }: { onExit?: () => void }) {
 ...
 return (
  <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
   <LabHeader onExit={onExit} title={t('lab.m12integration_m12_integration_lab')} />
  ...
  <div className="lg:hidden w-full px-4 py-4 md:px-6 grid grid-cols-2 gap-2 flex-shrink-0 z-10 relative mb-4">
   ...
   >
                     ...
                     {t('lab.m12integration_theory')}
                    </button>
   <button 
    onClick={() => setActiveMobileTab('lab')}
    className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
   >{t('lab.m12integration_lab')}</button>
  </div>
  <div className="p-6 flex flex-col lg:grid lg:grid-cols-3 gap-0 lg:gap-6 flex-grow lg:overflow-visible">
    ...
    <div className={`w-full bg-slate-50 dark:!bg-[#121212] p-6 rounded-xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] flex-col ${activeMobileTab === 'theory' ? 'flex' : 'hidden'} lg:flex`}>
     <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 flex items-center"><BookOpen className="w-5 h-5 mr-2 text-blue-600" />{t('lab.m12integration_theory_context')}</h2>
     <div className="prose prose-slate prose-sm text-slate-600 dark:text-[#a1a1aa]">
      {tab === 'solid' && (
       <>
        <p><strong>{t('lab.m12integration_solids_of_revolution_disc_meth')}</strong></p>
        <p><MathText>{t('lab.m12integration_revolving_the_region_under_f_x')}</MathText></p>
        <p><MathText>{t('lab.m12integration_the_volume_of_an_infinitesimal')}</MathText></p>
        <p className="font-semibold text-blue-700"><MathText>{"$$ V = \\int_{0}^{4} \\pi [k(-x^2 + 4x)]^2 dx $$"}</MathText></p>
        <p><MathText>{t('lab.m12integration_we_can_approximate_this_visual')}</MathText></p>
       </>
      )}
      {tab === 'dam' && (
       <>
        <p><strong>{t('lab.m12integration_hydrostatic_force_on_a_vertica')}</strong></p>
        <p><MathText>{t('lab.m12integration_the_pressure_exerted_by_a_flui')}</MathText></p>
        <p><MathText>{t('lab.m12integration_the_force_on_a_thin_horizontal')}</MathText></p>
        <p><MathText>{t('lab.m12integration_total_force_f_is_the_integral')}</MathText></p>
        <p className="font-semibold text-blue-700"><MathText>{"$$ F = \\int_{0}^{h} \\rho g (h - y) W dy $$"}</MathText></p>
        <p><MathText>{t('lab.m12integration_where_rho_1000_kg_m_and_g_9_8_')}</MathText></p>
       </>
      )}
      {tab === 'spring' && (
       <>
        <p><strong>{t('lab.m12integration_work_done_stretching_a_spring')}</strong></p>
        <p><MathText>{t('lab.m12integration_according_to_hooke_s_law_the_f')}</MathText></p>
        <p><MathText>{t('lab.m12integration_the_work_w_done_in_stretching_')}</MathText></p>
        <p className="font-semibold text-blue-700"><MathText>{"$$ W = \\int_{0}^{x} k s ds = \\frac{1}{2} k x^2 $$"}</MathText></p>
       </>
      )}
     </div>
    </div>
    ...
    <div className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:!bg-[#121212] p-6 rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] flex-col '' : ''} ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
     <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 flex items-center"><Activity className="w-5 h-5 mr-2 text-blue-600" />{t('lab.m12integration_interactive_visualizer')}</h2>
     ...
     {tab === 'solid' && (
      <div className="flex-grow flex flex-col">
       ...
       <label className="text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.m12integration_scale_factor')}<MathText>{t('lab.m12integration_k')}</MathText>): {kSolid.toFixed(1)}</label>
       <input type="range" min="0.5" max="2" step="0.1" value={kSolid} onChange={(e) => setKSolid(Number(e.target.value))} className="mb-2" />
       <label className="text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.m12integration_number_of_disks')}<MathText>{t('lab.m12integration_n')}</MathText>): {nDisks}</label>
     ...
     {tab === 'dam' && (
      <div className="flex-grow flex flex-col">
       ...
       <label className="text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.m12integration_water_depth')}<MathText>{t('lab.m12integration_h')}</MathText>): {damH} m</label>
     ...
     {tab === 'spring' && (
      <div className="flex-grow flex flex-col">
       <svg viewBox="0 0 400 300" className="w-full bg-slate-50 dark:bg-[#121212] rounded-lg shadow-inner border mb-4">
        ...
        <text x={spEnd + 20} y="160" fill="white" fontSize="12" textAnchor="middle" fontWeight="bold">{springK}  {t('lab.m12integration_n_m')}</text>
       </svg>
       <label className="text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.m12integration_spring_constant')}<MathText>{t('lab.m12integration_k')}</MathText>): {springK}  {t('lab.m12integration_n_m')}</label>
       <input type="range" min="10" max="100" step="10" value={springK} onChange={(e) => setSpringK(Number(e.target.value))} className="mb-2" />
       <label className="text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.m12integration_stretch_distance')}<MathText>{t('lab.m12integration_x')}</MathText>): {springX.toFixed(1)} m</label>
    ...
    <div className={`bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:!bg-[#121212] p-6 rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] rounded-t-none lg:rounded-t-xl border-t-0 lg:border-t flex-col ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
     <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 flex items-center"><Calculator className="w-5 h-5 mr-2 text-blue-600" />{t('lab.m12integration_assessment')}</h2>
     <div className={`bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6 text-sm text-blue-900 dark:bg-teal-950/20 dark:border-teal-900 dark:text-[#ffffff] `}>
      {tab === 'solid' && <p><MathText>{t('lab.m12integration_calculate_the_exact_volume_of_')} {kSolid}{t('lab.m12integration_use_pi_approx_3_14159')}</MathText></p>}
      {tab === 'dam' && <p><MathText>{t('lab.m12integration_calculate_the_total_hydrostati')} {damH}{t('lab.m12integration_m_width_w_100_m')}</MathText></p>}
      {tab === 'spring' && <p><MathText>{t('lab.m12integration_calculate_the_exact_work_done_')} {springX.toFixed(1)}{t('lab.m12integration_m_with_k')} {springK}{t('lab.m12integration_n_m_1')}</MathText></p>}
     </div>
     
     <div className="space-y-4">
      <div>
       <label className="block text-sm font-medium text-slate-700 dark:text-[#ffffff] mb-1">{t('lab.m12integration_your_answer')}</label>
       <input 
        type="text" 
        value={userAns} 
        ...
        placeholder={t('lab.m12integration_enter_numerical_value')}
       />
      </div>
      <button 
       onClick={checkAnswer}
      ...
      >
                                ...
                                {t('lab.m12integration_check_answer')}
                               </button>
