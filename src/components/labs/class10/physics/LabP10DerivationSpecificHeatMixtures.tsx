...
export default function LabP10DerivationSpecificHeatMixtures({ onExit }: { onExit?: () => void }) {
 ...
 { label: t('lab.p10_shMix_step2_label'), formula: 'Q_s = m_s c_s (T2 âˆ’ T3)', detail: t('lab.p10_shMix_step2_detail') },
 { label: t('lab.p10_shMix_step3_label'), formula: 'Q_w + Q_c = (m_w c_w + m_c c_c)(T3 âˆ’ T1)', detail: t('lab.p10_shMix_step3_detail') },
 { label: t('lab.p10_shMix_step4_label'), formula: 'c_s = (m_w c_w + m_c c_c)(T3 âˆ’ T1) / m_s (T2 âˆ’ T3)', detail: t('lab.p10_shMix_step4_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "Q_lost = m_s * c_s * (T_s - T_f)",
    "testHint": "Heat lost by hot solid Q = m_s c_s (T_s - T_f)."
  },
  {
    "testEquation": "Q_gained = (m_w * c_w + m_c * c_c) * (T_f - T_i)",
    "testHint": "Heat gained by water and calorimeter Q = (m_w c_w + m_c c_c)(T_f - T_i)."
  },
  {
    "testEquation": "Q_lost = Q_gained",
    "testHint": "By principle of conservation of energy, Heat Lost = Heat Gained."
  },
  {
    "testEquation": "c_s = ((m_w * c_w + m_c * c_c) * (T_f - T_i)) / (m_s * (T_s - T_f))",
    "testHint": "Solve for solid specific heat capacity c_s."
  }
];
