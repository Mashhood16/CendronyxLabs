...
import { Atom, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationRadioactiveDecay({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_radDecay_step1_label'), formula: 'N₁ = ½ N₀', detail: t('lab.p10_radDecay_step1_detail') },
 { label: t('lab.p10_radDecay_step2_label'), formula: 'N₂ = ½ × ½ N₀ = (½)² N₀', detail: t('lab.p10_radDecay_step2_detail') },
 { label: t('lab.p10_radDecay_step3_label'), formula: 'N = (½)ⁿ N₀ = N₀ / 2ⁿ', detail: t('lab.p10_radDecay_step3_detail') },
 { label: t('lab.p10_radDecay_step4_label'), formula: 'n = Δt / T½', detail: t('lab.p10_radDecay_step4_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "N = N_o / 2",
    "testHint": "After 1 half-life, half of the original nuclei remain."
  },
  {
    "testEquation": "N = N_o / 4",
    "testHint": "After 2 half-lives, one quarter (1/2^2) remain."
  },
  {
    "testEquation": "N = N_o / 2^n",
    "testHint": "After n half-lives, N = N_o / 2^n where n = t / T_{1/2}."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Radioactive Decay Formula")} />
 ...
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-emerald-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_radDecay_subtitle')}</p></div>
 </div>
 ...
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="N = N₀ / 2^(Δt / T½)" className="text-xl font-bold" /></div>
 ...
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_radDecay_adjust_desc')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_radDecay_no_label')}</span><span className="text-emerald-600 font-mono">{no}</span></div><input type="range" min="100" max="5000" step="50" value={no} onChange={e => { setNo(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-emerald-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_radDecay_hl_label')}</span><span className="text-emerald-600 font-mono">{halfLife} s</span></div><input type="range" min="1" max="60" step="1" value={halfLife} onChange={e => { setHalfLife(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-emerald-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_radDecay_time_label')}</span><span className="text-emerald-600 font-mono">{totalTime} s</span></div><input type="range" min={halfLife} max={60 * halfLife} step="1" value={totalTime} onChange={e => { setTotalTime(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-emerald-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_radDecay_trace1', { remaining: remaining.toFixed(0) })}</p>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_radDecay_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Atom className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_radDecay_practice_q')}</p>
 <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded p-2 mt-2"><p className="text-xs text-emerald-700 dark:text-emerald-300 font-mono">{t('lab.p10_radDecay_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_radDecay_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-emerald-500 outline-none" />
 ...
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_radDecay_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_radDecay_incorrect_fb')}</p></div>}
 </div>
