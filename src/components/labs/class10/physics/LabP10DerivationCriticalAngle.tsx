...
import { Eye, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationCriticalAngle({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_critical_step1_label'), formula: 'nâ‚ sinÎ¸c = nâ‚‚ sin 90Â°', detail: t('lab.p10_critical_step1_detail') },
 { label: t('lab.p10_critical_step2_label'), formula: 'nâ‚ sinÎ¸c = nâ‚‚', detail: t('lab.p10_critical_step2_detail') },
 { label: t('lab.p10_critical_step3_label'), formula: 'Î¸c = sinâ»Â¹(nâ‚‚ / nâ‚)', detail: t('lab.p10_critical_step3_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "n_1 sin(theta_c) = n_2",
    "testHint": "Apply Snell's law at the critical angle where theta_2 = 90 degrees."
  },
  {
    "testEquation": "n_1 sin(theta_c) = n_2",
    "testHint": "Since sin(90Â°) = 1, the right side reduces to n_2."
  },
  {
    "testEquation": "theta_c = asin(n_2 / n_1)",
    "testHint": "Isolate theta_c by taking the inverse sine (asin) of (n_2 / n_1)."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Critical Angle for TIR")} />
 ...
 <button onClick={() => setActiveMobileTab('theory')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? 'bg-fuchsia-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.derivation')}</button>
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-fuchsia-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_critical_subtitle')}</p></div>
 </div>
 <div className="bg-gradient-to-br from-fuchsia-500 to-purple-600 rounded-xl p-5 text-center shadow-lg mb-3">
 <p className="text-xs text-fuchsia-200 font-semibold uppercase tracking-wider">{t('lab.final_formula')}</p>
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="Î¸c = sinâ»Â¹(nâ‚‚ / nâ‚)" className="text-xl font-bold" /></div>
 <p className="text-xs text-fuchsia-200 mt-1">{t('lab.p10_critical_final_desc')}</p>
 </div>
 <div className="space-y-0">
 {steps.map((step, idx) => (
 <div key={idx} className="relative">
 <div className="flex gap-3"> <div className="flex flex-col items-center">
 ...
 <div className="flex items-center gap-2 mb-3"><Lightbulb className="w-5 h-5 text-fuchsia-500" /><h2 className="text-lg font-bold">{t('lab.see_in_action')}</h2></div>
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_critical_adjust_desc')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_critical_n1_label')}</span><span className="text-fuchsia-600 font-mono">{n1.toFixed(2)}</span></div><input type="range" min="1.33" max="2.5" step="0.01" value={n1} onChange={e => { setN1(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-fuchsia-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_critical_n2_label')}</span><span className="text-fuchsia-600 font-mono">{n2.toFixed(2)}</span></div><input type="range" min="1" max="1.5" step="0.01" value={n2} onChange={e => { setN2(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-fuchsia-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_critical_trace1')}</p>
 <p className="text-sm text-slate-400">{t('lab.p10_critical_trace2', { n2: n2.toFixed(2), n1: n1.toFixed(2), ratio: (n2/n1).toFixed(4) })}</p>
 <p className="border-t border-[#2a2a2a] pt-1 text-xs">{t('lab.p10_critical_trace3', { theta: thetaC.toFixed(1) })}</p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_critical_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Eye className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_critical_practice_q')}</p>
 <div className="bg-fuchsia-50 dark:bg-fuchsia-900/20 rounded p-2 mt-2"><p className="text-xs text-fuchsia-700 dark:text-fuchsia-300 font-mono">{t('lab.p10_critical_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_critical_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-fuchsia-500 outline-none" />
 <button onClick={checkAnswer} className="px-4 py-2 bg-fuchsia-600 hover:bg-fuchsia-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button>
 </div>
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_critical_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_critical_incorrect_fb')}</p></div>}
 </div>
