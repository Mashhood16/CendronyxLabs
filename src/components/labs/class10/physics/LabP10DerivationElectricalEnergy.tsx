...
import { Battery, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationElectricalEnergy({ onExit }: { onExit?: () => void }) {
 ...
 const e1 = v * i * timeSec;
 const e2 = i * i * (v / i) * timeSec;
 const e3 = (v * v) / (v / i) * timeSec;
 ...
 const steps = [
 { label: t('lab.p10_elecenergy_step1_label'), formula: 'E = qV', detail: t('lab.p10_elecenergy_step1_detail') },
 { label: t('lab.p10_elecenergy_step2_label'), formula: 'E = I t V', detail: t('lab.p10_elecenergy_step2_detail') }
 ];
  ...
  {
    "testEquation": "W = V * Q",
    "testHint": "Work done moving charge Q through potential difference V is W = V Q."
  },
  {
    "testEquation": "Q = I * t",
    "testHint": "Electric charge Q equals current I times time t."
  },
  {
    "testEquation": "E = V * I * t",
    "testHint": "Substitute Q = I t into work equation to get E = V I t."
  },
  {
    "testEquation": "E = I^2 * R * t",
    "testHint": "Substitute V = I R (Joule's Law of Heating)."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Electrical Energy E = Pt")} />
 ...
 <button onClick={() => setActiveMobileTab('theory')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.derivation')}</button>
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_elecenergy_subtitle')}</p></div>
 </div>
 ...
 <div className="text-lg font-bold text-white mt-1"><MathFormula formula="E = IVt = IÂ²Rt = VÂ²t/R" className="text-base font-bold" /></div>
 <p className="text-xs text-indigo-200 mt-1">{t('lab.p10_elecenergy_final_desc')}</p>
 </div>
 ...
 <div className="flex items-center gap-2 mb-3"><Lightbulb className="w-5 h-5 text-indigo-500" /><h2 className="text-lg font-bold">{t('lab.see_in_action')}</h2></div>
 <p className="text-sm text-slate-500 mb-4">{t('lab.adjust_sliders')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_elecenergy_p_label')}</span><span className="text-indigo-600 font-mono">{v} V</span></div><input type="range" min="3" max="240" step="1" value={v} onChange={e => { setV(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-indigo-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_power_i_label')}</span><span className="text-indigo-600 font-mono">{i} A</span></div><input type="range" min="0.1" max="10" step="0.1" value={i} onChange={e => { setI(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-indigo-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_elecenergy_t_label')}</span><span className="text-indigo-600 font-mono">{timeSec} s</span></div><input type="range" min="10" max="300" step="10" value={timeSec} onChange={e => { setTimeSec(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-indigo-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_elecenergy_trace1')}</p>
 <p className="text-sm text-slate-400">{t('lab.p10_elecenergy_trace2', { p: v.toFixed(0), t: timeSec.toFixed(0) })}</p>
 <p className="text-sm text-slate-400">{t('lab.p10_elecenergy_trace3', { e: e1.toFixed(0) })}</p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_elecenergy_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Battery className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_elecenergy_practice_q')}</p>
 <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded p-2 mt-2"><p className="text-xs text-indigo-700 dark:text-indigo-300 font-mono">{t('lab.p10_elecenergy_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_elecenergy_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-indigo-500 outline-none" />
 <button onClick={checkAnswer} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button>
 </div>
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_elecenergy_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_elecenergy_incorrect_fb')}</p></div>}
 </div>
