...
import { Box, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationVolumetricExpansion({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_volExp_step1_label'), formula: 'Î”V âˆ V_o Ã— Î”T', detail: t('lab.p10_volExp_step1_detail') },
 { label: t('lab.p10_volExp_step2_label'), formula: 'Î”V = Î² V_o Î”T', detail: t('lab.p10_volExp_step2_detail') },
 { label: t('lab.p10_volExp_step3_label'), formula: 'Î² = Î”V / (V_o Î”T)', detail: t('lab.p10_volExp_step3_detail') },
 { label: t('lab.p10_volExp_step4_label'), formula: 'Î² = 3Î±', detail: t('lab.p10_volExp_step4_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "dV = beta * V_o * dT",
    "testHint": "Volume change dV is proportional to initial volume V_o and temperature change dT."
  },
  {
    "testEquation": "beta = dV / (V_o * dT)",
    "testHint": "Rearrange equation to solve for volumetric expansion coefficient beta."
  },
  {
    "testEquation": "V_T = V_o(1 + beta * dT)",
    "testHint": "Volume at temperature T is V_T = V_o(1 + beta dT)."
  },
  {
    "testEquation": "beta = 3 * alpha",
    "testHint": "Volumetric expansion coefficient is three times linear expansion coefficient (beta = 3 alpha)."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Volumetric Thermal Expansion")} />
 ...
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-rose-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_volExp_subtitle')}</p></div>
 </div>
 ...
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="Î² = Î”V / (Vâ‚€ Î”T)" className="text-2xl font-bold" /></div>
 <p className="text-xs text-rose-200 mt-1">{t('lab.p10_volExp_final_desc')}</p>
 </div>
 ...
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_volExp_adjust_desc')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_volExp_vo_label')}</span><span className="text-rose-600 font-mono">{vo} mÂ³</span></div><input type="range" min="0.1" max="2" step="0.05" value={vo} onChange={e => { setVo(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-rose-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_volExp_dt_label')}</span><span className="text-rose-600 font-mono">{dt}Â°C</span></div><input type="range" min="10" max="200" step="5" value={dt} onChange={e => { setDt(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-rose-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_volExp_beta_label')}</span><span className="text-rose-600 font-mono">{betaInput.toExponential(1)}{t('lab.p10derivationvolumetricexpansion_c')}</span></div><input type="range" min="1e-5" max="1e-4" step="0.1e-5" value={betaInput} onChange={e => { setBetaInput(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-rose-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_volExp_trace1', { dv: (dV*1000).toFixed(2) })}</p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_volExp_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Box className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_volExp_practice_q')}</p>
 <div className="bg-rose-50 dark:bg-rose-900/20 rounded p-2 mt-2"><p className="text-xs text-rose-700 dark:text-rose-300 font-mono">{t('lab.p10_volExp_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_volExp_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-rose-500 outline-none" />
 ...
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_volExp_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_volExp_incorrect_fb')}</p></div>}
 </div>
