...
import { Link, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationSeriesResistance({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_ser_step1_label'), formula: 'V = Vâ‚ + Vâ‚‚ + Vâ‚ƒ', detail: t('lab.p10_ser_step1_detail') },
 { label: t('lab.p10_ser_step2_label'), formula: 'R_eq = Râ‚ + Râ‚‚ + Râ‚ƒ', detail: t('lab.p10_ser_step2_detail') },
 { label: t('lab.p10_ser_step3_label'), formula: 'R_eq > Râ‚, Râ‚‚, Râ‚ƒ individually', detail: t('lab.p10_ser_step3_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "V = V1 + V2 + V3",
    "testHint": "Total voltage in series circuit equals sum of voltages across each resistor."
  },
  {
    "testEquation": "V1 = I * R1",
    "testHint": "Apply Ohm's Law V = I R for each series resistor."
  },
  {
    "testEquation": "I * R_eq = I*R1 + I*R2 + I*R3",
    "testHint": "Substitute voltage drops into total voltage equation."
  },
  {
    "testEquation": "R_eq = R1 + R2 + R3",
    "testHint": "Divide both sides by common current I."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Series Resistance")} />
 ...
 <button onClick={() => setActiveMobileTab('theory')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.derivation')}</button>
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_ser_subtitle')}</p></div>
 </div>
 ...
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="R_eq = Râ‚ + Râ‚‚ + Râ‚ƒ" className="text-2xl font-bold" /></div>
 <p className="text-xs text-blue-200 mt-1">{t('lab.p10_ser_final_desc')}</p>
 </div>
 ...
 <div className="flex items-center gap-2 mb-3"><Lightbulb className="w-5 h-5 text-blue-500" /><h2 className="text-lg font-bold">{t('lab.see_in_action')}</h2></div>
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_ser_adjust_desc')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_ser_r1_label')}</span><span className="text-blue-600 font-mono">{r1} Î©</span></div><input type="range" min="50" max="500" step="10" value={r1} onChange={e => { setR1(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-blue-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_ser_r2_label')}</span><span className="text-blue-600 font-mono">{r2} Î©</span></div><input type="range" min="50" max="500" step="10" value={r2} onChange={e => { setR2(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-blue-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_ser_r3_label')}</span><span className="text-blue-600 font-mono">{r3} Î©</span></div><input type="range" min="50" max="500" step="10" value={r3} onChange={e => { setR3(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-blue-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_ser_trace1', { req, i: (current*1000).toFixed(1) })}</p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_ser_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Link className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_ser_practice_q')}</p>
 <div className="bg-blue-50 dark:bg-blue-900/20 rounded p-2 mt-2"><p className="text-xs text-blue-700 dark:text-blue-300 font-mono">{t('lab.p10_ser_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_ser_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-blue-500 outline-none" />
 ...
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_ser_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_ser_incorrect_fb')}</p></div>}
 </div>
