...
import { Thermometer, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationTempCoefficient({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_tempCo_step1_label'), formula: 'ρᴵ − ρ₀ ∝ ρ₀ × ΔT', detail: t('lab.p10_tempCo_step1_detail') },
 { label: t('lab.p10_tempCo_step2_label'), formula: 'ρᴵ − ρ₀ = α ρ₀ ΔT', detail: t('lab.p10_tempCo_step2_detail') },
 { label: t('lab.p10_tempCo_step3_label'), formula: 'α = (ρᴵ − ρ₀) / (ρ₀ ΔT)', detail: t('lab.p10_tempCo_step3_detail') },
 { label: t('lab.p10_tempCo_step4_label'), formula: 'ρᴵ = ρ₀ [1 + α(T − T₀)]', detail: t('lab.p10_tempCo_step4_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "d_rho = alpha * rho_o * dT",
    "testHint": "Resistivity change d_rho is proportional to initial resistivity rho_o and temperature change dT."
  },
  {
    "testEquation": "alpha = d_rho / (rho_o * dT)",
    "testHint": "Rearrange equation to solve for temperature coefficient alpha."
  },
  {
    "testEquation": "rho_T = rho_o(1 + alpha * dT)",
    "testHint": "Resistivity at temperature T is rho_T = rho_o(1 + alpha dT)."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Temperature Coefficient of Resistivity")} />
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_tempCo_subtitle')}</p></div>
 </div>
 ...
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="α = (ρₜ − ρ₀) / (ρ₀ ΔT)" className="text-xl font-bold" /></div>
 <p className="text-xs text-yellow-200 mt-1">{t('lab.p10_tempCo_final_desc')}</p>
 </div>
 ...
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_tempCo_adjust_desc')}</p>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_tempCo_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 ...
 <p className="text-base font-medium mb-2">{t('lab.p10_tempCo_practice_q')}</p>
 <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded p-2 mt-2"><p className="text-xs text-yellow-700 dark:text-yellow-300 font-mono">{t('lab.p10_tempCo_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_tempCo_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-yellow-500 outline-none" />
 ...
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_tempCo_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_tempCo_incorrect_fb')}</p></div>}
 </div>
