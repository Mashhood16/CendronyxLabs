...
import { Ear, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationEcho({ onExit }: { onExit?: () => void }) {
 ...
 const [tVal, setTVal] = useState(0.1);
 ...
 const steps = [
 { label: t('lab.p10_echo_step1_label'), formula: 'v = D / t', detail: t('lab.p10_echo_step1_detail') },
 { label: t('lab.p10_echo_step2_label'), formula: 'D = 2d', detail: t('lab.p10_echo_step2_detail') },
 { label: t('lab.p10_echo_step3_label'), formula: 'v = 2d / t', detail: t('lab.p10_echo_step3_detail') },
 { label: t('lab.p10_echo_step4_label'), formula: 'd = v Ã— t / 2', detail: t('lab.p10_echo_step4_detail') }
 ];
  ...
  {
    "testEquation": "2d = v * t",
    "testHint": "Sound travels to the obstacle and back, covering total distance 2d."
  },
  {
    "testEquation": "2d = v * t",
    "testHint": "Distance = speed Ã— total round-trip time."
  },
  {
    "testEquation": "d = (v * t) / 2",
    "testHint": "Divide total distance by 2 to get one-way distance d."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Depth via Echo (Ultrasound)")} />
 ...
 <button onClick={() => setActiveMobileTab('theory')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? 'bg-sky-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.derivation')}</button>
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-sky-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.simulation')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_echo_subtitle')}</p></div>
 </div>
 <div className="bg-gradient-to-br from-sky-500 to-blue-600 rounded-xl p-5 text-center shadow-lg mb-3">
 <p className="text-xs text-sky-200 font-semibold uppercase tracking-wider">{t('lab.final_formula')}</p>
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="d = v Ã— t / 2" className="text-2xl font-bold" /></div>
 <p className="text-xs text-sky-200 mt-1">{t('lab.p10_echo_final_desc')}</p>
 </div>
 ...
 <div className="flex items-center gap-2 mb-3"><Lightbulb className="w-5 h-5 text-sky-500" /><h2 className="text-lg font-bold">{t('lab.see_in_action')}</h2></div>
 <p className="text-sm text-slate-500 mb-4">{t('lab.adjust_sliders')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_echo_speed_label')}</span><span className="text-sky-600 font-mono">{v} {t('lab.p10derivationecho_m_s')}</span></div><input type="range" min="340" max="1500" step="10" value={v} onChange={e => { setV(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-sky-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_echo_time_label')}</span><span className="text-sky-600 font-mono">{tVal} s</span></div><input type="range" min="0.01" max="1" step="0.01" value={tVal} onChange={e => { setTVal(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-sky-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_echo_trace1', { vt })}</p>
 <p className="text-sm text-slate-400">{t('lab.p10_echo_trace2', { vt })}</p>
 <p className="border-t border-[#2a2a2a] pt-1 text-xs"><span className="text-green-400 font-bold">{t('lab.p10_echo_trace3', { d: d.toFixed(1) })}</span></p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_echo_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Ear className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_echo_practice_q')}</p>
 <p className="text-base font-medium">{t('lab.p10_echo_practice_q2')}</p>
 <div className="bg-sky-50 dark:bg-sky-900/20 rounded p-2 mt-2"><p className="text-xs text-sky-700 dark:text-sky-300 font-mono">{t('lab.p10_echo_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_echo_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-sky-500 outline-none" />
 <button onClick={checkAnswer} className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button>
 </div>
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300">{t('lab.p10_echo_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300">{t('lab.p10_echo_incorrect_fb')}</p></div>}
 </div>
