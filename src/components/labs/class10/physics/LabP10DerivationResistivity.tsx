...
import { Cable, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationResistivity({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_resist_step1_label'), formula: 'R âˆ L / A', detail: t('lab.p10_resist_step1_detail') },
 { label: t('lab.p10_resist_step2_label'), formula: 'R = Ï Ã— L / A', detail: t('lab.p10_resist_step2_detail') },
 { label: t('lab.p10_resist_step3_label'), formula: 'Ï = R Ã— A / L', detail: t('lab.p10_resist_step3_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "R = rho * L / A",
    "testHint": "Resistance R is directly proportional to length L and inversely to area A."
  },
  {
    "testEquation": "R = rho * L / A",
    "testHint": "Introduce resistivity constant rho."
  },
  {
    "testEquation": "R = rho * L / A",
    "testHint": "Formula for resistance R = rho * L / A."
  },
  {
    "testEquation": "rho = R * A / L",
    "testHint": "Rearrange equation to solve for resistivity rho."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Resistivity")} />
 ...
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-amber-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_resist_subtitle')}</p></div>
 </div>
 ...
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="Ï = R Ã— A / L" className="text-2xl font-bold" /></div>
 <p className="text-xs text-amber-200 mt-1">{t('lab.p10_resist_final_desc')}</p>
 </div>
 ...
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_resist_adjust_desc')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_resist_r_label')}</span><span className="text-amber-600 font-mono">{r} Î©</span></div><input type="range" min="1" max="50" step="1" value={r} onChange={e => { setR(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-amber-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_resist_l_label')}</span><span className="text-amber-600 font-mono">{l} m</span></div><input type="range" min="0.5" max="10" step="0.5" value={l} onChange={e => { setL(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-amber-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_resist_a_label')}</span><span className="text-amber-600 font-mono">{(a*1e6).toFixed(0)} {t('lab.p10derivationresistivity_mm')}</span></div><input type="range" min="0.2e-6" max="5e-6" step="0.1e-6" value={a} onChange={e => { setA(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-amber-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_resist_trace1', { rho: rho.toExponential(2) })}</p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_resist_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><Cable className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_resist_practice_q')}</p>
 <div className="bg-amber-50 dark:bg-amber-900/20 rounded p-2 mt-2"><p className="text-xs text-amber-700 dark:text-amber-300 font-mono">{t('lab.p10_resist_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_resist_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-amber-500 outline-none" />
 <button onClick={checkAnswer} className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button>
 </div>
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_resist_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_resist_incorrect_fb')}</p></div>}
 </div>
