...
import { FlaskRound, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationExpansionLiquids({ onExit }: { onExit?: () => void }) {
 ...
 const steps = [
 { label: t('lab.p10_expLiq_step1_label'), formula: 'Î³_r = Î³_a + Î³_g', detail: t('lab.p10_expLiq_step1_detail') },
 { label: t('lab.p10_expLiq_step2_label'), formula: 'Î³_r > Î³_a', detail: t('lab.p10_expLiq_step2_detail') },
 { label: t('lab.p10_expLiq_step3_label'), formula: 'Î³_a = Î³_r âˆ’ Î³_g', detail: t('lab.p10_expLiq_step3_detail') }
 ];
  ...
  const testSteps = [
  {
    "testEquation": "dV_real = dV_apparent + dV_glass",
    "testHint": "Real liquid expansion equals apparent expansion plus flask expansion."
  },
  {
    "testEquation": "gamma_r = gamma_a + gamma_g",
    "testHint": "Divide by V_o * dT to get the relationship between thermal expansion coefficients."
  },
  {
    "testEquation": "gamma_r = gamma_a + gamma_g",
    "testHint": "Real coefficient gamma_r = apparent gamma_a + vessel gamma_g."
  }
];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Real vs Apparent Expansion of Liquids")} />
 ...
 <button onClick={() => setActiveMobileTab('theory')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? 'bg-teal-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.derivation')}</button>
 <button onClick={() => setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-teal-600 text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.lab')}</button>
 </div>
 ...
 <div><h2 className="text-lg font-bold">{t('lab.step_by_step')}</h2><p className="text-xs text-slate-500">{t('lab.p10_expLiq_subtitle')}</p></div>
 </div>
 <div className="bg-gradient-to-br from-teal-500 to-emerald-600 rounded-xl p-5 text-center shadow-lg mb-3">
 <p className="text-xs text-teal-200 font-semibold uppercase tracking-wider">{t('lab.final_formula')}</p>
 <div className="text-2xl font-bold text-white mt-1"><MathFormula formula="Î³_r = Î³_a + Î³_g" className="text-2xl font-bold" /></div>
 <p className="text-xs text-teal-200 mt-1">{t('lab.p10_expLiq_final_desc')}</p>
 </div>
 ...
 <div className="flex items-center gap-2 mb-3"><Lightbulb className="w-5 h-5 text-teal-500" /><h2 className="text-lg font-bold">{t('lab.see_in_action')}</h2></div>
 <p className="text-sm text-slate-500 mb-4">{t('lab.p10_expLiq_adjust_desc')}</p>
 <div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_expLiq_gammaa_label')}</span><span className="text-teal-600 font-mono">{gammaA.toExponential(2)}</span></div><input type="range" min="0.5e-4" max="2e-4" step="0.1e-4" value={gammaA} onChange={e => { setGammaA(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-teal-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.p10_expLiq_gammag_label')}</span><span className="text-teal-600 font-mono">{gammaG.toExponential(2)}</span></div><input type="range" min="1e-5" max="5e-5" step="0.1e-5" value={gammaG} onChange={e => { setGammaG(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-teal-500" /></div>
 ...
 <p className="text-sm text-slate-400">{t('lab.p10_expLiq_trace1', { ga: gammaA.toExponential(2), gg: gammaG.toExponential(2), gr: gammaR.toExponential(2) })}</p>
 </div>
 </div>
 </div>
 ...
 <div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.p10_expLiq_real_life')}</p></div>
 </div>
 </div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><FlaskRound className="w-5 h-5 text-emerald-500" /> {t('lab.practice_apply')}</h2>
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3">
 <p className="text-base font-medium mb-2">{t('lab.p10_expLiq_practice_q')}</p>
 <div className="bg-teal-50 dark:bg-teal-900/20 rounded p-2 mt-2"><p className="text-xs text-teal-700 dark:text-teal-300 font-mono">{t('lab.p10_expLiq_practice_hint')}</p></div>
 </div>
 <div className="flex gap-2 mb-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} placeholder={t('lab.p10_expLiq_placeholder')} className="flex-1 px-3 py-3 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-teal-500 outline-none" />
 <button onClick={checkAnswer} className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button>
 </div>
 {checkResult === 'correct' && <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.p10_expLiq_correct_fb')}</p></div>}
 {checkResult === 'incorrect' && <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.p10_expLiq_incorrect_fb')}</p></div>}
 </div>
