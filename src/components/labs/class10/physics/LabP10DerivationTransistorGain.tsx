...
import { Cpu, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw, Target } from 'lucide-react';
...
export default function LabP10DerivationTransistorGain({ onExit }: { onExit?: () => void }) {
 ...
 { label: t('lab.p10_transGain_step4_label'), formula: 'I_C = Î² Ã— I_B', detail: t('lab.p10_transGain_step4_detail') }
  ...
  const testSteps = [
  {
    "testEquation": "I_E = I_B + I_C",
    "testHint": "Emitter current I_E equals sum of base current I_B and collector current I_C."
  },
  {
    "testEquation": "beta = I_C / I_B",
    "testHint": "Common-emitter current gain beta = I_C / I_B."
  },
  {
    "testEquation": "alpha = I_C / I_E",
    "testHint": "Common-base current gain alpha = I_C / I_E."
  },
  {
    "testEquation": "alpha = beta / (1 + beta)",
    "testHint": "Relationship between current gains alpha = beta / (1 + beta)."
  }
];
 ...
 <div className="text-lg font-bold text-white mt-1"><MathFormula formula="Î² = I_C/I_B, Î± = I_C/I_E, I_E = I_B + I_C" className="text-base font-bold" /></div>
