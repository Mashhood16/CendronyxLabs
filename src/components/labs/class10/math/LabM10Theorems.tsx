...
function QuadraticFormulaInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_discriminant_b_4ac')} {b}Â² âˆ’ 4({a})({cVal}) = {b*b} âˆ’ {4*a*cVal} = <strong>{discriminant}</strong><br />
 {discriminant >= 0 ? (
 ...
 {checkResult === 'correct' && <p className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> {t('lab.m10theorems_correct_x')} {-b/(2*a)} Â± âˆš{discriminant}/({2*a})</p>}
...
function VectorDiffInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 <div><span className="text-[10px]">xâ‚</span><input type="range" min="-5" max="5" step="1" value={ax} onChange={e => { setAx(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-emerald-500" /><span className="text-xs">{ax}</span></div>
 <div><span className="text-[10px]">yâ‚</span><input type="range" min="-5" max="5" step="1" value={ay} onChange={e => { setAy(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-emerald-500" /><span className="text-xs">{ay}</span></div>
 </div>
 </div>
 <div>
 <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">{t('lab.m10theorems_b_x_y')}</label>
 <div className="grid grid-cols-2 gap-2">
 <div><span className="text-[10px]">xâ‚‚</span><input type="range" min="-5" max="5" step="1" value={bx} onChange={e => { setBx(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-violet-500" /><span className="text-xs">{bx}</span></div>
 <div><span className="text-[10px]">yâ‚‚</span><input type="range" min="-5" max="5" step="1" value={by} onChange={e => { setBy(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-violet-500" /><span className="text-xs">{by}</span></div>
 </div>
 </div>
 </div>
 ...
 {t('lab.m10theorems_ab_b')}{bx}âˆ’{ax}, {by}âˆ’{ay}) = <strong>({compX}, {compY})</strong><br />
 
 {t('lab.m10theorems_ab')}{compX}Â² + {compY}Â²) = âˆš{compX*compX + compY*compY} = <strong>{mag.toFixed(2)}</strong>
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
...
function LawCosinesInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 <input type="range" min="1" max="179" step="1" value={alpha} onChange={e => { setAlpha(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-rose-500" /><span className="text-xs">{alpha}Â°</span></div>
 </div>
 ...
 <text x="35" y="82" className="text-[10px] fill-rose-600">Î±={alpha}Â°</text>
 </svg>
 </div>
 ...
 {t('lab.m10theorems_a_2')} {b}Â² + {c}Â² âˆ’ 2({b})({c}{t('lab.m10theorems_cos')}{alpha}Â°)<br />
 
 {t('lab.m10theorems_a_2')} {b*b} + {c*c} âˆ’ {2*b*c} Ã— {Math.cos(rad).toFixed(4)}<br />
...
function LawSinesInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 <div><label className="text-xs font-semibold">âˆ Î±</label>
 <input type="range" min="1" max="178" step="1" value={alpha} onChange={e => { setAlpha(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-sky-500" /><span className="text-xs">{alpha}Â°</span></div>
 <div><label className="text-xs font-semibold">âˆ Î²</label>
 <input type="range" min="1" max={178-alpha} step="1" value={beta} onChange={e => { setBeta(parseFloat(e.target.value)); setCheckResult('idle'); }} className="w-full accent-sky-500" /><span className="text-xs">{beta}Â°</span></div>
 </div>
 ...
 âˆ Î³ = 180Â° âˆ’ {alpha}Â° âˆ’ {beta}Â° = {gamma}Â°<br />
 
 {t('lab.m10theorems_a_sin_b_sin_b_a_sin_sin')}<br />
 
 {t('lab.m10theorems_b')} {a} {t('lab.m10theorems_sin')}{beta}{t('lab.m10theorems_sin_1')}{alpha}Â°)<br />
 
 {t('lab.m10theorems_b')} {a} Ã— {Math.sin(beta*Math.PI/180).toFixed(4)} / {Math.sin(alpha*Math.PI/180).toFixed(4)}<br />
...
function HerosFormulaInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 Î” = âˆš[{S.toFixed(1)}Ã—{S.toFixed(1)}âˆ’{a})Ã—({S.toFixed(1)}âˆ’{b})Ã—({S.toFixed(1)}âˆ’{c})]<br />
 Î” = âˆš[{S.toFixed(1)}Ã—{(S-a).toFixed(1)}Ã—{(S-b).toFixed(1)}Ã—{(S-c).toFixed(1)}]<br />
 Î” = âˆš{(S*(S-a)*(S-b)*(S-c)).toFixed(2)} = <strong>{area.toFixed(2)}</strong>
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
...
function CircumRadiusInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_r_abc_4')}{a}Ã—{b}Ã—{c}) / (4 Ã— {area.toFixed(2)})<br />
...
function InRadiusInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_s')} {S.toFixed(1)}, Î” = {area.toFixed(2)}<br />
 
 {t('lab.m10theorems_r_s')} {area.toFixed(2)} / {S.toFixed(1)} = <strong>{r.toFixed(2)}</strong><br />
 
 {t('lab.m10theorems_check_r_s')} {r.toFixed(2)}Ã—{S.toFixed(1)} = {(r*S).toFixed(2)} = Î” âœ“
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
...
function CircleChordInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_chord_half_length_r_d')}{radius}Â² âˆ’ {chordOffset}Â²)<br />
 = âˆš{radius*radius - chordOffset*chordOffset} = {halfChord.toFixed(1)}<br />
...
function TangentCircleInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_distance_between_centres_d')} {mode === 'external' ? 'râ‚ + râ‚‚' : 'râ‚ âˆ’ râ‚‚'} = <strong>{distance}</strong>
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
 ...
 {checkResult === 'incorrect' && <p className="text-xs text-red-500"><XCircle className="w-3 h-3 inline" /> {t('lab.m10theorems_d')} {mode === 'external' ? 'râ‚ + râ‚‚' : 'râ‚ âˆ’ râ‚‚'}</p>}
 </div>
 );
}
...
function CyclicQuadInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 <div className="space-y-4">
 <div>
 <label className="text-xs font-semibold">âˆ A</label>
 ...
 <span className="text-xs">{t('lab.m10theorems_a_3')} {angleA}Â°</span>
 </div>
 <div className="relative h-32 bg-slate-100 dark:bg-[#1c1b1b] rounded-lg overflow-hidden border border-slate-200 dark:border-[#2a2a2a]">
 ...
 <text x="80" y="12" className="text-[10px] fill-amber-600">A={angleA}Â°</text>
 <text x="150" y="80" className="text-[10px] fill-amber-600">B={angleB}Â°</text>
 <text x="80" y="112" className="text-[10px] fill-amber-600">C={angleC}Â°</text>
 <text x="12" y="80" className="text-[10px] fill-amber-600">D={angleD}Â°</text>
 </svg>
 </div>
 ...
 {t('lab.m10theorems_a_c')} {angleA}Â° + {angleC}Â° = <strong>180Â°</strong> âœ“<br />
 
 {t('lab.m10theorems_b_d')} {angleB}Â° + {angleD}Â° = <strong>180Â°</strong> âœ“<br />
...
function AlternateSegmentInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 <span className="text-xs">{t('lab.m10theorems_apt')} {angle}Â°</span>
 </div>
 <div className="relative h-32 bg-slate-100 dark:bg-[#1c1b1b] rounded-lg overflow-hidden border border-slate-200 dark:border-[#2a2a2a]">
 <svg viewBox="0 0 200 120" className="w-full h-full">
 ...
 {t('lab.m10theorems_apt')} {angle}{t('lab.m10theorems_therefore')} <strong>{t('lab.m10theorems_abp')} {angle}Â°</strong><br />
 ...
 {checkResult === 'correct' && <p className="text-xs text-emerald-600"><CheckCircle className="w-3 h-3 inline" /> {t('lab.m10theorems_correct_abp')} {angle}Â°</p>}
...
function SemicircleAngleInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_no_matter_where_p_is_on_the_se')} <strong>90Â°</strong>
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
...
const THEOREMS: TheoremModule[] = [
 ...
 formula: 'x = [-b Â± âˆš(bÂ² âˆ’ 4ac)] / 2a',
 formulaDesc: 'A universal formula to solve any quadratic equation axÂ² + bx + c = 0.',
 color: 'blue',
 steps: [
 { label: 'Start with standard form', content: 'ðŸ—ï¸ You\'re an architect designing a parabolic arch. The curve follows axÂ² + bx + c = 0. You need a formula that works for ANY arch shape. Start with the general equation: axÂ² + bx + c = 0.' },
 { label: 'Move constant term', content: 'First, shift the constant to the right: axÂ² + bx = âˆ’c. The arch\'s height (c) doesn\'t affect the shape â€” it just shifts it up or down.' },
 { label: 'Divide by a', content: 'Divide both sides by a: xÂ² + (b/a)x = âˆ’c/a. Now the coefficient of xÂ² is 1, making it easier to "complete the square."' },
 { label: 'Complete the square', content: 'Add (b/2a)Â² to both sides: xÂ² + (b/a)x + (b/2a)Â² = (b/2a)Â² âˆ’ c/a. The left simplifies to (x + b/2a)Â². The right becomes (bÂ²âˆ’4ac)/(4aÂ²).' },
 { label: 'Take square root and solve', content: 'x + b/2a = Â±âˆš(bÂ²âˆ’4ac)/(2a). Isolate x: x = [âˆ’b Â± âˆš(bÂ²âˆ’4ac)] / (2a). This is the Quadratic Formula! The discriminant D = bÂ²âˆ’4ac tells you everything: D > 0 means 2 real roots (arch crosses ground twice), D = 0 means 1 root (arch just touches ground), D < 0 means no real roots (arch never meets ground).' },
 ],
 ...
 color: 'amber',
 steps: [
 { label: 'Define position vectors', content: 'ðŸ—ºï¸ You\'re tracking two delivery drones. Drone A is at (2, 3) and drone B is at (7, 5). Their position vectors from the control tower (origin O) are Ã£ = OA and bâƒ— = OB.' },
 { label: 'Use triangle law of addition', content: 'To travel from O to B via A: OA + AB = OB. This is the triangle law of vector addition â€” the direct path OA plus AB equals the direct path OB.' },
 { label: 'Rearrange for AB', content: 'AB = OB âˆ’ OA. The vector from A to B is the difference between the two position vectors.' },
 { label: 'Substitute position vectors', content: 'AB = bâƒ— âˆ’ Ã£ = (xâ‚‚âˆ’xâ‚, yâ‚‚âˆ’yâ‚) = (7âˆ’2, 5âˆ’3) = (5, 2). The displacement between the drones is 5 units east and 2 units north. Its magnitude |AB| = âˆš(5Â²+2Â²) = âˆš29 â‰ˆ 5.39 units. This formula is fundamental in physics: velocity = position_B âˆ’ position_A / time!' },
 ],
 ...
 formula: 'aÂ² = bÂ² + cÂ² âˆ’ 2bc cos Î±',
 formulaDesc: 'Generalizes Pythagoras for ANY triangle, not just right-angled ones.',
 color: 'rose',
 steps: [
 { label: 'Draw altitude AD âŸ‚ BC', content: 'ðŸ”ï¸ You\'re a surveyor measuring a sloped plot of land. Triangle ABC has no right angle, but you can create one! Drop a perpendicular AD from A to side BC, creating two right triangles.' },
 { label: 'Express AD and BD in terms of c', content: 'In right triangle ABD: BD = c cos Î² and AD = c sin Î². The altitude splits side BC (length a) into two segments: BD and DC.' },
 { label: 'Find CD = a âˆ’ BD', content: 'CD = a âˆ’ c cos Î². This is the remaining base segment of the triangle.' },
 { label: 'Apply Pythagoras in right triangle ADC', content: 'bÂ² = ADÂ² + CDÂ² = (c sin Î²)Â² + (a âˆ’ c cos Î²)Â² = cÂ² sinÂ²Î² + aÂ² + cÂ² cosÂ²Î² âˆ’ 2ac cos Î².' },
 { label: 'Simplify using sinÂ²+cosÂ²=1', content: 'bÂ² = aÂ² + cÂ²(sinÂ²Î² + cosÂ²Î²) âˆ’ 2ac cos Î² = aÂ² + cÂ² âˆ’ 2ac cos Î². This is the Law of Cosines! When Î² = 90Â°, cos 90Â° = 0, so it becomes bÂ² = aÂ² + cÂ² â€” the Pythagorean Theorem! The Law of Cosines is Pythagoras supercharged for ANY triangle.' },
 ],
 ...
 icon: Triangle,
 title: 'Law of Sines',
 formula: 'a/sin Î± = b/sin Î² = c/sin Î³',
 formulaDesc: 'The ratio of a side to the sine of its opposite angle is constant.',
 color: 'sky',
 steps: [
 { label: 'Draw altitude h from B to AC', content: 'ðŸŒ„ A mountain expedition measures a triangular valley. Drop a perpendicular h from vertex B to base AC (extended if needed). This creates two right triangles.' },
 { label: 'Express h in two ways', content: 'In triangle ABD (right-angled at D): h = c sin Î±. In triangle CBD: h = a sin Î³ (since sin(Ï€âˆ’Î³) = sin Î³). Both represent the same altitude!' },
 { label: 'Equate the expressions', content: 'c sin Î± = a sin Î³ â†’ a/sin Î± = c/sin Î³. The ratio of side a to sin Î± equals the ratio of side c to sin Î³.' },
 { label: 'Repeat for other sides', content: 'By drawing altitudes to other sides, we get a/sin Î± = b/sin Î². Combining all: a/sin Î± = b/sin Î² = c/sin Î³. This is the Law of Sines. It\'s the foundation of triangulation â€” GPS satellites use this to pinpoint your location by measuring angles to multiple satellites!' },
 ],
 ...
 formula: '(a+b)/(aâˆ’b) = tan[(Î±+Î²)/2] / tan[(Î±âˆ’Î²)/2]',
 formulaDesc: 'Relates side lengths to tangent of half angle sums and differences.',
 color: 'indigo',
 steps: [
 { label: 'Start with Law of Sines', content: 'ðŸ“ From the Law of Sines: a/sin Î± = b/sin Î² â†’ a/b = sin Î± / sin Î². This ratio connects side lengths to the sines of their opposite angles.' },
 { label: 'Apply componendo-dividendo', content: 'Using the algebraic property: (a+b)/(aâˆ’b) = (sin Î± + sin Î²)/(sin Î± âˆ’ sin Î²). This transforms the ratio into a form we can simplify with trig identities.' },
 { label: 'Apply sum-to-product identities', content: 'sin Î± + sin Î² = 2 sin[(Î±+Î²)/2] cos[(Î±âˆ’Î²)/2] and sin Î± âˆ’ sin Î² = 2 cos[(Î±+Î²)/2] sin[(Î±âˆ’Î²)/2]. The 2\'s cancel, giving: (a+b)/(aâˆ’b) = {sin[(Î±+Î²)/2] cos[(Î±âˆ’Î²)/2]} / {cos[(Î±+Î²)/2] sin[(Î±âˆ’Î²)/2]}.' },
 { label: 'Simplify to tangent ratio', content: 'This simplifies to: (a+b)/(aâˆ’b) = tan[(Î±+Î²)/2] / tan[(Î±âˆ’Î²)/2]. The Law of Tangents! It\'s especially useful in surveying and navigation when you know two sides and the included angle, or two angles and a side â€” it avoids needing a calculator for square roots!' },
 ],
 interactive: LawCosinesInteractive,
 },
 {
 ...
 formula: 'cos(Î±/2) = âˆš[S(Sâˆ’a)/bc]',
 formulaDesc: 'Express half-angle cosines directly in terms of side lengths.',
 color: 'emerald',
 steps: [
 { label: 'Start with Law of Cosines', content: 'ðŸ“Š Start with cos Î± = (bÂ²+cÂ²âˆ’aÂ²)/(2bc). This gives cos of the full angle Î± in terms of the three side lengths.' },
 { label: 'Use double-angle identity', content: 'The double-angle identity says cos Î± = 2cosÂ²(Î±/2) âˆ’ 1. Substitute the Law of Cosines: 2cosÂ²(Î±/2) âˆ’ 1 = (bÂ²+cÂ²âˆ’aÂ²)/(2bc).' },
 { label: 'Solve for cosÂ²(Î±/2)', content: '2cosÂ²(Î±/2) = 1 + (bÂ²+cÂ²âˆ’aÂ²)/(2bc) = (2bc + bÂ² + cÂ² âˆ’ aÂ²)/(2bc) = [(b+c)Â² âˆ’ aÂ²]/(2bc).' },
 { label: 'Factor and substitute S', content: '[(b+c)Â² âˆ’ aÂ²] = (b+c+a)(b+câˆ’a). Since a+b+c = 2S (the semi-perimeter), and b+câˆ’a = 2(Sâˆ’a), we get: 2cosÂ²(Î±/2) = [2S Ã— 2(Sâˆ’a)]/(2bc). Divide by 2 and take the square root: cos(Î±/2) = âˆš[S(Sâˆ’a)/(bc)]. This formula is elegant â€” it expresses a trigonometric ratio purely in terms of side lengths, no angles needed!' },
 ],
 ...
 formula: 'sin(Î±/2) = âˆš[(Sâˆ’b)(Sâˆ’c)/bc]',
 formulaDesc: 'Express half-angle sines directly in terms of side lengths.',
 color: 'emerald',
 steps: [
 { label: 'Use identity 1âˆ’2sinÂ²(Î±/2) = cos Î±', content: 'ðŸ”„ Starting with the identity cos Î± = 1 âˆ’ 2sinÂ²(Î±/2), substitute the Law of Cosines: 1 âˆ’ 2sinÂ²(Î±/2) = (bÂ²+cÂ²âˆ’aÂ²)/(2bc).' },
 { label: 'Solve for sinÂ²(Î±/2)', content: '2sinÂ²(Î±/2) = 1 âˆ’ (bÂ²+cÂ²âˆ’aÂ²)/(2bc) = (2bc âˆ’ bÂ² âˆ’ cÂ² + aÂ²)/(2bc) = [aÂ² âˆ’ (bâˆ’c)Â²]/(2bc).' },
 { label: 'Factor as difference of squares', content: 'aÂ² âˆ’ (bâˆ’c)Â² = (aâˆ’b+c)(a+bâˆ’c). Using semi-perimeter S: aâˆ’b+c = 2(Sâˆ’b) and a+bâˆ’c = 2(Sâˆ’c).' },
 { label: 'Substitute and simplify', content: '2sinÂ²(Î±/2) = [2(Sâˆ’b) Ã— 2(Sâˆ’c)]/(2bc). Divide by 2 and take square root: sin(Î±/2) = âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)]. Together with the cosine half-angle formula, these form the basis for Hero\'s formula and are used extensively in navigation and astronomy!' },
 ],
 interactive: HerosFormulaInteractive,
 },
 {
 ...
 formula: 'Î” = Â½ bc sin Î±',
 formulaDesc: 'The area of a triangle using two sides and the included angle.',
 color: 'violet',
 steps: [
 { label: 'Basic area formula', content: 'ðŸŒ³ A land surveyor measures a triangular plot. They know two sides (b and c) and the angle between them (Î±). The basic area formula is: Area = Â½ Ã— base Ã— height.' },
 { label: 'Identify base and height', content: 'Take side c = AB as the base. The height is the perpendicular from C to AB, which is h = b sin Î± (from right triangle ACD).' },
 { label: 'Substitute into area formula', content: 'Area = Â½ Ã— AB Ã— h = Â½ Ã— c Ã— (b sin Î±) = Â½ bc sin Î±. This is the SAS (Side-Angle-Side) area formula. If Î± = 90Â°, sin 90Â° = 1, so Area = Â½ bc â€” the familiar right triangle area formula!' },
 ],
 interactive: LawCosinesInteractive,
 },
 {
 ...
 formula: 'Î” = Â½ aÂ² sin Î² sin Î³ / sin Î±',
 formulaDesc: 'The area of a triangle using one side and two angles.',
 color: 'violet',
 steps: [
 { label: 'Start with SAS formula', content: 'ðŸ“‹ When you know two angles (Î², Î³) and the included side a (AAS), you can still find area. Start from Area = Â½ ab sin Î³.' },
 { label: 'Use Law of Sines to find b', content: 'From Law of Sines: b/sin Î² = a/sin Î± â†’ b = a sin Î² / sin Î±. This expresses side b in terms of known angles and side a.' },
 { label: 'Substitute into area formula', content: 'Area = Â½ a Ã— (a sin Î² / sin Î±) Ã— sin Î³ = Â½ aÂ² (sin Î² sin Î³) / sin Î±. This is the AAS area formula. It\'s perfect when you can\'t measure both adjacent sides but you can measure angles â€” like measuring a mountain\'s profile from a distance!' },
 ],
 interactive: LawSinesInteractive,
 },
 {
 ...
 formula: 'Î” = âˆš[S(Sâˆ’a)(Sâˆ’b)(Sâˆ’c)]',
 formulaDesc: 'The area of a triangle from its three side lengths alone.',
 color: 'emerald',
 steps: [
 { label: 'Start with area = Â½ bc sin Î±', content: 'ðŸ“ You\'ve measured all three sides of a triangular garden: a=5m, b=6m, c=7m. You need the area but don\'t know any angles. Start with Î” = Â½ bc sin Î±.' },
 { label: 'Write sin Î± = 2 sin(Î±/2) cos(Î±/2)', content: 'Using the double-angle identity: sin Î± = 2 sin(Î±/2) cos(Î±/2). So Î” = bc Ã— sin(Î±/2) Ã— cos(Î±/2).' },
 { label: 'Substitute half-angle formulas', content: 'Substitute sin(Î±/2) = âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)] and cos(Î±/2) = âˆš[S(Sâˆ’a)/(bc)]. The bc factors outside the radicals multiply: bc Ã— âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)] Ã— âˆš[S(Sâˆ’a)/(bc)].' },
 { label: 'Simplify to Hero\'s formula', content: 'bc Ã— 1/(bc) Ã— âˆš[S(Sâˆ’a)(Sâˆ’b)(Sâˆ’c)] = âˆš[S(Sâˆ’a)(Sâˆ’b)(Sâˆ’c)]. This is Hero\'s (or Heron\'s) formula! For our garden: S = (5+6+7)/2 = 9, Area = âˆš[9Ã—4Ã—3Ã—2] = âˆš216 â‰ˆ 14.7mÂ². This 2,000-year-old formula, discovered by Hero of Alexandria, requires ONLY side lengths â€” no angles, no trigonometry!' },
 ],
 interactive: HerosFormulaInteractive,
 },
 {
 ...
 color: 'orange',
 steps: [
 { label: 'Draw circumscribed circle', content: 'ðŸŒ Imagine drawing a circle that passes through all three vertices of a triangle. This is the circumcircle, and its centre O is the circumcentre. Draw diameter BD.' },
 { label: 'Connect D to C', content: 'In the circumcircle, Î± (angle at A) and âˆ BDC are angles in the same segment, so âˆ BDC = Î±. Triangle BCD is right-angled at C (angle in a semicircle).' },
 { label: 'Apply sine in right triangle BCD', content: 'In right triangle BCD: BC/BD = sin(âˆ BDC) = sin Î±. Since BC = a and BD = 2R: a/(2R) = sin Î± â†’ R = a/(2 sin Î±).' },
 { label: 'Express in terms of sides and area', content: 'From area formula: Î” = Â½ bc sin Î± â†’ sin Î± = 2Î”/(bc). Substituting: R = a/(2 Ã— 2Î”/(bc)) = abc/(4Î”). The circumradius relates all three sides and the area! For an equilateral triangle, R = a/âˆš3. The larger the triangle, the larger the circumcircle!' },
 ],
 ...
 color: 'teal',
 steps: [
 { label: 'Draw the incircle', content: 'ðŸŽ¯ The incircle touches all three sides of a triangle from the inside. Its centre I (the incentre) is the intersection of the angle bisectors. The perpendicular distances from I to each side are all equal to r.' },
 { label: 'Split triangle into three', content: 'The incentre I divides Î”ABC into three smaller triangles: IAB, IBC, and IAC. Each has a height of r (the inradius).' },
 { label: 'Sum the areas', content: 'Total area Î” = Area(IBC) + Area(IAC) + Area(IAB) = Â½ar + Â½br + Â½cr = r(a+b+c)/2.' },
 { label: 'Solve for r', content: 'Since S = (a+b+c)/2 (semi-perimeter), we get Î” = rS â†’ r = Î”/S. The inradius is the ratio of the area to the semi-perimeter! For a right triangle with legs 3 and 4 (hypotenuse 5): Î” = 6, S = 6, so r = 1. The incircle has radius 1. This formula is used to find the radius of a pizza that fits exactly inside a triangular box!' },
 ],
 ...
 color: 'teal',
 steps: [
 { label: 'Draw the excircle opposite A', content: 'ðŸ”µ An excircle touches one side of the triangle and the extensions of the other two sides. The ex-centre Iâ‚ is opposite vertex A. Draw it and connect to all three vertices.' },
 { label: 'Split into three regions', content: 'The ex-centre Iâ‚ creates three regions: Iâ‚AB, Iâ‚AC (outside the triangle), and Iâ‚BC (partially inside). The total area Î” = Area(Iâ‚AB) + Area(Iâ‚AC) âˆ’ Area(Iâ‚BC).' },
 { label: 'Substitute baseÃ—height/2', content: 'Î” = Â½crâ‚ + Â½brâ‚ âˆ’ Â½arâ‚ = Â½râ‚(b + c âˆ’ a). Watch the minus sign â€” Iâ‚BC is partially outside the original triangle!' },
 { label: 'Simplify using S', content: 'b + c âˆ’ a = (a+b+c) âˆ’ 2a = 2S âˆ’ 2a = 2(Sâˆ’a). So Î” = Â½râ‚ Ã— 2(Sâˆ’a) = râ‚(Sâˆ’a). Rearranging: râ‚ = Î”/(Sâˆ’a). Similarly, râ‚‚ = Î”/(Sâˆ’b) and râ‚ƒ = Î”/(Sâˆ’c). Exradii are larger than the inradius and are used in advanced geometry proofs and triangle geometry problems!' },
 ],
 interactive: InRadiusInteractive,
 },
 ...
 color: 'indigo',
 steps: [
 { label: 'Take three non-collinear points', content: 'ðŸ“ Pick any three points A, B, C that don\'t lie on a straight line â€” like three trees in a park that aren\'t in a row. Connect them to form a triangle.' },
 ...
 { label: 'Draw the circle', content: 'With centre O and radius OA, draw the circle. It passes through A, B, and C perfectly. If the points were collinear (on a line), the perpendicular bisectors would be parallel and never meet â€” proving that three points on a line can never lie on a circle!' },
 ],
 ...
 formula: 'If OC âŸ‚ AB then AC = BC where O is centre.',
 formulaDesc: 'A line from the centre to the midpoint of a chord is perpendicular to the chord.',
 color: 'indigo',
 steps: [
 { label: 'Draw chord AB with midpoint C', content: 'âš½ Take a chord AB on a circle. Mark its midpoint C. Draw a line from the centre O to C.' },
 { label: 'Join OA and OB', content: 'OA and OB are radii, so OA = OB. Triangle OAB is isosceles with apex at O.' },
 { label: 'Use properties of isosceles triangle', content: 'In isosceles triangle OAB, the line from O to the midpoint C of the base AB is also the altitude (perpendicular). So OC âŸ‚ AB.' },
 { label: 'The perpendicular property', content: 'Therefore, the line from the centre to the midpoint of a chord is perpendicular to the chord. This is a key property used in circle geometry to find chord lengths and distances from the centre. It\'s why a spirit level works â€” the bubble\'s centre line is perpendicular to the surface!' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 formula: 'If OC âŸ‚ AB then C is the midpoint of chord AB.',
 formulaDesc: 'The perpendicular from the centre to a chord bisects it.',
 color: 'indigo',
 steps: [
 { label: 'Draw chord AB and perpendicular from O', content: 'ðŸŽ¯ Draw a chord AB and a perpendicular from centre O to meet AB at C (âˆ OCA = 90Â°).' },
 { label: 'Join OA and OB', content: 'OA and OB are radii, so OA = OB. Triangle OAB is isosceles.' },
 { label: 'Prove triangles OAC and OBC are congruent', content: 'OA = OB (radii), OC = OC (common side), and âˆ OCA = âˆ OCB = 90Â°. So triangle OAC â‰… OBC by RHS congruence. Therefore AC = BC.' },
 { label: 'C is the midpoint', content: 'Since AC = BC, C is the midpoint of chord AB. The perpendicular from the centre to a chord bisects it. This is the converse of Theorem 9.2. Together, these two theorems are the foundation of all chord calculations â€” used in designing arches, bridges, and circular structures!' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 formula: 'AB = CD â‡” OE = OF (where E, F are midpoints)',
 formulaDesc: 'Equal chords are at the same distance from the centre.',
 color: 'indigo',
 steps: [
 { label: 'Draw two equal chords', content: 'âœï¸ Draw two chords AB and CD of equal length. Draw perpendiculars OE and OF from the centre to each chord.' },
 ...
 { label: 'Use Pythagoras in triangles', content: 'In right triangles OAE and OCF: OEÂ² = OAÂ² âˆ’ AEÂ² and OFÂ² = OCÂ² âˆ’ CFÂ². Since OA = OC (radii) and AE = CF, we get OEÂ² = OFÂ², so OE = OF.' },
 { label: 'Equal chords are equidistant', content: 'Therefore, equal chords are equidistant from the centre. Imagine two guitar strings (chords) of the same length â€” they\'re the same distance from the centre of the circular sound hole. This is used in engineering to balance forces in circular structures!' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 formula: 'OE = OF â‡’ AB = CD',
 formulaDesc: 'Chords at equal distance from the centre are equal in length.',
 color: 'indigo',
 steps: [
 { label: 'Draw chords at equal distance', content: 'ðŸŽª Draw two chords AB and CD such that their perpendicular distances from the centre (OE and OF) are equal.' },
 { label: 'Use Pythagoras', content: 'In right triangles OAE and OCF: AEÂ² = OAÂ² âˆ’ OEÂ² and CFÂ² = OCÂ² âˆ’ OFÂ². Since OA = OC (radii) and OE = OF, we get AEÂ² = CFÂ², so AE = CF.' },
 { label: 'Double the half-lengths', content: 'Since E and F are midpoints (Theorem 9.3): AB = 2Ã—AE and CD = 2Ã—CF. Since AE = CF, we get AB = CD.' },
 ...
 formula: 'Arc AB â‰… Arc CD â‡’ AB = CD',
 formulaDesc: 'Congruent arcs subtend equal chords.',
 color: 'indigo',
 steps: [
 { label: 'Take two congruent arcs', content: 'ðŸŽµ Imagine two identical arcs on a circle â€” like two equal-length curved sections of a circular railway track.' },
 ...
 { label: 'Equal chords', content: 'Therefore, congruent arcs have equal chords. This is used in gear design â€” equal gear teeth (arcs) must have equal chord lengths for smooth meshing. In map-making, equal arcs of latitude and longitude create equal chords on the globe!' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 formula: 'AB = CD â‡’ Arc AB â‰… Arc CD',
 formulaDesc: 'Equal chords subtend congruent arcs.',
 color: 'indigo',
 steps: [
 { label: 'Draw two equal chords', content: 'ðŸ”— Draw two equal chords AB and CD on a circle. For example, two equal-length strings on a circular instrument.' },
 ...
 { label: 'Central angles are equal', content: 'From congruency: âˆ AOB = âˆ COD. The central angles subtended by the chords are equal.' },
 { label: 'Equal central angles mean equal arcs', content: 'The measure of an arc equals the measure of its central angle. Since âˆ AOB = âˆ COD, the arcs AB and CD are congruent. This completes the cycle: equal chords â†” equal central angles â†” congruent arcs!' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 formula: 'AB = CD â‡’ âˆ AOB = âˆ COD',
 formulaDesc: 'Equal chords subtend equal angles at the centre.',
 color: 'indigo',
 steps: [
 { label: 'Draw two equal chords', content: 'ðŸŽ¯ Draw two equal chords AB and CD in a circle. Connect their endpoints to the centre O.' },
 { label: 'Compare triangles', content: 'In triangles OAB and OCD: OA = OC (radii), OB = OD (radii), and AB = CD (given). So triangles OAB â‰… OCD by SSS congruence.' },
 { label: 'Corresponding angles are equal', content: 'From the congruence, âˆ AOB (corresponding to âˆ COD) = âˆ COD.' },
 { label: 'Equal central angles', content: 'Therefore, equal chords subtend equal angles at the centre. Picture a pizza sliced into equal-width crust pieces â€” equal chords around the edge mean equal central angles, giving you equal-sized slices! This theorem is why equal spacing on a circle creates equal angles, fundamental in polygon construction.' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 formula: 'âˆ AOB = âˆ COD â‡’ AB = CD',
 formulaDesc: 'Equal angles at the centre subtend equal chords.',
 color: 'indigo',
 steps: [
 { label: 'Draw chords with equal central angles', content: 'ðŸ“ Draw two chords AB and CD such that the central angles âˆ AOB = âˆ COD are equal.' },
 { label: 'Compare triangles', content: 'In triangles OAB and OCD: OA = OC (radii), OB = OD (radii), and âˆ AOB = âˆ COD (given). So triangles OAB â‰… OCD by SAS congruence.' },
 { label: 'Corresponding sides are equal', content: 'From the congruence, AB (corresponding to CD) = CD.' },
 { label: 'Converse is proved', content: 'This completes the set: Theorems 9.8 and 9.9 together prove that chords and their central angles have a perfect one-to-one correspondence. Equal chords â†” equal central angles. This is used in engineering to create evenly spaced holes on a circular flange!' },
 ],
 interactive: CircleChordInteractive,
 },
 ...
 formula: 'If line âŸ‚ radius at outer endpoint, then it is a tangent.',
 formulaDesc: 'A line perpendicular to a radius at its endpoint on the circle is a tangent.',
 color: 'purple',
 steps: [
 { label: 'Draw radius and perpendicular line', content: 'ðŸ”„ Draw a radial segment OA from centre O to point A on the circle. Draw a line PQ perpendicular to OA at the outer end A.' },
 { label: 'Check distance from centre', content: 'Since PQ âŸ‚ OA, the distance from centre O to any point on PQ is at least OA (perpendicular distance is shortest). The distance from O to any other point on PQ is > OA = radius.' },
 ...
 { label: 'PQ is a tangent', content: 'By definition, a line that touches a circle at exactly one point is a tangent. So PQ is tangent to the circle at A. This is the construction method for tangents â€” simply draw a perpendicular at the endpoint of a radius!' },
 ],
 ...
 color: 'purple',
 steps: [
 { label: 'Draw tangent and radius', content: 'â­• Draw a circle with centre O. Draw a tangent PQ touching the circle at point B. Draw the radial segment OB.' },
 ...
 color: 'purple',
 steps: [
 { label: 'Draw tangents from external point', content: 'ðŸ“ Take a point A outside the circle. Draw two tangents AB and AC touching the circle at B and C.' },
 { label: 'Join centre O to points', content: 'Join OA, OB, and OC. Since tangents are perpendicular to radii (Theorem 10.2): OB âŸ‚ AB and OC âŸ‚ AC, so âˆ OBA = âˆ OCA = 90Â°.' },
 { label: 'Prove triangles congruent', content: 'In right triangles OBA and OCA: OB = OC (radii), OA = OA (common side), and âˆ OBA = âˆ OCA = 90Â°. So triangles OBA â‰… OCA by RHS congruence.' },
 ...
 formula: 'd = râ‚ + râ‚‚ (distance between centres)',
 formulaDesc: 'If circles touch externally, the distance between centres equals the sum of radii.',
 color: 'purple',
 steps: [
 { label: 'Draw two circles touching externally', content: 'ðŸ”µðŸ”µ Draw two circles with centres A and B, radii râ‚ = 2cm and râ‚‚ = 1cm, that touch each other at exactly one point C.' },
 ...
 { label: 'A to C = râ‚, B to C = râ‚‚', content: 'AC is the radius of circle A (râ‚ = 2cm). BC is the radius of circle B (râ‚‚ = 1cm). Since C lies on line AB: AB = AC + BC = râ‚ + râ‚‚.' },
 ...
 formula: 'd = |râ‚ âˆ’ râ‚‚| (distance between centres)',
 formulaDesc: 'If circles touch internally, the distance equals the difference of radii.',
 color: 'purple',
 steps: [
 { label: 'Draw two circles touching internally', content: 'â­•ðŸ”µ Draw two circles â€” one larger (radius râ‚ = 3cm, centre A) and one smaller (radius râ‚‚ = 1cm, centre B) inside it, touching at exactly one point C.' },
 ...
 { label: 'AC = râ‚ (outer radius), BC = râ‚‚ (inner radius)', content: 'For the outer circle: AC = râ‚ = 3cm. For the inner circle: BC = râ‚‚ = 1cm. Since B lies inside and C is on the boundary: AB = AC âˆ’ BC = râ‚ âˆ’ râ‚‚.' },
 { label: 'Distance equals difference', content: 'AB = 3 âˆ’ 1 = 2 cm. If circles touch internally, the distance between centres equals the difference of their radii. This principle is used in ball bearings â€” the inner race (small circle) touches the outer race (large circle) internally!' },
 ],
 interactive: TangentCircleInteractive,
 },
 {
 ...
 formula: 'âˆ APT = âˆ ABP (between tangent and chord)',
 formulaDesc: 'Angle between tangent and chord equals angle in the alternate segment.',
 color: 'pink',
 steps: [
 { label: 'Draw tangent and chord', content: 'ðŸŽ¯ Draw a circle with tangent PT at point A. Draw chord AB through A, creating angle âˆ APT between the tangent and chord.' },
 ...
 { label: 'Measure both angles', content: 'âˆ APT is the angle between the tangent PT and chord PA. âˆ ABP is the angle in the alternate segment (the angle subtended by the same chord AP at point B on the alternate arc). Measuring shows âˆ APT = âˆ ABP.' },
 { label: 'Equality holds', content: 'The angle between a tangent and a chord through the point of contact equals the angle in the alternate segment. This is one of the most beautiful theorems in circle geometry! It\'s used in navigation â€” sailors use the angle between a coastline (tangent) and a line to a lighthouse (chord) to determine their position!' },
 ],
 ...
 formula: 'âˆ COD = 2âˆ CED (on the major arc)',
 formulaDesc: 'The central angle is double any inscribed angle subtended by the same arc.',
 color: 'cyan',
 steps: [
 { label: 'Draw central and inscribed angles', content: 'ðŸ“ Draw a circle with centre O and chord CD. Draw the central angle âˆ COD. Pick point E on the major arc (opposite side of CD from O). Draw inscribed angle âˆ CED.' },
 { label: 'Join OE and extend', content: 'Join O to E and extend to point F on the opposite side. Now âˆ COD = âˆ COF âˆ’ âˆ DOF. But âˆ COF = 2âˆ CEO (exterior angle = sum of opposite interior angles of triangle COE, and OC = OE makes it isosceles).' },
 { label: 'Apply isosceles triangle properties', content: 'Since OC = OE (radii), triangle COE is isosceles. The exterior angle âˆ COF = âˆ CEO + âˆ OCE = 2âˆ CEO. Similarly, âˆ DOF = 2âˆ DEO.' },
 { label: 'Central = 2 Ã— inscribed', content: 'âˆ COD = 2âˆ CEO + 2âˆ DEO = 2(âˆ CEO + âˆ DEO) = 2âˆ CED. The central angle ALWAYS equals twice the inscribed angle subtended by the same arc. This is why the angle in a semicircle is 90Â° â€” the central angle is 180Â° (a straight line), so the inscribed angle is 90Â°!' },
 ],
 ...
 formula: 'âˆ ACB = âˆ ADB = âˆ AEB (same segment)',
 formulaDesc: 'All angles subtended by a chord in the same segment are equal.',
 color: 'cyan',
 steps: [
 { label: 'Take chord AB and points on same segment', content: 'ðŸŽ¯ Draw a chord AB. Take three points C, D, E on the SAME segment (major or minor) of the circle. For example, all on the major arc AB.' },
 { label: 'Measure inscribed angles', content: 'Measure angles âˆ ACB, âˆ ADB, and âˆ AEB â€” all subtended by chord AB at points C, D, and E.' },
 { label: 'All angles are equal', content: 'Each of these angles equals HALF the central angle âˆ AOB (by Theorem 10.7). Since they all equal half of the same central angle, they must be equal to each other!' },
 { label: 'Angles in same segment are equal', content: 'Therefore, âˆ ACB = âˆ ADB = âˆ AEB. All angles in the same segment of a circle are equal. This is why any point on a circular arc \"sees\" a chord at the same angle â€” it\'s the principle behind the \"goal line\" in circular sports stadiums!' },
 ],
 interactive: SemicircleAngleInteractive,
 },
 {
 ...
 formula: 'âˆ APB = 90Â° (where AB is diameter)',
 formulaDesc: 'An angle inscribed in a semicircle is always 90Â°.',
 color: 'blue',
 steps: [
 { label: 'Draw a semicircle with diameter AB', content: 'ðŸŒ“ Draw a circle with diameter AB (passing through centre O). Take any point P on the semicircle arc.' },
 { label: 'Apply central angle theorem', content: 'The central angle for chord AB is âˆ AOB = 180Â° (a straight line through the diameter). By Theorem 10.7, the inscribed angle âˆ APB = Â½ Ã— âˆ AOB = Â½ Ã— 180Â° = 90Â°.' },
 { label: 'No matter where P is', content: 'This works for ANY point P on the semicircle! Move P anywhere along the arc, and âˆ APB always equals 90Â°.' },
 { label: 'Thales\' theorem', content: 'This theorem is attributed to Thales of Miletus (c. 600 BCE), making it one of the oldest known mathematical theorems. It\'s the foundation for constructing right angles (used by Egyptian pyramid builders!) and is the basis for the \"circle of possibilities\" â€” any point P on the circle gives a right angle. This is used in GPS triangulation and even in finding the optimal location for a cell tower!' },
 ],
 interactive: SemicircleAngleInteractive,
 },
 {
 ...
 formula: 'âˆ APB < 90Â° (major segment)',
 formulaDesc: 'An angle in a segment greater than a semicircle is acute.',
 color: 'cyan',
 steps: [
 { label: 'Draw a chord AB forming a major segment', content: 'ðŸ“ Draw chord AB that creates a major segment (larger than a semicircle). Take points P, Q, R on the major arc.' },
 { label: 'Measure inscribed angles', content: 'Measure âˆ APB, âˆ AQB, and âˆ ARB. Each of these angles is less than the angle in a semicircle (90Â°).' },
 { label: 'Central angle is less than 180Â°', content: 'The central angle âˆ AOB subtended by the major segment is less than 180Â°. By Theorem 10.7: âˆ APB = Â½âˆ AOB < Â½ Ã— 180Â° = 90Â°.' },
 { label: 'All angles are acute', content: 'Therefore, all angles in a major segment (greater than a semicircle) are acute (< 90Â°). This helps determine the nature of angles in circular segments â€” useful in architectural design of arches and domes!' },
 ],
 interactive: SemicircleAngleInteractive,
 },
 {
 ...
 formula: 'âˆ APB > 90Â° (minor segment)',
 formulaDesc: 'An angle in a segment less than a semicircle is obtuse.',
 color: 'cyan',
 steps: [
 { label: 'Draw a chord forming a minor segment', content: 'ðŸ“ Draw chord AB that forms a minor segment (smaller than a semicircle). Take points on the minor arc.' },
 { label: 'Central angle is > 180Â°', content: 'The central angle âˆ AOB subtended by the minor segment is GREATER than 180Â° (the major central angle). By Theorem 10.7: âˆ APB = Â½ Ã— (major central angle) > Â½ Ã— 180Â° = 90Â°.' },
 { label: 'All angles are obtuse', content: 'Therefore, all angles inscribed in a minor segment (less than a semicircle) are obtuse (> 90Â°).' },
 { label: 'Complete classification', content: 'Combined with Theorem 10.10: angle in a major segment < 90Â° < angle in a minor segment. The semicircle at exactly 90Â° is the boundary between acute and obtuse. This complete classification is a beautiful geometric result and explains why right angles are so special in circle geometry!' },
 ],
 interactive: SemicircleAngleInteractive,
 },
 {
 ...
 formula: 'âˆ A + âˆ C = 180Â°, âˆ B + âˆ D = 180Â°',
 formulaDesc: 'The sum of opposite angles in a cyclic quadrilateral is 180Â°.',
 color: 'cyan',
 steps: [
 { label: 'Draw a cyclic quadrilateral ABCD', content: 'ðŸ”· Draw a quadrilateral ABCD inscribed in a circle (all four vertices on the circle). This is called a cyclic quadrilateral.' },
 { label: 'Join centre O to vertices', content: 'Join O to A, B, C, D. The central angle subtended by arc BCD is âˆ BOD (reflex or not). The inscribed angle at A is âˆ BAD.' },
 { label: 'Apply central angle theorem', content: 'âˆ BAD = Â½âˆ BOD (reflex) and âˆ BCD = Â½âˆ BOD (the non-reflex part). The two central angles BOD (reflex) + BOD (non-reflex) = 360Â°.' },
 { label: 'Sum of opposite angles = 180Â°', content: 'âˆ A + âˆ C = Â½(âˆ BOD reflex + âˆ BOD) = Â½ Ã— 360Â° = 180Â°. Similarly, âˆ B + âˆ D = 180Â°. Opposite angles of a cyclic quadrilateral are supplementary! This is why a rectangular picture frame can always be inscribed in a circle â€” its opposite angles are 90Â° + 90Â° = 180Â°. This theorem is used in surveying to check if four points lie on a circle!' },
 ],
