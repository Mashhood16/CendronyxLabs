...
function QuadraticFormulaInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_discriminant_b_4ac')} {b}² âˆ’ 4({a})({cVal}) = {b*b} âˆ’ {4*a*cVal} = <strong>{discriminant}</strong><br />
 {discriminant >= 0 ? (
 ...
 {checkResult === 'correct' && <p className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> {t('lab.m10theorems_correct_x')} {-b/(2*a)} ± âˆš{discriminant}/({2*a})</p>}
...
function VectorDiffInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_ab')}{compX}² + {compY}²) = âˆš{compX*compX + compY*compY} = <strong>{mag.toFixed(2)}</strong>
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
...
function LawCosinesInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 <text x="35" y="82" className="text-[10px] fill-rose-600">Î±={alpha}°</text>
 </svg>
 </div>
 ...
 {t('lab.m10theorems_a_2')} {b}² + {c}² âˆ’ 2({b})({c}{t('lab.m10theorems_cos')}{alpha}°)<br />
 
 {t('lab.m10theorems_a_2')} {b*b} + {c*c} âˆ’ {2*b*c} × {Math.cos(rad).toFixed(4)}<br />
...
function LawSinesInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 âˆ Î³ = 180° âˆ’ {alpha}° âˆ’ {beta}° = {gamma}°<br />
...
function HerosFormulaInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 Î” = âˆš[{S.toFixed(1)}×{S.toFixed(1)}âˆ’{a})×({S.toFixed(1)}âˆ’{b})×({S.toFixed(1)}âˆ’{c})]<br />
 Î” = âˆš[{S.toFixed(1)}×{(S-a).toFixed(1)}×{(S-b).toFixed(1)}×{(S-c).toFixed(1)}]<br />
...
function InRadiusInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_check_r_s')} {r.toFixed(2)}×{S.toFixed(1)} = {(r*S).toFixed(2)} = Î” âœ“
 </p>
 </div>
 <div className="flex gap-2">
 <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)}
...
function CircleChordInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_chord_half_length_r_d')}{radius}² âˆ’ {chordOffset}²)<br />
...
function CyclicQuadInteractive({ onAnswer }: { onAnswer: (correct: boolean) => void }) {
 ...
 {t('lab.m10theorems_a_c')} {angleA}° + {angleC}° = <strong>180°</strong> âœ“<br />
 
 {t('lab.m10theorems_b_d')} {angleB}° + {angleD}° = <strong>180°</strong> âœ“<br />
...
const THEOREMS: TheoremModule[] = [
 ...
 formula: 'x = [-b ± âˆš(b² âˆ’ 4ac)] / 2a',
 formulaDesc: 'A universal formula to solve any quadratic equation ax² + bx + c = 0.',
 color: 'blue',
 steps: [
 { label: 'Start with standard form', content: 'ðŸ—ï¸ You\'re an architect designing a parabolic arch. The curve follows ax² + bx + c = 0. You need a formula that works for ANY arch shape. Start with the general equation: ax² + bx + c = 0.' },
 { label: 'Move constant term', content: 'First, shift the constant to the right: ax² + bx = âˆ’c. The arch\'s height (c) doesn\'t affect the shape — it just shifts it up or down.' },
 { label: 'Divide by a', content: 'Divide both sides by a: x² + (b/a)x = âˆ’c/a. Now the coefficient of x² is 1, making it easier to "complete the square."' },
 { label: 'Complete the square', content: 'Add (b/2a)² to both sides: x² + (b/a)x + (b/2a)² = (b/2a)² âˆ’ c/a. The left simplifies to (x + b/2a)². The right becomes (b²âˆ’4ac)/(4a²).' },
 { label: 'Take square root and solve', content: 'x + b/2a = ±âˆš(b²âˆ’4ac)/(2a). Isolate x: x = [âˆ’b ± âˆš(b²âˆ’4ac)] / (2a). This is the Quadratic Formula! The discriminant D = b²âˆ’4ac tells you everything: D > 0 means 2 real roots (arch crosses ground twice), D = 0 means 1 root (arch just touches ground), D < 0 means no real roots (arch never meets ground).' },
 ],
 ...
 { label: 'Substitute position vectors', content: 'AB = bâƒ— âˆ’ Ã£ = (xâ‚‚âˆ’xâ‚, yâ‚‚âˆ’yâ‚) = (7âˆ’2, 5âˆ’3) = (5, 2). The displacement between the drones is 5 units east and 2 units north. Its magnitude |AB| = âˆš(5²+2²) = âˆš29 â‰ˆ 5.39 units. This formula is fundamental in physics: velocity = position_B âˆ’ position_A / time!' },
 ],
 ...
 { label: 'Apply Pythagoras in right triangle ADC', content: 'b² = AD² + CD² = (c sin Î²)² + (a âˆ’ c cos Î²)² = c² sin²Î² + a² + c² cos²Î² âˆ’ 2ac cos Î².' },
 { label: 'Simplify using sin²+cos²=1', content: 'b² = a² + c²(sin²Î² + cos²Î²) âˆ’ 2ac cos Î² = a² + c² âˆ’ 2ac cos Î². This is the Law of Cosines! When Î² = 90°, cos 90° = 0, so it becomes b² = a² + c² — the Pythagorean Theorem! The Law of Cosines is Pythagoras supercharged for ANY triangle.' },
 ],
 ...
 { label: 'Equate the expressions', content: 'c sin Î± = a sin Î³ → a/sin Î± = c/sin Î³. The ratio of side a to sin Î± equals the ratio of side c to sin Î³.' },
 { label: 'Repeat for other sides', content: 'By drawing altitudes to other sides, we get a/sin Î± = b/sin Î². Combining all: a/sin Î± = b/sin Î² = c/sin Î³. This is the Law of Sines. It\'s the foundation of triangulation — GPS satellites use this to pinpoint your location by measuring angles to multiple satellites!' },
 ],
 ...
 color: 'indigo',
 steps: [
 { label: 'Start with Law of Sines', content: 'ðŸ“ From the Law of Sines: a/sin Î± = b/sin Î² → a/b = sin Î± / sin Î². This ratio connects side lengths to the sines of their opposite angles.' },
 ...
 { label: 'Simplify to tangent ratio', content: 'This simplifies to: (a+b)/(aâˆ’b) = tan[(Î±+Î²)/2] / tan[(Î±âˆ’Î²)/2]. The Law of Tangents! It\'s especially useful in surveying and navigation when you know two sides and the included angle, or two angles and a side — it avoids needing a calculator for square roots!' },
 ],
 interactive: LawCosinesInteractive,
 },
 {
 ...
 color: 'emerald',
 steps: [
 { label: 'Start with Law of Cosines', content: 'ðŸ“Š Start with cos Î± = (b²+c²âˆ’a²)/(2bc). This gives cos of the full angle Î± in terms of the three side lengths.' },
 { label: 'Use double-angle identity', content: 'The double-angle identity says cos Î± = 2cos²(Î±/2) âˆ’ 1. Substitute the Law of Cosines: 2cos²(Î±/2) âˆ’ 1 = (b²+c²âˆ’a²)/(2bc).' },
 { label: 'Solve for cos²(Î±/2)', content: '2cos²(Î±/2) = 1 + (b²+c²âˆ’a²)/(2bc) = (2bc + b² + c² âˆ’ a²)/(2bc) = [(b+c)² âˆ’ a²]/(2bc).' },
 { label: 'Factor and substitute S', content: '[(b+c)² âˆ’ a²] = (b+c+a)(b+câˆ’a). Since a+b+c = 2S (the semi-perimeter), and b+câˆ’a = 2(Sâˆ’a), we get: 2cos²(Î±/2) = [2S × 2(Sâˆ’a)]/(2bc). Divide by 2 and take the square root: cos(Î±/2) = âˆš[S(Sâˆ’a)/(bc)]. This formula is elegant — it expresses a trigonometric ratio purely in terms of side lengths, no angles needed!' },
 ],
 ...
 color: 'emerald',
 steps: [
 { label: 'Use identity 1âˆ’2sin²(Î±/2) = cos Î±', content: 'ðŸ”„ Starting with the identity cos Î± = 1 âˆ’ 2sin²(Î±/2), substitute the Law of Cosines: 1 âˆ’ 2sin²(Î±/2) = (b²+c²âˆ’a²)/(2bc).' },
 { label: 'Solve for sin²(Î±/2)', content: '2sin²(Î±/2) = 1 âˆ’ (b²+c²âˆ’a²)/(2bc) = (2bc âˆ’ b² âˆ’ c² + a²)/(2bc) = [a² âˆ’ (bâˆ’c)²]/(2bc).' },
 { label: 'Factor as difference of squares', content: 'a² âˆ’ (bâˆ’c)² = (aâˆ’b+c)(a+bâˆ’c). Using semi-perimeter S: aâˆ’b+c = 2(Sâˆ’b) and a+bâˆ’c = 2(Sâˆ’c).' },
 { label: 'Substitute and simplify', content: '2sin²(Î±/2) = [2(Sâˆ’b) × 2(Sâˆ’c)]/(2bc). Divide by 2 and take square root: sin(Î±/2) = âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)]. Together with the cosine half-angle formula, these form the basis for Hero\'s formula and are used extensively in navigation and astronomy!' },
 ],
 interactive: HerosFormulaInteractive,
 },
 {
 ...
 color: 'violet',
 steps: [
 { label: 'Basic area formula', content: 'ðŸŒ³ A land surveyor measures a triangular plot. They know two sides (b and c) and the angle between them (Î±). The basic area formula is: Area = ½ × base × height.' },
 ...
 { label: 'Substitute into area formula', content: 'Area = ½ × AB × h = ½ × c × (b sin Î±) = ½ bc sin Î±. This is the SAS (Side-Angle-Side) area formula. If Î± = 90°, sin 90° = 1, so Area = ½ bc — the familiar right triangle area formula!' },
 ],
 interactive: LawCosinesInteractive,
 },
 {
 ...
 formula: 'Î” = ½ a² sin Î² sin Î³ / sin Î±',
 formulaDesc: 'The area of a triangle using one side and two angles.',
 color: 'violet',
 steps: [
 { label: 'Start with SAS formula', content: 'ðŸ“‹ When you know two angles (Î², Î³) and the included side a (AAS), you can still find area. Start from Area = ½ ab sin Î³.' },
 { label: 'Use Law of Sines to find b', content: 'From Law of Sines: b/sin Î² = a/sin Î± → b = a sin Î² / sin Î±. This expresses side b in terms of known angles and side a.' },
 { label: 'Substitute into area formula', content: 'Area = ½ a × (a sin Î² / sin Î±) × sin Î³ = ½ a² (sin Î² sin Î³) / sin Î±. This is the AAS area formula. It\'s perfect when you can\'t measure both adjacent sides but you can measure angles — like measuring a mountain\'s profile from a distance!' },
 ],
 interactive: LawSinesInteractive,
 },
 {
 ...
 color: 'emerald',
 steps: [
 { label: 'Start with area = ½ bc sin Î±', content: 'ðŸ“ You\'ve measured all three sides of a triangular garden: a=5m, b=6m, c=7m. You need the area but don\'t know any angles. Start with Î” = ½ bc sin Î±.' },
 { label: 'Write sin Î± = 2 sin(Î±/2) cos(Î±/2)', content: 'Using the double-angle identity: sin Î± = 2 sin(Î±/2) cos(Î±/2). So Î” = bc × sin(Î±/2) × cos(Î±/2).' },
 { label: 'Substitute half-angle formulas', content: 'Substitute sin(Î±/2) = âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)] and cos(Î±/2) = âˆš[S(Sâˆ’a)/(bc)]. The bc factors outside the radicals multiply: bc × âˆš[(Sâˆ’b)(Sâˆ’c)/(bc)] × âˆš[S(Sâˆ’a)/(bc)].' },
 { label: 'Simplify to Hero\'s formula', content: 'bc × 1/(bc) × âˆš[S(Sâˆ’a)(Sâˆ’b)(Sâˆ’c)] = âˆš[S(Sâˆ’a)(Sâˆ’b)(Sâˆ’c)]. This is Hero\'s (or Heron\'s) formula! For our garden: S = (5+6+7)/2 = 9, Area = âˆš[9×4×3×2] = âˆš216 â‰ˆ 14.7m². This 2,000-year-old formula, discovered by Hero of Alexandria, requires ONLY side lengths — no angles, no trigonometry!' },
 ],
 interactive: HerosFormulaInteractive,
 },
 {
 ...
 { label: 'Apply sine in right triangle BCD', content: 'In right triangle BCD: BC/BD = sin(âˆ BDC) = sin Î±. Since BC = a and BD = 2R: a/(2R) = sin Î± → R = a/(2 sin Î±).' },
 { label: 'Express in terms of sides and area', content: 'From area formula: Î” = ½ bc sin Î± → sin Î± = 2Î”/(bc). Substituting: R = a/(2 × 2Î”/(bc)) = abc/(4Î”). The circumradius relates all three sides and the area! For an equilateral triangle, R = a/âˆš3. The larger the triangle, the larger the circumcircle!' },
 ],
 ...
 { label: 'Sum the areas', content: 'Total area Î” = Area(IBC) + Area(IAC) + Area(IAB) = ½ar + ½br + ½cr = r(a+b+c)/2.' },
 { label: 'Solve for r', content: 'Since S = (a+b+c)/2 (semi-perimeter), we get Î” = rS → r = Î”/S. The inradius is the ratio of the area to the semi-perimeter! For a right triangle with legs 3 and 4 (hypotenuse 5): Î” = 6, S = 6, so r = 1. The incircle has radius 1. This formula is used to find the radius of a pizza that fits exactly inside a triangular box!' },
 ],
 ...
 { label: 'Substitute base×height/2', content: 'Î” = ½crâ‚ + ½brâ‚ âˆ’ ½arâ‚ = ½râ‚(b + c âˆ’ a). Watch the minus sign — Iâ‚BC is partially outside the original triangle!' },
 { label: 'Simplify using S', content: 'b + c âˆ’ a = (a+b+c) âˆ’ 2a = 2S âˆ’ 2a = 2(Sâˆ’a). So Î” = ½râ‚ × 2(Sâˆ’a) = râ‚(Sâˆ’a). Rearranging: râ‚ = Î”/(Sâˆ’a). Similarly, râ‚‚ = Î”/(Sâˆ’b) and râ‚ƒ = Î”/(Sâˆ’c). Exradii are larger than the inradius and are used in advanced geometry proofs and triangle geometry problems!' },
 ],
 interactive: InRadiusInteractive,
 },
 ...
 color: 'indigo',
 steps: [
 { label: 'Take three non-collinear points', content: 'ðŸ“ Pick any three points A, B, C that don\'t lie on a straight line — like three trees in a park that aren\'t in a row. Connect them to form a triangle.' },
 ...
 color: 'indigo',
 steps: [
 { label: 'Draw chord AB and perpendicular from O', content: 'ðŸŽ¯ Draw a chord AB and a perpendicular from centre O to meet AB at C (âˆ OCA = 90°).' },
 { label: 'Join OA and OB', content: 'OA and OB are radii, so OA = OB. Triangle OAB is isosceles.' },
 { label: 'Prove triangles OAC and OBC are congruent', content: 'OA = OB (radii), OC = OC (common side), and âˆ OCA = âˆ OCB = 90°. So triangle OAC â‰… OBC by RHS congruence. Therefore AC = BC.' },
 ...
 { label: 'Use Pythagoras in triangles', content: 'In right triangles OAE and OCF: OE² = OA² âˆ’ AE² and OF² = OC² âˆ’ CF². Since OA = OC (radii) and AE = CF, we get OE² = OF², so OE = OF.' },
 ...
 { label: 'Use Pythagoras', content: 'In right triangles OAE and OCF: AE² = OA² âˆ’ OE² and CF² = OC² âˆ’ OF². Since OA = OC (radii) and OE = OF, we get AE² = CF², so AE = CF.' },
 ...
 color: 'indigo',
 steps: [
 { label: 'Take two congruent arcs', content: 'ðŸŽµ Imagine two identical arcs on a circle — like two equal-length curved sections of a circular railway track.' },
 ...
 { label: 'Equal central angles mean equal arcs', content: 'The measure of an arc equals the measure of its central angle. Since âˆ AOB = âˆ COD, the arcs AB and CD are congruent. This completes the cycle: equal chords ←” equal central angles ←” congruent arcs!' },
 ],
 interactive: CircleChordInteractive,
 },
 {
 ...
 { label: 'Join centre O to points', content: 'Join OA, OB, and OC. Since tangents are perpendicular to radii (Theorem 10.2): OB âŸ‚ AB and OC âŸ‚ AC, so âˆ OBA = âˆ OCA = 90°.' },
 { label: 'Prove triangles congruent', content: 'In right triangles OBA and OCA: OB = OC (radii), OA = OA (common side), and âˆ OBA = âˆ OCA = 90°. So triangles OBA â‰… OCA by RHS congruence.' },
 ...
 color: 'purple',
 steps: [
 { label: 'Draw two circles touching internally', content: 'â­•ðŸ”µ Draw two circles — one larger (radius râ‚ = 3cm, centre A) and one smaller (radius râ‚‚ = 1cm, centre B) inside it, touching at exactly one point C.' },
 ...
 { label: 'Distance equals difference', content: 'AB = 3 âˆ’ 1 = 2 cm. If circles touch internally, the distance between centres equals the difference of their radii. This principle is used in ball bearings — the inner race (small circle) touches the outer race (large circle) internally!' },
 ],
 interactive: TangentCircleInteractive,
 },
 {
 ...
 { label: 'Central = 2 × inscribed', content: 'âˆ COD = 2âˆ CEO + 2âˆ DEO = 2(âˆ CEO + âˆ DEO) = 2âˆ CED. The central angle ALWAYS equals twice the inscribed angle subtended by the same arc. This is why the angle in a semicircle is 90° — the central angle is 180° (a straight line), so the inscribed angle is 90°!' },
 ],
 ...
 { label: 'Measure inscribed angles', content: 'Measure angles âˆ ACB, âˆ ADB, and âˆ AEB — all subtended by chord AB at points C, D, and E.' },
 ...
 { label: 'Angles in same segment are equal', content: 'Therefore, âˆ ACB = âˆ ADB = âˆ AEB. All angles in the same segment of a circle are equal. This is why any point on a circular arc \"sees\" a chord at the same angle — it\'s the principle behind the \"goal line\" in circular sports stadiums!' },
 ],
 interactive: SemicircleAngleInteractive,
 },
 {
 ...
 formula: 'âˆ APB = 90° (where AB is diameter)',
 ...
 { label: 'Apply central angle theorem', content: 'The central angle for chord AB is âˆ AOB = 180° (a straight line through the diameter). By Theorem 10.7, the inscribed angle âˆ APB = ½ × âˆ AOB = ½ × 180° = 90°.' },
 { label: 'No matter where P is', content: 'This works for ANY point P on the semicircle! Move P anywhere along the arc, and âˆ APB always equals 90°.' },
 ...
 formula: 'âˆ APB < 90° (major segment)',
 ...
 { label: 'Measure inscribed angles', content: 'Measure âˆ APB, âˆ AQB, and âˆ ARB. Each of these angles is less than the angle in a semicircle (90°).' },
 { label: 'Central angle is less than 180°', content: 'The central angle âˆ AOB subtended by the major segment is less than 180°. By Theorem 10.7: âˆ APB = ½âˆ AOB < ½ × 180° = 90°.' },
 ...
 formula: 'âˆ APB > 90° (minor segment)',
 ...
 { label: 'Central angle is > 180°', content: 'The central angle âˆ AOB subtended by the minor segment is GREATER than 180° (the major central angle). By Theorem 10.7: âˆ APB = ½ × (major central angle) > ½ × 180° = 90°.' },
 ...
 formula: 'âˆ A + âˆ C = 180°, âˆ B + âˆ D = 180°',
 ...
 { label: 'Apply central angle theorem', content: 'âˆ BAD = ½âˆ BOD (reflex) and âˆ BCD = ½âˆ BOD (the non-reflex part). The two central angles BOD (reflex) + BOD (non-reflex) = 360°.' },
 { label: 'Sum of opposite angles = 180°', content: 'âˆ A + âˆ C = ½(âˆ BOD reflex + âˆ BOD) = ½ × 360° = 180°. Similarly, âˆ B + âˆ D = 180°. Opposite angles of a cyclic quadrilateral are supplementary! This is why a rectangular picture frame can always be inscribed in a circle — its opposite angles are 90° + 90° = 180°. This theorem is used in surveying to check if four points lie on a circle!' },
 ],
