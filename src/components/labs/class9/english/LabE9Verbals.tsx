...
export default function LabE9Verbals({ onExit }: { onExit?: () => void }) {
 ...
 return (
 <div className=" bg-slate-50 dark:bg-[#000000] flex flex-col font-sans min-h-screen lg:h-screen overflow-x-hidden w-full">
 ...
 {/* Window 1: Theory */}
 <section className={`w-full rounded-xl shadow-sm p-6 border border-slate-200 dark:border-[#1c1b1b] flex-col min-h-0 lg:overflow-y-auto ${activeMobileTab === 'theory' ? 'flex' : 'hidden'} lg:flex`}>
 <VocabularyPanel words={['running', 'swimming', 'broken', 'hidden', 'interesting', 'exciting', 'exhausted', 'gerund', 'participle', 'infinitive', 'reading', 'baking']} title="Key Words" />
 ...
 {t('lab.e9verbals_retry_assessment')}
    ...
    <section className={`bg-slate-100 dark:bg-[#0a0a0a] rounded-xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] relative flex flex-col min-h-0 overflow-y-auto p-8 ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
      ...
      {/* Current Item */}
      <div className="z-10 w-full max-w-lg flex-1 flex flex-col items-center justify-center mx-auto">
        {!isSimulationRunning && queue.length > 0 && (
          <div className="text-center p-8 rounded-xl shadow-xl border border-slate-200 dark:border-[#2a2a2a] w-full">
        ...
        {isSimulationRunning && currentItem && (
          <div className="text-center p-8 rounded-xl shadow-xl border-2 border-indigo-200 dark:border-indigo-800 animate-in zoom-in duration-300 w-full">
