...
export default function LabP9DerivationKE({ onExit }: { onExit?: () => void }) {
 ...
 const testSteps=[{testEquation:'E_k = Fd',testHint:'Kinetic energy equals the work done, which is force times distance.'},{testEquation:'E_k = ma × (v/2 × t)',testHint:'Substitute d = (v/2)t into E_k = Fd, using F = ma.'},{testEquation:'E_k = m × (v/t) × (v/2 × t)',testHint:'Replace acceleration a with v/t in the kinetic energy expression.'},{testEquation:'E_k = ½mv²',testHint:'After cancellations, kinetic energy = one-half mass × velocity squared.'}];
 ...
 const steps = [
 { label: t('lab.ke_step1_label'), formula: 'Eâ‚– = W = F × d', detail: t('lab.ke_step1_detail') },
 { label: t('lab.ke_step2_label'), formula: 'Eâ‚– = (m × a) × d', detail: t('lab.ke_step2_detail') },
 { label: t('lab.ke_step3_label'), formula: 'd = (v/2) × t, a = v/t', detail: t('lab.ke_step3_detail') },
 { label: t('lab.ke_step4_label'), formula: 'Eâ‚– = ½mv²', detail: t('lab.ke_step4_detail') },
 ];
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Kinetic Energy (Eâ‚– = ½mv²)")} />
 ...
 <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-3 border border-yellow-200 dark:border-yellow-800"><p className="text-xs font-bold text-yellow-700 dark:text-yellow-300">âš¡ The v² effect</p><div className="mt-1 space-y-1"><div className="flex justify-between text-xs"><span className="text-slate-500">½ speed ({Math.max(1,(velocity*0.5)).toFixed(1)} m/s)</span><span className="text-yellow-600 font-mono">{(0.5*mass*Math.max(1,velocity*0.5)**2).toFixed(1)} J</span></div><div className="flex justify-between text-xs"><span className="text-slate-500">Current ({velocity} m/s)</span><span className="text-orange-600 font-mono font-bold">{kineticEnergy.toFixed(1)} J</span></div><div className="flex justify-between text-xs"><span className="text-slate-500">2× speed ({Math.min(velocity*2,20)} m/s)</span><span className="text-red-600 font-mono">{(0.5*mass*Math.min(velocity*2,20)**2).toFixed(1)} J (×{Math.min((velocity*2)**2/velocity**2,20**2/velocity**2).toFixed(0)})</span></div></div></div>
