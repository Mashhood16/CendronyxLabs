...
import { Mountain, CheckCircle, XCircle, Lightbulb, ArrowRight, BookOpen, BrainCircuit, HelpCircle, Trophy, EyeOff, Sparkles, RefreshCcw } from 'lucide-react';
...
export default function LabP9DerivationGPE({ onExit }: { onExit?: () => void }) {
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Gravitational PE (Eₚ = mgh)")} />
 ...
 <button onClick={()=>setActiveMobileTab('theory')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab==='theory'?'bg-emerald-600 text-white shadow-md':'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.derivation')}</button>
 <button onClick={()=>setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab==='lab'?'bg-emerald-600 text-white shadow-md':'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.simulation')}</button>
 </div>
 ...
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5"><div className="flex items-center gap-2 mb-3"><Lightbulb className="w-5 h-5 text-emerald-500" /><h2 className="text-lg font-bold">{t('lab.see_in_action')}</h2></div><p className="text-sm text-slate-500 mb-4">{t('lab.gpe_adjust_desc')}</p><div className="space-y-3">
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.gpe_mass_label')}</span><span className="text-emerald-600 font-mono">{mass} kg</span></div><input type="range" min="0.5" max="50" step="0.5" value={mass} onChange={e=>{setMass(parseFloat(e.target.value));setCheckResult('idle');}} className="w-full accent-emerald-500" /></div>
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.gpe_height_label')}</span><span className="text-emerald-600 font-mono">{height} m</span></div><input type="range" min="1" max="30" step="0.5" value={height} onChange={e=>{setHeight(parseFloat(e.target.value));setCheckResult('idle');}} className="w-full accent-emerald-500" /></div>
 <div className="relative h-40 bg-white dark:bg-[#1c1b1b] rounded-lg overflow-hidden border border-slate-300 dark:border-[#2a2a2a]"><div className="absolute bottom-0 left-0 right-0 h-5 bg-gradient-to-t from-emerald-600 to-emerald-500 flex items-center justify-center"><span className="text-[7px] text-white font-bold">GROUND</span></div><div className="absolute left-1/2 -translate-x-1/2 w-10 h-10 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-lg shadow-lg flex items-center justify-center text-[8px] font-bold text-white transition-all duration-300" style={{bottom:`${5+(height/30)*62}%`}}>{mass} kg</div><div className="absolute left-2 bottom-5 w-2" style={{height:`${(height/30)*62}%`}}><div className="w-0.5 h-full bg-emerald-400/40 mx-auto" /><div className="absolute -top-2.5 left-1 text-[7px] text-emerald-500 font-mono">h={height}m</div><div className="absolute top-1/2 -translate-y-1/2 -left-0.5 text-emerald-400 text-[7px]">{'▲'}</div><div className="absolute bottom-0 -left-0.5 text-emerald-400 text-[7px]">{'▼'}</div></div>{[5,10,15,20,25].filter(m=>m<=height).map(m=>{const pct=(m/30)*62+5;return<div key={m} className="absolute left-11 w-5 border-t border-dashed border-slate-300 dark:border-[#2a2a2a]" style={{bottom:`${pct}%`}}><span className="text-[6px] text-slate-400">{m}m</span></div>;})}</div>
 <div className="bg-[#000000] rounded-lg p-4 border border-[#1c1b1b] space-y-1"><p className="text-xs text-slate-500 font-semibold uppercase">Derivation Trace</p><p className="text-sm text-slate-400">{t('lab.gpe_trace1')}</p><p className="text-sm text-slate-400">{t('lab.gpe_trace2',{f:(mass*9.81).toFixed(1)})}</p><p className="text-sm text-slate-400">{t('lab.gpe_trace3',{h:height})}</p><p className="border-t border-[#2a2a2a] pt-1 text-xs"><span className="text-green-400 font-bold">{t('lab.gpe_trace4',{e:potentialEnergy.toFixed(0)})}</span></p>{potentialEnergy>500&&<p className="text-xs text-amber-400">Could power a 60W bulb for {(potentialEnergy/60).toFixed(0)}s!</p>}</div>
 </div></div>
 <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800 mt-2"><div className="flex items-start gap-2"><Lightbulb className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" /><div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.gpe_real_life')}</p></div></div></div>
 ...
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3"><p className="text-base font-medium mb-2">{t('lab.gpe_practice_q')}</p><p className="text-base font-medium">{t('lab.gpe_practice_q2')}</p><div className="bg-emerald-50 dark:bg-emerald-900/20 rounded p-2 mt-2"><p className="text-xs text-emerald-700 dark:text-emerald-300 font-mono">{t('lab.gpe_practice_hint')}</p></div></div>
 <div className="flex gap-2 mb-2"><input type="number" value={userAns} onChange={e=>setUserAns(e.target.value)} placeholder={t('lab.gpe_placeholder')} className="flex-1 px-3 py-2 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-emerald-500 outline-none" /><button onClick={checkAnswer} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button></div>
 {checkResult==='correct'&&<div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.gpe_correct_feedback')}</p></div>}
 {checkResult==='incorrect'&&<div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.gpe_incorrect_feedback')}</p></div>}
 </div>
...
<div className="flex-1 overflow-y-auto p-4 lg:p-6">
 ...
 {steps.map((_, idx) => (
 <div key={idx} className={`flex-1 h-1.5 rounded-full transition-all ${completedSteps[idx] ? 'bg-emerald-500' : idx === currentStep ? 'bg-purple-400 animate-pulse' : 'bg-slate-200 dark:bg-[#1c1b1b]'}`} />
 ))}
 </div>
 </div>
 {testFullyCompleted ? (
 ...
 <div
  ...
  className={`relative rounded-xl border transition-all cursor-pointer overflow-hidden
   ...
   ${isActive ? 'border-purple-400 dark:border-purple-600 bg-white dark:bg-[#1c1b1b] shadow-md shadow-purple-500/10 ring-1 ring-purple-400/30' : ''}
 ...
 >
  ...
  <div className="px-4 py-3 flex items-center gap-3">
   <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 transition-all
    ...
    ${isActive ? 'bg-purple-500 text-white shadow-sm' : ''}
   ...
   {isActive && !isCompleted && <div className="w-2 h-2 rounded-full bg-purple-500 animate-pulse shrink-0" />}
  </div>
 ...
 <div className="hidden lg:block w-full lg:w-[380px] xl:w-[420px] shrink-0">
  <div className="bg-white dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#1c1b1b] p-5 sticky top-4">
   ...
   <div className="flex items-center gap-2 mb-4">
    <div className="w-8 h-8 rounded-full bg-purple-500 text-white flex items-center justify-center text-xs font-bold shrink-0">{currentStep + 1}</div>
    <div>
