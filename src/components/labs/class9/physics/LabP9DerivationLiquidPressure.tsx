...
export default function LabP9DerivationLiquidPressure({ onExit }: { onExit?: () => void }) {
 ...
 const testSteps=[{testEquation:'P = F/A',testHint:'Pressure is defined as force divided by the area over which it acts.'},{testEquation:'m = ÏV',testHint:'Mass equals density multiplied by volume.'},{testEquation:'V = Ah',testHint:'Volume of a column = cross-sectional area × height.'},{testEquation:'P = Ïgh',testHint:'After substituting F = ÏAhg into P = F/A, the area cancels.'}];
 ...
 { label: t('lab.lp_step2_label'), formula: 'F = m × g = Ï × V × g', detail: t('lab.lp_step2_detail') },
 { label: t('lab.lp_step3_label'), formula: 'V = A × h', detail: t('lab.lp_step3_detail') },
 { label: t('lab.lp_step4_label'), formula: 'P = Ï × g × h', detail: t('lab.lp_step4_detail') },
 ];
 ...
 <button onClick={()=>setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab==='lab'?'bg-cyan-600 text-white shadow-md':'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.simulation')}</button>
 </div>
 ...
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3"><p className="text-base font-medium mb-2">{t('lab.lp_practice_q')}</p><p className="text-base font-medium">{t('lab.lp_practice_q2')}</p><div className="bg-cyan-50 dark:bg-cyan-900/20 rounded p-2 mt-2"><p className="text-xs text-cyan-700 dark:text-cyan-300 font-mono">{t('lab.lp_practice_hint')}</p></div></div>
 <div className="flex gap-2 mb-2"><input type="number" value={userAns} onChange={e=>setUserAns(e.target.value)} placeholder={t('lab.lp_placeholder')} className="flex-1 px-3 py-2 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-cyan-500 outline-none" /><button onClick={checkAnswer} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button></div>
 {checkResult==='correct'&&<div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.lp_correct_feedback')}</p></div>}
 {checkResult==='incorrect'&&<div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.lp_incorrect_feedback')}</p></div>}
 </div>
...
<div className="flex-1 overflow-y-auto p-4 lg:p-6">
 ...
 {steps.map((_, idx) => (
 <div key={idx} className={`flex-1 h-1.5 rounded-full transition-all ${completedSteps[idx] ? 'bg-emerald-500' : idx === currentStep ? 'bg-cyan-400 animate-pulse' : 'bg-slate-200 dark:bg-[#1c1b1b]'}`} />
 ))}
 </div>
 </div>
 {testFullyCompleted ? (
 ...
 <div
  ...
  className={`relative rounded-xl border transition-all cursor-pointer overflow-hidden
   ...
   ${isActive ? 'border-cyan-400 dark:border-cyan-600 bg-white dark:bg-[#1c1b1b] shadow-md shadow-cyan-500/10 ring-1 ring-cyan-400/30' : ''}
 ...
 >
  ...
  <div className="px-4 py-3 flex items-center gap-3">
   <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 transition-all
    ...
    ${isActive ? 'bg-cyan-500 text-white shadow-sm' : ''}
   ...
   {isActive && !isCompleted && <div className="w-2 h-2 rounded-full bg-cyan-500 animate-pulse shrink-0" />}
  </div>
 ...
 <div className="hidden lg:block w-full lg:w-[380px] xl:w-[420px] shrink-0">
  <div className="bg-white dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#1c1b1b] p-5 sticky top-4">
   ...
   <div className="flex items-center gap-2 mb-4">
    <div className="w-8 h-8 rounded-full bg-cyan-500 text-white flex items-center justify-center text-xs font-bold shrink-0">{currentStep + 1}</div>
    <div>
