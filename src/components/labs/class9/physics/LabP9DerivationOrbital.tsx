...
export default function LabP9DerivationOrbital({ onExit }: { onExit?: () => void }) {
 ...
 <LabHeader onExit={onExit} title={t("Derivation: Orbital Speed of a Satellite")} />
 ...
 <button onClick={()=>setActiveMobileTab('lab')} className={`py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab==='lab'?'bg-indigo-600 text-white shadow-md':'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}>{t('lab.tab.simulation')}</button>
 </div>
 ...
 <div><div className="flex justify-between text-xs font-semibold"><span>{t('lab.orbital_radius_label')}</span><span className="text-indigo-600 font-mono">{radius.toLocaleString()} km</span></div><input type="range" min="6400" max="42000" step="100" value={radius} onChange={e=>{setRadius(parseFloat(e.target.value));setCheckResult('idle');}} className="w-full accent-indigo-500" /><div className="flex justify-between text-[9px] text-slate-400 mt-0.5"><span>{t('lab.p9derivationorbital_earth_surface_6_378_km')}</span><span>{t('lab.p9derivationorbital_geo_35_786_km')}</span></div></div>
 <div className="relative h-32 bg-[#000000] rounded-lg overflow-hidden border border-[#1c1b1b]">
 <div className="absolute left-1/2 top-1/2 w-12 h-12 bg-blue-600 rounded-full -translate-x-1/2 -translate-y-1/2 shadow-lg border-4 border-blue-400/30"><span className="absolute inset-0 flex items-center justify-center text-[6px] text-white font-bold">{t('lab.p9derivationorbital_earth')}</span></div>
 ...
 <div className="absolute top-2 right-2 bg-black/80 rounded px-2 py-1 border border-[#1c1b1b]"><div className="text-xs text-green-400 font-mono font-bold">{speed.toFixed(0)} m/s</div><div className="text-[8px] text-slate-500">T = {(period/60).toFixed(1)} min</div></div>
 </div>
 ...
 <p className="text-sm text-slate-400">{t('lab.orbital_trace1')}</p><p className="text-sm text-slate-400">{t('lab.orbital_trace2',{r:rM.toLocaleString()})}</p><p className="text-sm text-slate-400">{t('lab.orbital_trace3',{T:period.toFixed(0)})}</p>
 ...
 <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800 mt-2"><div className="flex items-start gap-2"><Lightbulb className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" /><div><p className="font-bold text-base text-amber-700 dark:text-amber-300">{t('lab.real_life_application')}</p><p className="text-sm text-amber-700 dark:text-amber-300 mt-1">{t('lab.orbital_real_life')}</p></div></div></div>
 <div className="bg-white dark:bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5">
 ...
 <div className="bg-slate-50 dark:bg-[#1c1b1b] rounded-lg p-4 border border-slate-200 dark:border-[#2a2a2a] mb-3"><p className="text-base font-medium mb-2">{t('lab.orbital_practice_q')}</p><p className="text-base font-medium">{t('lab.orbital_practice_q2')}</p><div className="bg-indigo-50 dark:bg-indigo-900/20 rounded p-2 mt-2"><p className="text-xs text-indigo-700 dark:text-indigo-300 font-mono">{t('lab.orbital_practice_hint')}</p></div></div>
 <div className="flex gap-2 mb-2"><input type="number" value={userAns} onChange={e=>setUserAns(e.target.value)} placeholder={t('lab.orbital_placeholder')} className="flex-1 px-3 py-2 text-sm border border-slate-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#121212] focus:ring-2 focus:ring-indigo-500 outline-none" /><button onClick={checkAnswer} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg transition-colors">{t('lab.check')}</button></div>
 {checkResult==='correct'&&<div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" /><p className="text-xs text-emerald-700 dark:text-emerald-300"><strong>{t('lab.correct')}</strong> {t('lab.orbital_correct_feedback')}</p></div>}
 {checkResult==='incorrect'&&<div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800 flex items-center gap-2"><XCircle className="w-5 h-5 text-red-500 shrink-0" /><p className="text-xs text-red-700 dark:text-red-300"><strong>{t('lab.incorrect')}</strong> {t('lab.orbital_incorrect_feedback')}</p></div>}
 </div>
...
<div className="flex-1 overflow-y-auto p-4 lg:p-6">
 ...
 {steps.map((_, idx) => (
 <div key={idx} className={`flex-1 h-1.5 rounded-full transition-all ${completedSteps[idx] ? 'bg-emerald-500' : idx === currentStep ? 'bg-indigo-400 animate-pulse' : 'bg-slate-200 dark:bg-[#1c1b1b]'}`} />
 ))}
 </div>
 </div>
 {testFullyCompleted ? (
 ...
 <div
  ...
  className={`relative rounded-xl border transition-all cursor-pointer overflow-hidden
   ...
   ${isActive ? 'border-indigo-400 dark:border-indigo-600 bg-white dark:bg-[#1c1b1b] shadow-md shadow-indigo-500/10 ring-1 ring-indigo-400/30' : ''}
 ...
 >
  ...
  <div className="px-4 py-3 flex items-center gap-3">
   ...
   {isActive && !isCompleted && <div className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse shrink-0" />}
  </div>
 ...
 <div className="hidden lg:block w-full lg:w-[380px] xl:w-[420px] shrink-0">
  <div className="bg-white dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#1c1b1b] p-5 sticky top-4">
   ...
   <div className="flex items-center gap-2 mb-4">
    <div className="w-8 h-8 rounded-full bg-indigo-500 text-white flex items-center justify-center text-xs font-bold shrink-0">{currentStep + 1}</div>
    <div>
