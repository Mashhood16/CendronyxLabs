...
import LabShellJunior from '../../class8/computer/LabShellJunior';
...
const SIMPLE_PUNCTUATION_TASKS = [
 ...
 correctOrder: ["We", "have", "won", "the", "match"],
 correctPunc: "!"
 },
 {
 id: 't3',
...
export default function LabE6SentencesAffixes({ onExit }: { onExit?: () => void }) {
 ...
 activeTab === 'affixes'
 ? 'bg-white dark:bg-[#2a2a2a] shadow-sm text-indigo-600 dark:text-indigo-400'
 ...
 {t('lab.e6sentencesaffixes_affix_factory')}
 </>
 ) : (
 <>
 <Quote className="w-6 h-6 mr-2 text-pink-500" />
 {t('lab.e6sentencesaffixes_transformer')}
 </>
 )}
 </h2>
 ...
 {/* Vocabulary Panel */}
 <VocabularyPanel
 words={['happy', 'care', 'friend', 'understand', 'act', 'The', 'dog', 'barking', 'loudly']}
 title="Key Words"
 />
 {activeTab === 'affixes' ? (
 ...
 : 'border border-slate-200 dark:border-[#1c1b1b] hover:border-indigo-300 dark:hover:border-indigo-600'
 }`}
 >
 {bw.word}
 </button>
 ...
 <div className="space-y-2 pt-6 border-t border-slate-200 dark:border-[#1c1b1b] mt-auto">
 <button
 onClick={checkAffixCombo}
 className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold transition-colors flex items-center justify-center whitespace-nowrap flex-shrink-0 dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:border-transparent dark:shadow-lg dark:shadow-indigo-500/40"
 >
 ...
 className="w-full py-3 px-4 bg-pink-600 hover:bg-pink-700 text-white rounded-xl font-semibold transition-colors flex items-center justify-center whitespace-nowrap flex-shrink-0 dark:bg-pink-500 dark:hover:bg-pink-400 dark:border-transparent dark:shadow-lg dark:shadow-pink-500/40"
 >
 ...
 {activeTab === 'affixes' ? (
 <div className={`w-full max-w-2xl ${theme.card.bg} rounded-3xl p-8 shadow-sm border ${theme.card.border} text-center flex flex-col items-center`}>
 ...
 activePrefix
 ? 'bg-blue-100 border-blue-400 text-blue-700 dark:bg-blue-900/50 dark:border-blue-500 dark:text-blue-200 text-3xl font-bold'
 ...
 : 'border-slate-300 dark:border-slate-600 text-slate-600 dark:text-[#ffffff]'
 }`}
 >
 {p}
 </button>
 ...
 <div className={`h-20 w-full flex items-center justify-center rounded-2xl border-4 border-dashed transition-all ${
 activeSuffix
 ? 'bg-green-100 border-green-400 text-green-700 dark:bg-green-900/50 dark:border-green-500 dark:text-green-200 text-3xl font-bold'
 : 'border-slate-200 dark:border-[#2a2a2a]'
 }`}>
 ...
 {/* Feedback Area */}
 {affixFeedback && (
 <div className={`w-full p-4 rounded-xl flex items-center justify-center space-x-3 text-lg font-medium animate-in fade-in slide-in-from-bottom-4 ${
 affixFeedback.type === 'success'
 ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300'
 : affixFeedback.type === 'error'
 ...
 <div className={`w-full max-w-3xl ${theme.card.bg} rounded-3xl p-8 shadow-sm border ${theme.card.border} flex flex-col items-center`}>
 ...
 <button onClick={() => moveWordLeft(idx)} disabled={idx === 0} className="p-1 bg-slate-100 dark:bg-[#1a1a1a] rounded disabled:opacity-30 border border-slate-200 dark:border-[#2a2a2a]">
 <ArrowLeft className="w-4 h-4" />
 </button>
 <button onClick={() => moveWordRight(idx)} disabled={idx === userWords.length - 1} className="p-1 bg-slate-100 dark:bg-[#1a1a1a] rounded disabled:opacity-30 border border-slate-200 dark:border-[#2a2a2a]">
 ...
 <LabShellJunior
 onExit={onExit}
 title={t('lab.e6sentencesaffixes_sentences_word_parts')}
