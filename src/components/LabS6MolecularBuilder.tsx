  ...
  return (
    <div className="overflow-y-auto flex flex-col h-screen bg-slate-100 dark:bg-slate-800 font-sans">
      ...
      <div className="flex-1 flex p-6 gap-6">
        ...
        <div className="w-80 bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col">
          ...
          <div className="flex gap-2 mb-6">
            {(['H2O', 'CO2', 'CH4'] as const).map(mol => (
              <button 
                ...
                className={`flex-1 py-2 rounded font-bold border-2 ${molecule === mol ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-slate-200 dark:border-slate-700 dark:border-slate-500 hover:border-purple-300'}`}
              >
                {mol}
              </button>
            ))}
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Click to add atoms to the builder to form {requirements[molecule].name}.</p>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <span className="font-medium text-slate-700 dark:text-slate-200">Oxygen (Red)</span>
            </button>
          </div>

          <button onClick={reset} className="mt-auto flex items-center justify-center py-3 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-100 font-bold bg-slate-100 dark:bg-slate-800 rounded-lg">
            <RefreshCcw className="w-5 h-5 mr-2" /> Reset Canvas
          </button>
        </div>

        {/* Right Panel - Assembly Canvas */}
        <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-8 flex flex-col items-center justify-center relative bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0MCIgaGVpZ2h0PSI0MCI+PHBhdGggZD0iTTAgMGg0MHY0MEgweiIgZmlsbD0ibm9uZSIvPPHBhdGggZD0iTTAgMGgxdjQwSDB6TTAgMGg0MHYxSDB6IiBmaWxsPSJyZ2JhKDAsMCwwLDAuMDUpIi8+PC9zdmc+')]">
          ...
          {isComplete() ? (
            ...
            <div className="absolute top-8 p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-xl text-slate-500 dark:text-slate-400 font-medium text-center">
          ...
          <div className="relative w-64 h-64 flex items-center justify-center">
            {molecule === 'H2O' && isComplete() && (
              <div className="relative w-full h-full">
                ...
                <div className="absolute top-[60%] right-[30%] w-16 h-3 bg-slate-300 dark:bg-slate-800 rounded-full origin-right rotate-45 shadow-sm"></div>
              </div>
            )}
            ...
            {molecule === 'CO2' && isComplete() && (
              <div className="relative w-full h-full flex items-center justify-center gap-1">
                ...
                <div className="flex flex-col gap-1 z-0 -mx-4"><div className="w-12 h-3 bg-slate-300 dark:bg-slate-800 rounded-full"></div><div className="w-12 h-3 bg-slate-300 dark:bg-slate-800 rounded-full"></div></div>
                <div className="w-20 h-20 bg-slate-800 dark:bg-slate-800 rounded-full shadow-[inset_-5px_-5px_15px_rgba(0,0,0,0.5)] z-10 flex items-center justify-center text-white font-bold text-2xl">C</div>
                <div className="flex flex-col gap-1 z-0 -mx-4"><div className="w-12 h-3 bg-slate-300 dark:bg-slate-800 rounded-full"></div><div className="w-12 h-3 bg-slate-300 dark:bg-slate-800 rounded-full"></div></div>
                <div className="w-16 h-16 bg-red-500 rounded-full shadow-[inset_-5px_-5px_15px_rgba(0,0,0,0.3)] z-10 flex items-center justify-center text-white font-bold text-2xl">O</div>
              </div>
            )}
