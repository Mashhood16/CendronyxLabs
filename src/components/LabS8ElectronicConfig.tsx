    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans">
      ...
      <div className="flex-1 p-6 flex flex-col md:flex-row gap-6 max-w-6xl mx-auto w-full">
        ...
        <div className="w-full md:w-64 flex flex-col gap-4">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-4">
            <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-3 border-b pb-2">Group IA (Alkali)</h3>
            <div className="flex flex-col gap-2">
              {ELEMENTS.filter(e => e.group === 'IA').map(e => (
                <button 
                  key={e.symbol}
                  onClick={() => setSelected(e)}
                  className={`p-2 text-left rounded-lg font-bold transition-colors ${selected.symbol === e.symbol ? 'bg-blue-600 text-white' : 'bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200'}`}
                >
                  {e.name} ({e.symbol})
                </button>
              ))}
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-4">
            <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-3 border-b pb-2">Group IIA (Alkaline Earth)</h3>
            <div className="flex flex-col gap-2">
              {ELEMENTS.filter(e => e.group === 'IIA').map(e => (
                <button 
                  key={e.symbol}
                  onClick={() => setSelected(e)}
                  className={`p-2 text-left rounded-lg font-bold transition-colors ${selected.symbol === e.symbol ? 'bg-green-600 text-white' : 'bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200'}`}
                >
                  {e.name} ({e.symbol})
                </button>
              ))}
        ...
        <div className="flex-1 bg-slate-900 dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-800 dark:border-slate-500 p-6 flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="relative w-80 h-80 flex items-center justify-center">
            ...
            {selected.electrons.map((count, shellIndex) => {
              ...
              return (
                <div 
                  key={shellIndex}
                  className={`absolute rounded-full border border-slate-700 dark:border-slate-500/50 flex items-center justify-center animate-spin-slow`}
                  style={{ 
                    width: radius * 2, 
                    height: radius * 2,
                ...
                >
                  ...
                  {Array.from({ length: count }).map((_, eIdx) => {
                    ...
                    return (
                      <div 
                        key={eIdx}
                        className={`absolute w-3 h-3 rounded-full ${isOutermost ? (selected.group === 'IA' ? 'bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,0.8)]' : 'bg-green-400 shadow-[0_0_10px_rgba(74,222,128,0.8)]') : 'bg-slate-400 dark:bg-slate-800'}`}
                        style={{
          ...
          <div className="mt-12 bg-slate-800 dark:bg-slate-800 border border-slate-700 dark:border-slate-500 px-6 py-4 rounded-xl text-center">
