  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-pink-50 font-sans">
      ...
      <div className="flex-1 p-8 flex flex-col items-center">
        <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-pink-100 max-w-2xl w-full text-center mb-8">
          <h2 className="text-2xl font-bold text-pink-800 mb-4">Crystallization</h2>
          <p className="text-slate-600 dark:text-slate-300 mb-6">A supersaturated solution of sugar and water is poured into jars. Seeded wooden sticks are dangled in. Over a week, as the water evaporates and cools, massive sugar crystals form.</p>
          ...
          <div className="mt-4 font-bold text-lg text-slate-700 dark:text-slate-200">Time Elapsed: {days} Days</div>
        </div>
        ...
        <div className="flex gap-16 justify-center mt-12 w-full flex-wrap">
          ...
          {[
            ...
            <div key={idx} className="relative w-32 h-64 flex justify-center">
               ...
               <div className="absolute top-4 w-2 h-56 bg-amber-200 z-10 flex flex-col justify-end items-center pb-4">
                  ...
                  >
                     <div className={`absolute inset-0 ${theme.bg} opacity-90 backdrop-blur-sm rounded ${theme.dropShadow} flex flex-wrap content-end overflow-hidden border border-white/20`} style={{ clipPath: 'polygon(20% 0, 80% 0, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0 80%, 0 20%)' }}>
                       ...
                       <div className="w-full h-1/2 bg-slate-50 dark:bg-slate-900/20 absolute top-0 -rotate-12 transform origin-top-left"></div>
               ...
               {/* Jar */}
               <div className="absolute bottom-0 w-32 h-40 border-4 border-slate-300 dark:border-slate-700 dark:border-slate-500 bg-slate-50 dark:bg-slate-900/10 backdrop-blur-sm rounded-b-xl rounded-t-sm z-20 flex justify-center items-end shadow-inner overflow-hidden">
        ...
        {days === 7 && (
          <div className="mt-16 p-6 bg-slate-50 dark:bg-slate-900 shadow-lg text-slate-800 dark:text-slate-100 rounded-xl border-t-4 border-pink-500 max-w-2xl text-center">
