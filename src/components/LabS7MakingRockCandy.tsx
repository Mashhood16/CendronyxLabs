...
export default function LabS7MakingRockCandy({ onExit }: LabProps) {
  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-pink-50 font-sans">
      <div className="bg-white border-b border-pink-200 p-4 flex items-center justify-between shrink-0">
        <button onClick={onExit} className="flex items-center text-slate-600 hover:text-pink-600 font-medium">
          <ArrowLeft className="w-5 h-5 mr-2" /> Back to Dashboard
        </button>
        <h1 className="text-xl font-bold text-slate-800">Unit 7: Making Rock Candy</h1>
      </div>

      <div className="flex-1 p-8 flex flex-col items-center">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-pink-100 max-w-2xl w-full text-center mb-8">
        ...
        <div className="flex gap-16 justify-center mt-12 w-full flex-wrap">
          ...
          {[
            ...
            <div key={idx} className="relative w-32 h-64 flex justify-center">
               ...
               <div className="absolute top-4 w-2 h-56 bg-amber-200 z-10 flex flex-col justify-end items-center pb-4">
                  ...
                  >
                     <div className={`absolute inset-0 ${theme.bg} opacity-90 backdrop-blur-sm rounded ${theme.dropShadow} flex flex-wrap content-end overflow-hidden border border-white/20`} style={{ clipPath: 'polygon(20% 0, 80% 0, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0 80%, 0 20%)' }}>
                       ...
                       <div className="w-full h-1/2 bg-white/20 absolute top-0 -rotate-12 transform origin-top-left"></div>
               ...
               {/* Jar */}
               <div className="absolute bottom-0 w-32 h-40 border-4 border-slate-300 bg-white/10 backdrop-blur-sm rounded-b-xl rounded-t-sm z-20 flex justify-center items-end shadow-inner overflow-hidden">
        ...
        {days === 7 && (
          <div className="mt-16 p-6 bg-white shadow-lg text-slate-800 rounded-xl border-t-4 border-pink-500 max-w-2xl text-center">
