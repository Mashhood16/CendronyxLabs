          ...
          <div className="grid grid-cols-2 gap-8">
            {materials.map(mat => (
              <div key={mat.id} className="bg-slate-700 dark:bg-slate-800 rounded-xl border border-slate-600 dark:border-slate-500 p-6 flex flex-col">
                ...
                <div className="h-24 bg-slate-900 dark:bg-slate-800 rounded-lg border border-slate-800 dark:border-slate-500 mb-6 flex items-center justify-center relative overflow-hidden">
                  {/* Wires */}
                  <div className="absolute top-1/2 left-0 w-1/3 h-1 bg-slate-500 dark:bg-slate-800"></div>
                  <div className="absolute top-1/2 right-0 w-1/3 h-1 bg-slate-500 dark:bg-slate-800"></div>
                  
                  {/* The Material in middle */}
                  <div className="px-4 py-2 bg-slate-600 dark:bg-slate-800 border border-slate-500 dark:border-slate-500 rounded text-xs font-bold text-slate-300 z-10 relative">
                    {mat.name}
                  </div>

                  {/* Bulb */}
                  <div className="absolute top-2 right-4">
                    <div className={`w-6 h-6 rounded-full transition-colors ${tested[mat.id] ? (mat.type === 'Conductor' ? 'bg-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.8)]' : 'bg-slate-600 dark:bg-slate-800') : 'bg-slate-600 dark:bg-slate-800'}`}></div>
                  </div>
                </div>
