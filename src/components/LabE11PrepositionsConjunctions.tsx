import { useState } from 'react';
import LabHeader from './LabHeader';
import { Check, BookOpen, Puzzle, AlertCircle, List } from 'lucide-react';
import { useTranslate } from "../i18n";
import { useLab } from '../store';

const PUZZLES = [
 {
 id: 1,
 topic: "Coordinating Conjunctions",
 text: "I was very tired, {gap} I went to bed early.",
 options: ["for", "and", "so", "but"],
 answer: "so",
 feedback: "'so' indicates a result or consequence."
 },
 {
 id: 2,
 topic: "Subordinating Conjunctions",
 text: "{gap} it was raining, we decided to stay indoors.",
 options: ["Although", "Because", "Unless", "Until"],
 answer: "Because",
 feedback: "'Because' introduces a cause or reason."
 },
 {
 id: 3,
 topic: "Prepositions of Place",
 text: "The keys are hidden {gap} the mat.",
 options: ["over", "under", "through", "into"],
 answer: "under",
 feedback: "'under' indicates position beneath something."
 },
 {
 id: 4,
 topic: "Correlative Conjunctions",
 text: "She is {gap} intelligent but also very kind.",
 options: ["neither", "either", "not only", "both"],
 answer: "not only",
 feedback: "'not only... but also' are used together to link two qualities."
 },
 {
 id: 5,
 topic: "Interjections",
 text: "{gap} ! I didn't see you there.",
 options: ["Alas", "Oh", "Well", "Bravo"],
 answer: "Oh",
 feedback: "'Oh' expresses surprise."
 },
 {
 id: 6,
 topic: "Phrasal Prepositions",
 text: "We stayed home {gap} the bad weather.",
 options: ["instead of", "because of", "in spite of", "according to"],
 answer: "because of",
 feedback: "'because of' shows the reason for staying home."
 }
];

export default function LabE11PrepositionsConjunctions({ onExit }: { onExit?: () => void }) {
    const { t } = useTranslate();
 const [activeMobileTab, setActiveMobileTab] = useState<'theory' | 'lab'>('theory');
 const [currentPuzzleIndex, setCurrentPuzzleIndex] = useState(0);
 const [selectedOption, setSelectedOption] = useState<string | null>(null);
 const [isCorrect, setIsCorrect] = useState<boolean | null>(null);

 const puzzle = PUZZLES[currentPuzzleIndex];

 const handleCheck = () => {
 if (selectedOption === puzzle.answer) {
  setIsCorrect(true);
 } else {
  setIsCorrect(false);
 }
 };

 const handleNext = () => {
 setSelectedOption(null);
 setIsCorrect(null);
 setCurrentPuzzleIndex((prev) => (prev + 1) % PUZZLES.length);
 };

 return (
 <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans text-slate-900 dark:text-[#ffffff] min-h-screen lg:h-screen overflow-x-hidden w-full">
  {/* Header */}
  <LabHeader onExit={onExit} title={t('lab.e11prepositionsconjunctions_prepositions_conjunctions')} />

  {/* Mobile Tab Navigation */}
  <div className="lg:hidden px-4 pt-4 md:px-6 flex gap-2 shrink-0">
   <button 
   onClick={() => setActiveMobileTab('theory')}
   className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'theory' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
  >
   
                    {t('lab.e11prepositionsconjunctions_theory')}
                   </button>
   <button 
   onClick={() => setActiveMobileTab('lab')}
   className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
  >{t('lab.e11prepositionsconjunctions_lab')}</button>
  </div>

  <main className="flex-grow p-4 md:p-6 flex flex-col lg:grid lg:grid-cols-3 gap-0 lg:gap-6 lg: lg:overflow-visible">
  {/* Window 1: Theory */}
  <section className={`w-full rounded-xl shadow-sm p-6 border border-slate-200 dark:border-[#1c1b1b] flex-col overflow- ${activeMobileTab === 'theory' ? 'flex' : 'hidden'} lg:flex`}>
   <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2 shrink-0">
   <BookOpen className="w-5 h-5 text-[#4158D1]" />
   
                        {t('lab.e11prepositionsconjunctions_grammar_theory')}
                        </h2>
   <div className="prose prose-sm text-slate-600 dark:text-[#a1a1aa] overflow-y-auto h-[500px] pr-2">
   <h3 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-2">{t('lab.e11prepositionsconjunctions_prepositions')}</h3>
   <p className="mb-4">
    
                             {t('lab.e11prepositionsconjunctions_a_preposition_is_a_word_or_gro')}
                            </p>
   <ul className="list-disc pl-5 mb-4 space-y-2">
    <li><strong>{t('lab.e11prepositionsconjunctions_time')}</strong>  {t('lab.e11prepositionsconjunctions_indicates_when_an_event_happen')} <em>at</em>  {t('lab.e11prepositionsconjunctions_5_pm')} <em>on</em>  {t('lab.e11prepositionsconjunctions_monday')} <em>in</em>  {t('lab.e11prepositionsconjunctions_may')} <em>{t('lab.e11prepositionsconjunctions_during')}</em>  {t('lab.e11prepositionsconjunctions_the_test')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_place')}</strong>  {t('lab.e11prepositionsconjunctions_indicates_where_something_is_l')} <em>in</em>  {t('lab.e11prepositionsconjunctions_the_box')} <em>on</em>  {t('lab.e11prepositionsconjunctions_the_table')} <em>at</em>  {t('lab.e11prepositionsconjunctions_the_door')} <em>{t('lab.e11prepositionsconjunctions_under')}</em>  {t('lab.e11prepositionsconjunctions_the_bed')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_direction')}</strong>  {t('lab.e11prepositionsconjunctions_shows_movement_towards_a_desti')} <em>to</em>  {t('lab.e11prepositionsconjunctions_school_walk')} <em>{t('lab.e11prepositionsconjunctions_into')}</em>  {t('lab.e11prepositionsconjunctions_the_room_look')} <em>{t('lab.e11prepositionsconjunctions_towards')}</em>  {t('lab.e11prepositionsconjunctions_the_sky')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_phrasal_prepositions')}</strong>  {t('lab.e11prepositionsconjunctions_two_or_more_words_acting_as_a_')} <em>{t('lab.e11prepositionsconjunctions_because_of')}</em>  {t('lab.e11prepositionsconjunctions_the_rain')} <em>{t('lab.e11prepositionsconjunctions_in_spite_of')}</em>  {t('lab.e11prepositionsconjunctions_the_delay')} <em>{t('lab.e11prepositionsconjunctions_according_to')}</em>  {t('lab.e11prepositionsconjunctions_the_rules')}</li>
   </ul>

   <h3 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-2 mt-6">{t('lab.e11prepositionsconjunctions_conjunctions')}</h3>
   <p className="mb-4">
    
                             {t('lab.e11prepositionsconjunctions_a_conjunction_connects_clauses')}
                            </p>
   <ul className="list-disc pl-5 mb-4 space-y-2">
    <li><strong>{t('lab.e11prepositionsconjunctions_coordinating_conjunctions')}</strong>  {t('lab.e11prepositionsconjunctions_join_parts_of_a_sentence_that_')}<br/>{t('lab.e11prepositionsconjunctions_example_i_wanted_to_go')} <strong>{t('lab.e11prepositionsconjunctions_but')}</strong>  {t('lab.e11prepositionsconjunctions_it_was_raining')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_subordinating_conjunctions')}</strong>  {t('lab.e11prepositionsconjunctions_join_a_dependent_clause_to_an_')}<br/>{t('lab.e11prepositionsconjunctions_example_we_stayed_inside')} <strong>{t('lab.e11prepositionsconjunctions_because')}</strong>  {t('lab.e11prepositionsconjunctions_it_was_cold')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_correlative_conjunctions')}</strong>  {t('lab.e11prepositionsconjunctions_work_in_pairs_to_join_words_or')}<br/>{t('lab.e11prepositionsconjunctions_example_she_is')} <strong>{t('lab.e11prepositionsconjunctions_not_only')}</strong>  {t('lab.e11prepositionsconjunctions_intelligent')} <strong>{t('lab.e11prepositionsconjunctions_but_also')}</strong>  {t('lab.e11prepositionsconjunctions_very_kind')}</li>
   </ul>

   <h3 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-2 mt-6">{t('lab.e11prepositionsconjunctions_interjections')}</h3>
   <p className="mb-4">
    
                             {t('lab.e11prepositionsconjunctions_an_interjection_is_a_word_or_p')}
                            </p>
   <ul className="list-disc pl-5 mb-4 space-y-2">
    <li><strong>{t('lab.e11prepositionsconjunctions_joy_surprise')}</strong>  {t('lab.e11prepositionsconjunctions_wow_oh_aha')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_pain')}</strong>  {t('lab.e11prepositionsconjunctions_ouch_ow')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_sorrow')}</strong>  {t('lab.e11prepositionsconjunctions_alas')}</li>
    <li><strong>{t('lab.e11prepositionsconjunctions_hesitation')}</strong>  {t('lab.e11prepositionsconjunctions_uh_well_hmm')}</li>
   </ul>
   </div>
  </section>

  {/* Window 2: Controls */}
  <section className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:bg-[#121212] lg:dark:bg-[#1c1b1b] rounded-xl shadow-sm p-6 border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#2a2a2a] flex-col '' : ''} rounded-t-none lg:rounded-t-xl border-t-0 lg:border-t ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2 shrink-0">
   <List className="w-5 h-5 text-[#4158D1]" />
   
                        {t('lab.e11prepositionsconjunctions_options')}
                        </h2>
   <div className="flex-grow overflow-y-auto flex flex-col gap-4">
   <div className="flex justify-between items-center mb-2">
    <div className="flex items-center gap-2 text-[#4158D1] font-bold uppercase tracking-wider text-sm">
    <Puzzle size={18} />
    {puzzle.topic}
    </div>
    <span className="text-xs font-medium px-2 py-1 bg-white dark:bg-[#2a2a2a] text-slate-600 dark:text-[#a1a1aa] rounded border border-slate-200 dark:border-[#333]">
    {currentPuzzleIndex + 1} / {PUZZLES.length}
    </span>
   </div>

   <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mb-2">{t('lab.e11prepositionsconjunctions_select_the_correct_word_to_com')}</p>

   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
    {puzzle.options.map(opt => (
    <button
     key={opt}
     disabled={isCorrect === true}
     onClick={() => {
     if (isCorrect !== true) {
      setSelectedOption(opt);
      setIsCorrect(null);
     }
     }}
     className={`px-4 py-3 rounded-lg font-semibold text-sm transition-all shadow-sm ${ selectedOption === opt ? 'bg-[#4158D1] text-white shadow-md ring-2 ring-[#4158D1]/30 dark:ring-indigo-900' : 'bg-white dark:bg-[#121212] dark:border-[#1c1b1b] text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-[#1c1b1b] hover:bg-slate-50 dark:bg-[#121212] dark:text-white dark:border-[#333] dark:hover:bg-[#2a2a2a]' } ${isCorrect === true ? 'opacity-50 cursor-not-allowed transform-none' : ''}`}
    >
     {opt}
    </button>
    ))}
   </div>

   <div className="mt-auto flex flex-col gap-4">
    {!isCorrect && isCorrect !== false && (
    <button 
     onClick={handleCheck}
     disabled={!selectedOption}
     className={`w-full py-3 rounded-lg font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 ${ selectedOption ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-200 text-slate-400 cursor-not-allowed dark:bg-[#2a2a2a] dark:text-[#71717a]' }`}
    >
     <Check size={18} />  {t('lab.e11prepositionsconjunctions_check_answer')}
                                     </button>
    )}
    
    {isCorrect === true && (
    <div className={`flex flex-col items-center bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-lg border border-emerald-200 dark:border-emerald-800/50 `}>
     <div className="text-emerald-600 dark:text-emerald-400 font-bold text-lg flex items-center gap-2 mb-2">
     <Check size={20} />  {t('lab.e11prepositionsconjunctions_correct')}
                                          </div>
     <p className="text-sm text-slate-600 dark:text-[#a1a1aa] text-center mb-4">{puzzle.feedback}</p>
     <button 
     onClick={handleNext}
     className={`w-full py-3 bg-[#4158D1] hover:bg-indigo-600 text-white rounded-lg font-bold shadow-md transition-all flex-col `}
     >
     
                                          {t('lab.e11prepositionsconjunctions_next_puzzle')}
                                          </button>
    </div>
    )}
    
    {isCorrect === false && (
    <div className={`flex flex-col items-center bg-rose-50 dark:bg-rose-900/20 p-4 rounded-lg border border-rose-200 dark:border-rose-800/50 `}>
     <div className="text-rose-600 dark:text-rose-400 font-bold text-sm flex items-center gap-2">
     <AlertCircle size={18} />  {t('lab.e11prepositionsconjunctions_incorrect_try_another_option')}
                                          </div>
    </div>
    )}
   </div>
   </div>
  </section>

  {/* Window 3: Simulation */}
  <section className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-100 dark:bg-[#121212] lg:dark:bg-[#0a0a0a] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] relative items-center justify-center p-8 lg:min-h-[35vh] lg:min-h-[500px] flex-col '' : ''} ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   <h2 className="absolute top-6 left-6 text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
   <Puzzle className="w-5 h-5 text-[#4158D1]" />
   
                        {t('lab.e11prepositionsconjunctions_sentence_construction')}
                        </h2>
   <div className="w-full max-w-2xl px-6">
   <div className="text-2xl md:text-3xl font-medium leading-relaxed text-slate-800 dark:text-[#ffffff] py-12 px-8 rounded-2xl shadow-sm border border-slate-200 dark:border-[#333] text-center">
    {puzzle.text.split('{gap}').map((part, idx) => (
    <span key={idx}>
     {part}
     {idx === 0 && (
     <span 
      className={`inline-flex items-center justify-center min-w-[120px] mx-3 px-4 py-2 border-b-4 rounded-t-sm transition-all duration-300 ${ selectedOption ? isCorrect === true ? 'bg-emerald-100 border-emerald-500 text-emerald-800 dark:bg-emerald-900/40 dark:border-emerald-500 dark:text-emerald-300' : isCorrect === false ? 'bg-rose-100 border-rose-500 text-rose-800 dark:bg-rose-900/40 dark:border-rose-500 dark:text-rose-300' : 'bg-indigo-50 border-[#4158D1] text-[#4158D1] dark:bg-[#4158D1]/20 dark:border-[#4158D1] dark:text-indigo-300' : 'bg-slate-50 dark:bg-[#000000] border-slate-300 text-slate-400 dark:bg-[#1c1b1b] dark:border-[#444]' }`}
     >
      {selectedOption || "___"}
     </span>
     )}
    </span>
    ))}
   </div>
   </div>
  </section>
  </main>
 </div>
 );
}
