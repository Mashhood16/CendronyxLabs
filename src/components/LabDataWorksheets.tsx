      ...
      <div className="flex-1 flex lg:overflow-hidden">
                 ...
                 <tbody>
                    {currentData.map((row, i) => (
                      <tr key={i} className="border-b border-slate-200 dark:border-[#1c1b1b]">
                         <td className="p-0 border-r border-slate-200 dark:border-[#1c1b1b]">
                            <input 
                              type="text" 
                              value={row.label} 
        ...
        <div className="flex-1 bg-slate-50 dark:bg-[#121212] border-l border-slate-300 dark:border-[#1c1b1b] p-8 flex flex-col justify-center items-center relative shadow-inner bg-[url('https://www.transparenttextures.com/patterns/graphy.png')]">
           ...
           <div className="w-full max-w-md h-80 bg-slate-50 dark:!bg-[#121212] border border-slate-300 dark:border-[#1c1b1b] shadow-xl rounded-xl p-8 flex items-end justify-between relative">
              ...
              {scenario === 'heights' && (
                 <div className="w-full h-full flex items-end justify-between border-l-2 border-b-2 border-slate-400 dark:border-[#1c1b1b] pb-0 px-4 pt-4 gap-4">
                    {currentData.map((d, i) => (
                       <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group">
                          <div 
                            className="w-full bg-blue-500 rounded-t transition-all duration-500 group-hover:bg-blue-400 shadow-inner /20 dark:border-teal-900 dark:bg-blue-500 dark:hover:bg-blue-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-blue-500/40"
                            style={{ height: `${(d.val / maxVal) * 100}%` }}
                          ></div>
                          <span className="text-xs font-bold text-slate-500 dark:text-[#71717a] mt-2 truncate w-full text-center">{d.label}</span>
                       </div>
                    ))}
                 </div>
              )}
              ...
              {scenario === 'bills' && (
                 <div className="w-full h-full border-l-2 border-b-2 border-slate-400 dark:border-[#1c1b1b] relative">
                    ...
                    <div className="absolute -bottom-8 w-full flex justify-between">
                       {currentData.map((d, i) => (
                          <span key={i} className="text-xs font-bold text-slate-500 dark:text-[#71717a] truncate w-12 text-center" style={{ marginLeft: i === 0 ? '-24px' : i === currentData.length-1 ? '0' : '-12px' }}>{d.label}</span>
                       ))}
                    </div>
                 </div>
              )}
                       ...
                       {currentData.map((d, i) => {
                         ...
                         return (
                           <div key={i} className="flex items-center gap-2">
                             ...
                             <span className="text-xs font-bold text-slate-600 dark:text-[#a1a1aa]">{d.label}</span>
                           </div>
                         );
                       })}
                    </div>
