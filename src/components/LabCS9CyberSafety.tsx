        ...
        <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl shadow-sm flex flex-col gap-4 border border-slate-100">
                    ...
                    <div className="text-sm text-slate-600 dark:text-slate-300 space-y-4">
                        ...
                        <div className="space-y-2">
                            <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2"><ShieldAlert className="w-4 h-4 text-red-500"/> Phishing</h3>
                        ...
                        <div className="space-y-2">
                            <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2"><MessageSquare className="w-4 h-4 text-orange-500"/> Cyberbullying</h3>
                        ...
                        <div className="space-y-2">
                            <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-yellow-500"/> Fake News</h3>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl shadow-sm flex flex-col items-center border border-slate-100">
                    ...
                    <div className="w-full max-w-xs bg-slate-900 dark:bg-slate-800 rounded-[2.5rem] p-3 shadow-2xl border-4 border-slate-800 dark:border-slate-500 relative mt-4 h-[500px] flex flex-col">
                        <div className="absolute top-0 inset-x-0 h-6 flex justify-center z-10">
                            <div className="w-32 h-6 bg-slate-800 dark:bg-slate-800 rounded-b-xl"></div>
                        </div>
                        
                        <div className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-3xl overflow-hidden flex flex-col relative">
                            {/* Screen Header */}
                            ...
                            <div className="flex-1 p-4 flex flex-col justify-end">
                                {currentScenario !== -1 ? (
                                    <div className="animate-in slide-in-from-bottom-4 fade-in">
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-semibold">{scenarios[currentScenario].sender}</p>
                                        <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-2xl rounded-tl-none shadow-sm text-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 mb-4">
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full text-center p-4">
                                        ...
                                        <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">Simulation Complete</h3>
                                        <p className="text-sm text-slate-600 dark:text-slate-300 mt-2">Check the Analysis panel for your results.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl shadow-sm flex flex-col h-full border border-slate-100">
                    ...
                    <div className="mb-6">
                        <div className="flex justify-between items-end mb-2">
                            <span className="font-semibold text-sm text-slate-700 dark:text-slate-200">Overall Safety Score</span>
                    ...
                    <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-500 mb-6 shadow-inner space-y-3">
                        <h3 className="font-semibold text-sm text-slate-700 dark:text-slate-200 border-b pb-1 border-slate-200 dark:border-slate-700 dark:border-slate-500">Action History</h3>
                        {chatLog.length === 0 ? (
                            <p className="text-xs text-slate-500 dark:text-slate-400 italic text-center mt-4">Make choices on the device to log data.</p>
                        ) : (
                            chatLog.map((log, i) => (
                                <div key={i} className={`p-3 rounded border text-xs shadow-sm ${log.safe ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                                    ...
                                    <p className="text-slate-600 dark:text-slate-300 mb-2">You chose: "{log.userAction}"</p>
                    ...
                    <div className="bg-teal-50 p-5 rounded-lg border border-teal-200 shadow-sm">
                        ...
                        <button 
                            ...
                            className={`w-full py-2 rounded-md transition-colors text-sm font-semibold shadow-sm ${currentScenario === -1 ? 'bg-teal-600 text-white hover:bg-teal-700' : 'bg-slate-300 dark:bg-slate-800 text-slate-500 dark:text-slate-400 cursor-not-allowed'}`}
                        >
                            Generate Report
                        </button>
                        
                        {assessmentScore !== null && (
                            <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-900 rounded border border-teal-100 shadow-sm">
                                <p className="text-sm font-bold text-slate-800 dark:text-slate-100">Score: {assessmentScore}/100</p>
                                <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">{finalFeedback}</p>
                            </div>
                        )}
                    </div>
                </div>
