  ...
  return (
    <div className="overflow-y-auto flex flex-col h-screen bg-slate-50 dark:bg-slate-900 font-sans select-none">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 p-8 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col items-center w-full max-w-lg">
          
          <div className="relative w-48 h-48 mb-8">
            {/* Beaker */}
            <div className="absolute inset-0 bg-slate-100 dark:bg-slate-800/50 rounded-b-[40px] border-4 border-t-0 border-slate-300 dark:border-slate-700 dark:border-slate-500 flex flex-col justify-end overflow-hidden z-10">
               {/* Milk / Liquid */}
               <div className={`w-full transition-all duration-1000 flex items-center justify-center ${isSeparated ? 'h-3/4 bg-yellow-100/80 backdrop-blur-sm' : 'h-3/4 shadow-[inset_0_-10px_20px_rgba(0,0,0,0.05)]'}`} style={!isSeparated ? {backgroundColor: '#f8fafc'} : undefined}>
                 ...
                 {/* Curds (Casein) */}
                 {isSeparated && (
                   <div className="w-24 h-16 bg-slate-50 dark:bg-slate-900 border border-slate-100 rounded-3xl animate-bounce shadow-md flex items-center justify-center flex-wrap gap-1 p-2">
                     <div className="w-4 h-4 bg-slate-50 dark:bg-slate-900 rounded-full" />
                     <div className="w-6 h-5 bg-slate-50 dark:bg-slate-900 rounded-full" />
                     <div className="w-5 h-5 bg-slate-50 dark:bg-slate-900 rounded-full" />
                     <div className="w-7 h-6 bg-slate-50 dark:bg-slate-900 rounded-full" />
                   </div>
                 )}
               </div>
            </div>
          ...
          <div className="flex flex-col items-center gap-4 w-full">
            <button 
              ...
              className={`w-full py-4 rounded-xl font-bold text-lg shadow-md transition-transform active:scale-95 ${isSeparated ? 'bg-slate-200 dark:bg-slate-800 text-slate-400' : 'bg-amber-100 hover:bg-amber-200 text-amber-800 border border-amber-300'}`}
            >
