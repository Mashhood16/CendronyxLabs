...
export default function LabE8AdjectivesPoetry({ onExit }: { onExit?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900 dark:bg-slate-900 font-sans select-none text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">
      <header className="flex items-center p-4 bg-white dark:bg-slate-800 dark:bg-slate-800 shadow-sm z-10 border-b border-slate-200 dark:border-slate-700 dark:border-slate-700">
             ...
             <div className="p-4">
               {activeTab === 'poetry' ? (
                 <div className="space-y-6">
                    <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-700">
                       <button onClick={() => setPoemIndex(Math.max(0, poemIndex - 1))} disabled={poemIndex === 0} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Prev Poem</button>
                       <span className="font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">Poem {poemIndex + 1} of {poetryData.length}</span>
                       <button onClick={() => setPoemIndex(Math.min(poetryData.length - 1, poemIndex + 1))} disabled={poemIndex === poetryData.length - 1} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Next Poem</button>
                    </div>

                    <div className="space-y-4">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 flex items-center gap-2">
                        ...
                        {currentPoem.questions.map((q, i) => {
                            ...
                            return (
                                <div key={i} className="bg-white dark:bg-slate-800 dark:bg-slate-700/50 p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-600">
                                    <p className="font-medium text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-3">{i+1}. {q.q}</p>
                                    <div className="grid grid-cols-2 gap-2">
                                        {q.options.map(opt => (
                                            <button
                                                ...
                                                disabled={isAnswered}
                                                className={`whitespace-nowrap flex-shrink-0 py-2 px-3 rounded-md border text-sm font-medium transition-colors ${
                                                    poetryAnswers[poemIndex]?.[i] === opt
                                                        ? isCorrect ? 'bg-green-100 dark:bg-green-900/50 dark:bg-green-900/60 border-green-500 text-green-700 dark:text-green-300 dark:bg-green-900/30 dark:border-green-500 dark:text-green-400' 
                                                                  : 'bg-red-100 dark:bg-red-900/50 dark:bg-red-900/60 border-red-500 text-red-700 dark:text-red-300 dark:bg-red-900/30 dark:border-red-500 dark:text-red-400'
                                                        : 'bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 dark:text-slate-300 hover:bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700'
                                                }`}
                                            >
                                                {opt}
                                            </button>
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-700">
                       <button onClick={() => setAdjIndex(Math.max(0, adjIndex - 1))} disabled={adjIndex === 0} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Prev</button>
                       <span className="font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">Exercise {adjIndex + 1} of {adjectiveData.length}</span>
                       <button onClick={() => setAdjIndex(Math.min(adjectiveData.length - 1, adjIndex + 1))} disabled={adjIndex === adjectiveData.length - 1} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Next</button>
                    </div>

                    <div className="bg-white dark:bg-slate-800 dark:bg-slate-700/50 p-5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-600">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-4 flex items-center gap-2">
                           <Type size={20} className="text-purple-500 dark:text-purple-400" /> Adjectives & Phrases
                        </h3>
                        <p className="text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 text-lg mb-6 italic bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900 dark:bg-slate-800 p-3 rounded border border-slate-200 dark:border-slate-700 dark:border-slate-600">"{currentAdj.sentence}"</p>
                        
                        {currentAdj.type === 'fill' ? (
                            <div>
                                <p className="mb-3 text-slate-600 dark:text-slate-400 dark:text-slate-300 font-medium">Upgrade the adjective <span className="font-bold text-blue-600 dark:text-blue-400 dark:text-blue-400">({currentAdj.base})</span> to the correct degree:</p>
                                <div className="flex gap-2 items-center">
                                    <input
                                        type="text"
                                        value={adjAnswers[adjIndex] || ''}
                                        onChange={(e) => handleAdjAnswer(e.target.value)}
                                        className="flex-1 min-w-0 p-2 border border-slate-300 dark:border-slate-600 dark:border-slate-600 rounded bg-white dark:bg-slate-800 dark:bg-slate-800 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"
                            ...
                            <div>
                                <p className="mb-4 text-slate-600 dark:text-slate-400 dark:text-slate-300 font-medium">{currentAdj.question}</p>
                                <div className="space-y-2">
                                    {currentAdj.options?.map(opt => {
                                        ...
                                        let btnClass = "bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 dark:text-slate-300 hover:bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700";
                                        if (showResult) {
         ...
         <div className="w-full lg:w-2/3 flex flex-col bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900 p-8 overflow-y-auto items-center justify-center relative">
            {activeTab === 'poetry' ? (
                <div className="relative w-full max-w-lg bg-amber-50 dark:bg-amber-900/50 dark:bg-amber-900/10 border-2 border-amber-200 dark:border-amber-700/50 p-10 rounded-xl shadow-xl font-serif">
                    ...
                    <h2 className="text-3xl font-bold text-center text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-amber-500 mb-8 border-b border-amber-200 dark:border-amber-700/50 pb-4">
                        {currentPoem.title}
                    </h2>
                    
                    <div className="whitespace-pre-wrap text-xl leading-loose text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 italic text-center">
                        {currentPoem.text}
                    </div>
                ...
                <div className="flex flex-col items-center justify-center w-full max-w-md bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-2xl shadow-lg p-10 border border-slate-200 dark:border-slate-700 dark:border-slate-700">
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-10 flex items-center gap-3">
                       <Sparkles className="text-purple-500 dark:text-purple-400" />
                       Descriptive Power Meter
                    </h2>
                    
                    <div className="relative w-32 h-64 bg-slate-200 dark:bg-slate-900/50 dark:bg-slate-700/50 rounded-full overflow-hidden border-4 border-slate-300 dark:border-slate-600 dark:border-slate-600 shadow-inner">
                        <div 
                        ...
                        />
                        <div className="absolute inset-0 flex flex-col items-center justify-between py-4 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 drop-shadow-[0_2px_2px_rgba(255,255,255,0.8)] dark:drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] text-sm font-bold z-10">
                            <span>MAX</span>
                            <span>MID</span>
                            <span>MIN</span>
                        </div>
                    ...
                    <div className="mt-8 text-center">
                        <p className="text-xl font-medium text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-2">
