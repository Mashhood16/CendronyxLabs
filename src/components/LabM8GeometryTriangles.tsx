  ...
  const renderControls = () => {
    switch (mode) {
      case 'similar-triangles':
        return (
          <div className="space-y-4">
            <h3 className="text-xl font-semibold dark:text-slate-100">Navigational Distance</h3>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium dark:text-slate-200">Person's Shadow ($s_1$): {personShadow} m</label>
              <input 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium dark:text-slate-200">Tower's Shadow ($s_2$): {towerShadow} m</label>
              <input 
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-600 space-y-3">
              <label className="text-sm font-medium dark:text-slate-200">Calculate Tower Height ($h_2$):</label>
              <div className="flex flex-wrap gap-2">
                ...
                />
                <span className="self-center font-medium text-slate-600 dark:text-slate-400">meters</span>
              </div>
            </div>
          </div>
        );
      case 'compass-constructions':
        return (
          <div className="space-y-4">
            <h3 className="text-xl font-semibold dark:text-slate-100">Compass Constructions</h3>
            <p className="text-slate-700 dark:text-slate-300 text-sm">
            ...
            <div className="flex flex-col gap-2 mt-4">
              <label className="text-sm font-medium dark:text-slate-200">Compass Radius: {compassRadius} units</label>
              <input 
            ...
            <div className="flex flex-col gap-3 pt-4">
              <button
                ...
                className="w-full py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 font-medium rounded-lg transition-colors whitespace-nowrap flex-shrink-0"
              >
  ...
  const renderSimulation = () => {
    switch (mode) {
      case 'similar-triangles':
        ...
        return (
          <svg viewBox="0 0 200 100" className="w-full h-full max-h-[60vh] bg-sky-50 dark:bg-slate-800 rounded-xl">
            {/* Ground */}
            ...
            {/* Labels */}
            <text x={px - 5} y={groundY - p_height_px - 5} fontSize="6" fill="currentColor" className="dark:text-slate-300">h1={personHeight}m</text>
            <text x={px - p_shadow_px/2 - 5} y={groundY + 8} fontSize="6" fill="currentColor" className="dark:text-slate-300">s1={personShadow}m</text>
            
            <text x={tx - 8} y={groundY - t_height_px - 5} fontSize="6" fill="currentColor" className="dark:text-slate-300">h2=?</text>
            <text x={tx - t_shadow_px/2 - 5} y={groundY + 8} fontSize="6" fill="currentColor" className="dark:text-slate-300">s2={towerShadow}m</text>
      ...
      case 'compass-constructions':
        ...
        return (
          <svg viewBox="-100 -100 200 200" className="w-full h-full max-h-[60vh] bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
            <defs>
            ...
            <text x={ptA.x - 10} y={ptA.y - 5} fontSize="8" fill="currentColor" className="dark:text-slate-200 font-bold">A</text>
            
            <circle cx={ptB.x} cy={ptB.y} r="2" fill="#1e293b" className="dark:fill-slate-100" />
            <text x={ptB.x + 5} y={ptB.y - 5} fontSize="8" fill="currentColor" className="dark:text-slate-200 font-bold">B</text>
  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none text-slate-900 dark:text-slate-100">
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <div className="w-full lg:w-1/3 p-6 bg-white dark:bg-slate-800 shadow-lg lg:overflow-y-auto flex flex-col gap-6 border-r border-slate-200 dark:border-slate-700">
          <div className="space-y-4">
            <h2 className="text-lg font-semibold dark:text-slate-200">Select Lab</h2>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => setMode('similar-triangles')}
                className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-md font-medium whitespace-nowrap flex-shrink-0 transition-colors ${
        ...
        <div className="w-full lg:w-2/3 p-6 bg-slate-100 dark:bg-slate-900 flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="w-full max-w-3xl aspect-video bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-2 flex items-center justify-center overflow-hidden">
            {renderSimulation()}
          </div>
        </div>
      </div>
