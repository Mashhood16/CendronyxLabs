    ...
    <div className="flex h-screen font-sans bg-slate-50 text-slate-800">
      <div className="flex-1 p-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex gap-8 max-w-5xl mx-auto w-full">
          {/* Palette */}
          <div className="w-64 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="flex flex-col gap-4">
              {nodes.filter(n => n.placedAt === null).map(node => (
                <div 
                  ...
                  className={`
                    p-4 bg-blue-50 border-2 border-blue-400 text-blue-900 font-medium text-center shadow-sm cursor-grab active:cursor-grabbing hover:-translate-y-1 transition-transform
                    ${node.shape === 'pill' ? 'rounded-full' : ''}
                    ${node.shape === 'parallelogram' ? 'skew-x-[-15deg]' : ''}
                    ${node.shape === 'diamond' ? 'rotate-45 p-6 w-24 h-24 mx-auto flex items-center justify-center' : ''}
                  `}
                >
          ...
          {/* Canvas */}
          <div className="flex-1 bg-white p-8 rounded-xl shadow-inner border-2 border-slate-200 flex flex-col items-center">
    ...
    >
      {node && (
        <div 
          ...
          className={`
            p-4 bg-emerald-50 border-2 border-emerald-500 text-emerald-900 font-bold text-center shadow-lg cursor-pointer hover:bg-rose-50 hover:border-rose-500 hover:text-rose-900 absolute z-10 w-48
            ${node.shape === 'pill' ? 'rounded-full' : ''}
            ${node.shape === 'parallelogram' ? 'skew-x-[-15deg]' : ''}
            ${node.shape === 'diamond' ? 'rotate-45 p-6 w-28 h-28 mx-auto flex items-center justify-center' : ''}
          `}
        >
          <div className={`${node.shape === 'diamond' ? '-rotate-45 w-[140%] text-xs' : ''} ${node.shape === 'parallelogram' ? 'skew-x-[15deg]' : ''}`}>
            {node.label}
