        ...
        <div className="flex gap-8 flex-1">
          ...
          <div className="w-80 flex flex-col gap-6">
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center text-center gap-2 p-3 rounded-lg border-2 text-xs font-bold transition-colors h-24 ${
                        topic === t.id ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'
                      }`}
                    >
              ...
              <div className="flex gap-2">
                {Object.keys(colorClasses).map(color => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-4 transition-transform ${colorScheme === color ? 'border-slate-800 scale-110' : 'border-transparent hover:scale-110'}`}
            ...
            <button 
              ...
              className="mt-auto py-4 bg-slate-800 hover:bg-slate-900 text-white font-bold rounded-xl shadow-sm disabled:opacity-50 transition-colors"
            >
              Submit Poster Entry
            </button>
          </div>
          ...
          {/* Canvas Area */}
          <div className="flex-1 bg-slate-200 rounded-xl shadow-inner border border-slate-300 p-8 flex items-center justify-center relative overflow-hidden">
            {/* The Poster */}
            ...
            {isFinished && (
              <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center p-12 text-center animate-in fade-in">
                ...
                <button 
                  ...
                  className="mt-8 px-6 py-3 bg-slate-200 hover:bg-slate-300 rounded-lg font-bold text-slate-700"
                >
                  Edit Poster
                </button>
              </div>
