  ...
  return (
    <div className="lg:overflow-y-auto flex flex-col h-screen font-sans bg-slate-50 dark:!bg-[#000000] text-slate-800 dark:text-white">
      ...
      <div className="flex-1 flex p-6 gap-6">
            ...
            <div>
              ...
              <div className="flex bg-slate-200 dark:bg-[#121212] rounded-lg p-1 border border-slate-300 dark:border-[#1c1b1b]">
                <button 
                  ...
                  className={`flex-1 py-2 text-sm font-bold rounded-md ${circuitType === 'series' ? 'bg-white dark:bg-[#121212] text-slate-900 dark:text-white shadow' : 'text-slate-500 dark:text-[#a1a1aa] hover:text-slate-700 dark:hover:text-slate-300'}`}
                >Series</button>
                <button 
                  onClick={() => { setCircuitType('parallel'); setSwitchClosed(false); }}
                  className={`flex-1 py-2 text-sm font-bold rounded-md ${circuitType === 'parallel' ? 'bg-white dark:bg-[#121212] text-slate-900 dark:text-white shadow' : 'text-slate-500 dark:text-[#a1a1aa] hover:text-slate-700 dark:hover:text-slate-300'}`}
                >Parallel</button>
              </div>
            </div>
        ...
        {/* Interactive Canvas */}
        <div className="flex-1 rounded-2xl border border-slate-300 dark:border-[#1c1b1b] p-8 flex items-center justify-center relative overflow-hidden bg-[#000000] dark:bg-slate-950">
          ...
          <div className="relative w-full max-w-2xl h-96 flex flex-col items-center justify-between p-12">
                ...
                <div key={i} className={`flex flex-col items-center bg-slate-950 p-2 ${circuitType === 'parallel' && i > 0 ? (i === 1 ? 'absolute left-1/3 -translate-x-1/2' : 'absolute left-2/3 -translate-x-1/2') : ''}`}>
                  <div className="relative">
                    ...
                    {switchClosed && brightness > 0 && (
                      <div 
                        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-yellow-400 blur-xl transition-all duration-300 dark:bg-yellow-500 dark:hover:bg-yellow-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-yellow-500/40"
                        style={{ width: `${Math.min(brightness * 60, 150)}px`, height: `${Math.min(brightness * 60, 150)}px`, opacity: Math.min(brightness * 0.5, 1) }}
                      ></div>
                    )}
                  </div>
                  <div className="w-6 h-4 bg-slate-600 dark:bg-[#121212] mt-1 rounded"></div>
                </div>
              ))}
            </div>
            ...
            <div className="relative z-10 w-3/5 flex justify-between items-center -mb-8">
               ...
               <div className="bg-slate-950 px-2 flex items-center justify-center w-24">
                 <div className="w-4 h-4 rounded-full bg-slate-600 dark:bg-[#121212]"></div>
                 <div className={`w-16 h-2 bg-slate-400 dark:bg-[#121212] origin-left transition-transform duration-300 ${switchClosed ? 'rotate-0' : '-rotate-30'}`}></div>
                 <div className="w-4 h-4 rounded-full bg-slate-600 dark:bg-[#121212]"></div>
               </div>
            </div>
