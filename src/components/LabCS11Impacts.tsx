      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 overflow-hidden min-h-0">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg shrink-0">
            <button 
              className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'bias' ? 'bg-slate-50 dark:bg-slate-900 shadow text-blue-600' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:bg-slate-800'}`}
              onClick={() => { setActiveTab('bias'); setFeedback(null); setAns(''); }}
            >
              Data Bias Evaluator
            </button>
            <button 
              className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'a11y' ? 'bg-slate-50 dark:bg-slate-900 shadow text-purple-600' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:bg-slate-800'}`}
          ...
          <div className="text-slate-600 dark:text-slate-300 space-y-4 text-sm mt-2">
            {activeTab === 'bias' ? (
              ...
              <>
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-md mt-4">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col overflow-y-auto">
          ...
          {activeTab === 'bias' && (
            <div className="flex-1 flex flex-col gap-4 min-h-[400px]">
              <div className="flex gap-4 p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border shrink-0">
              ...
              <div className="flex-1 border rounded-lg bg-slate-50 dark:bg-slate-900 relative overflow-hidden flex items-center justify-center">
              ...
              <div className="grid grid-cols-2 gap-4 text-center shrink-0">
                <div className="p-3 bg-slate-50 dark:bg-slate-900 border rounded-lg shadow-sm">
                  <div className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-1">Pop. Mean Connectivity</div>
                  <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">{popMean.toFixed(1)}</div>
                </div>
          ...
          {activeTab === 'a11y' && (
            <div className="flex-1 flex flex-col relative border rounded-lg bg-slate-100 dark:bg-slate-800 overflow-hidden min-h-[400px]">
              <div className="p-4 border-b bg-slate-50 dark:bg-slate-900 flex justify-between items-center z-10 relative shadow-sm">
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold transition-all ${srMode ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:bg-slate-800'}`}
                >
              ...
              <div className={`flex-1 p-8 bg-slate-50 dark:bg-slate-900 transition-all duration-500 ${srMode ? 'blur-[4px] grayscale opacity-40' : ''}`}>
                <h1 className="text-3xl font-bold mb-4 text-slate-800 dark:text-slate-100">Travel Blog</h1>
                <p className="text-slate-600 dark:text-slate-300 mb-6">Check out my recent trip to Paris!</p>
                <div className="w-full h-40 bg-slate-200 dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-700 dark:border-slate-500 flex items-center justify-center text-slate-500 dark:text-slate-400 mb-6 font-mono text-sm">
              ...
              {srMode && (
                <div className="absolute bottom-4 left-4 right-4 bg-slate-900 dark:bg-slate-800/95 text-green-400 font-mono text-sm p-4 rounded-xl shadow-2xl border border-slate-700 dark:border-slate-500 animate-in slide-in-from-bottom-4">
                  <div className="flex items-center gap-2 mb-3 text-white font-sans font-bold border-b border-slate-700 dark:border-slate-500 pb-2">
            ...
            {activeTab === 'bias' ? (
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <p className="text-sm text-blue-900 mb-4 leading-relaxed">
                  ...
                  <code className="bg-slate-50 dark:bg-slate-900 px-2 py-1 rounded border">Bias = Sample Mean - Population Mean</code>
                </p>
