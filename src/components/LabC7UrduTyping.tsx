      ...
      <div className="flex-1 p-8 flex flex-col overflow-y-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col flex-1 overflow-hidden">
          ...
          <div className="border-b border-slate-200 bg-slate-100 p-2 flex gap-2 items-center">
            ...
            <span className="text-sm text-slate-500 ml-4 font-medium italic">
          ...
          {/* Text Area */}
          <textarea
            className={`flex-1 p-8 text-2xl resize-none outline-none ${urduMode ? 'text-right font-urdu' : 'text-left font-sans'}`}
      ...
      <div className="w-80 bg-white p-6 border-l border-slate-200 shadow-[-10px_0_20px_rgba(0,0,0,0.05)] z-10 flex flex-col overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          {Object.entries(urduMap).map(([eng, urd]) => {
            if (eng === ' ' || eng === '\n') return null;
            return (
              <div key={eng} className="flex justify-between items-center bg-slate-50 p-2 rounded border border-slate-100">
                <span className="font-mono font-bold text-slate-500">{eng.toUpperCase()}</span>
                <span className="text-xl font-bold text-slate-800">{urd}</span>
              </div>
            )
          })}
        </div>
