    ...
    return (
        <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none">
            ...
            <main className="flex-1 p-6 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Left Column: Theory */}
                <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col h-full overflow-y-auto">
                    ...
                    {activeTab === 'mass-energy' && (
                        <div className="space-y-4 text-slate-600 dark:text-slate-300 leading-relaxed">
                            ...
                            <span className="text-xl font-semibold text-center block text-slate-800 dark:text-slate-100">E = Δm c²</span>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg text-sm font-mono border border-slate-200 dark:border-slate-700 dark:border-slate-500 space-y-1 mt-4">
                    ...
                    {activeTab === 'pet-scan' && (
                        <div className="space-y-4 text-slate-600 dark:text-slate-300 leading-relaxed">
                            ...
                            <span className="text-xl font-semibold text-center block text-slate-800 dark:text-slate-100 mt-2 mb-2">E = 2m<sub>e</sub>c²</span>
                            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg text-sm font-mono border border-slate-200 dark:border-slate-700 dark:border-slate-500 space-y-1 mt-4">
                                <p className="text-indigo-700 font-bold mb-2">Reference Data:</p>
                    ...
                    {activeTab === 'synchrotron' && (
                        <div className="space-y-4 text-slate-600 dark:text-slate-300 leading-relaxed">
                            ...
                            <span className="text-xl font-semibold text-center block text-slate-800 dark:text-slate-100">p = q B r</span>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg text-sm font-mono border border-slate-200 dark:border-slate-700 dark:border-slate-500 mt-4">
                                <p className="text-indigo-700 font-bold mb-2">Reference Data:</p>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col h-full">
                    <div className="flex gap-2 mb-6 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                        <button onClick={() => handleTabChange('mass-energy')} className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'mass-energy' ? 'bg-slate-50 dark:bg-slate-900 shadow-sm text-indigo-700' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}>Fission</button>
                        <button onClick={() => handleTabChange('pet-scan')} className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'pet-scan' ? 'bg-slate-50 dark:bg-slate-900 shadow-sm text-indigo-700' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}>PET Scan</button>
                        <button onClick={() => handleTabChange('synchrotron')} className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'synchrotron' ? 'bg-slate-50 dark:bg-slate-900 shadow-sm text-indigo-700' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}>Synchrotron</button>
                    </div>
                    
                    <div className="flex-1 flex flex-col justify-center items-center">
                        <svg width="100%" height="300" viewBox="0 0 300 300" className="bg-slate-900 dark:bg-slate-800 rounded-xl overflow-hidden shadow-inner">
                            <SvgGrid />
                            ...
                            {activeTab === 'mass-energy' && (
                                <div className="flex justify-center gap-4">
                                    ...
                                    <button 
                                        ...
                                        className="flex items-center gap-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:bg-slate-800 text-slate-800 dark:text-slate-100 px-6 py-2 rounded-full font-medium transition-colors"
                                    >
                ...
                <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col h-full overflow-y-auto">
                    ...
                    {activeTab === 'mass-energy' && (
                        <div className="space-y-5">
                            <p className="text-sm text-slate-600 dark:text-slate-300">Simulate a U-235 fission event. Use the reference data to calculate the missing mass, then convert it to energy.</p>
                            
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Calculate Mass Defect (Δm)</label>
                                <div className="flex gap-2 items-center">
                                    <input 
                                        type="number"
                                        value={fissionDeltaM}
                                        onChange={e => { setFissionDeltaM(e.target.value); setFissionCorrect(null); }}
                                        className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                        placeholder="e.g. 0.2148"
                                    />
                                    <span className="text-slate-600 dark:text-slate-300 font-medium">amu</span>
                                </div>
                            </div>
                            
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Calculate Energy Released (E)</label>
                                <div className="flex gap-2 items-center">
                                    <input 
                                        type="number"
                                        value={fissionEnergy}
                                    ...
                                    />
                                    <span className="text-slate-600 dark:text-slate-300 font-medium">MeV</span>
                                </div>
                            </div>
                    ...
                    {activeTab === 'pet-scan' && (
                        <div className="space-y-5">
                            <p className="text-sm text-slate-600 dark:text-slate-300">A positron and electron annihilate completely. Determine the energy of a single emitted gamma photon.</p>
                            
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Gamma Photon Energy (E<sub>γ</sub>)</label>
                                <div className="flex gap-2 items-center">
                                    <input 
                                        type="number"
                                        ...
                                        placeholder="e.g. 511"
                                    />
                                    <span className="text-slate-600 dark:text-slate-300 font-medium">keV</span>
                                </div>
                            </div>
                    ...
                    {activeTab === 'synchrotron' && (
                        <div className="space-y-5">
                            <div className="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-md text-sm space-y-2 font-mono">
                                <p className="text-indigo-700 font-bold border-b border-slate-200 dark:border-slate-700 dark:border-slate-500 pb-1">Sensor Readout:</p>
                                <p>B-Field : {syncBField.toFixed(1)} T</p>
                                <p>Radius  : {syncRadius} m</p>
                            </div>

                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Calculate Proton Momentum (p)</label>
                                <div className="flex gap-2 items-center">
                                    <input 
                                        type="number"
                                        value={syncMomentum}
                                        onChange={e => { setSyncMomentum(e.target.value); setSyncCorrect(null); }}
                                        className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-0"
                                        placeholder="e.g. 9.6"
                                    />
                                    <span className="text-slate-600 dark:text-slate-300 font-medium whitespace-nowrap text-sm">× 10<sup>-18</sup> kg·m/s</span>
                                </div>
                            </div>
