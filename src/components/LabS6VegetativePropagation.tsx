...
export default function LabS6VegetativePropagation({ onExit }: LabProps) {
 ...
 <div className="flex flex-col min- lg: bg-emerald-50 font-sans min-h-screen lg:h-screen overflow-x-hidden w-full">
  ...
  <div className="w-full max-w-5xl bg-slate-50 dark:!bg-[#121212] rounded-2xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-8">
   ...
   <h3 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 flex items-center gap-2"><Edit2 className="w-5 h-5" /> Data Collection Table</h3>
   ...
   <table className="w-full text-left border-collapse">
    ...
    <tr className="bg-slate-100 dark:bg-[#121212] text-slate-700 dark:text-[#ffffff]">
     <th className="p-4 border-b border-slate-200 dark:border-[#1c1b1b] font-bold w-1/3">Observation Parameter</th>
     <th className="p-4 border-b border-slate-200 dark:border-[#1c1b1b] border-l font-bold w-1/3">Group 1 (Grown from Seeds)</th>
     <th className="p-4 border-b border-slate-200 dark:border-[#1c1b1b] border-l font-bold w-1/3">Group 2 (Grown from Cuttings)</th>
    </tr>
    </thead>
   ...
   <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 flex items-start gap-4">
    ...
    <div>
     ...
     <button onClick={() => setSubmitted(false)} className="mt-4 px-4 py-2 bg-emerald-600 text-white text-sm font-bold rounded hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-emerald-500/40">Edit Data</button>
    </div>
   </div>
   )}
