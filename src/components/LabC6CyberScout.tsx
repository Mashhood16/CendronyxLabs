      ...
      <div className="flex-1 px-8 pb-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 dark:border-slate-500 overflow-hidden flex flex-col">
          ...
          <div className="bg-slate-200 dark:bg-slate-800 border-b border-slate-300 dark:border-slate-700 dark:border-slate-500 p-3 flex items-center gap-4">
            ...
            <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-md flex items-center px-3 py-1.5 shadow-inner">
              <Globe className="w-4 h-4 text-slate-400 mr-2" />
              <span className="text-sm text-slate-500 dark:text-slate-400">https://www.search-simulator.edu</span>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-50 dark:bg-slate-900">
              ...
              <div className="w-full max-w-2xl flex relative shadow-lg rounded-full">
                ...
                <input 
                  ...
                  className="w-full text-xl py-4 pl-16 pr-8 rounded-full border-2 border-slate-200 dark:border-slate-700 dark:border-slate-500 focus:border-blue-500 outline-none transition-colors"
                />
              </div>
              <button 
                onClick={() => setHasSearched(true)}
                disabled={query.trim().length === 0}
                className="mt-8 px-8 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold rounded-lg disabled:opacity-50"
              >
                Simulated Search
              </button>
            </div>
              ...
              <div className="flex-1 px-8 pb-8 overflow-y-auto">
                <div className="flex items-center gap-4 mb-8 border-b border-slate-200 dark:border-slate-700 dark:border-slate-500 pb-6">
                  ...
                  <input 
                    ...
                    value={query}
                    onChange={e => setQuery(e.target.value)}
                    className="flex-1 text-lg py-2 px-4 rounded-lg border border-slate-300 dark:border-slate-700 dark:border-slate-500"
                  />
                  <button onClick={() => setHasSearched(false)} className="px-4 py-2 bg-blue-600 text-white rounded font-bold">Search</button>
                </div>

                <div className="text-sm text-slate-500 dark:text-slate-400 mb-6">About 1 results (0.02 seconds)</div>

                <div className="bg-slate-50 dark:bg-slate-900 p-8 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 max-w-3xl">
                  ...
                  {!joined ? (
                    <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-500">
                      <h3 className="font-bold text-lg mb-4">Registration Requirements:</h3>
                      <ul className="list-disc pl-5 mb-6 space-y-2 text-slate-700 dark:text-slate-200">
