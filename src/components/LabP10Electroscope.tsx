  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 max-w-7xl mx-auto w-full">
          ...
          <div>
            ...
            <div className="mb-5">
              <div className="flex justify-between mb-1">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Rod Charge (Q_rod)</label>
                <span className={`text-sm font-mono ${qRod > 0 ? 'text-blue-600' : qRod < 0 ? 'text-red-600' : 'text-slate-600 dark:text-slate-300'}`}>
                  {qRod > 0 ? '+' : ''}{qRod} μC
                </span>
              </div>
              <input 
            ...
            <div className="mb-5">
              <div className="flex justify-between mb-1">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Rod Distance (d)</label>
              ...
              />
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">d = 0 means touching (Conduction).</p>
            </div>

            <button 
              onClick={handleGround}
              className="w-full py-2 bg-slate-800 dark:bg-slate-800 text-white rounded font-medium hover:bg-slate-700 dark:bg-slate-800 transition-colors"
            >
        ...
        {/* Column 2: Simulation */}
        <div className="bg-slate-100 dark:bg-slate-800 rounded-2xl shadow-inner border border-slate-300 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col items-center justify-center relative overflow-hidden">
          
          {/* Angle Display overlay */}
          <div className="absolute top-4 right-4 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-300 dark:border-slate-700 dark:border-slate-500 shadow-sm flex flex-col items-center z-20">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Deflection Angle θ</span>
            <span className="text-2xl font-mono font-bold text-slate-800 dark:text-slate-100">{measuredAngle.toFixed(1)}°</span>
          </div>
          ...
          <div className="relative w-[300px] h-[400px] flex justify-center">
            ...
            >
               ...
               <div className="absolute -right-16 text-slate-600 dark:text-slate-300 font-mono text-sm bg-slate-50 dark:bg-slate-900/80 px-1 rounded">Rod</div>
            </div>
            ...
            <div className="absolute bottom-10 flex flex-col items-center">
              ...
              <div className="w-20 h-6 bg-yellow-500 rounded-full border-2 border-yellow-600 z-20 flex justify-center items-center shadow-md relative">
                 <div className="absolute top-1/2 -translate-y-1/2 text-slate-800 dark:text-slate-100 text-xs font-bold mix-blend-overlay">Disc</div>
              </div>
              
              {/* Insulator plug */}
              <div className="w-12 h-6 bg-slate-800 dark:bg-slate-800 -mt-2 z-10" />
          ...
          <div className="absolute bottom-4 left-4 flex gap-2">
            <div className="bg-slate-50 dark:bg-slate-900/80 px-2 py-1 rounded text-xs border border-slate-300 dark:border-slate-700 dark:border-slate-500 text-slate-600 dark:text-slate-300 font-mono shadow-sm">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col h-full overflow-hidden">
          ...
          <div>
            ...
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">(Hint: What happens to the electrons on the leaf when a positive rod is brought near the disc?)</p>
              <div className="flex gap-2 items-center">
                <input 
                  ...
                  className="border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded px-3 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button 
                  onClick={checkAnswer}
