...
export default function LabM7FinancialArithmetic({ onExit }: { onExit?: () => void }) {
 ...
 return (
 <div className="flex flex-col min- bg-slate-50 dark:!bg-[#000000] font-sans text-slate-800 dark:text-[#ffffff] select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
 {/* Header */}
 ...
 {t('lab.m7financialarithmetic_interactive_workspace')}
 </h2>

 {activeTab === 'business' && (
 <div className="space-y-4 flex-1">
 <p className="text-sm text-slate-600 dark:text-[#71717a]">{t('lab.m7financialarithmetic_calculate_profit_loss_and_taxe')}</p>
 <div>
 <label className="block text-sm font-medium mb-1">{t('lab.m7financialarithmetic_cost_price_rs')} {costPrice}</label>
 ...
 <div>
 <label className="block text-sm font-medium mb-1">{t('lab.m7financialarithmetic_selling_price_rs')} {sellingPrice}</label>
 ...
 <div>
 <label className="block text-sm font-medium mb-1">{t('lab.m7financialarithmetic_tax_rate')} {taxRate}</label>
 ...
 <h3 className="font-semibold mb-2 text-indigo-900 dark:text-indigo-100">{t('lab.m7financialarithmetic_solve_it')}</h3>
 <p className="text-sm mb-4">{t('lab.m7financialarithmetic_what_is_the_net_amount_receive')}</p>
 <div className="flex space-x-2">
 <input type="number" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} className="flex-1 min-w-0 px-3 py-2 border rounded-md dark:bg-slate-700 dark:border-[#1c1b1b] focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder={t('lab.m7financialarithmetic_enter_amount')} />
 <button onClick={checkAnswer} className="whitespace-nowrap flex-shrink-0 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors dark:text-white dark:text-white dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-indigo-500/40">{t('lab.m7financialarithmetic_check')}</button>
 </div>
 ...
 {activeTab === 'zakat' && (
 <div className="space-y-4 flex-1">
 <p className="text-sm text-slate-600 dark:text-[#71717a]">{t('lab.m7financialarithmetic_zakat_is_calculated_at_2_5_on_')}</p>
 <div>
 <label className="block text-sm font-medium mb-1">{t('lab.m7financialarithmetic_wealth_amount_rs')} {wealthAmount.toLocaleString()}</label>
 ...
 <h3 className="font-semibold mb-2 text-emerald-900 dark:text-emerald-100">{t('lab.m7financialarithmetic_solve_it')}</h3>
 <p className="text-sm mb-4">{t('lab.m7financialarithmetic_calculate_the_zakat_payable_on')}</p>
 <div className="flex space-x-2">
 <input type="number" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} className="flex-1 min-w-0 px-3 py-2 border rounded-md dark:bg-slate-700 dark:border-[#1c1b1b] focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder={t('lab.m7financialarithmetic_enter_zakat')} />
 <button onClick={checkAnswer} className="whitespace-nowrap flex-shrink-0 px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition-colors dark:text-white dark:text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-emerald-500/40">{t('lab.m7financialarithmetic_check')}</button>
 </div>
 {feedback && <p className={`mt-2 text-sm font-medium ${feedback.includes('Correct') ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{feedback}</p>}
 </div>
 </div>
 ...
 {activeTab === 'ushr' && (
 <div className="space-y-4 flex-1">
 <p className="text-sm text-slate-600 dark:text-[#71717a]">{t('lab.m7financialarithmetic_ushr_is_10_for_natural_irrigat')}</p>
 <div>
 <label className="block text-sm font-medium mb-1">{t('lab.m7financialarithmetic_produce_value_rs')} {produceValue.toLocaleString()}</label>
 ...
 <div>
 <label className="block text-sm font-medium mb-2">{t('lab.m7financialarithmetic_irrigation_type')}</label>
 ...
 <span>{t('lab.m7financialarithmetic_natural_rain')}</span>
 </label>
 <label className="flex items-center space-x-2 cursor-pointer">
 ...
 <span>{t('lab.m7financialarithmetic_artificial_tube_well')}</span>
 </label>
 </div>
 </div>
 <div className="mt-8 p-4 bg-amber-50 dark:bg-amber-900/30 rounded-lg border border-amber-100 dark:border-amber-800">
 <h3 className="font-semibold mb-2 text-amber-900 dark:text-amber-100">{t('lab.m7financialarithmetic_solve_it')}</h3>
 <p className="text-sm mb-4">{t('lab.m7financialarithmetic_calculate_the_ushr_payable_on_')}</p>
 <div className="flex space-x-2">
 <input type="number" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} className="flex-1 min-w-0 px-3 py-2 border rounded-md dark:bg-slate-700 dark:border-[#1c1b1b] focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder={t('lab.m7financialarithmetic_enter_ushr')} />
 <button onClick={checkAnswer} className="whitespace-nowrap flex-shrink-0 px-4 py-2 bg-amber-600 text-white rounded-md hover:bg-amber-700 transition-colors dark:text-white dark:text-white dark:bg-amber-500 dark:hover:bg-amber-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-amber-500/40">{t('lab.m7financialarithmetic_check')}</button>
 </div>
 {feedback && <p className={`mt-2 text-sm font-medium ${feedback.includes('Correct') ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{feedback}</p>}
 </div>
 </div>
 ...
 <h3 className="text-xl font-bold">{t('lab.m7financialarithmetic_transaction_breakdown')}</h3>
 </div>
 
 <div className="flex justify-between items-center p-4 bg-slate-100 dark:bg-slate-700 rounded-lg">
 <span>{t('lab.m7financialarithmetic_selling_price')}</span>
 ...
 <span>{t('lab.m7financialarithmetic_tax')}{taxRate}%):</span>
 ...
 <span>{t('lab.m7financialarithmetic_net_amount')}</span>
 ...
 <span>{t('lab.m7financialarithmetic_cost_rs')} {costPrice}</span>
 ...
 <h3 className="text-xl font-bold">{t('lab.m7financialarithmetic_zakat_calculation')}</h3>
 </div>
 ...
 <span className="text-3xl font-bold bg-white dark:bg-[#121212] dark:border-[#1c1b1b]/80 dark:bg-[#121212]/80 px-4 py-2 rounded-lg backdrop-blur-sm shadow-sm">
 ...
 <span>{t('lab.m7financialarithmetic_zakat_amount_2_5')}</span>
 ...
 <h3 className="text-xl font-bold">{t('lab.m7financialarithmetic_ushr_calculation')}</h3>
 </div>
 ...
 <span>{t('lab.m7financialarithmetic_ushr_amount')}</span>
