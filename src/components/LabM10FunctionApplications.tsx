        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col">
          <div className="flex space-x-2 mb-6">
            <button 
              ...
              className={`flex-1 flex items-center justify-center py-2 rounded-lg font-medium transition-colors ${mode === 'virus' ? 'bg-purple-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:bg-slate-800'}`}
            >
              <Biohazard size={18} className="mr-2" /> Viral Spread
            </button>
            <button 
              onClick={() => handleModeChange('depreciation')}
              className={`flex-1 flex items-center justify-center py-2 rounded-lg font-medium transition-colors ${mode === 'depreciation' ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:bg-slate-800'}`}
            >
          ...
          {/* Visualizer */}
          <div className="relative h-64 bg-slate-100 dark:bg-slate-800 rounded-xl mb-6 overflow-hidden border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex items-center justify-center p-4">
            {mode === 'virus' ? (
              <div className="w-full h-full flex flex-col items-center justify-center">
                <div className="grid gap-[2px] bg-slate-300 dark:bg-slate-800 p-2 rounded-lg" style={{ gridTemplateColumns: `repeat(${gridCols}, minmax(0, 1fr))` }}>
                  {Array.from({ length: gridCells }).map((_, i) => (
                    <div key={i} className={`w-3 h-3 sm:w-4 sm:h-4 rounded-sm ${i < infectedCount ? 'bg-purple-600' : 'bg-slate-50 dark:bg-slate-900'}`} />
                  ))}
                </div>
                <div className="mt-4 font-bold text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-slate-900 px-4 py-1 rounded-full shadow-sm">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col">
          ...
          <div className="flex-1 min-w-0 space-y-6">
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-500">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">(Round to nearest whole number)</p>
