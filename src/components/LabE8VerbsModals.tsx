    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900 dark:bg-slate-900 font-sans select-none text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
               ...
               {activeTab === 'verbs' ? (
                 <div className="space-y-6">
                    <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-700">
                       <button onClick={() => setVerbIndex(Math.max(0, verbIndex - 1))} disabled={verbIndex === 0} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Prev</button>
                       <span className="font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">Verb {verbIndex + 1} of {verbData.length}</span>
                    ...
                    <div className="bg-white dark:bg-slate-800 dark:bg-slate-700/50 p-5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-600">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-4">Identify the Verb Type</h3>
                        
                        <p className="text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 text-lg mb-6 p-4 bg-indigo-50 dark:bg-indigo-900/50 dark:bg-indigo-900/20 rounded-lg border border-indigo-100 dark:border-indigo-800/50 text-center">
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-700">
                       <button onClick={() => setModalIndex(Math.max(0, modalIndex - 1))} disabled={modalIndex === 0} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Prev</button>
                       <span className="font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">Scenario {modalIndex + 1} of {modalData.length}</span>
                       <button onClick={() => setModalIndex(Math.min(modalData.length - 1, modalIndex + 1))} disabled={modalIndex === modalData.length - 1} className="whitespace-nowrap flex-shrink-0 px-3 py-1 rounded bg-white dark:bg-slate-800 dark:bg-slate-700 shadow disabled:opacity-50 text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 transition-opacity">Next</button>
                    </div>

                    <div className="bg-white dark:bg-slate-800 dark:bg-slate-700/50 p-5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-600">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-2">Choose the Right Modal</h3>
                        ...
                        <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/50 dark:bg-amber-900/20 border-l-4 border-amber-400 rounded-r shadow-sm">
                            <p className="text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 italic text-lg">"{currentM.scenario}"</p>
                        </div>
         ...
         <div className="w-full lg:w-2/3 flex flex-col bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900 p-8 overflow-y-auto items-center justify-center relative">
            {activeTab === 'verbs' ? (
                <div className="w-full max-w-lg bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-2xl shadow-xl border-4 border-indigo-200 dark:border-indigo-900/50 overflow-hidden">
                    ...
                    <div className="h-72 flex flex-col items-center justify-center relative bg-gradient-to-b from-slate-50 to-slate-200 dark:from-slate-800 dark:to-slate-900 p-6 overflow-hidden">
                        {displayMode === 'idle' && (
                            <div className="text-slate-400 dark:text-slate-300 font-medium text-xl animate-pulse flex flex-col items-center gap-4">
                        ...
                        {displayMode === 'Transitive' && (
                            <div className="flex items-center space-x-6 animate-[bounce_1s_ease-in-out_infinite]">
                                <div className="bg-blue-500 text-white p-4 rounded-xl font-bold shadow-lg text-lg">Subject</div>
                                <div className="text-3xl font-bold text-blue-600 dark:text-blue-400 dark:text-blue-400">→</div>
                                <div className="bg-purple-500 text-white p-4 rounded-xl font-bold shadow-lg text-lg flex flex-col items-center">
                                ...
                                <div className="bg-green-500 text-white p-4 rounded-xl font-bold shadow-lg text-lg">Object</div>
                            </div>
                        )}
                        ...
                        {displayMode === 'Linking' && (
                           <div className="flex items-center space-x-6">
                                ...
                                <div className="bg-orange-500 text-white p-4 rounded-xl font-bold shadow-lg text-lg">Subject Complement</div>
                           </div>
                        )}
                        
                        {displayMode === 'Helping' && (
                           <div className="flex items-center space-x-4">
                                <div className="bg-teal-500 text-white p-5 rounded-full font-bold shadow-lg flex flex-col items-center animate-[spin_4s_linear_infinite]">
                                    <Settings size={24} />
                                    <span className="text-xs mt-1">AUX</span>
                                </div>
                                <div className="text-3xl font-bold text-teal-500 dark:text-teal-400">+</div>
                                <div className="bg-purple-500 text-white p-5 rounded-xl font-bold shadow-lg text-lg">
                                    Main Verb
                                </div>
                           </div>
                        )}
                        ...
                        {displayMode === 'Infinitive' && (
                           <div className="flex items-center bg-yellow-400 text-slate-900 dark:text-slate-300 dark:text-slate-300 p-6 rounded-2xl font-bold shadow-xl border-4 border-yellow-500 text-2xl transform hover:scale-105 transition-transform">
                ...
                <div className="w-full max-w-md flex flex-col items-center bg-white dark:bg-slate-800 dark:bg-slate-800 p-10 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700">
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 mb-10 flex items-center gap-2">
                       <Gauge className="text-emerald-500 dark:text-emerald-400" />
                       Modal Strength Gauge
                    </h2>
                    
                    <div className="relative w-72 h-36 overflow-hidden bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/60 dark:bg-slate-900 rounded-t-full border-t-8 border-l-8 border-r-8 border-slate-300 dark:border-slate-600 dark:border-slate-700 shadow-inner">
                        {/* gradient arc */}
                        ...
                        {/* Needle */}
                        <div 
                            className="absolute bottom-0 left-1/2 w-1.5 h-28 bg-slate-800 dark:bg-slate-200 origin-bottom transition-transform duration-1000 cubic-bezier(0.4, 0, 0.2, 1) rounded-t-full shadow-lg"
                            style={{ transform: `translateX(-50%) rotate(${rotation}deg)` }}
                        >
                            {/* pivot */}
                            <div className="absolute -bottom-3 -left-2.5 w-6 h-6 bg-slate-800 dark:bg-slate-200 rounded-full border-4 border-white dark:border-slate-800 shadow-md"></div>
                        </div>
                    </div>
                    
                    <div className="flex justify-between w-72 mt-4 text-sm font-bold text-slate-600 dark:text-slate-400 dark:text-slate-300 uppercase tracking-wider">
                        <span>0% (Might)</span>
                        <span>100% (Must)</span>
                    </div>
