    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col gap-4">
            ...
            <p className="text-slate-600 dark:text-slate-300">A soluble salt like NaCl can be prepared by titrating an acid and an alkali, followed by evaporating the water to crystallize the salt.</p>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col items-center relative">
            ...
            <div className="flex gap-4 mb-8">
                <button onClick={() => setNaohAdded(true)} disabled={naohAdded} className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded hover:bg-slate-200 dark:bg-slate-800 disabled:opacity-50 text-sm font-medium">Add 25mL NaOH</button>
                <button onClick={() => setHclAdded(true)} disabled={!naohAdded || hclAdded} className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded hover:bg-slate-200 dark:bg-slate-800 disabled:opacity-50 text-sm font-medium">Add 25mL HCl</button>
            </div>
            ...
            <div className="relative w-64 h-64 flex flex-col items-center justify-end">
                ...
                {/* Burner */}
                <div className="w-16 h-12 border-x-4 border-t-4 border-slate-700 dark:border-slate-500 bg-slate-600 dark:bg-slate-800 rounded-t relative flex justify-center mt-2">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 flex flex-col gap-6">
            <div className="bg-orange-50 p-5 rounded-xl border border-orange-200 mt-auto">
                ...
                <div className="flex gap-2">
                    <input type="text" value={assessmentAns} onChange={(e) => setAssessmentAns(e.target.value)} placeholder="Process name..." className="flex-1 px-3 py-2 border border-orange-300 rounded focus:outline-none focus:ring-2 focus:ring-orange-500 bg-slate-50 dark:bg-slate-900" />
