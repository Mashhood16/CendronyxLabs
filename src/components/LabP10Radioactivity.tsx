...
const { recordLabData, setLabScore } = useLab();
 ...
 <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
  <LabHeader onExit={onExit} title={t('lab.p10radioactivity_unit_20_radioactivity_attenuat')} subtitle={t('lab.subtitle_quantitative_analysis_ionizing')} />
  ...
  <div className="lg:hidden w-full px-4 py-4 md:px-6 grid grid-cols-2 gap-2 flex-shrink-0 z-10 relative mb-4">
   ...
   >
                     ...
                     {t('lab.p10radioactivity_theory')}
                    </button>
   <button 
    onClick={() => setActiveMobileTab('lab')}
    className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
   >{t('lab.10radioactivity_lab')}</button>
  </div>
  ...
  <div className="flex flex-col gap-6 ${activeMobileTab === 'theory' ? 'flex' : 'hidden'} lg:flex">
   ...
   <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 flex items-center gap-2"><BookOpen className="w-5 h-5 text-indigo-600"/>{t('lab.10radioactivity_theory')}</h2>
   <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mb-4">
    
                             {t('lab.p10radioactivity_radiation_intensity')} <strong>I</strong>  {t('lab.p10radioactivity_decreases_exponentially_as_it_')} <strong>x</strong>:
   </p>
   <div className={`bg-indigo-50 p-3 rounded-lg text-center font-mono text-indigo-900 mb-4 font-bold border border-indigo-100 dark:bg-[#121212] dark:border-[#1c1b1b] dark:text-[#ffffff] flex-col `}>
                             ...
                             {t('lab.p10radioactivity_i_i_e_x')}
                            </div>
   ...
   <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-indigo-600"/>{t('lab.10radioactivity_setup')}</h2>
   
   <div className="space-y-5">
    <div>
    <div className="text-sm font-medium mb-2 text-slate-700 dark:text-[#ffffff]">{t('lab.10radioactivity_sourcetype')}</div>
    ...
    <div>
    <div className="text-sm font-medium mb-2 text-slate-700 dark:text-[#ffffff]">{t('lab.10radioactivity_absorbermaterial')}</div>
    <div className="flex gap-2">
    ...
    <div className="flex justify-between text-sm font-medium mb-1">
     <span className="text-slate-700 dark:text-[#ffffff]">{t('lab.10radioactivity_absorberthickness')}</span>
  ...
  <div className={`w-full bg-[#000000] dark:!bg-[#121212] rounded-2xl shadow-sm border border-[#1c1b1b] dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] flex flex-col relative overflow- lg:h-full lg:min-h-[35vh] lg:min-h-[500px]  'flex' : 'hidden'} lg:flex order-first lg:order-none rounded-b-none lg:rounded-b-xl border-b-0 lg:border-b`}>
   ...
   <Info className="w-4 h-4 text-indigo-400" />  {t('lab.p10radioactivity_virtual_geiger_muller_bench')}
                        </div>
   ...
   <div className="text-sm text-slate-400">{t('lab.10radioactivity_gmtubecountrate')}</div>
   <div className="font-mono text-3xl font-bold text-amber-400">
    {currentCPM} <span className="text-sm text-amber-600">{t('lab.10radioactivity_cpm')}</span>
   </div>
   </div>
  </div>
  ...
  <div className="flex flex-col gap-6">
   ...
   <div className="flex justify-between items-center mb-4">
    <h2 className="text-lg font-bold text-slate-800 dark:text-[#ffffff] flex items-center gap-2"><Table2 className="w-5 h-5 text-indigo-600"/>{t('lab.10radioactivity_datalog')}</h2>
    <button 
    onClick={recordData}
    disabled={!sourceOpen}
    ...
    >
                                 ...
                                 {t('lab.p10radioactivity_record_data')}
                                 </button>
   </div>
   ...
   <div className="flex-1 lg:overflow-y-auto border border-slate-200 dark:border-[#1c1b1b] rounded-lg mb-4">
    ...
    <thead className="bg-slate-50 dark:bg-[#121212] sticky top-0">
     <tr>
     <th className="p-3 font-semibold text-slate-700 dark:text-[#ffffff]">{t('lab.10radioactivity_source')}</th>
     <th className="p-3 font-semibold text-slate-700 dark:text-[#ffffff]">{t('lab.10radioactivity_mat')}</th>
     <th className="p-3 font-semibold text-slate-700 dark:text-[#ffffff]">{t('lab.p10radioactivity_x_mm')}</th>
     <th className="p-3 font-semibold text-slate-700 dark:text-[#ffffff]">{t('lab.10radioactivity_cpm')}</th>
     </tr>
    </thead>
    <tbody className="divide-y divide-slate-100">
     {loggedData.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-slate-400 italic">{t('lab.10radioactivity_no_data')}</td></tr>}
     {loggedData.map((d, i) => (
   ...
   <div className="h-32 border border-slate-200 dark:border-[#1c1b1b] rounded-lg p-2 relative bg-slate-50 dark:bg-[#121212]">
    <div className="absolute top-2 left-2 text-[10px] text-slate-400 font-bold">{t('lab.10radioactivity_cpmvsthickness')}</div>
   ...
   <div className="flex gap-2">
    <input 
    type="text" 
    placeholder={t('lab.p10radioactivity_t_lab_10radioactivity_eg005')}
    value={assessmentAns}
    ...
    <button onClick={checkAssessment} className="bg-amber-600 hover:bg-amber-500 px-4 py-2 rounded-lg font-bold text-sm dark:text-white dark:text-white dark:bg-amber-500 dark:hover:bg-amber-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-amber-500/40">
                                 ...
                                 {t('lab.p10radioactivity_check')}
                                 </button>
   </div>
   {assessmentStatus === 'correct' && <div className="mt-3 text-emerald-400 text-sm font-bold">{t('lab.10radioactivity_correct00693mm')}</div>}
   {assessmentStatus === 'incorrect' && <div className="mt-3 text-rose-400 text-sm font-bold">{t('lab.p10radioactivity_incorrect_hint_ln_2_x')}</div>}
   </div>
  </div>
