...
export default function LabS8HydraulicElevator({ onExit }: LabProps) {
  ...
  return (
    <div 
      className="overflow-y-auto flex flex-col h-screen bg-slate-50 font-sans select-none"
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border p-6 flex flex-col items-center justify-center relative min-h-[500px] overflow-hidden">
          ...
          <div className="relative w-[500px] h-80 flex items-end">
            ...
            <div className="absolute left-10 bottom-0 flex flex-col items-center">
              ...
              {/* Plunger Handle */}
              <div 
                className="w-12 h-4 bg-slate-800 rounded-sm cursor-ns-resize z-30"
                style={{ transform: `translateY(${pushed}px)` }}
                onPointerDown={handlePointerDown}
              >
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-4 h-32 bg-slate-700 -z-10" />
                {/* Rubber stopper */}
                <div className="absolute top-32 left-1/2 -translate-x-1/2 w-10 h-4 bg-slate-900 rounded-b-sm" />
              </div>
              ...
              <div className="w-4 h-8 bg-cyan-500/80 border-x-4 border-slate-300 z-10" />
            </div>

            {/* Connecting Tube */}
            <div className="absolute left-16 bottom-0 w-80 h-4 bg-cyan-500/80 border-y-4 border-slate-300 z-0" />
            ...
            <div className="absolute right-10 bottom-0 flex flex-col items-center">
              ...
              >
                <div className="w-24 h-12 bg-red-500 rounded-t-2xl rounded-b-md flex justify-center items-end pb-1 border-b-4 border-slate-800 shadow-xl relative">
                  <div className="absolute -bottom-3 left-2 w-6 h-6 bg-slate-900 rounded-full border-2 border-slate-400" />
                  <div className="absolute -bottom-3 right-2 w-6 h-6 bg-slate-900 rounded-full border-2 border-slate-400" />
                </div>
                {/* Platform */}
                <div className="w-32 h-2 bg-slate-700 mt-2 rounded-sm" />
              </div>

              {/* Plunger / Elevator shaft */}
              <div className="w-16 h-40 bg-slate-700 z-10 transition-all duration-75 absolute" style={{ bottom: `${10 + elevatorLift}px` }}>
                 <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-20 h-4 bg-slate-900 rounded-b-sm" />
              </div>
              ...
              <div className="w-6 h-8 bg-cyan-500/80 border-x-4 border-slate-300 z-10" />
