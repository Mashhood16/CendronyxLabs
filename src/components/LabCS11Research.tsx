    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 dark:bg-slate-900 font-sans select-none">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-200 dark:border-slate-700 dark:border-slate-500 overflow-y-auto">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-4">1. UI & Survey Design</h2>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">2. Research Workspace</h2>
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
              <button 
                onClick={() => setView('form')}
                className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-colors ${view === 'form' ? 'bg-slate-50 dark:bg-slate-900 shadow-sm text-indigo-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}
              >
                <LayoutList size={16} className="mr-2" /> Form Builder
              </button>
              <button 
                onClick={() => setView('dashboard')}
                className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-colors ${view === 'dashboard' ? 'bg-slate-50 dark:bg-slate-900 shadow-sm text-indigo-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}
              >
          ...
          {view === 'form' ? (
            <div className="flex-1 flex flex-col space-y-4 overflow-y-auto pr-2">
              {questions.map((q, index) => (
                <div key={q.id} className="p-4 border border-slate-200 dark:border-slate-700 dark:border-slate-500 rounded-lg bg-slate-50 dark:bg-slate-900 relative group">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Question {index + 1} ({q.type})</span>
                    <button onClick={() => removeQuestion(q.id)} className="text-red-400 hover:text-red-600 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                  <div className="flex items-center bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-md px-3 py-2">
                    <Edit3 size={16} className="text-slate-400 mr-2" />
                    <input 
                      type="text" 
                      value={q.text}
                      onChange={(e) => updateQuestion(q.id, e.target.value)}
                      className="flex-1 outline-none text-sm text-slate-800 dark:text-slate-100"
                      placeholder="Type your question here..."
                    />
                  </div>
                  {q.type === 'choice' && (
                    <div className="mt-3 pl-8 space-y-2">
                      <div className="flex items-center text-sm text-slate-500 dark:text-slate-400"><div className="w-4 h-4 rounded-full border border-slate-400 dark:border-slate-500 mr-2"></div> Option 1</div>
                      <div className="flex items-center text-sm text-slate-500 dark:text-slate-400"><div className="w-4 h-4 rounded-full border border-slate-400 dark:border-slate-500 mr-2"></div> Option 2</div>
                    </div>
                  )}
                  {q.type === 'text' && (
                    <div className="mt-3 pl-8">
                      <div className="border-b border-slate-300 dark:border-slate-700 dark:border-slate-500 w-full h-8"></div>
                    </div>
                  )}
                </div>
              ))}
              <div className="flex space-x-3 pt-4">
                ...
                <button onClick={() => addQuestion('text')} className="flex-1 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 dark:border-slate-500 py-2 rounded-lg text-sm font-medium hover:bg-slate-100 dark:bg-slate-800 flex items-center justify-center transition-colors">
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 dark:border-slate-500 p-6 relative overflow-hidden">
              ...
              <div className="w-full h-64 flex items-end justify-around space-x-2 pb-8 border-b-2 border-slate-300 dark:border-slate-700 dark:border-slate-500 relative">
                ...
                {mockData.map((data, i) => (
                  <div key={i} className="flex flex-col items-center flex-1 group">
                    ...
                    <span className="text-xs font-medium text-slate-600 dark:text-slate-300 mt-2 truncate w-full text-center">{data.label}</span>
                  </div>
                ))}
              </div>
              <p className="mt-8 text-sm text-slate-500 dark:text-slate-400 italic text-center">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-4">3. Analysis & Logic</h2>
          
          <div className="space-y-6 flex-1">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700 dark:text-slate-200 block">Task 1: Form Structure</label>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Construct a survey containing at least 3 questions, mixing both "Choice" and "Text" types.</p>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700 dark:text-slate-200 block">Task 2: Qualitative Analysis</label>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">What is a critical risk when writing survey questions that might skew your data unreliably?</p>
              <textarea 
                value={q1}
                onChange={(e) => setQ1(e.target.value)}
                className="w-full h-24 p-3 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded-lg text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none"
            ...
            <button 
              ...
              className="w-full bg-slate-800 dark:bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-700 dark:bg-slate-800 transition-colors shadow-sm"
            >
