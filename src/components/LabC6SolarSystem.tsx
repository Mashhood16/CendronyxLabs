...
import { ArrowLeft, Circle, Type, Palette, Trash2, Undo } from 'lucide-react';
    ...
    switch (item.type) {
      ...
      case 'orbit':
        return (
          <div 
            key={item.id} 
            className="absolute rounded-full border-2 border-dashed border-slate-600"
            style={{ 
              left: item.x - (item.size! / 2), 
              top: item.y - (item.size! / 2), 
              width: item.size, 
  ...
  return (
    <div className="flex h-screen font-sans bg-slate-50 text-slate-800">
      <div className="flex-1 p-8 flex flex-col overflow-y-auto">
        ...
        <h1 className="text-3xl font-bold mb-2">Solar System 3D Creator</h1>
        <p className="text-slate-600 mb-8">Draw planets using 3D shapes, fill them with colors, draw orbits, and label them.</p>

        <div className="flex gap-6 flex-1 h-[600px]">
          {/* Toolbar */}
          <div className="w-72 bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col gap-6 overflow-y-auto">
            <div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border-2 text-left font-bold text-sm transition-colors ${
                    selectedTool === 'planet' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 hover:border-slate-300 text-slate-600'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border-2 text-left font-bold text-sm transition-colors ${
                    selectedTool === 'orbit' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 hover:border-slate-300 text-slate-600'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border-2 text-left font-bold text-sm transition-colors ${
                    selectedTool === 'label' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 hover:border-slate-300 text-slate-600'
                  }`}
                >
              ...
              <div className="flex flex-wrap gap-2">
                {colors.map(c => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full shadow-sm border-2 transition-transform ${currentColor === c ? 'border-slate-800 scale-110' : 'border-transparent hover:scale-105'}`}
            ...
            <div className="mt-auto flex gap-2 pt-4 border-t border-slate-100">
              <button onClick={undo} disabled={items.length === 0} className="flex-1 flex flex-col items-center justify-center p-3 rounded-lg border-2 border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-50">
                <Undo className="w-5 h-5 mb-1" />
                <span className="text-xs font-bold">Undo</span>
              </button>
              <button onClick={clear} disabled={items.length === 0} className="flex-1 flex flex-col items-center justify-center p-3 rounded-lg border-2 border-slate-200 text-red-600 hover:bg-red-50 disabled:opacity-50">
          ...
          <div 
            ...
            className={`flex-1 bg-slate-900 rounded-xl shadow-inner border border-slate-800 relative overflow-hidden ${selectedTool ? 'cursor-crosshair border-blue-500' : 'cursor-default'}`}
            style={{
          ...
          >
            ...
            {items.length === 0 && !selectedTool && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-slate-400 flex flex-col items-center gap-4 bg-slate-800/80 p-8 rounded-2xl border border-slate-700">
