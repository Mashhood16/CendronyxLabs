...
export default function LabC10CondensationPolymerisation({ onExit }: LabProps) {
 ...
 return (
  <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
   <LabHeader onExit={onExit} title={t('lab.c10condensationpolymerisation_condensation_polymerisation_pe')} />
  ...
  <div className="lg:hidden w-full px-4 py-4 md:px-6 grid grid-cols-2 gap-2 flex-shrink-0 z-10 relative mb-4">
   ...
   >
                     ...
                     {t('lab.c10condensationpolymerisation_theory')}
                    </button>
   <button 
    onClick={() => setActiveMobileTab('lab')}
    className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
   >{t('lab.c10condensationpolymerisation_lab')}</button>
  </div>
  <div className="flex flex-col lg:grid lg:grid-cols-3 gap-0 lg:gap-4 p-4 flex-grow lg:overflow-visible">
    <div className={`w-full bg-slate-50 dark:!bg-[#121212] rounded-lg shadow-lg p-4 flex-col space-y-4 border border-slate-200 dark:border-[#1c1b1b] ${activeMobileTab === 'theory' ? 'flex' : activeMobileTab === 'lab' ? 'flex mb-4' : 'hidden'} lg:flex lg:order-none`}>
     <h2 className="text-xl font-semibold flex items-center"><Info className="mr-2 text-indigo-600"/>  {t('lab.c10condensationpolymerisation_setup_theory')}</h2>
     <p className="text-sm text-slate-700 dark:text-[#ffffff]">
      
                           {t('lab.c10condensationpolymerisation_condensation_polymerisation_jo')}
                          </p>
     
     <div className={`space-y-4 mt-4 ${activeMobileTab === 'theory' ? 'block' : 'hidden'} lg:block`}>
      <div>
       <label className="block text-sm font-medium">{t('lab.c10condensationpolymerisation_terephthalic_acid_moles')} {acidMoles}</label>
      ...
      <div>
       <label className="block text-sm font-medium">{t('lab.c10condensationpolymerisation_ethylene_glycol_moles')} {alcoholMoles}</label>
      ...
      <div>
       <label className="block text-sm font-medium">{t('lab.c10condensationpolymerisation_temperature_c')} {temp}</label>
      ...
      <div className="flex space-x-2 pt-4">
       <button onClick={handleReact} disabled={isReacting || progress > 0} className="flex-1 bg-green-600 text-white py-2 rounded hover:bg-green-700 disabled:opacity-50 flex justify-center items-center dark:bg-green-500 dark:hover:bg-green-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-green-500/40">
        <Play className="w-4 h-4 mr-1"/>  {t('lab.c10condensationpolymerisation_react')}
                                    </button>
       <button onClick={handleReset} className="flex-1 bg-slate-600 dark:bg-[#121212] text-white py-2 rounded hover:bg-slate-700 dark:bg-[#121212] flex justify-center items-center dark:bg-cyan-400 dark:text-black dark:hover:bg-cyan-300 dark:border-transparent">
        <RotateCcw className="w-4 h-4 mr-1"/>  {t('lab.c10condensationpolymerisation_reset')}
                                    </button>
      </div>
     </div>
    </div>
    ...
    <div className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:!bg-[#121212] rounded-lg shadow-lg p-4 flex-col items-center justify-center border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] relative overflow- '' : ''} ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
     <h2 className="text-xl font-semibold absolute top-4 left-4">{t('lab.c10condensationpolymerisation_molecular_view')}</h2>
     <div className="w-full h-64 mt-12 bg-slate-100 dark:bg-[#121212] rounded border border-slate-300 dark:border-[#1c1b1b] relative flex items-center justify-center overflow-hidden">
      <svg width="100%" height="100%" viewBox="0 0 500 200">
       <g transform="translate(50, 100)">
        ...
        <text x="20" y="5" fill="white" fontSize="12" textAnchor="middle">{t('lab.c10condensationpolymerisation_acid')}</text>
        
        <rect x={60 - (20 * progress / 100)} y="-15" width="40" height="30" fill="#ef4444" rx="4" />
        <text x={80 - (20 * progress / 100)} y="5" fill="white" fontSize="12" textAnchor="middle">{t('lab.c10condensationpolymerisation_diol')}</text>

        <rect x={120 - (40 * progress / 100)} y="-15" width="40" height="30" fill="#3b82f6" rx="4" />
        <text x={140 - (40 * progress / 100)} y="5" fill="white" fontSize="12" textAnchor="middle">{t('lab.c10condensationpolymerisation_acid')}</text>

        <rect x={180 - (60 * progress / 100)} y="-15" width="40" height="30" fill="#ef4444" rx="4" />
        <text x={200 - (60 * progress / 100)} y="5" fill="white" fontSize="12" textAnchor="middle">{t('lab.c10condensationpolymerisation_diol')}</text>
      ...
      <div className="absolute bottom-2 left-2 text-sm font-semibold">{t('lab.c10condensationpolymerisation_limiting_reactant')} {acidMoles < alcoholMoles ? 'Acid' : alcoholMoles < acidMoles ? 'Diol' : 'None (Equimolar)'}</div>
      <div className="absolute bottom-2 right-2 text-sm font-semibold text-blue-600">{t('lab.c10condensationpolymerisation_water_released')} {waterMass.toFixed(1)} g</div>
     </div>
     <div className="mt-4 text-center w-full px-4">
      <p className="text-sm font-mono overflow-x-auto whitespace-nowrap">
       
                                {t('lab.c10condensationpolymerisation_n_hooc_c_h_cooh_n_ho_ch_ch_oh_')}<sub className="text-xs">n</sub>  {t('lab.c10condensationpolymerisation_2n_h_o')}
                               </p>
     </div>
    </div>
    ...
    <div className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:!bg-[#121212] rounded-lg shadow-lg p-4 flex-col space-y-4 border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] '' : ''} rounded-t-none lg:rounded-t-xl border-t-0 lg:border-t ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
     <h2 className="text-xl font-semibold flex items-center"><Database className="mr-2 text-indigo-600"/>  {t('lab.c10condensationpolymerisation_data_analysis')}</h2>
     ...
     <button onClick={recordData} disabled={progress < 100} className="w-full bg-indigo-100 text-indigo-700 py-2 rounded font-medium hover:bg-indigo-200 disabled:opacity-50">
                           ...
                           {t('lab.c10condensationpolymerisation_record_final_yield')}
                          </button>
     ...
     <div className="flex-1 lg:overflow-y-auto">
      <table className="w-full text-sm text-left border-collapse">
       <thead className="bg-slate-100 dark:bg-[#121212]">
        <tr>
         <th className="p-2 border">{t('lab.c10condensationpolymerisation_acid_mol')}</th>
         <th className="p-2 border">{t('lab.c10condensationpolymerisation_diol_mol')}</th>
         <th className="p-2 border">{t('lab.c10condensationpolymerisation_h_o_g')}</th>
        </tr>
       </thead>
       <tbody>
        {data.map((d, i) => (
     ...
     <div className="bg-slate-100 dark:bg-[#121212] p-3 rounded">
      <h3 className="font-semibold text-sm mb-2 flex items-center"><CheckCircle className="w-4 h-4 mr-1 text-green-600"/>  {t('lab.c10condensationpolymerisation_assessment')}</h3>
      <p className="text-xs mb-2">If {unknownMoles}  {t('lab.c10condensationpolymerisation_moles_of_terephthalic_acid_ful')}</p>
      <div className="flex space-x-2">
       <input type="number" value={assessmentAns} onChange={(e) => setAssessmentAns(e.target.value)} placeholder={t('lab.c10condensationpolymerisation_mass_g')} className="flex-1 px-2 py-1 border rounded text-sm"/>
       <button onClick={checkAnswer} className="bg-indigo-600 text-white px-3 py-1 rounded text-sm hover:bg-indigo-700 dark:text-white dark:text-white dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:text-white dark:border-transparent dark:shadow-lg dark:shadow-indigo-500/40">{t('lab.c10condensationpolymerisation_check')}</button>
      </div>
      {isCorrect !== null && (
