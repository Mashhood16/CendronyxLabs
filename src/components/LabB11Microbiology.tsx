...
export default function LabB11Microbiology({ onExit }: { onExit?: () => void }) {
 ...
 <div className="flex flex-col min- lg: bg-slate-50 dark:!bg-[#000000] font-sans select-none min-h-screen lg:h-screen overflow-x-hidden w-full">
  <LabHeader onExit={onExit} title={t('lab.b11microbiology_microbiology_antibiotic_resist')} />
  ...
  <div className="lg:hidden w-full px-4 py-4 md:px-6 grid grid-cols-2 gap-2 flex-shrink-0 z-10 relative mb-4">
   ...
   >
                     ...
                     {t('lab.b11microbiology_theory')}
                    </button>
   <button 
    onClick={() => setActiveMobileTab('lab')}
    className={`w-full py-3 text-sm font-bold rounded-xl transition-all text-center ${activeMobileTab === 'lab' ? 'bg-[#4158D1] text-white shadow-md' : 'bg-white dark:bg-[#1c1b1b] text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700'}`}
   >{t('lab.b11microbiology_lab')}</button>
  </div>
  ...
  <div className={`w-full bg-slate-50 dark:!bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-5 flex-col gap-6 ${activeMobileTab === 'theory' ? 'flex' : 'hidden'} lg:flex`}>
   ...
   <h2 className="text-xl font-bold text-slate-800 dark:text-[#ffffff] flex items-center gap-2 mb-3">
    <BookOpen className="text-blue-600" />  {t('lab.b11microbiology_theory_setup')}
                            </h2>
   <p className="text-slate-600 dark:text-[#a1a1aa] text-sm mb-4 leading-relaxed">
    
                             {t('lab.b11microbiology_the')} <strong>{t('lab.b11microbiology_kirby_bauer_disc_diffusion_tes')}</strong>  {t('lab.b11microbiology_measures_the_sensitivity_of_ba')}
                            </p>
   </div>
   ...
   <h3 className="font-semibold text-slate-700 dark:text-[#ffffff]">{t('lab.b11microbiology_inoculation_controls')}</h3>
   
   <div className="space-y-2">
    <label className="text-sm font-medium text-slate-700 dark:text-[#ffffff] block">{t('lab.b11microbiology_select_bacteria_strain')}</label>
    <select 
    value={bacteria} 
  ...
  <div className={`w-full bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:!bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] p-5 flex-col items-center relative overflow- '' : ''} ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   <div className="absolute top-5 left-5 right-5 flex justify-between items-center z-10">
   <h2 className="text-xl font-bold text-slate-800 dark:text-[#ffffff] flex items-center gap-2">
    <Activity className="text-blue-600" />  {t('lab.b11microbiology_agar_plate_view')}
                            </h2>
   <div className="bg-slate-100 dark:bg-[#121212] text-slate-500 dark:text-[#71717a] text-xs font-bold px-2 py-1 rounded">{t('lab.b11microbiology_interactive_model')}</div>
   </div>
   ...
   {progress === 1 && (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 bg-white dark:bg-[#121212] dark:border-[#1c1b1b] lg:bg-slate-50 dark:bg-[#121212] lg:dark:bg-[#121212]/95 p-3 rounded-xl shadow-lg border border-slate-200 dark:border-[#2a2a2a] lg:dark:border-[#1c1b1b] w-4/5 z-20 backdrop-blur-sm ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex rounded-t-none lg:rounded-t-xl border-t-0 lg:border-t">
    <div className="flex items-center justify-between w-full">
     <span className="text-[10px] font-bold text-slate-500 dark:text-[#71717a] uppercase tracking-wider">{t('lab.b11microbiology_measurement_caliper')}</span>
  ...
  <div className={`bg-slate-50 dark:!bg-[#121212] rounded-xl shadow-sm border border-slate-200 dark:border-[#1c1b1b] p-5 flex-col gap-6 ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   <div className={`bg-slate-50 dark:bg-[#121212] p-5 rounded-lg border border-slate-200 dark:border-[#1c1b1b] flex-1 flex-col ${activeMobileTab === 'lab' ? 'flex' : 'hidden'} lg:flex`}>
   <h3 className="font-bold text-slate-800 dark:text-[#ffffff] flex items-center gap-2 mb-4 text-lg">
    <ShieldAlert className="text-blue-600" />  {t('lab.b11microbiology_resistance_profiling')}
                            </h3>
   <p className="text-sm text-slate-600 dark:text-[#a1a1aa] mb-6 leading-relaxed">
    
                             {t('lab.b11microbiology_measure_the_diameter_of_the_zo')}
                            </p>
   
   <div className="space-y-4 flex-1">
    <div className="space-y-1">
    <label className="text-xs font-semibold text-slate-500 dark:text-[#71717a] uppercase">{t('lab.b11microbiology_select_antibiotic_disc')}</label>
    <select 
    ...
    <div className="space-y-1">
    <label className="text-xs font-semibold text-slate-500 dark:text-[#71717a] uppercase">{t('lab.b11microbiology_interpretation')}</label>
    <select 
     value={assessResult} onChange={(e) => setAssessResult(e.target.value)}
     className="w-full px-3 py-2 border border-slate-300 dark:border-[#1c1b1b] rounded text-sm focus:outline-blue-500 bg-slate-50 dark:bg-[#121212]"
    >
     <option value="">{t('lab.b11microbiology_select_profile')}</option>
     <option value="S">{t('lab.b11microbiology_susceptible_zone_20mm')}</option>
     <option value="I">{t('lab.b11microbiology_intermediate_zone_15_19mm')}</option>
     <option value="R">{t('lab.b11microbiology_resistant_zone_lt_15mm')}</option>
    </select>
    </div>
    ...
    >
                                 ...
                                 {t('lab.b11microbiology_check_answer')}
                                 </button>
