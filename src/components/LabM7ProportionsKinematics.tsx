      ...
      <main className="flex-1 min-w-0 flex overflow-hidden">
        ...
        <div className="w-[400px] lg:w-[450px] flex flex-col border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 overflow-y-auto z-10 shadow-[2px_0_10px_rgba(0,0,0,0.05)]">
          ...
          <div className="flex bg-slate-200 dark:bg-slate-700 rounded-lg p-1 mb-6 shrink-0">
            <button 
              className={`flex-1 py-2 px-4 rounded-md text-sm font-semibold transition-colors ${activeTab === 'proportions' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow' : 'text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white'}`}
              onClick={() => setActiveTab('proportions')}
            >
              Proportions
            </button>
            <button 
              className={`flex-1 py-2 px-4 rounded-md text-sm font-semibold transition-colors ${activeTab === 'kinematics' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow' : 'text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white'}`}
          ...
          {activeTab === 'proportions' && (
            <div className="space-y-6 flex-1 pr-2">
              <div className="flex gap-2 mb-4 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700">
                <button 
                  onClick={() => setPropMode('recipe')} 
                  className={`flex-1 py-1 px-3 text-sm font-bold rounded-md transition-colors ${propMode === 'recipe' ? 'bg-blue-600 text-white shadow' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}
                >
                  Recipe (Direct)
                </button>
                <button 
                  onClick={() => setPropMode('taps')} 
                  className={`flex-1 py-1 px-3 text-sm font-bold rounded-md transition-colors ${propMode === 'taps' ? 'bg-blue-600 text-white shadow' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}
                >
                  Tank (Inverse)
                </button>
              </div>
                ...
                <>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-xl border border-blue-200 dark:border-blue-800">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      />
                      <button onClick={checkRecipeAnswer} className="whitespace-nowrap flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors">
              ...
              {propMode === 'taps' && (
                <>
                  <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <button onClick={handleStartTank} className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors">
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-xl border border-blue-200 dark:border-blue-800">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      />
                      <button onClick={checkTapsAnswer} className="whitespace-nowrap flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors">
                        <Check className="w-5 h-5" /> Check
                      </button>
                    </div>
                    {tapsFeedback !== 'idle' && (
          ...
          {activeTab === 'kinematics' && (
            <div className="space-y-6 flex-1 pr-2">
              ...
              <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <button onClick={handleStartCar} className="mt-4 w-full bg-emerald-600 hover:bg-emerald-700 text-white py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-xl border border-blue-200 dark:border-blue-800">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  />
                  <button onClick={checkKinAnswer} className="whitespace-nowrap flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors">
                    <Check className="w-5 h-5" /> Check
                  </button>
                </div>
                {kinFeedback !== 'idle' && (
        ...
        <div className="flex-1 min-w-0 flex items-center justify-center bg-slate-100 dark:bg-slate-900/50 p-6 overflow-y-auto relative">
          ...
          {activeTab === 'proportions' && propMode === 'recipe' && (
            <div className="flex flex-col items-center justify-center w-full h-full">
              <h2 className="text-2xl font-bold mb-12 text-slate-700 dark:text-slate-300">
                Ingredients Required
              </h2>
              <div className="flex gap-12 items-end h-64 border-b-4 border-slate-300 dark:border-slate-600 pb-2 px-12">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest text-sm">Flour</span>
                  <span className="text-orange-600 dark:text-orange-400 font-black text-xl">{(baseRecipe.flour / 4) * recipeServings}g</span>
                </div>
                
                <div className="flex flex-col items-center gap-3">
                  <div className="w-24 bg-slate-50 dark:bg-slate-800/50 border-x-4 border-b-4 border-slate-200 dark:border-slate-700 rounded-b-xl relative overflow-hidden shadow-inner" style={{ height: '200px' }}>
                  ...
                  <span className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest text-sm">Sugar</span>
                  <span className="text-slate-600 dark:text-slate-400 font-black text-xl">{(baseRecipe.sugar / 4) * recipeServings}g</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest text-sm">Milk</span>
          ...
          {activeTab === 'proportions' && propMode === 'taps' && (
            <div className="flex flex-col items-center justify-center w-full h-full relative">
              <div className="relative w-full max-w-xl h-[400px] flex flex-col items-center justify-end">
                ...
                {/* Tank */}
                <div className="w-80 h-56 border-8 border-slate-300 dark:border-slate-600 rounded-b-3xl relative overflow-hidden bg-white/40 dark:bg-slate-800/40 backdrop-blur shadow-2xl">
                  {/* Water level */}
                  <div 
                  ...
                  >
                    {/* Surface reflection */}
                    <div className="w-full h-2 bg-white/30"></div>
                  </div>
                </div>
                
                {/* Time Display */}
                <div className="absolute -right-20 top-1/2 transform -translate-y-1/2 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 w-48 text-center">
                  <div className="text-sm text-slate-500 dark:text-slate-400 font-bold uppercase mb-2 tracking-widest">Time Taken</div>
          ...
          {activeTab === 'kinematics' && (
            <div className="flex flex-col items-center justify-center w-full h-full p-8">
              <div className="w-full max-w-4xl flex flex-col items-center">
                {/* Environment */}
                <div className="w-full h-40 bg-sky-200 dark:bg-sky-900/50 rounded-t-3xl relative overflow-hidden flex items-end border-t border-x border-slate-200 dark:border-slate-700">
                  {/* Decorative sun */}
                  <div className="absolute top-8 right-12 w-16 h-16 bg-yellow-300 rounded-full opacity-80 blur-[2px]"></div>
                </div>
                
                {/* Road */}
                <div className="w-full h-32 bg-slate-600 dark:bg-slate-800 relative overflow-hidden flex items-center shadow-inner border-x border-slate-200 dark:border-slate-700">
                ...
                {/* Road Labels */}
                <div className="w-full flex justify-between px-6 mt-4 text-slate-500 dark:text-slate-400 font-bold text-lg">
                  <span>0 km</span>
                  <span>{distance} km</span>
                </div>

                {/* Dashboard */}
                <div className="mt-12 bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 flex gap-16 text-center">
                  <div>
                    <div className="text-sm text-slate-500 dark:text-slate-400 font-bold uppercase mb-2 tracking-widest">Distance</div>
                  ...
                  <div>
                    <div className="text-sm text-slate-500 dark:text-slate-400 font-bold uppercase mb-2 tracking-widest">Time Elapsed</div>
