    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <div className="flex-1 p-4 md:p-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Column 1: Theory */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/50 dark:border-slate-700 dark:border-slate-700 p-6 flex flex-col h-[calc(100vh-8rem)] overflow-y-auto overflow-x-hidden">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-3 bg-blue-100 dark:bg-blue-900 rounded-xl">
                <BookOpen className="w-6 h-6 text-blue-600 dark:text-blue-400 dark:text-blue-400" />
              </div>
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-300 dark:text-white">Morphology Theory</h2>
            </div>
            ...
            <div className="space-y-6">
              ...
              <section>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 dark:text-slate-300 leading-relaxed mb-3">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                  <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Common Suffixes:</h4>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/50 dark:border-slate-700 dark:border-slate-700 p-6 flex flex-col h-[calc(100vh-8rem)]">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-300 dark:text-white">Word Forge</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">Combine bases and suffixes to craft adjectives</p>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300">1. Select Base Word</label>
                      ...
                      <button
                        ...
                        className={`px-4 py-3 rounded-xl text-left text-sm font-medium transition-all ${
                          selectedBase === b
                            ...
                            : 'bg-slate-100 dark:bg-slate-900 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700/50 dark:border-slate-700 dark:border-slate-600'
                        }`}
                      >
                ...
                {/* Suffix Selector */}
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300">2. Select Suffix</label>
                  <div className="grid gap-2">
                    {suffixes.map(s => (
                      <button
                        key={s}
              ...
              {/* Forge Action */}
              <div className="mt-4 flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700/50 dark:border-slate-700 dark:border-slate-700 relative">
                
                <div className="flex items-center justify-center gap-4 mb-6 w-full">
                  <div className={`flex-1 p-3 text-center rounded-xl font-bold text-lg ${selectedBase ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 dark:text-blue-300' : 'bg-slate-100 dark:bg-slate-900 dark:bg-slate-800 text-slate-400'}`}>
                    {selectedBase || '?'}
                  </div>
                  <div className="text-2xl text-slate-400 font-bold">+</div>
                  <div className={`flex-1 p-3 text-center rounded-xl font-bold text-lg ${selectedSuffix ? 'bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 dark:text-purple-300' : 'bg-slate-100 dark:bg-slate-900 dark:bg-slate-800 text-slate-400'}`}>
                ...
                {forgedLog.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {forgedLog.map((log, idx) => (
                      <div key={idx} className="bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700/50 dark:border-slate-700 dark:border-slate-700 flex justify-between items-center">
                        <div>
                          <span className="font-bold text-slate-800 dark:text-slate-300 dark:text-slate-300">{log.result}</span>
                          <span className="text-xs text-slate-500 dark:text-slate-400 ml-2">({log.base} + -{log.suffix})</span>
                        </div>
