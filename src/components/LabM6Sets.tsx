...
export default function LabM6Sets({ onExit }: { onExit?: () => void }) {
  ...
  const renderItem = (item: Item) => (
    <div
      ...
      className="px-4 py-2 bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 rounded-xl shadow-md text-sm font-bold cursor-grab active:cursor-grabbing hover:border-indigo-400 dark:hover:border-indigo-500 hover:-translate-y-1 transition-all z-30 select-none text-slate-700 dark:text-slate-200 inline-block"
    >
      {item.name}
    </div>
  );
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-sans select-none">
      ...
      <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 shadow-sm z-10 shrink-0 border-b border-slate-200 dark:border-slate-700">
        ...
        <div className="flex gap-2 bg-slate-100 dark:bg-slate-900 p-1 rounded-lg border border-slate-200 dark:border-slate-700">
          <button onClick={() => setActiveScenario('biology')} className={`px-4 py-2 rounded-md font-medium transition-all ${activeScenario === 'biology' ? 'bg-white dark:bg-slate-800 shadow text-indigo-600 dark:text-indigo-400' : 'hover:text-indigo-600 dark:hover:text-indigo-400'}`}>
            Biology Lab
          </button>
          <button onClick={() => setActiveScenario('students')} className={`px-4 py-2 rounded-md font-medium transition-all ${activeScenario === 'students' ? 'bg-white dark:bg-slate-800 shadow text-indigo-600 dark:text-indigo-400' : 'hover:text-indigo-600 dark:hover:text-indigo-400'}`}>
            Student Clubs
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-full max-w-md p-6 overflow-y-auto border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col gap-6 shrink-0 shadow-lg z-0">
          ...
          <div className="bg-indigo-50 dark:bg-indigo-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800/50">
            ...
            <div className="flex flex-col gap-2 text-sm bg-white dark:bg-slate-800 p-3 rounded-xl border border-indigo-100 dark:border-indigo-800">
          ...
          <div className="flex-1 min-w-0 bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-5 border border-slate-200 dark:border-slate-700/50 flex flex-col min-h-[250px]">
             <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-slate-700 dark:text-slate-300">Item Pool</h3>
             ...
             <div 
                className="flex-1 min-w-0 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800 p-4 flex flex-wrap content-start gap-3 transition-colors"
                onDragOver={onDragOver} 
                onDrop={(e) => handleDrop(e, 'pool')}
             >
                {items.filter(i => i.region === 'pool').map(renderItem)}
                {items.filter(i => i.region === 'pool').length === 0 && (
                  <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500 font-bold italic text-center text-sm">
                    All items placed!
                  </div>
                )}
             </div>
          ...
          <div className="flex flex-col gap-3">
            <button onClick={checkAnswers} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl shadow-lg hover:shadow-xl transition-all active:scale-[0.98]">
              Verify Organization
            </button>
        ...
        <div className="flex-1 min-w-0 p-8 bg-slate-100/50 dark:bg-slate-950 flex items-center justify-center overflow-y-auto">
          
          <div 
             className="relative w-full max-w-4xl aspect-[4/3] min-h-[500px] bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col p-8 overflow-hidden transition-colors"
             onDragOver={onDragOver}
             onDrop={(e) => handleDrop(e, 'none')}
          >
             <h3 className="text-slate-400 dark:text-slate-500 font-black tracking-widest uppercase text-xl mb-4 shrink-0 px-4">
               Universe
             </h3>
             ...
             <div className="flex-1 min-w-0 relative flex items-center justify-center">
                ...
                <div className="relative w-[600px] h-[400px] pointer-events-none">
                   ...
                   {/* Set A Circle */}
                   <div 
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-[350px] h-[350px] rounded-full border-[8px] border-blue-500 bg-blue-500/10 dark:bg-blue-500/20 shadow-lg flex flex-col items-start pt-16 pl-12 pointer-events-auto transition-colors hover:bg-blue-500/20 dark:hover:bg-blue-500/30"
                      onDragOver={onDragOver}
                      onDrop={(e) => { e.stopPropagation(); handleDrop(e, 'A'); }}
                   >
                     <div className="bg-blue-500 text-white font-black px-4 py-1.5 rounded-full text-sm mb-4 shadow-sm whitespace-nowrap">Set A: {scenario.setA}</div>
                   ...
                   {/* Set B Circle */}
                   <div 
                      className="absolute right-0 top-1/2 -translate-y-1/2 w-[350px] h-[350px] rounded-full border-[8px] border-pink-500 bg-pink-500/10 dark:bg-pink-500/20 shadow-lg flex flex-col items-end pt-16 pr-12 pointer-events-auto transition-colors hover:bg-pink-500/20 dark:hover:bg-pink-500/30"
                      onDragOver={onDragOver}
                      onDrop={(e) => { e.stopPropagation(); handleDrop(e, 'B'); }}
                   >
                     <div className="bg-pink-500 text-white font-black px-4 py-1.5 rounded-full text-sm mb-4 shadow-sm whitespace-nowrap">Set B: {scenario.setB}</div>
             ...
             <div className="h-28 w-full shrink-0 border-t-2 border-dashed border-slate-200 dark:border-slate-800 pt-4 flex flex-col items-center">
