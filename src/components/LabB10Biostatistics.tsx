      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 overflow-y-auto">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Biostatistics Basics</h2>
          </div>
        ...
        <div className="lg:col-span-1 bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-500 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Interactive Data Tasks</h2>
            <button
              onClick={handleReset}
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-8">
            {/* Task 1: Sorting */}
            <div>
              <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-2">1. Sort the Data (Ascending)</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Click two cards to swap their positions.</p>
                  ...
                  <button
                    ...
                    className={`w-14 h-14 text-xl font-bold rounded-lg transition-all ${
                      selectedIdx === idx
                        ...
                        : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 hover:bg-indigo-100 border border-slate-200 dark:border-slate-700 dark:border-slate-500'
                    }`}
                  >
                    {val}
                  </button>
            ...
            {/* Task 2: Charting */}
            <div>
              <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-2">2. Plot the Raw Data</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                Click inside the grid columns to set the bar height for each plot.
              </p>
              <div className="flex justify-center mb-4 text-sm font-semibold text-slate-700 dark:text-slate-200">
                Raw Data: A=15, B=30, C=10, D=25, E=20
              </div>
              <div className="relative border-l-2 border-b-2 border-slate-800 dark:border-slate-500 w-full max-w-sm mx-auto h-64 flex mt-8">
                <div className="absolute -left-6 top-0 bottom-0 flex flex-col justify-between text-xs text-slate-600 dark:text-slate-300 font-medium">
                ...
                <div className="flex flex-1 justify-around h-full items-end relative">
                  <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
                    {[30, 25, 20, 15, 10, 5, 0].map((v) => (
                      <div key={v} className="w-full border-t border-slate-200 dark:border-slate-700 dark:border-slate-500 h-0" />
                    ))}
                  </div>
                  ...
                  {cropYields.map((c) => (
                    <div
                      key={c.plot}
                      className="w-12 h-full flex flex-col justify-end group relative z-10 border-x border-transparent hover:border-slate-300 dark:border-slate-700 dark:border-slate-500 transition-colors"
                    >
                      ...
                      ></div>
                      <div className="absolute -bottom-6 w-full text-center text-sm font-bold text-slate-700 dark:text-slate-200">
                        {c.plot}
                      </div>
                    </div>
                  ))}
          ...
          <div className="flex flex-col gap-6 flex-1">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 dark:border-slate-500 rounded focus:outline-none focus:border-indigo-500"
              />
            </div>
