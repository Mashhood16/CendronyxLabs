...
export default function LabS8ThermalConductivity({ onExit }: LabProps) {
 ...
 className={`p-3 text-left rounded-lg font-bold transition-all border-2 ${selected.id === m.id ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-200 dark:border-[#1c1b1b] bg-slate-50 dark:bg-[#121212] hover:border-slate-300 dark:border-[#1c1b1b] text-slate-700 dark:text-[#ffffff]'}`}
 >
 {m.name}
 </button>
 ))}
 ...
 {/* Action Area */}
 <div className="flex-1 bg-[#000000] dark:!bg-[#121212] rounded-2xl shadow-sm border border-[#1c1b1b] dark:border-[#1c1b1b] p-6 flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]">
 ...
 <div 
 key={i}
 className={`absolute w-4 h-6 bg-slate-200 dark:bg-[#121212] rounded-sm shadow-md transition-all duration-500 ${isMelted ? 'opacity-0 translate-y-16 scale-y-150 bg-slate-300 dark:bg-[#121212]/50' : ''}`}
 style={{ left: `${pos}%`, top: isMelted ? '20px' : '-4px' }}
 >
 {!isMelted && <div className="absolute -bottom-2 w-full h-2 bg-slate-200 dark:bg-[#121212] clip-path-triangle" />}
 </div>
 );
 })}
