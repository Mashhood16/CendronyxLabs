...
export default function Class10Physics() {
  ...
  const derivationModules = useMemo(
    () => LAB_MODULES.filter(m => m.classLevel === classId && m.subject === subjectId && m.id.startsWith('p10_deriv_')),
    []
  );
        ...
        <div className={`flex gap-2 mb-6 ${theme.innerCard.bg} border ${theme.innerCard.border} p-1.5 rounded-xl w-full sm:w-fit`}>
          <button
            ...
            className={`flex flex-1 sm:flex-none justify-center items-center gap-2 px-3 sm:px-5 py-2.5 text-sm font-bold rounded-lg transition-all ${
              activeTab === 'labs'
                ? 'bg-white dark:bg-[#121212] text-teal-600 dark:text-teal-400 shadow-sm'
            ...
            {filteredModules.length === 0 ? (
              ...
              <div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredModules.filter(m => !m.id.startsWith('p10_deriv_')).map((lab) => {
