    ...
    <Layout>
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={`text-2xl md:text-3xl font-bold ${centralTheme.text.primary} font-outfit tracking-tight`}>{t('settings.title')}</h1>
            <p className={`${centralTheme.text.muted} text-sm md:text-base`}>{t('settings.subtitle')}</p>
          </div>
        </div>

        {/* Profile & Account */}
        <div className={`${centralTheme.card.bg} rounded-2xl border ${centralTheme.border.default} shadow-sm p-4 sm:p-6`}>
          <h2 className={`text-lg font-bold ${centralTheme.text.primary} mb-4 flex items-center gap-3`}>
          ...
          {user ? (
                  ...
                  {isEditing ? (
                    <div className="flex gap-2">
                      <input
                        ...
                        className={`flex-1 px-3 py-2 border rounded-xl text-sm outline-none ${centralTheme.input.bg} ${centralTheme.input.border} ${centralTheme.input.text} ${centralTheme.input.focus}`}
                        autoFocus
                        onKeyDown={(e) => e.key === 'Enter' && handleSaveName()}
                      />
                      <button onClick={handleSaveName} className="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-colors dark:bg-cyan-400 dark:text-black dark:hover:bg-cyan-300 dark:border-transparent">{t('settings.save')}</button>
                      <button onClick={() => setIsEditing(false)} className={`px-4 py-2 ${centralTheme.button.default} text-sm font-semibold rounded-xl transition-colors`}>{t('settings.cancel')}</button>
                    </div>
                  ) : (
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`${centralTheme.text.primary} font-semibold`}>{user.name}</span>
                      <button onClick={() => { setEditName(user.name); setIsEditing(true); }} className="text-sm text-blue-600 hover:text-blue-700 font-medium">{t('settings.edit')}</button>
                    </div>
                  )}
                  <p className={`text-xs ${centralTheme.text.subtle} mt-1`}>{user.email}</p>
                </div>
              </div>
              <div className={`rounded-xl p-4 border ${centralTheme.innerCard.bg} ${centralTheme.innerCard.border}`}>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className={`text-xs font-bold ${centralTheme.text.subtle} uppercase tracking-wider`}>{t('settings.class_label')}</span>
                    <p className={`text-sm font-semibold ${centralTheme.text.secondary}`}>{t("Class")} {user.classLevel}</p>
                  </div>
                  <div>
                    <span className={`text-xs font-bold ${centralTheme.text.subtle} uppercase tracking-wider`}>{t('settings.section_label')}</span>
                    <p className={`text-sm font-semibold ${centralTheme.text.secondary}`}>{user.section}</p>
                  </div>
                </div>
              </div>
              <button
                onClick={() => { logout(); navigate('/login'); }}
                className={`w-full px-4 py-3 ${centralTheme.error.bg} ${centralTheme.error.text} border ${centralTheme.error.border} font-semibold rounded-xl transition-colors text-sm flex items-center justify-center gap-2`}
              >
            ...
            <div className="text-center py-4">
              <p className={`${centralTheme.text.muted} mb-3`}>{t('settings.not_signed_in')}</p>
              <button onClick={() => navigate('/login')} className="px-6 py-2 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors text-sm dark:bg-cyan-400 dark:text-black dark:hover:bg-cyan-300 dark:border-transparent">{t('settings.log_in')}</button>
            </div>
          )}
        </div>
        ...
        <div className={`${centralTheme.card.bg} rounded-2xl border ${centralTheme.border.default} shadow-sm p-4 sm:p-6`}>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className={`text-sm ${centralTheme.text.muted}`}>{t('settings.theme')}</span>
              <div className={`flex ${centralTheme.innerCard.bg} rounded-xl p-1`}>
                <button
                  onClick={() => setTheme('light')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${theme === 'light' ? `bg-white dark:bg-[#121212] ${centralTheme.text.primary} shadow-sm` : `${centralTheme.text.subtle} hover:${centralTheme.text.primary}`}`}
                >{t('settings.light')}</button>
                <button
                  onClick={() => setTheme('dark')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${theme === 'dark' ? `bg-white dark:bg-[#121212] ${centralTheme.text.primary} shadow-sm` : `${centralTheme.text.subtle} hover:${centralTheme.text.primary}`}`}
                >{t('settings.dark')}</button>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <span className={`text-sm ${centralTheme.text.muted}`}>{t('settings.language')}</span>
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as Language)}
                className={`px-3 py-2 rounded-xl text-sm font-medium border transition-colors ${centralTheme.input.bg} ${centralTheme.input.border} ${centralTheme.input.text}`}
              >
        ...
        <div className={`${centralTheme.card.bg} rounded-2xl border ${centralTheme.border.default} shadow-sm p-4 sm:p-6`}>
          <h2 className={`text-lg font-bold ${centralTheme.text.primary} mb-4 flex items-center gap-3`}>
            ...
            {t('settings.storage_cache')}
          </h2>
          <div className="space-y-4">
            {storageInfo && (
              <div className={`rounded-xl p-4 ${centralTheme.innerCard.bg} border ${centralTheme.innerCard.border}`}>
            ...
            <button
              ...
              className={`w-full px-4 py-3 ${centralTheme.warning.bg} ${centralTheme.warning.text} border ${centralTheme.warning.border} font-semibold rounded-xl transition-colors text-sm flex items-center justify-center gap-2`}
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
              {t('settings.clear_cached_labs')}
            </button>
            <button
        ...
        <div className={`${centralTheme.card.bg} rounded-2xl border ${centralTheme.border.default} shadow-sm p-4 sm:p-6`}>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className={`text-sm ${centralTheme.text.muted}`}>{t('settings.font_size')}</span>
              <div className="flex items-center gap-2">
                <button onClick={() => setFontSize(String(Math.max(12, parseInt(fontSize) - 2)))} className={`w-8 h-8 rounded-lg ${centralTheme.button.ghost} font-bold transition-colors text-sm`}>A-</button>
                <span className={`text-sm font-semibold ${centralTheme.text.primary} w-12 text-center`}>{fontSize}px</span>
                <button onClick={() => setFontSize(String(Math.min(24, parseInt(fontSize) + 2)))} className={`w-8 h-8 rounded-lg ${centralTheme.button.ghost} font-bold transition-colors text-sm`}>A+</button>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <span className={`text-sm ${centralTheme.text.muted}`}>{t('settings.reduce_animations')}</span>
              <button
