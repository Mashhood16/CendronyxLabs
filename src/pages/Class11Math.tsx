            ...
            {filteredModules.filter(m => !m.id.includes('_theorem_')).length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredModules.filter(m => !m.id.includes('_theorem_')).map((lab) => {
                  ...
                  return (
                    <div key={lab.id} onClick={() => isBuilt && goToLab(`/class/${classId}/${subjectId}/lab/${lab.id}`)} className={`relative group rounded-2xl overflow-hidden transition-all duration-300 flex flex-col h-full ${isBuilt ? `${theme.card.bg} border ${theme.border.default} hover:-translate-y-2 hover:shadow-xl hover:border-transparent cursor-pointer hover:gradient-border` : `${theme.card.bg} border border-dashed ${theme.border.default} opacity-70`}`}>
          ...
          <div className="flex-1">
            {theoremModules.length === 0 ? (
              <div className={`flex flex-col items-center justify-center py-20 ${theme.card.bg} rounded-2xl border border-dashed ${theme.border.default}`}>
                ...
                <p className="${theme.text.subtle} max-w-md text-center">{t("We are developing interactive theorem proofs for Class 11 Mathematics. Check back soon!")}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
