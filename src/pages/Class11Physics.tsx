...
export default function Class11Physics() {
  ...
  const derivationModules = useMemo(
    () => LAB_MODULES.filter(m => m.classLevel === classId && m.subject === subjectId && m.id.startsWith('p11_deriv_')),
    []
  );
    ...
    <Layout>
      <div className="flex flex-col min-h-[calc(100vh-80px)]">
        ...
        <div className="relative overflow-hidden rounded-2xl mb-6 shadow-lg">
          <div className="absolute inset-0 bg-gradient-to-br from-sky-600 via-blue-600 to-indigo-600"></div>
        ...
        <div className="flex gap-2 mb-6 bg-slate-100 dark:bg-[#1c1b1b] p-1.5 rounded-xl w-full sm:w-fit">
          <button onClick={() => setActiveTab('labs')} className={`flex flex-1 sm:flex-none justify-center items-center gap-2 px-3 sm:px-5 py-2.5 text-sm font-bold rounded-lg transition-all ${activeTab === 'labs' ? 'bg-white dark:bg-[#121212] text-sky-600 dark:text-sky-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>
            <Beaker className="w-4 h-4" /> {t("Labs")} <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-sky-100 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400">{readyCount - derivationModules.length}</span>
          </button>
          <button onClick={() => setActiveTab('derivations')} className={`flex flex-1 sm:flex-none justify-center items-center gap-2 px-3 sm:px-5 py-2.5 text-sm font-bold rounded-lg transition-all ${activeTab === 'derivations' ? 'bg-white dark:bg-[#121212] text-sky-600 dark:text-sky-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>
            <GraduationCap className="w-4 h-4" /> {t("Derivations")} <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400">{derivationModules.length}</span>
          </button>
        </div>

        {activeTab === 'labs' && (
          <div className="flex-1">
            {filteredModules.filter(m => !m.id.startsWith('p11_deriv_')).length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 glass rounded-2xl border border-dashed border-slate-200/50 dark:border-neutral-900/50">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center mb-4 opacity-50"><Rocket className="w-10 h-10 text-white" /></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredModules.filter(m => !m.id.startsWith('p11_deriv_')).map((lab) => {
                    ...
                    <div key={lab.id}                    onClick={() => isBuilt && goToLab(`/class/${classId}/${subjectId}/lab/${lab.id}`)} className={`relative group rounded-2xl overflow-hidden transition-all duration-300 flex flex-col h-full ${isBuilt ? 'glass border border-slate-200/50 dark:border-[#1c1b1b]/50 hover:-translate-y-2 hover:shadow-xl hover:border-transparent cursor-pointer hover:gradient-border' : 'glass border border-dashed border-slate-200/50 dark:border-[#1c1b1b]/50 opacity-70'}`}>
                      ...
                      <div className="p-6 flex-1 flex flex-col bg-transparent">
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                          ...
                          {isBuilt ? <span className="text-xs font-bold text-white px-3 py-1.5 rounded-lg bg-gradient-to-r from-sky-500 to-blue-600 group-hover:scale-105 transition-transform inline-flex items-center gap-1">{t("Launch")} <ArrowRight className="w-3 h-3" /></span> : <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("Soon")}</span>}
                        </div>
                      </div>
                    </div>
                  );
                ...
                return (
                  <div key={lab.id}                    onClick={() => isBuilt && goToLab(`/class/${classId}/${subjectId}/lab/${lab.id}`)} className={`relative group rounded-2xl overflow-hidden transition-all duration-300 flex flex-col h-full ${isBuilt ? 'glass border border-slate-200/50 dark:border-[#1c1b1b]/50 hover:-translate-y-2 hover:shadow-xl hover:border-transparent cursor-pointer hover:gradient-border' : 'glass border border-dashed border-slate-200/50 dark:border-[#1c1b1b]/50 opacity-70'}`}>
                    <div className={`relative h-32 bg-gradient-to-br ${lab.bg} overflow-hidden`}>
                      <div className="absolute inset-0 opacity-20 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMiIgY3k9IjIiIHI9IjEiIGZpbGw9IiNmZmYiLz48L3N2Zz4=')]"></div>
                      {isBuilt && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 pointer-events-none"></div>}
                      <div className="absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-black/20 backdrop-blur-md text-white text-xs font-bold tracking-wider border border-white/20">{t("Derivation &middot; Class 11 Physics")}</div>
                      {hasCalculator(lab) && <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-lg bg-amber-400/90 text-amber-900 text-[10px] font-bold backdrop-blur-sm shadow-sm">{t("🧮 Calc")}</div>}
                    ...
                    <div className="p-6 flex-1 flex flex-col bg-transparent">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                        ...
                        {isBuilt ? <span className="text-xs font-bold text-white px-3 py-1.5 rounded-lg bg-gradient-to-r from-purple-500 to-sky-600 group-hover:scale-105 transition-transform inline-flex items-center gap-1">{t("Start Derivation")} <ArrowRight className="w-3 h-3" /></span> : <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("Soon")}</span>}
                      </div>
                    </div>
                  </div>
                );
